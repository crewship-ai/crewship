...
| 4 | `/issues` + API | **Issue → runs → journal.** `issues/{id}/runs` returns `assignments` rows, whose `id` is an assignment id, not a `pipeline_runs` id, so it must not be handed to `/activity?run=` (the run panel resolves by pipeline run id and shows "This run's record is not loaded"). Add `pipeline_run_id`/`trace_id` and `agent_slug` to each row where a pipeline run exists; render the run list (not only `runs[0]`); link rows with a pipeline run to `/activity?pipeline=…&run=<pipeline_run_id>` and rows without one to the issue's own assignment surface; add `mission_id` to the journal URL keys and a "Journal for this issue →" link; project `mission_id` onto `/api/v1/runs` so a journal run can name its issue. This is the §1 answer. |
