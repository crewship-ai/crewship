...
- **Grandfathered crews tightened to restricted egress.** Existing crews still on the legacy `network_mode=free` default are backfilled to `restricted` on upgrade, so they stop getting unrestricted outbound access. A restricted crew still reaches all built-in LLM/CLI provider domains plus any it explicitly allowlists; if a crew genuinely needs open egress, re-open it to `free` (or add the domains it needs) from its network policy. See [Container isolation](/security/container-isolation).
