...
description: "Self-hosted runtime for AI coding agents. Each agent gets its own Linux container with full tooling, the whole environment runs on your hardware and is backed up by default — code, data, conversations, AI cost ledger, all of it."
---

# Welcome to Crewship

Crewship is a **self-hosted runtime for AI coding agents**. Each agent gets its own Linux container — a real machine where it can install services, run databases, mount volumes, and build a complete working system. The whole environment runs on **your hardware** and is packaged into portable, encrypted backups, so nothing your agents create ever disappears. Scale your Claude Code, Gemini, or OpenCode session into a fleet of governed agents with cost budgets, approval gates, and audit logs built in.
