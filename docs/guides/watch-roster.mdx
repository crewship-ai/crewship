...
`--crew` accepts either a crew slug or a CUID — slugs are resolved against `/api/v1/crews` before each render. `--watch` re-renders the table on an interval (short-polling — the server has no presence SSE stream yet); `--interval` sets the poll cadence (default `5s`, floored at `1s`). See [`crewship presence`](/cli/presence).
