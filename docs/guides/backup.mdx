...
#   such column: issue_counters.crew_id (1 row(s)).
...
The case that produces it is a bundle taken before a **non-additive** migration — one that re-keys or renames a table rather than adding to it. Restoring across one is not something the restore path can repair on its own: it does not know what the old column meant. What it can do is refuse to be quiet about it, so you check the tables it names instead of finding out months later.
