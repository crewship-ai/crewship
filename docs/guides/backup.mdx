...
- **Workspace migration to another host.** Moving a workspace from a dev VM to a prod host (or between two prod hosts) is exactly what bundles are for. Create on the source, scp the file, `crewship backup restore --as-workspace <new-slug>` on the destination, provision the crews, then `crewship backup restore … --files-only` to land their filesystem state. The `--as-workspace` rename avoids the "two `acme` workspaces colliding" hazard; the third step is not optional — without it the rows migrate and every crew's workspace files and memory tree stay behind.
...
| **`--as-workspace` / `--as-crew` rename** | Restore the bundle under a new slug instead of the original. Skips the docker phase — the crews it just created have no containers yet — and names the crews whose files are therefore not landed. Finish with `crewship crew provision` then `crewship backup restore … --files-only`. Avoids "two `acme` workspaces colliding" during DR drills. |
...
#     crewship crew provision <crew> -w ws_new123
#     crewship backup restore bundle.tar.zst -w ws_new123 --files-only

# 2. Provision the forked crews so there is somewhere for the files to land.
crewship crew provision engineering -w ws_new123
crewship crew provision research -w ws_new123
...
(exec is how every section is written now). Start it with `crewship crew provision engineering`
```

`crew provision` is the right command for this: it reconciles an existing
stopped container back to running rather than rebuilding it.
...
crewship crew provision <crew-slug> -w ws_new123
crewship crew provision <another-crew-slug> -w ws_new123
