...
|---|---|
| **running** | Pauses for the duration of the file collection (a paused filesystem is a consistent one), then resumes — on success, on failure, always. If the resume itself fails, the backup is reported as failed with `container left paused; manual unpause required` rather than pretending nothing happened. |
...
| **stopped / created** | Collects as-is, without starting it. Docker's archive API reads a stopped container's filesystem fine, and a stopped container is trivially consistent — nothing in it can write. The container stays stopped. |
