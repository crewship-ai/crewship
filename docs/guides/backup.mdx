...
Inside `payload.age`, workspace-scope bundles carry two distinct notions of "memory" — don't confuse them:

- **Per-crew agent memory** (`memory/<slug>` section) — the running agent's own markdown under the container's `/output` mount. Collected by the docker phase per crew.
...
crewship crew provision <crew-slug>
crewship crew provision <another-crew-slug>
```
