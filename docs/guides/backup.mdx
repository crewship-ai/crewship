...
| Port exposures | The row lands (description, status, history) with a re-keyed secret nobody can present. The exposure is bound to the source's container anyway — re-request it on the fork after its crews are started. |
| Pipeline webhooks | The row lands (name, pipeline binding, rate limit) with a re-keyed secret. Re-create the webhook on the fork — or rotate it — to hand senders the new URL. |
| Page public links / page webhooks | The rows land with re-keyed digests; the tokens were hashed at rest on the source (#1888) and are unrecoverable by design, so the capabilities arrive revoked. Mint new ones from the page. |
