...
A workspace on `crewship-prod.example.com` should produce a bundle every night, ship it to a separate backup host, and keep 14 days on disk locally as a fast-rollback safety net.
...
| `POST` | `/api/v1/backups/{filename}/verify` | End-to-end SHA-256 + AGE decrypt check. No DB writes. |
