...
| **missing from the daemon** | The crew's database rows are still backed up, the filesystem section is not, and the create log says so (`crew container not found on daemon; backing up DB rows only`). A container that vanishes *mid*-collection fails the whole backup — an incomplete export never gets sealed as a complete bundle. |
