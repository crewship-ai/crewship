...
- Otherwise it returns a **curated fallback** — a hand-maintained list (`config/models.json`, shared with the web pickers) of the
...
<Note>
  **Discovery is not pricing, and the two answer for different provider sets.**
  This endpoint covers the four providers above, from a live call or a curated
  list, and answers *"which model ids may I type"*. What a model **costs** comes
  from an embedded models.dev snapshot covering eight providers —
  `amazon-bedrock`, `anthropic`, `deepseek`, `google`, `mistral`, `openai`,
  `openrouter`, `xai` — which is neither a discovery surface nor a claim that
  Crewship can call those models. `crewship model price` queries it offline.
  See [Multi-provider LLM configuration](/guides/multi-provider-llm#the-model-catalog).

  `GOOGLE` is the visible seam between the two: its models are discoverable here
  and priced by the catalog, but there is no Gemini codec in the provider
  registry, so it cannot back an evaluator slot.
</Note>
