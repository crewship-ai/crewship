...
The agent runs through a non-login `docker exec`, which does not read `/etc/profile.d`. The build therefore puts the agent's own tool directories at the front of the image `PATH` — `/home/agent/.local/bin` and `/opt/mise/data/shims` — in the image `ENV`, in `/etc/environment` and in the runtime container environment, so every mise tool and every agent-installed CLI resolves without a login shell. The runtime prepends the same directories even when a login PATH was captured at build time: a login shell does not see mise's shims either. You do not need to spell these out in `containerEnv.PATH`; a `PATH` you set there is kept behind them.
...
The verified binaries are recorded with the image. Creating an agent on an adapter the image was not verified for, or moving an agent to one, enqueues a rebuild; a message sent meanwhile waits for the build and is delivered when it is ready.
