...
A typical session that adds one Python dependency emits exactly one `container.snapshot` entry (the new `pip` line), which then survives compaction the same way other observability entries do — see [Crew Journal — `container.snapshot`](/guides/crew-journal#container-snapshot--container-actuals) for the payload schema.
