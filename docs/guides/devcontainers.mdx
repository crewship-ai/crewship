...
description: "Bring your own Linux base image, declare devcontainer features and mise tools, and let Crewship provision a cached per-crew image. The sidecar is bind-mounted from the host — no custom runtime image needed."
icon: "container"
---
...
<Tabs>
  <Tab title="UI (Crew Wizard)">
    ...
    - Base image dropdown (populated from `/api/v1/admin/runtime-images` catalog)
    - Feature picker (searchable, powered by `/api/v1/admin/devcontainer-features`)
    - mise tool picker (searchable, powered by `/api/v1/admin/mise-tools`)
  ...
  <Tab title="REST API">
    ...
    Server-side validation rejects blobs with whitespace/control chars in the image ref, non-allowlisted feature IDs, or JSON > 64 KB.
  </Tab>
</Tabs>
...
Provisioning is **asynchronous**. The CLI command returns immediately after the backend enqueues a job; you then poll for status.
...
- To roll out a new sidecar or entrypoint across existing crews, rebuild the sidecar (`make build:sidecar`) and then restart each crew's container (`docker rm -f crewship-team-<slug>` — it will be recreated on the next message). A future `crewship crew restart <slug>` will formalize this.
...
The legacy image is no longer published by CI (`agent-runtime.yml` workflow deleted). Pulling its old tag still works but sits frozen; nothing in Crewship itself assumes it any more.
