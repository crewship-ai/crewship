...
| Route | Purpose |
|---|---|
| `/chat` | The chat index, and the **Chat** entry in the left rail and the mobile nav sheet. Lists recent conversations across every agent — merged client-side from `GET /agents` plus a capped fan-out of `GET /agents/{id}/chats`, newest activity first — with the agent roster beneath it. It holds no WebSocket: picking a thread is what opens a live connection, on the page below. |
