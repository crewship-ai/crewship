...
- **One reasoning pass = one thinking block.** While the model reasons, the block header shows a live shimmering "Thinking… Ns" timer; when the answer starts, it collapses to "Thought for N seconds". Transient progress lines never split a pass into multiple stub blocks — a streamed turn renders identically to the same turn reloaded from history.
