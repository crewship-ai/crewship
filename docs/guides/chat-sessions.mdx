...
Drops you into Viktor's most recent session. To start fresh, click **New conversation** in the left column. To jump to a specific past conversation, paste the deep-link URL `?session=<chatId>`. To approach from the canvas instead, use `/crews?crew=<slug>&agent=viktor` — see [Routes](#routes).
...
| Route | Purpose |
|---|---|
| `/chat` | The chat surface, and the **Chat** entry in the left rail and the mobile nav sheet. It is **not an index** — it opens straight into your freshest conversation and rewrites the address bar to that conversation's `/chat/<agentSlug>?session=<chatId>` shape, so it holds a live WebSocket from the moment it loads. The left column lists conversations across every agent, merged client-side from `GET /agents` plus a capped fan-out of `GET /agents/{id}/chats`, newest activity first. Agents are an attribute of a conversation here, not a level above it. |
...
<Accordion title="Footguns and surprising behaviour (14)">
...
it does not by itself grant manage-participants rights (see [Group chats](#group-chats)).
