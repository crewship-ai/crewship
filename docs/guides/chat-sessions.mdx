...
| `DELETE` | `/api/v1/agents/{agentId}/chats/{chatId}` | Delete the session, its messages, and read cursors — irreversible. Allowed for the chat's creator or anyone with agent-edit rights. `routine iterate` uses this to remove its one-shot grader/optimizer sessions. Returns `204`; cross-tenant or mis-nested ids return `404`. |
