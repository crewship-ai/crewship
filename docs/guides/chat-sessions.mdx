...
If no auxiliary model is configured, or the summarize call fails, the orchestrator falls back to plain newest-first truncation — the historical behavior — so a misconfigured or slow aux model can never fail a run. In that mode the oldest turns drop with a `…(truncated)` marker on the boundary message, exactly as before.
