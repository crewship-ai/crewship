...
| **Pending review** | `status='PENDING_REVIEW'` | Guided-crew hire that hasn't been operator-approved yet. Nothing starts the agent until status flips to `IDLE`: not a chat message, not `/assign`, not an `@mention`, not a mission task. |
