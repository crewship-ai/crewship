...
`PENDING_REVIEW`). An approval that nobody answers before its timeout leaves
the queue row `timeout` with the agent untouched. Both are recoverable with
...
been decided in the **current** cycle still returns `409` — a deny writes
`agents.expired_at` in the same transaction as its queue decision, and the
approve's conditional `UPDATE … WHERE expired_at IS NULL` is what rejects it.
