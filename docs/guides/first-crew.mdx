...
<Steps>
  ...
  <Step title="Add agents to the crew">
    ...
    - **Crew:** Optional for an `AGENT` — leave it empty for a **workspace-wide agent** (no crew; you can assign one later). A `LEAD` must belong to a crew.
  ...
  <Step title="Assign credentials">
    ...
    Agents created from templates or through the internal API have credentials auto-assigned (`autoAssignCredentials` matches on provider). Agents created via the CLI or UI assign credentials manually -- creation does not prompt for one. An agent that ends up with none, and no crew credential to inherit, is flagged on its overview canvas: the Credentials cell warns "No credential assigned" instead of staying quiet.
  </Step>
</Steps>

## The Crews screen

`/crews` is the fleet: crews on the left, the selected crew or agent on the
right.

- **The explorer** groups crews by what needs a person first — *Needs
  attention* (an agent in error, a failed or pending container build, a
  credential whose CLI is missing from the image, an agent waiting on your
  decision), then *Running*, then *Idle*. The idle majority folds after six
  behind "N more crews · Show all". The line under the search is the
  workspace's real size (`103 crews · 308 agents`, from the API's
  `X-Total-Count`), or "N crews · M agents match" while you search; a search
  that matches nothing says so and offers Clear.
- **Needs you** sits above a crew's tabs and only when there is something to
  do. Each row carries its verb: *Build now* / *Retry build* (rebuild the
  container image), *Install gh* (adds the devcontainer feature the readiness
  check named and rebuilds), *Connect* (a crew tool without a credential —
  opens it on Integrations), *Inspect* (an agent in error), *Review* (an agent
  waiting on a decision, in the inbox). A healthy crew shows no strip.
- **Settings → Integrations** shows each crew tool's transport, bound agents
  and auth state, with Connect beside a gap.
- **The roster** (no selection) is a table on a desktop and a one-column list
  on a phone; the onboarding guide agent is never listed.
- **A selected crew** opens on four tabs: **Overview** (the crew at a
  glance — its team, health and what needs a person; settings and delete live
  behind **Crew administration**), **Team** (its agents, and the people
  assigned to it), **Work** (the crew's issues, missions and shared routines) and **Memory** — the
  knowledge the crew and its agents have retained, by scope (*Agent knowledge*,
  *Shared with crew*, *Shared with workspace*), with **Refresh**, an export for
  the agent and crew scopes, a link to the pending **Knowledge proposals** in
  the inbox, and an **About me** view of what agents have saved about *you*
  (saved preferences and per-agent notes, each with a **Forget**). See
  [Agent memory](/guides/agent-memory).

Lists are windowed at 500 rows; when a fleet is larger the explorer says how
many rows are not loaded and loads them on ask.
...
|-------|------|---------|---------|
| Alex | Engineering Lead (LEAD) | FULL | CLAUDE_CODE |
| Sam | Backend Engineer (AGENT) | CODING | CLAUDE_CODE |
| Robin | Frontend Engineer (AGENT) | CODING | CLAUDE_CODE |
