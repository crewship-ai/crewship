...
<Warning>
**Every memory route requires the per-agent token.** The whole memory surface —
...
provisioned (every crew started by a current orchestrator). A request without
it is refused with `403 {"error":"per-agent token required"}`, because several
agents share one sidecar and a token-less call cannot be attributed to a
member. Crews that predate per-agent tokens keep working unauthenticated.
The token is in each agent's environment, so `-H "Authorization: Bearer
