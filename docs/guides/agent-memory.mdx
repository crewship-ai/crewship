---
title: "Memory System"
description: "4-tier persistent memory (agent, crew, workspace, session) with FTS5 search, dynamic budget allocation, and multi-scope queries."
icon: "brain"
---

<Note>
  The **workspace memory** tier is read and written only by the deprecated
  Captain and COORDINATOR. See [Captain](/guides/captain) and
  [Coordinator](/guides/coordinator) for deprecation context. Agent, crew,
  and session memory tiers are **not** deprecated.
</Note>

# Memory System

Crewship has a 4-tier memory hierarchy that gives agents persistent knowledge across sessions, shared context within crews, and organization-wide visibility for coordinators.
...
```
WORKSPACE MEMORY (Coordinator/Captain)
  Host: ~/.crewship/memory/{workspace-id}/
  Who writes: Coordinator        Who reads: Coordinator only
  Budget: 20% of system prompt
...
<Tip>
Budget is **dynamic**. If crew memory is empty, the agent reclaims the full budget. If workspace memory is empty (non-coordinator), crew and agent split the whole budget.
</Tip>
...
Lives on the host at `~/.crewship/memory/{workspace-id}/`. Only the Coordinator (Captain) agent reads and writes workspace memory.

```
~/.crewship/memory/{workspace-id}/
  WORKSPACE.md          # Org-level strategy, KPIs, policies
  crews/
    dev.md              # Crew summaries
    marketing.md
...
[WORKSPACE MEMORY]                    (Coordinator only)
--- WORKSPACE.md ---
...org-level strategy...
...
| Agent Role | Workspace | Crew (max) | Agent (remainder) |
|------------|-----------|------------|-------------------|
| **Coordinator** | 20% | 40% of remaining | rest |
| **Lead** (in crew) | -- | 40% | rest |
| **Agent** (in crew) | -- | 40% | rest |
| **Solo agent** | -- | -- | 100% |

<Accordion title="Budget examples">
**Coordinator with all tiers populated:**
- Workspace: 3,000 chars (20%)
- Crew: 4,800 chars (40% of 12,000 remaining)
- Agent: 7,200 chars
...
# Check workspace memory status
crewship memory status -p ~/.crewship/memory/ws-123 -S workspace
...
| Operation | Agent | Lead | Coordinator |
|-----------|-------|------|-------------|
| Read own agent memory | Yes | Yes | Yes |
| Write own agent memory | Yes | Yes | Yes |
| Read crew shared memory | Yes | Yes | Yes |
| Write crew shared memory | No | **Yes** | No |
| Read workspace memory | No | No | **Yes** |
| Write workspace memory | No | No | **Yes** |
