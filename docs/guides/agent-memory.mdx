...
| **Budget** | The character allowance for memory content in the system prompt. Default 15,000 chars. The crew tier is capped at 40% of total; the agent tier reclaims the remainder. Empty tiers reclaim their entire share. A `[MEMORY BUDGET]` meter renders per-tier and total usage into the prompt itself — see [Reading Memory](#2-reading-memory-system-prompt-injection). |
...
[MEMORY BUDGET]
Crew: 1980 of 6000 chars, 33%
Agent: 6432 of 9000 chars, 71%
Total: 8412 of 15000 chars, 56%
[END MEMORY BUDGET]
...
carries a one-line-per-tier usage meter: how many of the wake-time character
budget's chars each allocated tier (`Pins`, `Crew`, `Workspace`, `Agent` —
only the tiers actually allocated for this run) actually used, plus a
`Total` line. The wording mirrors `memory.write`'s own overflow-guidance
usage string byte for byte (`<used> of <cap> bytes, <pct>%`, described
above) except for the unit — chars in the assembled prompt, not bytes on a
tier's file:
...
budget — the default 15,000-char wake-time cap, split 10% pins / 40% crew /
15% workspace / agent remainder — the meter says so explicitly instead of
letting the model believe it saw the whole snapshot:

```
[MEMORY BUDGET]
Agent: 9000 of 9000 chars, 100%
Total: 9000 of 9000 chars, 100%
