...
<Steps>
  ...
  <Step title="Search explicitly when you need to">
    ...
    <Warning>
      The legacy `POST /memory/search` route is a **boot-agent-only** surface — the agent whose exec started the crew's shared sidecar. Any other member of the crew gets `403` on `scope=agent` and `scope=both` there (`scope=crew` is shared and always works); see the warning at the end of [Searching Memory](#3-searching-memory).
      ...
      There, `scope` is one of `agent`, `crew`, `both`. With `both`, results from the two engines are merged and sorted by BM25 score, each carrying its origin. (RRF fusion of sparse + dense scores applies on the `hybrid=true` path, when an embedding model is configured — not to the default `both` merge.)
    </Warning>
  </Step>
...
and searches that agent's own tier.

The sidecar HTTP API below takes a `scope` parameter and is the **boot agent's**
surface: it serves the tier of the agent the sidecar was booted for. If your crew
shares a sidecar and you are not that agent, these routes answer `403` and you
want `/mcp/memory/<your-slug>` (or the MCP tools) instead — see the warning at
...
Crews that predate per-agent tokens keep working unauthenticated. The token is
in each agent's environment, so `-H "Authorization: Bearer
...
<Warning>
**The legacy `/memory/*` routes serve the boot agent's tier only.** They resolve
the memory directory from the sidecar's boot agent and do not switch tiers per
caller. A crew member that is *not* the boot agent is therefore refused on those
five routes, with a pointer to its own surface:

```
403 {"error":"this route serves only the boot agent's memory tier;
              use /mcp/memory/<your-slug> for your own"}
```

Use `/mcp/memory/<slug>` (or the memory MCP tools, which every adapter injects
automatically) — that transport resolves the acting agent from its token and
serves that agent's own tier. Serving each agent its own tier on the legacy
routes as well is planned; until then they are a boot-agent surface, not a
per-agent one.
</Warning>
...
- **Crew reindex lags up to 60 seconds.** The lead sidecar reindexes the crew tier on a 60-second tick. Writes to `CREW.md` or `topics/*.md` are searchable on the next tick, not immediately — if you need instant visibility, hit `POST /memory/reindex?scope=crew`. The crew tier is shared, so this works from any member of the crew — unlike `scope=agent` on that route, which serves the boot agent's tier only.
