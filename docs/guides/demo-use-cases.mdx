...
| 10 | `pack-ci-watch` | github | The legacy [nightly CI watch pack](/guides/demo-packs#nightly-ci-watch): the delivered script, the token-zero probe, `crewship seed verify --pack ci-watch`. |
| 11 | `pack-docs-drift` | github | The legacy [docs drift pack](/guides/demo-packs#docs-drift) through `seed verify --pack`, with this checkout as the fact-check source. |
| 12 | `chat-reply-inbox` | model | A saved reply from Morgan appears in Inbox → Messages when the recipient is not watching the chat live. |
...
account. The default seed is the Harbor Goods business demo — local sample
data, no third-party accounts — and `crewship seed verify` is its acceptance
test. The use cases with a model need the seed's model credential to be
...
The pack use cases (10, 11) additionally need a workspace that still carries
the legacy pack content — the current seed installs the business stories and
not the packs — and the GitHub token at seed time; see
[Demo packs → Credentials](/guides/demo-packs#credentials). Without either
they SKIP, and so does `seed verify --pack`.
