...
description: "Thirteen product use cases as thirteen scripts — run one at a time against a seeded install, each narrated for an audience and verified by assertions."
icon: "list-checks"
...
thirteen units under `scripts/demo/`, one script per use case, each runnable
...
scripts/demo/run.sh --all              # run all thirteen, one summary table
...
| 13 | `chat-reply-inbox` | model | A saved reply from Morgan appears in Inbox → Messages when the recipient is not watching the chat live. |
