...
## How an Agent Requests a New Credential

Agents **cannot create or store credentials themselves** — credential lifecycle
requires human attribution (the internal mutation route rejects any write without
an `X-Caller-User-Id`). The `/secrets/{agent-slug}/` files are read-only, and writing
a file there does **not** register anything in the vault.

When an autonomous agent needs a credential it doesn't have — or needs to record one
for the crew, such as a connection string for a service it just provisioned — it raises
a **`CREDENTIAL` escalation** through the sidecar. A human then resolves it (via the
[inbox](/guides/inbox) or `crewship escalation resolve`) and the value is stored encrypted
in the vault:

```bash
curl -s -X POST http://localhost:9119/escalate \
  -H "Content-Type: application/json" \
  -d '{"from":"<agent-slug>","reason":"<what credential and why>","context":"<details>","type":"CREDENTIAL"}'
```

The agent's call blocks until a human responds (up to 5 minutes). This guidance is part
of the agent's operational preamble, so every agent — lead, non-lead, and standalone —
knows to escalate rather than improvise a local file (which would silently fail to persist).
