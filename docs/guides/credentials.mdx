...
<Note>
`from` is **advisory**. The sidecar attributes the escalation to its own bound
agent identity, ignoring the value in the body — so one agent in a crew
container cannot raise an escalation (or propose a credential) attributed to a
sibling. Attribution is non-repudiable regardless of what the caller sends.
</Note>
