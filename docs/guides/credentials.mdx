...
Enforcement is at the injection point, not just the UI. The clearest case is
`/keeper/execute` (the L3/L4 per-command injection path): the credential lookup
and the pre-injection re-validation both require `expires_at IS NULL OR
expires_at > now`, so a lease that lapsed mid-session — even after the Keeper
already decided **ALLOW** — is treated exactly like a missing assignment and the
secret is never injected. A captured lease is unusable after its TTL.
