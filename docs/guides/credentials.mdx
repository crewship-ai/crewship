...
- **Keeper enabled** — the `SECRET` is **not** written to the agent's
  ...
  a compromised process to scrape. Each withheld `SECRET` is recorded in the
  server log at `WARN` (env-var name only, never the value), so the gate is
  observable rather than a silent skip. Withholding also happens once at the
  config resolver, so the plaintext never leaves the server process — the env,
  file, and MCP delivery paths gate on Keeper as defense-in-depth.
