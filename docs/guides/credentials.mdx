...
- **Keeper enabled** — the `SECRET` is **not** written to the agent's
  ...
  trail. There is no file to read and nothing under `/secrets/{agent-slug}/` for
  a compromised process to scrape.

This gate is **`SECRET`-only**. `CLI_TOKEN`, `GENERIC_SECRET`, `USERPASS`,
`SSH_KEY`, and `CERTIFICATE` are always delivered as files regardless of Keeper
state — CLI tools and generic secret consumers are expected to read them from
disk, and the prompt makes no withhold claim for them.

<Warning>
**Known gap — MCP config references.** The env-withholding above covers the
standard credential-delivery paths (files and the direct env builder). It does
**not** yet cover MCP credential injection: if a `SECRET`'s env-var name is
referenced by an MCP server config (e.g. `Authorization: Bearer ${DB_PASSWORD}`),
Crewship still passes the value into that MCP process's environment even when
Keeper is enabled — the MCP process runs inside the agent container and cannot
call the Keeper API itself, so withholding the value would simply break the MCP
server. Until this is resolved, **do not reference `SECRET`-typed credentials
from MCP configs when you rely on Keeper withholding** — the value is readable in
the agent's environment and bypasses the `/keeper/*` audit trail. Closing the gap
(routing MCP secrets through the sidecar, or rejecting `SECRET`-typed MCP refs
under Keeper) is a tracked follow-up.
</Warning>
