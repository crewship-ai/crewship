...
<Note>
`from` must name an agent **in the same crew** — the sidecar validates it
against the crew roster and rejects a slug outside the crew (so an escalation
can't be attributed to a non-member). Note the per-crew sidecar is shared across
the crew's agents, so it cannot cryptographically prove which sibling sent a
given call; full per-agent non-repudiation requires per-agent auth.
</Note>
