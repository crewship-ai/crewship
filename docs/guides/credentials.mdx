...
- **An `L1`–`L4` chip beside the name** — in the left rail, in the credential
  table, in the detail header, and on the overview's lists. A credential whose
  server did not report a tier shows `L?`, never `L1`: "we were not told" and
  "unguarded" are different claims.
