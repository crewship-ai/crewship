...
Fields are **not** delivered into containers yet. `credentials.encrypted_value`
and `credentials.username` remain the source for every existing injection path,
unchanged, and nothing was backfilled into the field table — so adding fields
to a credential cannot affect a running agent.
