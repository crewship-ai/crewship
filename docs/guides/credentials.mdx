...
- **Keeper enabled** — the `SECRET` is **not** written to the agent's
  ...
  (`/keeper/request` for the raw value, or `/keeper/execute` to run a command
  with the secret injected), which enforces access control and writes an audit
  trail. Withholding happens **once, at the config resolver** — the plaintext is
