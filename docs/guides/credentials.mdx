...
Enforcement is at the injection point, not just the UI. It covers **both**
credential-injection paths:

- **`/keeper/execute`** (the L3/L4 per-command injection path): the credential
  lookup and the pre-injection re-validation both require `expires_at IS NULL OR
  expires_at > now`, so a lease that lapsed mid-session — even after the Keeper
  already decided **ALLOW** — is treated exactly like a missing assignment and
  the secret is never injected.
- **Boot-delivered provider keys** (`API_KEY` / `AI_CLI_TOKEN`, e.g. the
  Anthropic key handed to an agent container at start): these expire too. The
  internal credential listing the sidecar consumes at boot — and the sidecar's
  credential reaper (which reconciles the in-memory store every ~60s) — apply the
  same `expires_at > now` gate. So a leased provider key is not delivered once
  its TTL has already lapsed, and a key delivered while valid is **evicted from
  the container's in-memory store within one reaper interval** of its lease
  lapsing. The container does not keep a leased key for its whole lifetime.

A captured lease is unusable after its TTL, whichever path would inject it.

Lease state is visible on the agent's credential list:

```bash
crewship agent credentials viktor
# ID            NAME           PROVIDER   TYPE      ENV VAR              LEASE
# clx…          prod-ssh       NONE       SECRET    PROD_SSH             expires 2026-07-23T11:30:00Z
# clx…          anthropic-key  ANTHROPIC  API_KEY   ANTHROPIC_API_KEY    standing
```

An expired lease renders as `EXPIRED <timestamp>`.
