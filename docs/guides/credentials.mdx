...
| `ENDPOINT_URL` | An OpenAI-compatible model endpoint (Ollama/LM Studio/llama.cpp/remote inference), configured like a key but **not a secret** — the URL is shown on read. Used to point OpenCode agents at a local model without a server env var. Workspace-scoped as the default; assign to an agent to override. | `http://host.docker.internal:11434/v1` |
