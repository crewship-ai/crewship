...
### 1. Cursor `--approve-mcps`

Without it, MCP servers are LISTED in `cursor-agent mcp list` but NEVER INVOKED in `--print` mode (forum #143045 + #148397). Crewship's adapter adds the flag automatically when any MCP source is configured.
