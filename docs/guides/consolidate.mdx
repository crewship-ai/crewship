...
If you want to tune journal compaction retention, edit `RunnerOptions.CompactionOlderThan` and restart. Per-workspace retention for `memory_versions` (a separate sweep, run by the Iter 4 coordinator) is configurable at runtime via `PATCH /api/v1/admin/memory/config` — see [Memory Observability — Per-workspace retention](/guides/memory-observability#per-workspace-retention).
