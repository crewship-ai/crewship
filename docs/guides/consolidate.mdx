...
The summarizer resolves the **`curator` auxiliary slot** -- the slot the Judge
...
```bash
crewship keeper aux set curator --provider anthropic --model claude-haiku-4-5
crewship keeper aux set curator --provider ollama --model qwen2.5:7b
```

If that slot has no buildable provider -- the common case being the shipped
...
If you want to tune journal compaction retention, edit `RunnerOptions.CompactionOlderThan` and restart. Per-workspace retention for `memory_versions` (a separate Iter 4 worker sweep) is configurable at runtime via `PATCH /api/v1/admin/memory/config` — see [Memory Observability — Per-workspace retention](/guides/memory-observability#per-workspace-retention).
...
| Condition | Response |
|---|---|
| Run started | `{"accepted": true, "triggered": true, "worker_id": "csd_..."}` |
