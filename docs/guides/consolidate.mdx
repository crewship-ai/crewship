...
Compaction target: `info` and `notice` entries older than 30 days that are NOT type `summary.generated`, `memory.consolidated`, or any checkpoint/fork/approval/regression. Rolls up into one `system.compaction` entry per (crew, day) bucket with a count and kind breakdown in the payload, then deletes the originals in a single transaction.
