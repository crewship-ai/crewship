...
The sequence is **stop → swap → start → health-check**:

1. **Stop** the systemd unit (so the binary is swapped without a live process).
2. **Swap** the binary + companions in place (checksum-verified; the previous
   binary is kept as `<binary>.bak`).
3. **Start** the unit on the new binary — migrations run here, behind the
   automatic pre-migration snapshot.
4. **Health-check** `http://127.0.0.1:<CREWSHIP_PORT>/healthz` until it returns
...
binary from `.bak`, and starts it again — so a bad release doesn't leave your
server down. Because migrations run in step 3, a rollback may also need the
**schema** rolled back: if the restored old binary then refuses to boot with a
version-skew error, restore the pre-migration snapshot too:
