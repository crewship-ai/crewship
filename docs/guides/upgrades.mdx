...
2. **Restore the matching pre-migration snapshot** with `crewship db restore-snapshot`:
...
`restore-snapshot` refuses to run while a local server is up, only accepts a
genuine `*.pre-migrate-*.bak` snapshot of your database, and copies the
current database aside to `<db>.before-restore-<ts>` first — so the restore is
itself reversible. Then start the older binary; it boots against the restored
schema.
