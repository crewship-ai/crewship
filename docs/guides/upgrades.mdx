...
next start, behind an automatic snapshot so a bad upgrade is a one-step
rollback.
...
|---|---|
| **Homebrew** (`brew install crewship-ai/tap/crewship`) | Runs `brew upgrade crewship` for you. |
| **Installer / tarball** (`install.sh`, or a downloaded archive) | Downloads the release asset for your OS/arch, **verifies its SHA-256** against the signed `checksums.txt`, and **atomically swaps** the binary in place — plus the bundled `crewship-sidecar` and `entrypoint.sh` when they sit alongside it. Then it re-runs the new binary's `version` as a sanity check. |
...
half-written binary. `self-update` only acts when a newer release actually
exists — otherwise it reports you're already current and exits.
