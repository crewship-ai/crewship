...
| **Status, Priority** | Pickers in the Properties card. |
| **Owner, Delegate** | Two separate chips on the identity card and two rows in Properties: the **owner** is the accountable person, the **delegate** is the agent doing the work. They are independent — delegating never clears the owner, and an owner is not enough to Start; that needs a delegate. `crewship issue update <id> --assignee <email> --assignee-type user` sets the owner, `--assignee <agent>` the delegate. |
| **Current work** | Who holds the issue right now (a person or an agent), the latest execution's stage and workers, the acceptance policy, and the buttons that move work between people and agents. See [Work, hand-offs and Lead review](#work-hand-offs-and-lead-review). |
...
The **Current work** card is the issue's work contract (#2448): at any
moment the issue is either *with an agent* (`work_mode: agent` — Start may
...
1. Workers run (or the bound routine does). Stage `working`.
2. When every worker has finished, the Lead gets a review task with the
   acceptance criteria and the results. Stage `reviewing`.
3. The Lead answers `approve`, `request_changes` naming a worker, or
   `needs_human`. `approve` on verified work is `accepted` (issue to
   `REVIEW`, or `DONE` when acceptance is not required). `request_changes`
   opens a second attempt for that worker at once; after the second attempt,
   or on `needs_human`, or when the Lead approved despite a failed worker,
   the stage is `needs_human` and the issue waits in `REVIEW` with an inbox
   notification.
4. **Approve** / **Request changes** on the identity card are your half. A
   `request_changes` returns the issue to `TODO` and marks the execution
   `changes_requested`; the next Start opens a fresh attempt.

A take-over, hand-off, brief change or Stop while an execution is
...
see [CLI → issue](/cli/issue) and the
[API reference](/api-reference/issues#transfer-issue-work).
...
child. Add `?force=true` to the request (`crewship issue update <id>
--status DONE --force`; the check lives on the same `PATCH` every status
...
- **Plain Stop is cooperative, not instant — a hard variant exists now.**
  ...
  --hard`) additionally ends that step's own tmux session (`tmux
  kill-session -t agent-{slug}`) and, if it is still alive after a short
  grace period, sends `KILL` to the session's process group — both as a
  new exec into its own container, never by host pid and never a
  container-level kill, so a sibling agent sharing the same crew container
  is unaffected. A run that never had a tmux session is reported
  `NOT_FOUND` rather than signalled blind. Stop also
