...
so it gets a fresh key and fires normally. Deferred (pending) runs are keyed on
their row id and likewise fire once. If the idempotency store is unreachable a
scheduled fire fails **closed** (recorded `FAILED` + alerted), never double-run.
