...
**Enabling.** Set `COMPOSIO_API_KEY` (a project API key) on the server; presence of the key auto-enables the provider. Optionally pin `COMPOSIO_BASE_URL`. Billing is by tool calls (Composio free tier = 20K/mo), not by number of users or connected accounts.
