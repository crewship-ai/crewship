...
<Note>
**The managed Composio UI is now the default.** The `/integrations` page has
three tabs — **Outgoing notifications**, **Incoming webhooks** and
**Tools (MCP)**. Tools (MCP) is the managed integration platform (Composio):
a KPI header plus sections for App catalog, Connected accounts, Agent access,
Tools, Triggers, and MCP endpoints — so you can connect apps via OAuth without
standing up and maintaining your own MCP servers. Incoming webhooks is where
routine, agent and Page endpoints that *receive* HTTP requests are created
(see [Incoming webhooks](#incoming-webhooks) below).
...
Crewship supports two integration mechanisms: **OAuth 2.0 connections** for service authentication and **MCP (Model Context Protocol) servers** for extending agent tool capabilities. A third, simpler surface — **incoming webhooks** — lets an external service start work in Crewship with a signed HTTP request and no connected account at all.

## Incoming webhooks

**Integrations → Incoming webhooks** lists every endpoint an outside service
can POST to, grouped by target: **Endpoints** (all of them), then
**Routines**, **Agents** and **Pages**. A routine endpoint runs a saved
routine with the request body as its `event` input; an agent endpoint starts
a background run; a Page endpoint writes a webhook or script panel. Select a
target to see its endpoints and, for routines and agents, its **Recent
receipts** — the deliveries the server accepted, with the run or work item
each produced. The selection is linkable
(`/integrations?tab=incoming&section=routine&target=<slug>`).

**Creating one.** Click **Add integration → Incoming webhook** (or **Add
endpoint** inside a target), pick the target kind and target, and name the
endpoint. Routine endpoints have an **Advanced settings → Webhook format**
selector:

| Format | Use it for | Signature the sender must provide |
|---|---|---|
| **Signed JSON** (default; `ingress_profile: crewship`) | Any service you control — a script, a CI job, another Crewship | `X-Crewship-Signature: sha256=<HMAC-SHA256 of the raw body>`, optionally with `X-Crewship-Timestamp` |
| **GitHub pull requests** (`ingress_profile: github`) | A GitHub repository webhook; only `pull_request` `opened` / `synchronize` / `reopened` start the routine, `ping` is acknowledged | GitHub's own `X-Hub-Signature-256` — paste the secret into the repository's webhook settings, content type `application/json`, event **Pull requests** |

The format is fixed once the endpoint exists; to switch a sender's protocol,
create a new endpoint. Collapsing Advanced settings does not change the
selection.

**Where the URL and secret show.** On **Create endpoint** the dialog shows the
**Receiving URL** and the **Signing secret** once, each with a copy button.
The routine URL contains the token and is masked afterwards; the secret is
never displayed again. Copy both now. A GitHub-format URL ends in
`/github-pull-request` — the suffix is part of what you paste into GitHub.
Later, an endpoint row offers enable/disable, **Copy URL** (agents only —
their URL holds no secret), **Rotate secret** (a new secret shown once; the
URL survives) and **Delete** (senders get `404`; existing receipts are kept).

**From the CLI.** The same endpoints are `crewship routine webhooks create
--slug <routine> [--ingress-profile github]`, `crewship routine webhooks fire
<url> --secret … --body …` to send a signed test delivery, and `crewship
routine webhooks receipts list --webhook <id>` to read what arrived — see
[`crewship routine webhooks`](/cli/routine#crewship-routine-webhooks) and the
...
|---|---|---|
| `Composio rejected the configured API key (… HTTP 401/403 …)` | The effective key (workspace or `COMPOSIO_API_KEY` env) is a placeholder, revoked, or from the wrong project | `crewship integration composio key set --key ak_…`, or fix the server env |
| `Composio rate-limited the request (… HTTP 429 …)` | Project hit Composio's rate limit | Retry shortly |
| `Composio upstream error (… HTTP 5xx …)` | Composio itself is degraded | Retry shortly; check Composio status |
| `Composio rejected the request (… HTTP 4xx …)` | Bad request parameters (e.g. an invalid toolkit slug) | The included upstream message says what to change |
| `Composio API unreachable (…)` | Network failure between the Crewship server and Composio (DNS, dial, TLS, timeout) | Check the server's outbound connectivity and `COMPOSIO_BASE_URL` |
...
### Crew tools on /integrations

**Integrations → Tools (MCP) → Crew tools** lists every crew-scoped MCP server
in the workspace: the crew it belongs to (a link to that crew's Settings), how
many agents may call it, and its auth state as a pill — *Connected*, *No
credential*, *Expired*, or *No auth needed* for a `stdio` server that takes
none. The gaps sort first.

**Connect** on a row without a credential binds one to every agent in the crew
(`PATCH /agents/{id}/integrations/{binding}` for agents that already have a
binding, `POST /agents/{id}/integrations` for the rest), either from the vault
or through OAuth for an HTTP endpoint. A crew's own **Needs you** strip and its
Settings → Integrations list link here as `/integrations?tab=tools&section=crew-tools&server=<id>`,
which opens that row connecting.

The same list on the CLI: `crewship integration crews-overview`.

### Who gets which server

Every workspace- and crew-scoped MCP server stores its audience in a
`default_access` column:

| Value | Who resolves the server |
| --- | --- |
| `all` (default for new servers) | every agent in the server's scope |
| `bound-only` | only agents with an explicit agent binding |

An agent binding attaches a credential, a config override or an opt-out to
**one** agent's use of a server. It never changes the audience, so granting one
agent an integration cannot take it away from anyone else.

```bash
crewship integration access github bound-only        # workspace scope
crewship integration crew update eng <id> --access bound-only   # crew scope
crewship integration resolve pepa                    # what one agent actually gets
```

<Warning>
Before Crewship 2026.08 the audience was inferred from the number of bindings:
a server nobody was bound to resolved for everyone, and the first binding
anywhere in the workspace silently revoked it from every other agent. Upgrading
freezes each existing server at the audience it effectively had — servers that
already had a binding become `bound-only`, the rest become `all` — so nothing
changes at the moment of the upgrade.
</Warning>
...
[MCP guide](/guides/mcp-multi-cli#stdio-servers-local-subprocess).
