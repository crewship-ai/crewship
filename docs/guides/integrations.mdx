...
Crewship syncs with the official MCP registry at `registry.modelcontextprotocol.io/v0/servers`. The `SyncMCPRegistry` function (`internal/api/mcp_registry.go`) fetches and caches all registered servers locally.
