...
stored refresh token (`docs/prd/provider-logins.md` §5.3) is what makes a
Google login usable for a long run; until it lands, a run that outlives the
token fails on its next request. The file shape is taken from the Gemini CLI
