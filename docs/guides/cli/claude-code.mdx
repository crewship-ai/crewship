...
`--tools` is **always** passed with the agent's per-profile built-in allowlist (`MINIMAL` → `Read,Glob,Grep`; `CODING` → `+Write,Edit,Bash,WebFetch,WebSearch`; `FULL` → `+NotebookEdit`). This removes harness-internal tools (`TaskCreate`, `ToolSearch`, `Agent`, `Workflow`, `Cron*`, …) that have no Crewship backing from the agent's context. MCP tools are unaffected. See [Tool profiles & the built-in tool allowlist](/guides/cli-adapters#tool-profiles-the-built-in-tool-allowlist).
