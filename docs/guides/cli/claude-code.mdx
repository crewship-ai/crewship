...
`--bare` is recommended by Anthropic for SDK use (will become the `-p` default). It disables auto-discovery of `~/.claude/CLAUDE.md`, plugins, hooks, skills, MCP — Crewship sets these explicitly via flags.
