...
|---------|-------------|---------|
| **Watchdog enabled** | Master toggle for behavior monitoring + DENY notifications in this workspace | Inherits the server config until explicitly set |
| **Security contact** | A named OWNER/ADMIN whose inbox receives Keeper findings directly (realtime) | Unset — findings fan out to the MANAGER role |
...
Keeper findings push an `inbox.updated` event in realtime, so the bell badge
updates without a refresh; a configured security contact additionally gets a
direct `notification.created` ping on their user channel.

<Note>
  A workspace with no explicit setting **inherits the server config** —
  upgrading Crewship never silently changes what the watchdog does. The
  workspace toggle governs the *behavioral* layer only: the credential-access
  gatekeeper enforcement stays server-configured (`KEEPER_ENABLED`), so an
  in-app toggle can never weaken credential isolation.
</Note>
