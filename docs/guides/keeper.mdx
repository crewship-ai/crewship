...
The model that backs Keeper — the credential access gatekeeper **and** the
auxiliary evaluators — is a **per-workspace, vault-backed setting**, chosen
in-app rather than only via server env. This is what makes a **no-API-key
Keeper** possible: point governance at a local Ollama classifier and the whole
watchdog runs on your own hardware.
