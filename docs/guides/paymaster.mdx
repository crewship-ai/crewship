...
5. **`telemetry.LLMMiddleware` stamps** the same counts as `gen_ai.usage.cached_input_tokens` + `gen_ai.usage.cache_creation_tokens` on the `llm.call` span — see [Tracing](/guides/tracing).
