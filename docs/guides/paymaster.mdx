...
CREATE TABLE cost_ledger (
  ...
  quota_window          TEXT,             -- 'requests' | 'tokens' (most-restrictive)
  ...
  rate_input_per_m      REAL NOT NULL DEFAULT 0,
  rate_output_per_m     REAL NOT NULL DEFAULT 0,
  rate_cached_in_per_m  REAL NOT NULL DEFAULT 0,
...
-- [v62] Partial index for the Subscriptions panel — avoids a full-ledger scan
-- when grouping flat-rate rows by plan.
CREATE INDEX idx_cost_ledger_workspace_billing
  ON cost_ledger (workspace_id, billing_mode);
...
func EnforceQuota(
    ...
    window QuotaWindow,     // "requests" | "tokens" — display only
...
Every metered row writes the four `rate_*_per_m` columns from `RateCard(provider, model)` at the moment of write. When `pricing.go` is later edited (a provider repriced, a new model was added), historical rollups stay consistent: the old row continues to imply its old rate. New rollup queries can either trust `cost_usd` directly or recompute from `rate_*_per_m * tokens` to verify nothing has drifted.

Zero is honest for ollama/local (genuinely free) and for unknown-provider lookups (we couldn't price). NULL would erase that distinction, so the columns are NOT NULL with default 0.
