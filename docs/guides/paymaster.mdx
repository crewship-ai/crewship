...
It is no longer the only rate source: an **embedded models.dev snapshot** (`internal/modelcatalog`, 8 providers) sits underneath it and prices models the table has never heard of. The table stays authoritative — see [The four-step lookup](#the-four-step-lookup) — and `crewship model price --provider <p> --model <m>` reports which source answered for any pair.
