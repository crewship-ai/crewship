...
contract. In short: scripts byte-identical to the seed, no routine of the
pack already FAILED in this workspace, the probe against an independent read
of the source, the agent's counts against the probe, no secret in the report,
the notification in the inbox, the panels written by that run.
