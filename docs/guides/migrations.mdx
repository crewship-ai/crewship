...
**At review time**, `scripts/lint-migrations` (CI job "Lint migrations") fingerprints every migration entry — version, name, *and the body of the referenced SQL* — and compares against the PR's base ref. Changing an entry that already exists on `main` fails the check. So a forked schema does not reach `main`.
