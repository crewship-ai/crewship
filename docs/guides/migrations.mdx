...
SQLite has no HMAC and no SHA-256, so the digest cannot be computed in a
migration. The file adds the column and the index; the hashing runs the first
time the owning component is constructed, which on the server path is before
...
**The key comes from `ENCRYPTION_KEY`.** Not from
`CREWSHIP_ADMIN_TOKEN_HMAC_KEY`, which is deliberately optional —
`internal/api/cli_token.go` disables the entire ADMIN tier when it is unset —
and a migration that has to preserve every already-published URL on every
existing instance cannot depend on a variable an instance is allowed not to
have. `ENCRYPTION_KEY` cannot be absent on a running server: `crewship start`
calls `secrets.LoadOrGenerate` before the database is opened, which generates
and persists it when missing, and then refuses to boot at all if
`encryption.VerifyCurrentKey` cannot resolve it. The value actually used is a
subkey derived from it (`HMAC-SHA256(ENCRYPTION_KEY, "crewship/capability-token-hash/v1")`),
so the AES key is never used directly as an HMAC key.

Every stored digest carries a scheme prefix (`hk1:` keyed, `sh1:` unkeyed
fallback) and lookups accept either, so an instance that had no key when a row
was written keeps resolving it once one appears, and a future key rotation adds
