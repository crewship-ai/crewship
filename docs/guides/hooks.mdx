...
- `post_tool_call` -- the observation exists (`orchestrator.PostToolCallObserver`, wired to the built-in F4.2 behavior monitor), but nothing forwards it to `hooks.Dispatch` for user-registered hooks yet.
