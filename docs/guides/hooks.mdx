...
- `pre_agent_start`, `post_agent_stop` -- orchestrator agent lifecycle.
- `on_approval_requested` -- emitted by `harbormaster.Enqueue` when a new request lands in the queue.
...
Shell hooks require `OWNER` role at registration time -- an ADMIN cannot create a shell hook because arbitrary code execution on the host is a privilege escalation vector. The store enforces this:

```go
return nil, hooks.ErrShellHookNotAllowed // wrong role at register time
```

Also requires `allowedShell=true` on the hook row, confirming the operator consciously allowed shell execution for this entry.
