...
Open an agent, go to **Memory**, and use **Export** on the tab bar. It downloads the whole scope as an OKF bundle — the same archive the CLI writes, built by the server so the browser never has to know the format.

The button exports the scope you are looking at: the agent's own memory from an agent canvas, and the crew-shared tier when no agent is named.

<Note>
**Import is CLI-only, on purpose.** In the terminal the plan is the default and `--apply` is a second, deliberate step. As a button that becomes skippable, and the plan is the only thing standing between an operator and overwriting an agent's memory. Tracked on #1748.
</Note>
...
| Method | Route | Role |
|---|---|---|
| `GET` | `/api/v1/memory/export?crew_id=&agent_slug=` | OWNER/ADMIN — JSON document payload |
| `GET` | `/api/v1/memory/export?format=zip&crew_id=…&agent_slug=…` | OWNER/ADMIN — `application/zip` OKF download |
| `POST` | `/api/v1/memory/import` | OWNER/ADMIN — mapped documents only |

Export JSON is `{ "format", "documents", "skipped" }`. Each document has
`path`, `tier`, `scope`, `title`, `tags`, `sources`, and `body`; `scope`,
`title`, `tags`, and `sources` may be omitted when empty. Each skipped item has
`source` and `reason`. A missing non-zip scope returns an empty document and
skipped array; a missing zip scope returns `404`.

Import accepts `{ "crew_id", "agent_slug", "documents" }`. Each document
uses the same fields as export, with `path`, `tier`, and `body` required. The
request is capped at 32 MiB. A successful request returns `200` even when
individual documents are rejected or fail:

```json
{
  "written": ["AGENT.md"],
  "rejected": [{ "path": "secrets.md", "kind": "secret" }],
  "failed": [{ "path": "daily/2026-08-01.md", "reason": "write failed" }]
}
```

`rejected` entries may also include `detail`; `failed` entries contain only
`path` and `reason`. Empty documents are rejected with `400`, an oversized
request with `413`, and an unknown tier or invalid target with `400`/`404`.
