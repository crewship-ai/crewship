...
Every issue event goes through one emitter, and it writes exactly two payload
keys — whatever happened:
...
| `details` | Human-readable prose, e.g. `"BACKLOG → TODO"`. For display and for `{{ event.payload.details }}` — **not** for matching. |

Which means: **"fire when an issue moves to DONE" is not expressible as a
predicate.** The target status appears only inside `details`, which is prose
and would break the day someone rewords it. Match the action, and let the
routine read `{{ event.payload.details }}` and decide:
