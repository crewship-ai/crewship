...
spec:
  devcontainer:
    ...
    env:
      DATABASE_URL: postgres://postgres:${POSTGRES_PASSWORD}@postgres:5432/app
...
4. Wait for `healthcheck.test` to report HEALTHY (capped at 60 s across all sidecars).
...
Add this line at the top of any manifest and your editor (VS Code, Cursor, JetBrains with the YAML language server) gets autocomplete and inline validation:
