...
to the crew's lead agent), is injected as an env var into the sidecar, and is
appended to every crew agent's `env_refs` automatically so agents can reach the
service with the right value. Fields and defaults:
