...
Set the window with `crewship workspace update --approvals-retention-days N` (`PATCH /api/v1/workspaces/{id}`, see [Workspaces](/api-reference/workspaces)). Unlike the credential/audit-log retention pair, there is no "0 = keep forever" here -- every terminal decision this table records is also captured durably in the Crew Journal (`journal_entries`, via `AfterDecide`), so the row itself is disposable operational history rather than the compliance record. See [Migrations -- 20260901134904](/guides/migrations).
