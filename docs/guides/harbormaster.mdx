...
The [hooks](/guides/hooks) system fires `on_approval_requested` when a new request is enqueued (wired via `harbormaster.Enqueue` calling into the hooks dispatcher). Use it to page oncall, post to Slack, or auto-escalate:
