...
The same dial governs the `/api/v1/internal/*` routes an agent reaches through its sidecar. **This is a behaviour change on the default level**: on `guided`, a crew that has never had its policy touched will now see agents stopped where they previously were not.

| Level | Create a crew | Create an agent | Create a mission | Create a cron schedule |
|---|---|---|---|---|
| `strict` | **Rejected** | **Rejected** | Inbox approve | **Rejected** |
| `guided` | **Inbox approve** | **Inbox approve** | Auto + non-blocking inbox | Auto + non-blocking inbox |
| `trusted` | **Inbox approve** | **Inbox approve** | Auto-log to journal | Auto-log to journal |
| `full` | Auto + non-blocking inbox | Auto + non-blocking inbox | Auto + journal | Auto-log to journal |

**Why crews and agents block where missions and schedules do not.** A crew and an agent are *durable principals* — each one will act again tomorrow, on its own. Creating an agent additionally writes a caller-supplied `system_prompt`, which is persona authorship reached through an INSERT; direct persona writes are refused at *every* level, so an agent quietly acquiring a permanent colleague whose prompt it wrote itself is exactly what an operator wants to see before it happens. A mission creates no principal and is pinned to the acting agent's own crew, and a schedule stays an operator-editable row — for both, a blocking hold bought the same visibility a notice does while stopping ordinary work.

Note that this is an **oversight** decision, not a security one. The autonomy escape it was introduced alongside — an agent in a `strict` crew creating a `guided` one and acting there — is closed by two other things: `strict` refuses crew creation outright, and a created crew **inherits its creator's autonomy level** rather than the column default, so no child crew is ever more permissive than its parent. Loosening the `guided` cells would cost visibility, not containment.

### What a routine may do to the board

The same dial governs the [`crewship` step](/guides/routines#crewship-—-act-on-crewship’s-own-nouns) — a routine writing back into Crewship's own nouns. Three more actions:

| Level | Write to an issue<br/>(update · comment · link) | Create an assignment | Raise an escalation |
|---|---|---|---|
| `strict` | Inbox approve | Inbox approve | Auto-log to journal |
| `guided` | Auto-log to journal | Auto + non-blocking inbox | Auto-log to journal |
| `trusted` | Auto-log to journal | Auto-log to journal | Auto-log to journal |
| `full` | Auto + journal | Auto + journal | Auto + journal |
...
crewship approvals approve <id>     # releases the sentinel
crewship approvals deny <id>        # leaves it inert — nothing is deleted
```

The approvals queue is the **only** door, and `crewship approvals approve` requires OWNER or ADMIN — which is why the waitpoint is addressed to ADMIN rather than to MANAGER. In particular, `crewship hire approve` does **not** release a held agent: that command decides *ephemeral* hires an operator asked for with `crewship hire`, a MANAGER-tier action. An agent-created agent arrives with a system prompt another agent wrote, so releasing it is an OWNER/ADMIN decision. Pointing `hire approve` at one answers `409` and prints the `crewship approvals approve <id>` that does work.

**Denying does not delete anything, and neither does a lapsed hold.** The seven-day timeout leaves the artefact inert rather than turning into a green light — the gate fails closed. So does an unwired policy resolver: if the gate is not wired, everything holds.

Every arm — refused, held or allowed — writes an audit row carrying `decision`, `autonomy_level` and `policy_action`. There is no silent allow.
...
The F4.2 behavior monitor (Keeper Phase 2) samples tool calls — one in every N per crew, N from the workspace's [watchdog governance](/guides/keeper#watchdog-governance-per-workspace), default 5 — and runs them through a `behavior_evaluator`. The evaluator returns `ALLOW | WARN | DENY | ESCALATE`. `behavior_mode` controls what `DENY` means:

- **`warn`** (default) — `DENY` is degraded to `WARN`; the agent's tool call has already run and nothing is interrupted. Recommended for new crews — false positives don't break workflows.
- **`block`** — `DENY` is a *block* verdict. Recommended for crews with established behavior-monitor confidence (the operator has seen the warn-mode signal pattern for weeks and is ready to trust DENY as a real signal).

<Warning>
  **Detection, not authorization — but every verdict is recorded.** The
  monitor sees a tool call *after* it has executed (Crewship parses the driven
  CLI's stream; there is no pre-dispatch hook), so no verdict can undo the call
  it sampled, and on the live sampled path the agent's next tool call is not
  held either — even a `block`-mode `DENY` only records the interrupt
  (a `hook.blocked` journal entry).

  What the sampler sees **is** persisted, on both trigger paths alike: every
  fired sample writes a `keeper_requests` row, and the inbox fan-out below is
  the same whether the verdict came from the live sampler or from
  `crewship keeper review run behavior` — `WARN` → non-blocking inbox item,
  `ESCALATE` → operator review (blocking in `block` mode at strict/guided),
  `DENY` in `block` mode → blocking inbox item, `ALLOW` → journal telemetry.
  Read the monitor as detection with a full audit trail, not as an
  authorization boundary; see
  [Keeper → Watchdog governance](/guides/keeper#watchdog-governance-per-workspace).
</Warning>
...
{ "enabled": true, "reason": "graduated to autonomous after 3 weeks supervised" }
```

Requires `ADMIN`+. The CLI exposes the same control on `agent update`:

```bash
crewship agent update viktor --self-learning --learning-reason "graduated to autonomous"
crewship agent update viktor --self-learning=false --learning-reason "drift observed, reverting to inbox"
```
...
<Accordion title="Example 1: trusted crew + self_learning OFF agent">
...
<Accordion title="Example 2: full crew + self_learning ON agent (max autonomy)">
...
<Accordion title="Example 3: strict crew + self_learning ON agent">
...
```bash
crewship policy set --crew my-crew --level trusted --behavior warn \
...
CLI: `crewship agent update <id> --self-learning[=false] --learning-reason "…"` (must be run on its own — it can't be combined with other field updates in the same command).
