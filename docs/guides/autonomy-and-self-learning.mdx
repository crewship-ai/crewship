...
<Warning>
  **What a verdict actually does depends on which path produced it.** The
  ...
  it sampled. On the **live sampled path** the only effect of any verdict is a
  `hook.blocked` journal entry for a `block`-mode `DENY`; `WARN` and `ESCALATE`
  from the sampler are not written anywhere, no inbox item is opened, and the
  agent's next tool call is not held. The inbox rows described in the Phase 2
  docs — a non-blocking warning, a blocking approval that pauses the crew — are
  written by the synchronous review endpoint, which is what
  `crewship keeper review run behavior` and an operator-authored hook call. Read
  the monitor as detection and an audit signal, not as an authorization
  boundary; see [Keeper → Watchdog governance](/guides/keeper#watchdog-governance-per-workspace).
</Warning>
