...
Nothing to change. `crewship pipeline list` keeps working as an alias for `crewship routine list`. New subcommands (schedules / webhooks / waitpoints / validate / watch / logs) are only available under the `routine` name; pipeline alias doesn't expose them. Recommended: switch your scripts to `routine` so both old and new commands are available.
