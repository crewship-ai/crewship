...
# Seed the OWNER account using a build-arg secret
ARG BOOTSTRAP_EMAIL
ARG BOOTSTRAP_PASSWORD
RUN crewship admin reset-password \
...
The HTTP API isn't running during `RUN` steps — `crewship admin` is the only path that can produce a logged-in-able user before the server boots for the first time. Pair with `--secret` BuildKit mounts so the password doesn't bake into the image layer.
...
For the user/workspace data model these commands operate on, see [`/api-reference/users`](/api-reference/users) and the workspace endpoints — the schemas are identical; the CLI just bypasses the HTTP layer that normally fronts them.
