---
title: "Admin CLI"
description: "Host-side `crewship admin` subcommands that act on the SQLite database file on this machine — the recovery surface for an operator who's locked out of the UI. Says --local, because the CLI cannot guess which database you mean."
icon: "wrench"
...
## `--local`: the CLI cannot guess which database you mean

The file the admin CLI resolves and the server your CLI is configured for are **two different targets**, and until [#2086](https://github.com/crewship-ai/crewship/issues/2086) nothing in the CLI acknowledged that. It resolved `~/.crewship/crewship.db` and ignored `--server`, `CREWSHIP_SERVER` and `--profile` entirely — so `crewship admin list-users` against a populated server printed `(no users …)` and exited **0**.

Two changes:

- **`list-users` reads the server** (`GET /api/v1/admin/users`), scoped to the current workspace. It is the one verb in this group with a route, and it now uses it.
- **Every other verb here refuses to run when a server is named**, unless you pass `--local`. The refusal names the server, names the file, and names the flag. A server on `localhost` is not an exemption — `crewshipd` routinely runs with its own `DATABASE_URL` (every development clone uses `file:./crewship.db`), so "same host" says nothing about "same database".

So the recovery commands in this guide carry `--local`. It costs one word and it is the difference between resetting the password you meant to reset and resetting one in a database nobody was looking at.

<Warning>
**`list-users` needs `--local` on a fresh host too**, and it is the one command here where that is not conditional on a server being named. The other verbs are *gated*: they refuse only when `--server` / `--profile` / `CREWSHIP_SERVER` / a `crewship login` names a target, and run against the file unchanged when nothing does. `list-users` has a route, so it takes it — and `requireAuthAndWorkspace()` then fails with "not logged in" on a machine that has no CLI config at all, where the old build printed the table.

That is on purpose. Falling back to the local file whenever the login failed would answer "who exists on the server" from a file that may belong to a different instance — [#2086](https://github.com/crewship-ai/crewship/issues/2086) with an extra step, in exactly the situation (locked out, server down, login broken) where the wrong answer costs the most. The failure carries the hint, and the fix is one word:

```bash
crewship admin list-users --local
```

Scripts that ran bare `crewship admin list-users` for host-side recovery need that flag added, whether or not the box has ever seen a server URL.
</Warning>

<Warning>
Because the write happens straight against SQLite, an admin-CLI action does **not** produce a `journal_entries` row the way HTTP-fronted actions do — it never passes through the audit middleware. Treat these commands as out-of-band operator actions and record them in your own change log if you need an audit trail.
</Warning>
...
- **You can't log in at all.** Forgot password, sole owner of the workspace, mailer not configured, OAuth provider misconfigured — any combination where the web `/forgot` flow won't deliver a reset link. `crewship admin reset-password --email=… --local` mints a new hash directly.
...
- **CI / unattended provisioning needs a seeded admin.** A Dockerfile that bakes a baseline workspace can run `crewship admin reset-password --email=admin@example.com --password=$BOOTSTRAP_PASSWORD --local` non-interactively against the freshly migrated database, then start the server. The HTTP API isn't available yet at that point, so this is the only path.
- **You need to verify which email actually exists.** Operators routinely guess at email casing (`Admin@…` vs `admin@…`) before a reset. `crewship admin list-users --local` dumps the table so the reset command targets the right row instead of silently failing. (Without `--local` it asks the server, which is workspace-scoped and does not report lockout state.)
...
|------|-------------------|
| **Data directory** | The directory holding `crewship.db` and adjacent state. Defaults to `~/.crewship`; overridable via `CREWSHIP_DATA_DIR`. A `DATABASE_URL` env var takes precedence over both and points the admin CLI straight at a DSN. If the resolved database file is missing the command aborts before any SQL runs. |
| **Bcrypt hash** | The on-disk form of a user password. Stored in `users.hashed_password`; admin `reset-password` calls `bcrypt.GenerateFromPassword` with cost factor 12, so a hash written by the CLI is indistinguishable from one minted by the web signup flow. |
| **Workspace role** | Persisted in the `workspace_members` table per `(workspace_id, user_id)`. `promote` accepts `OWNER`, `ADMIN`, `MANAGER`, `MEMBER`, or `VIEWER`. `OWNER` is the only role that can delete a workspace or demote another `OWNER`. |
| **`--workspace` resolution** | If the user belongs to exactly one workspace, `promote` infers it. If they belong to multiple, the flag is required and a clear error (`user belongs to multiple workspaces — pass --workspace=<slug>`) tells the operator to disambiguate. No silent "picks the first one" behaviour. |
| **`reset-password`** | Mints a new bcrypt hash for `--email=…`. Interactive password prompt by default; `--password=…` for scripted use (CI bootstrap); `--password-stdin` reads the password from stdin so it never lands in shell history or `ps`. ALSO revokes every active session for that user in the same transaction (with `revoked_reason='password_change'`) and clears any lockout (`failed_login_count=0`, `locked_until=NULL`) so any leaked cookie can't outlive the recovery — no separate restart needed. |
...
| **`list-users`** | Default: `GET /api/v1/admin/users` on the targeted server, workspace-scoped (EMAIL / NAME / CREATED / ROLE), needs a login. With `--local`: reads the `users` table and renders it via `text/tabwriter`. Email, name, created-at, lockout status (`-` / `LOCKED until <ts>` / `expired <ts>`), failed-login count, workspace roles. Footer summarizes the active-lockout count with a hint to `reset-password` when any exist. Accepts `--locked-only` to filter to just the currently-locked accounts — which requires `--local`, since lockout state is not on the API.  Usually run before `reset-password` to verify the email row exists with the expected casing. |
| **`promote`** | Rewrites a single `workspace_members` row to the requested role. Errors if the user isn't a member of the resolved workspace. **Unlike the HTTP role endpoints, it does _not_ guard against demoting the last `OWNER`** of a workspace — a direct write can leave a workspace ownerless, so double-check with `list-users` first. |
...
```bash
# Stop the process you started with `crewship start` (Ctrl-C in its terminal),
# or if you run it as a systemd unit:
sudo systemctl stop crewshipd
```

The admin CLI opens the database with a 30-second `busy_timeout`, so a write that races a running server will wait rather than fail outright. Stop the server anyway — a half-applied change racing the server's own connection is exactly the failure mode to avoid.
...
```bash
crewship admin list-users --locked-only --local
```

The admin CLI lowercases the `--email` argument before querying, so it can only target rows whose stored email is already lowercase. Since web signup stores email verbatim, run `list-users` first to confirm the row exists in lowercase form; if the row you want isn't matched, `reset-password` fails loudly rather than silently creating a new one.
...
# New password: ********
# Confirm password: ********
# ✓ Password reset for Pavel Srba (admin@example.com).
#   2 active session(s) revoked.
...
```bash
crewship admin invalidate-sessions --email=alice@example.com --local
...
The role must be one of `OWNER`, `ADMIN`, `MANAGER`, `MEMBER`, or `VIEWER`. Note that `promote` writes the row directly and does **not** stop you from demoting the last `OWNER` of a workspace — verify with `list-users` first if you're changing an owner.
...
The new password is now active. The user logs in normally. Because the write bypassed the HTTP layer, it leaves **no** entry on the in-app audit timeline — note the recovery in your own operational log if you need it traced.
...
ssh prod-server
sudo systemctl stop crewshipd
sudo -u crewship crewship admin list-users --local
# admin@example.com   OWNER   2025-09-01 14:22   2026-04-30 09:15
sudo -u crewship crewship admin reset-password --email=admin@example.com --local
# New password: ********
sudo systemctl start crewshipd
```
...
crewship admin promote \
  ...
  --workspace=marketing \
  --local
# ✓ Promoted lead@example.com to OWNER in workspace "Marketing" (marketing).
sudo systemctl start crewshipd
```

Because the change is a direct DB write it does not appear on the in-app audit timeline, so a postmortem reader won't see how the workspace got unstuck unless the operator records the rescue elsewhere — note it in your incident log.
...
| `list-users` | *(read-only)* | None — reads `users` joined to `workspace_members`/`workspaces` for email, name, created-at, `locked_until`, `failed_login_count`, and per-workspace roles. |
| `promote` | `workspace_members.role` | Role must be one of `OWNER`/`ADMIN`/`MANAGER`/`MEMBER`/`VIEWER`; if `--workspace` omitted, user must belong to exactly one workspace. Errors if the user isn't a member of the target workspace. |
...
| `sessions list` | *(read-only)* | Email must exist. Dumps `user_sessions` for the user (all rows by default; `--active-only` filters to non-revoked, unexpired; `--limit` caps, default 50). |

None of these writes append to `journal_entries` — they bypass the HTTP audit middleware entirely. If you need the change traced, record it out of band.
...
- **Stop the server first, every time.** Two processes writing the same SQLite file is the failure mode to avoid. The admin CLI does *not* lock the server out — it opens the DB with a 30s `busy_timeout`, so a racing write just waits — but a half-applied change interleaved with the server's own writes is still a partially-applied-state risk. Stop the server, then run the command.
...
- **The admin CLI lowercases `--email`.** Every admin subcommand runs `strings.ToLower` on the `--email` argument before querying. Web signup stores emails **as typed**, so a user who registered as `Admin@example.com` cannot be targeted by these commands (the lowercased `admin@example.com` won't match the stored row). Run `list-users` first: if the row you need shows mixed case, fix it through the app, not the CLI.
...
- **`CREWSHIP_DATA_DIR` overrides `~/.crewship`.** On a host where the service was started with a custom data dir (common on dev VMs), running `crewship admin` with default env points at the wrong empty database, silently fails to find the user, and prints a misleading "no such email" error. Every command now prints the path it resolved, so check that line before you believe the answer. Export the same `CREWSHIP_DATA_DIR` the service uses (there is no `--data-dir` flag). If the service was configured with an explicit `DATABASE_URL` instead, set that — it takes precedence over `CREWSHIP_DATA_DIR` in the admin CLI's resolution.
