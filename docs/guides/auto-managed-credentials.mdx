...
description: "How sidecar database passwords are generated and stored automatically; rotation is available as a follow-up action, not at apply time."
icon: "shield-check"
---

# Auto-managed credentials

When a crew declares a sidecar from a known datastore image
...
declares a recognised datastore image (`postgres`, `mysql`, `mariadb`, `mongo`,
...
**Opting out is explicit and enforced.** If you override the service's
...
[Redis](#redis-is-delivered-via-a-command-arg-not-an-env-var) below.
</Note>
...
| `redis` | — (see below) | `REDIS_PASSWORD` |
...
[Customising](#customising-the-auto-credential)).
...
When you override the `command:` (or the auth env) on a **recognised
...
- **Command-injected datastores (Redis):** your `command:` must include
  the datastore's auth flag followed by a non-empty argument
  (`--requirepass <secret>` for Redis). A command with no flag, or the
  flag with an empty argument, fails the apply.
- **Env-injected datastores (Postgres, MySQL, Mongo, …):** the auth env
  key you set (e.g. `POSTGRES_PASSWORD`) must be non-empty. Setting it to
  `""` fails the apply — it is neither "auth" nor "let Crewship generate
  one," it is an ambiguous half-config, so we reject it.

```yaml
services:
  # Deliberately run an unauthenticated datastore on the crew-private
  # bridge — an explicit, acknowledged opt-out from the always-auth rule.
  - name: redis
    image: redis:7-alpine
    command: ["redis-server"]        # no --requirepass
    allow_unauthenticated: true      # required, or apply fails closed
```
...
<Warning>
**Breaking change (#1363).** Manifests that previously ran a recognised
datastore image with an operator `command:` (or auth env) that did **not**
configure authentication — e.g. `command: ["redis-server"]` with no
`--requirepass`, or `POSTGRES_PASSWORD: ""` — used to apply silently and
boot an **open** datastore. They now **fail `crewship apply`**. Fix each
one by either (a) adding the auth yourself (`--requirepass <secret>`, or a
non-empty password env), or (b) setting `allow_unauthenticated: true` on
the service to keep the open behaviour on purpose.
</Warning>
...
- `SPEC-4` — Auto-managed sidecar credentials (internal design doc). This guide is
  the canonical user-facing reference.
- [`internal/manifest/known_sidecars.go`](https://github.com/crewship-ai/crewship/blob/main/internal/manifest/known_sidecars.go)
