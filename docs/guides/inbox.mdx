...
- **Sender avatars.** Every row and the detail header render a coloured avatar keyed on `sender_type` (agent / crew / pipeline / system) so an agent message is visually distinct from a routine failure at a glance.
...
|---------|--------------|
| `crewship inbox list` | Show unread items (default). `--state all\|read\|resolved`, `--kind waitpoint\|escalation\|failed_run\|message`, `--limit`, `--format json\|yaml\|quiet`. JSON now includes `payload` + `sender_type`. |
