...
1. `UPDATE agents SET status='IDLE' WHERE id=? AND status='PENDING_REVIEW'` — guarded WHERE so a concurrent fire/soft-delete returns 404 rather than silently writing nothing
...
4. WebSocket broadcast `agent.ready` so the agent canvas + crew roster repaint instantly
...
- **Approve** lands the proposed action and writes a `keeper.approval` journal entry
- **Reject** discards the proposal and writes a `keeper.rejection` journal entry; the evaluator gets the rejection as a `prior_lesson` hint on its next run for the same context (dup-suppression — see [F4.4 negative-learning](/api-reference/admin#post-apiv1internalkeepernegative-learning))
