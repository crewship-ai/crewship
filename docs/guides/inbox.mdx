...
| `memory_consolidation` | The consolidator distils journal entries into candidate rules, or Keeper's memory-health sweep suggests one | **Accept** / **Reject** — `/consolidate/proposed/{id}/approve\|reject`. OWNER/ADMIN only, so the row is addressed to `ADMIN`. |
| `schedule_missed` | A cron schedule was overdue and dropped or deferred occurrences per its `catchup_policy` | **Run now** — fires the schedule out of cycle (`/pipeline-schedules/{id}/run`, the same call as `crewship routine schedules now`) — or Dismiss. |
| `schedule_circuit_breaker_tripped` | A schedule auto-disabled itself after N consecutive failures, to stop the spam and the cost bleed | **Re-enable schedule** — `PATCH /pipeline-schedules/{id}` with `{"enabled": true}`, the same call as `crewship routine schedules enable`. OWNER/ADMIN only. |
...
Decisions are gated by the role the **server** enforces, not by what the UI feels like offering. `/pipelines/waitpoints/{token}/approve` and the escalation lifecycle are `roleCreate` (MANAGER and up); `/skills/proposed/approve` and `/consolidate/proposed/{id}/approve` are `roleManage` (OWNER/ADMIN only). When the caller cannot decide, the card **names who can** instead of handing them a button that answers 403 — and the rows themselves are addressed to the role that can act, a pairing `TestInboxTargetRoleMatchesDecider` reads out of the source and requires to agree.
