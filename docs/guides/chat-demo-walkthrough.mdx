...
| 1 | `/chat` — the conversations column, the facets, a thread that names itself | Browser | 2 min |
...
Top to bottom: a search box that filters titles the page already has, a
**New conversation** button, then one segmented strip of three facets — **All**,
**Unread**, **Live** — each carrying its own count. Under that, **Conversations**,
newest activity first.

</Step>

<Step title="The facets">

**Click Unread, then open the row it shows.** The count drops immediately and
...
whatever the facet says — and the counts are computed *without* that pin, so
...
back to 0 when the reply lands.
...
talked to. Expand it and click `casey`. That mints a conversation id locally and
