...
The column is one list for everything you can talk in. Under the **Chat** heading sit **New chat** and a search box, then three collapsible sections: **People** (your direct messages with colleagues), **Agents** (every agent, with its sessions folded under it — expand an agent to see them, click its name to open the freshest one) and **Team spaces** (private groups and workspace channels). The agent is an attribute of a session row, the way a sender is an attribute of an email, not a folder you have to open first.

**New chat** is a menu, not a button: **Chat with an agent** opens the agent picker, **Message a person**, **Create a group** and **Create a channel** open the matching dialog. Which sections are open, and which agents are expanded, are remembered for the tab.

</Step>

<Step title="The scopes">

**Open Activity & filters** at the foot of the column and **click Routines.** The agent sessions you were reading are replaced by routine steps, stacked under the agent that ran them.
...
active scope is named on the **Activity & filters** summary so it is visible
when the foot is folded.

</Step>

<Step title="Unread, Running, and one agent">

The same foot carries two checkboxes: **Unread agent sessions** and **Running agents**. They **compose** with the scope — "unread routines" is a real question — and with the search box above, which matches agent names and session titles.

**Tick Unread agent sessions, then open the row it shows.** The count on the agent's row drops immediately and
...
**Running** means the agent is working right now. It is driven by the
`agent.status` workspace event rather than by a roster fetched once at mount, so
it flips at the moment a run does: send a message and watch the agent appear, then
drop out when the reply lands. The dot rides the agent's portrait on the row
it belongs to.

**Collapse** (the button beside the **Chat** heading) folds the column to a
narrow rail with the expand button in it — same control, same place, same
behaviour as the other explorers.
...
An agent nobody has talked to still has a row under **Agents**, reading "No sessions yet" once expanded. Click its name — or **New chat ▸ Chat with an agent**, which
opens a picker over every agent rather than choosing one for you — and
...
with `casey`. **New chat ▸ Chat with an agent** would give you one, but it
mints the id locally and only shows the draft; to have a second session with
a stable, shareable id, mint it from the CLI and open its deep link:
