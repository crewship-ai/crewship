...
| **Trigger node** | The single synthetic entry-point node that sits above every chain — labels the run's `triggered_via` (issue id, schedule cron, webhook name, sub-run, or `manual`). |
| **Step node** | One node per DSL step (`agent_run`, `http`, `script`, `code`, `transform`, `query`, `wait`, `notify`, `foreach`, `call_pipeline`). Its border colour is the status pip; its body shows the rendered prompt or expression. |
| **Sequencing edge** | Solid arrow. Drawn from a step to every other step that lists it in `needs[]`. Means *must run first*. Routed along dagre's own path so a skip edge steps around the nodes it passes instead of cutting across them. |
| **Data-flow edge** | Dashed arrow, labelled with a JSON path. Parsed from `{{ steps.X.output[.path] }}` templates in any input field. Means *this value flows here*. |
...
| **Overview mode** | What the canvas shows when no run is selected — the cross-feature chain: the automation or issue that triggers a routine, the routine, the last run of it, the agent that executed that run, and anything the run left in the inbox. Click a last-run node to flip into trace mode for that run. |
...
3. **Click the run row.** The canvas paints the trace graph top to bottom. Status pips light up live from the workspace's `pipeline.step.*` events — no refresh needed.
...
| **Steps** | One node per DSL step (`agent_run`, `http`, `script`, `code`, `transform`, `query`, `wait`, `notify`, `foreach`, `call_pipeline`) | DSL `steps[]` |
...
The layout is top-to-bottom via dagre, so the chain reads like a flowchart and parallel branches sit side by side at the same depth. Depth is the dimension a routine grows without bound, so it runs down the page — the direction the pane can scroll. The canvas also refuses to zoom out past legibility: a long routine overflows and pans rather than shrinking until its labels disappear.

Sequencing edges show *what must run first*; data-flow edges show *what value flows where* — distinct line styles so the two graph topologies don't visually collide.

<Note>
A `foreach` step draws as a single node. Its per-item body (`foreach.steps`) is not expanded onto the canvas yet, so a loop shows as one box regardless of how many items it processes.
</Note>
...
- **Input** — the step's declared DSL inputs with `{{ steps.X.output }}` / `{{ inputs.Y }}` refs resolved against the run's outputs (a `resolved` badge marks what flowed in from upstream). Every value renders through the shared content-aware renderer: a markdown prompt shows **formatted** (headings, bold, links, fenced code via shiki), JSON shows pretty / as a table, other code is syntax-highlighted — no plain-text `<pre>` walls. Each renderable value carries a **raw ⇄ rendered** toggle for copy-paste fidelity.
- **Output** — the recorded text from `run.step_outputs[stepId]`, through the same renderer (markdown / JSON / code detected automatically) with the same raw toggle.
...
The renderer is one shared component (`components/features/activity/output-view.tsx`) driven by pure detection (`lib/output-language.ts` `analyzeOutput`) — the same treatment the agent's chat reply gets, so a step's input/output reads consistently across the Input, Output, and per-action drill-down surfaces.

### Agent action drill-down

For an `agent_run` step, the detail panel carries a **waterfall of the agent's internal tool calls** — every Bash command, file `Read`/`Write`/`Edit`, and MCP tool call the agent made inside that step, positioned by start time and duration (`lib/trace/sub-spans.ts`, persisted as `run.agent_span` journal entries). Expand any action to see:

- **input** — the full (scrubbed, capped) arguments the tool was called with — e.g. the exact `python3 parse_vypis.py` command and its `description`, or an MCP call's params. Older runs fall back to a one-line **command** summary.
- **output** — the tool result body (scrubbed, capped tail): a script's stdout, a file's contents, an MCP reply. This is what lets you confirm *a deterministic script actually ran and what it returned*, post-hoc and from another session — not just that it was invoked.

Both are captured for every CLI adapter (the recorder taps the shared tool-call/tool-result stream, so it's adapter-agnostic) and size-bounded per action — **input at 2 KB** (args are context) and **output at 16 KB** (roomy on purpose: a strict-JSON deliverable, like a month of parsed transactions, must survive whole, since a JSON body cut mid-object no longer renders as structured data) — plus a per-step cap of 200 actions, so a chatty agent can't bloat the journal. When a value *is* cut, it's marked `...(truncated)` and the action shows a **`truncated` chip** — the full, untruncated result is always on the step's **Output** tab (`run.step_outputs`), so the cap never hides data, it just points at where the whole thing lives. Secrets in an argument or a result are redacted before capture. The same data is returned by `GET /api/v1/workspaces/{ws}/pipeline-runs/{runId}` under each step's `sub_spans[]`, and renders through the same shared `OutputView` as the Input/Output tabs.

The per-action **input/output bodies are fetched lazily**: the live canvas poll (which refetches this run on every step event + a 3 s tick) requests only span *metadata* — name, status, duration, and the truncated flags — so a watched long run's payload stays flat as actions accumulate. Opening a step's Actions tab fetches that one step's I/O on demand (`?io_step=<stepId>`; `?include_io=1` returns it for every step, e.g. for a CLI dump). The `truncated` chips render in the light view too, before the body arrives.
...
With **no run selected**, the canvas switches to overview mode: a "what's wired to what" snapshot of the chain a piece of work actually travels, left to right.
...
| Node | What it shows | Where clicking it goes |
|------|---------------|------------------------|
| **Automation** | The rule that fires a routine — its name, the event it listens for (`issue.created`, …), and whether it is enabled. A disabled rule still appears, dimmed: "wired up but off" is worth seeing. | Nowhere — the surface that edits it is the routine it fires, one node to the right. |
| **Issue** | An issue that binds a routine (`routine_id`). Issues with no routine binding are left off — there is no chain to draw. | Nowhere from the canvas; the issue opens from the rail row. |
| **Routine** | The routine itself, with its slug and invocation count. Surfaces if an issue binds it, an automation fires it, or it has run. | `/routines?slug=<slug>` |
| **Run** | The most recent run of that routine, with status, trigger source, and an "awaiting" badge when it is parked on a waitpoint. | Sets `?run=<id>` — the canvas flips into trace mode. |
| **Agent** | The agent that executed the run, with its real avatar and its crew. | `/crews?agent=<slug>` |
| **Inbox item** | What the run left for a human — a waitpoint, an escalation, a message — with its kind and title, ringed when it is blocking. | `/inbox?item=<id>` |

Solid edges mean *this happened* (routine → run → agent); dashed edges mean *this is wired* or *this is waiting* (automation → routine, issue → routine, run → inbox item).

This is the answer to "I just opened /activity, what should I look at" — the overview surfaces the chains the workspace is currently running on, and where each one is currently stuck, without forcing you to pick a run blindly from the rail.

Every column past the first three is drawn only when the page has the data for it: an agent appears when the run can be attributed to one, an inbox item when it names the run that produced it. Nothing is invented to fill a column.
...
- **A new overview node type needs a registered width, not just a component.** The overview layout hands dagre a width per node type and converts the centre dagre returns back into a top-left position — so the same number has to be used twice. Both reads go through `OVERVIEW_NODE_WIDTH` in `lib/trace/build-overview-graph.ts`, and `overviewNodeTypes` (`components/features/activity/overview-nodes.tsx`) is typed against the same union, so a component registered without a width fails to compile. It used to be two parallel ternaries that both fell back to the run width, and a new node type ranked correctly while rendering half a card off its column, silently.
