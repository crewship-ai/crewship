...
The bar is realtime-first: it seeds from `GET /api/v1/runs?status=RUNNING` (agent runs) and `GET /api/v1/workspaces/{ws}/pipelines/runs/active` (routine runs) on load, then adds rows on `run.started` / `pipeline.run.started` and removes them on the terminal events. Nothing in flight → the badge disappears and the dropdown reads "Nothing running right now". It's purely a launcher into detail — no actions live here.
