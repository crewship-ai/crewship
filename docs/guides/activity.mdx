...
Both are captured for every CLI adapter (the recorder taps the shared tool-call/tool-result stream, so it's adapter-agnostic) and both are size-bounded per action (2 KB each, marked `...(truncated)`) and per step (200 actions), so a chatty agent can't bloat the journal. Secrets in an argument or a result are redacted before capture. The same data is returned by `GET /api/v1/workspaces/{ws}/pipeline-runs/{runId}` under each step's `sub_spans[]`.
