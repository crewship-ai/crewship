...
<Note>
  The cadences below are when each evaluator fires **on its own**. Any of the
  four can also be run on demand — `crewship keeper review run <slot>`, or the
  **run now** link on the evaluator's row in Admin → Keeper. A manual run lands
  in this panel like any other: same evaluator, same `keeper_requests` row, same
  escalation. See [Running a review on demand](/guides/keeper#running-a-review-on-demand).
</Note>
...
Sampled post-tool-call. The orchestrator's `EventPostToolCall` hook fires on a per-crew rate (default 1-in-5, per-workspace via watchdog governance) and runs the tool-call shape through the evaluator. Every fired sample lands here — the live sampler and `crewship keeper review run behavior` write the same rows. Each row carries:

- `tool_name` (`shell_exec`, `http_post`, `file_write`, etc.)
- `tool_args_snippet` (truncated args — secrets scrubbed before storage)
- `decision` + `risk_score`
- `should_block` (true only when crew's `behavior_mode='block'` AND decision is DENY or ESCALATE at strict/guided autonomy)
