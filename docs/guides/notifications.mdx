...
crewship notify prefs set --category agents.approval --channel nch_abc123 --state immediate
...
`agents.approval` and `agents.escalation` **bypass the rate gate entirely** — a blocking human-in-the-loop item is never dropped for being "too frequent."
...
crewship notifychannel deliveries --category agents.approval --channel nch_abc123
```
...
{ "type": "shoutrrr", "provider": "slack", "fields": {"webhook_url": "https://hooks.slack.com/services/T0/B0/XXX"},
  "categories": ["agents.approval", "agents.escalation"], "min_priority": "medium" }
