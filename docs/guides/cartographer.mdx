...
| **Advisory restore** | `POST /api/v1/checkpoints/{id}/restore` returns `(checkpoint, divergence_list)` without mutating anything. The mission keeps running. The UI shows the diff and lets the operator decide whether to fork. |
...
| **Fork** | `POST /api/v1/checkpoints/{id}/fork`. Atomically: creates a fresh mission with a new `mission_id`, captures a new checkpoint in that mission at the *source* checkpoint's cursor, sets `fork_of` on the new checkpoint to the source, and writes a `fork.created` journal entry to the new mission. |
