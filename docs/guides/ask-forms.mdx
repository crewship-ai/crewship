...
Marketing Assistant, an **Investigate a missing inquiry** form — the
submission ID, the expected destination, what happened, and an optional
screenshot. Render it with `crewship agent ask-preview casey check-form`,
or read it at `cmd/crewship/seeddata/askforms/casey.json`.
