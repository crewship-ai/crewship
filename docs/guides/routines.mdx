...
<Note>
**Overwriting a shared file needs the crew container running.** A first-time delivery writes the shared tree host-side, but once a crew is provisioned its container (UID 1001) owns `/crew/shared`, so an **overwrite** — re-delivery of changed content, or `routine import --force` — is routed through the running container as that UID. If the crew container is stopped, the overwrite returns `409` with a "start the crew and retry" message rather than a silent failure.
</Note>
...
`routine result` answers "what did run X produce?" — it re-fetches the run and prints the final `output` (structured JSON is pretty-printed; `--format json` for scripting). `routine logs` now includes the output too; use `result` when the deliverable is all you want.
