...
<Warning>
  These proxy variables are **reserved**. Setting `HTTP_PROXY`, `HTTPS_PROXY`,
  `NO_PROXY` (or their lower-case spellings) in `script.env` is ignored and
  logged — a routine cannot switch off the egress fence that constrains it.
  Before this was enforced, script steps ran with no proxy at all and the crew
  allowlist did not apply to them ([#1473](https://github.com/crewship-ai/crewship/issues/1473)).
</Warning>
