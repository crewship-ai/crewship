...
| **Runs · 7 days** | The week by verdict — green passed, red failed, amber not yet judged, grey cancelled — with the day's spend as a line on its own axis. The bars sum to the runs that day: a cancelled run is stacked rather than dropped, because a bar shorter than its own day is a chart lying by omission. |
| **Recently failing** | What broke most recently, freshest first. |
...
A routine's **monthly budget** is on its own card here, next to what it costs. It used to be a workspace-wide roll-up on the overview, which put a third card about money on one row — and before that a tab nobody opened.
