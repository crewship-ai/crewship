...
The main **Routines** list is the one list: each row carries the recipe's name,
purpose, step count, how it runs (manual or its plan), how its last run went and
when. A run waiting for a decision sits above the list as a banner. Search and
the result filters (All / Need me / Running / Failed / Never run) sit above the
rows; **Show hidden** reveals temporary routines and release-verification
fixtures. **Calendar** and **Recent runs** are the other views. Missing
descriptions are summarized from actual step actions; an unreadable recipe is
