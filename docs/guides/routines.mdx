...
| **Recent runs** | The last ten invocations with trigger, duration, cost and age. Live runs pin to the top with their current step. Each row opens that run in [Activity](/guides/activity). |
...
Wait points are not here: they belong to a *run*, and [Activity](/guides/activity) is where the run is.
