...
- **Versions** — publishing creates an immutable version; rollback is one API call
...
| **Recipe** | atomic | the routine's definition | In this guide, the CLI and the UI, a routine's *recipe* is its published definition — the steps, inputs and declared effects that a version archives. It is unrelated to the [`/api/v1/recipes`](/api-reference/recipes) crew bundles, which install a crew with its credentials and MCP servers |
...
The explorer sidebar carries search, the status buckets and the routine list,
as on every page; on a phone it opens over the main view. The main **Routines**
list applies the same filters: each row carries the recipe's name, purpose, step
count, how it runs (manual or its plan), how its last run went and when. A run
waiting for a decision sits above the list as a banner. **Calendar** and
**Recent runs** are the other views. Missing descriptions are summarized from
actual step actions; an unreadable recipe is shown as unavailable.

Selecting a routine uses `?slug=` in the URL. Reload and browser history preserve
that selection. A selected run adds `&run=` and keeps its recorded recipe version.

### The detail page

| View | What it answers |
| --- | --- |
| **Overview** | Three answers first: *What it does* (readable steps, List or Map), *Last time* (the last run and its verdict) and *Before you run* (the questions asked, declared results, what a run can reach, and how it runs). Status chrome, metadata and access fold into **Technical details**. **Edit** opens the recipe as a page at `?view=edit`; nothing there is live until Publish is confirmed. |
| **History** | Recorded runs, their verdicts and how they started. Each opens its run detail. Composition depth is under Technical details. |
| **Versions** | Published recipes and an explicit comparison of chosen versions. Comparison starts real work and can change connected systems or incur costs. |
| **Plan** | Repeating plans, one-time starts, webhooks and the automations that can start this routine, with their saved inputs, shared with Calendar. |

Steps use an explicit name when provided, otherwise a description of their action.
Stable IDs remain secondary technical identifiers. Empty schedules and results
collapse to a short row. The run page leads with its verdict, error and failed
step, and opens failed, current or waiting steps automatically. Missing evidence
is distinguished from an empty result. Stopping a run does not undo prior actions.

Waiting decisions appear in the run and the authenticated **Inbox**. Typed forms
use the same decision contract in both places. A routine proposed for review also
has an Inbox item showing its requested access and version changes. Review actions
remain subject to server permissions. Recurring and automation starts retain their
recorded provenance; the stored `triggered_via` value alone does not distinguish them.

The icon and colour are independent metadata: changing them does not publish a
recipe version. Routine budgets remain guardrails, not invoices.

### Edit and Test

**New routine** offers AI authoring, copying an existing recipe or writing one.
These paths use the same durable draft. **Edit** is one scrolling document:
identity, Inputs, steps and schedule. A selected step has **Edit / Test** tools.
**Code** edits the same buffer, preserving unsupported keys and expressions.
**Publication changes** is a collapsed review inside the document.

- **Save draft** saves work without publishing or running it. A conflicting edit
  reports the conflict and preserves local work.
- **Test routine** checks the recipe, references and static prerequisites. It does
  not run agents, scripts or HTTP calls and cannot prove a real run will succeed.
- **Test this step** uses sample data. Transforms calculate a result; agent, HTTP
  and script steps require an explicit replacement output and make no external
  calls. Unsupported step types cannot execute through this test.
- **Publish** performs the required recipe test and publishes the draft after
  applicable risk review. It is one explicit action, separate from saving a draft.
- **Run** starts actual work. It can change data in connected systems and incur
  costs. With inputs, it first presents their typed form. A new run is recorded
  in History; an existing run retains its accepted snapshot after later edits.

Sample data, publication review, decision-form authoring and version comparison
remain available. There is no general sandbox guarantee. The technical
`fixture_test`, `test_run` and `dry_run` API/CLI names retain their existing contracts.
...
<CardGroup cols={3}>
  ...
  <Card title="UI" icon="browser">
    Open `/routines`, choose **New routine**, then write or copy a recipe.
    Edit the document or its Code view, use **Save draft** for unfinished work
    and **Publish** to make it available. Existing routines use the same editor
    through **Edit**. The server enforces permissions, draft revisions and the
    publication test proof for both paths.
  </Card>
...
<Note>
The routine editor uses the draft/test/publish flow. Its UI does not expose a
skip-test checkbox. The legacy `/save` contract remains for compatible API/CLI
clients, including its separately permission-checked `skip_test_gate` option.
</Note>
...
dsl_version: "1.0"
name: summarize-text
# comments are fine — YAML only, dropped on conversion, same as they
# always were once JSON.Marshal'd a hand-authored JSON file
description: Summarize input text in 3 bullets.
inputs:
  - name: text
    type: string
...
steps:
  - id: summarize
    ...
    complexity: fast
    prompt: |
      Summarize the following text in exactly 3 bullets:
      {{ inputs.text }}
```
...
| `agentless` | boolean | Token-zero guarantee: the routine may only contain steps that can never invoke an LLM — `agent_run`, `call_pipeline` and `crewship` are rejected. See [Agentless routines](#agentless-routines). |
...
A watermark can go wrong in a way that is silent and permanent: write a cursor
that is too far ahead (a future timestamp, an id from the wrong source) and every
later run finds nothing to do — and reports success. `crewship routine state` is
the operator surface for that.

```bash
# Which cursor is stuck, and when was it last written?
crewship routine state list nightly-import

# Rewind one schedule's watermark to a known-good value
crewship routine state set nightly-import last_id 41200 --schedule psched_abc123

# Drop one key, or the whole bucket
crewship routine state rm    nightly-import last_id --schedule psched_abc123
crewship routine state clear nightly-import          --schedule psched_abc123
```

`state list` shows **every** bucket by default — because state is isolated per
schedule, the stuck cursor is usually in a bucket you would not have guessed.
The `UPDATED` column is normally the tell: a cursor frozen three days ago next to
schedules that ran this morning.

Mutations are **ADMIN-tier**. A watermark governs what every future *unattended*
run does, so rewriting one carries the same blast radius as disabling the
routine. `clear` is deliberately bucket-scoped — there is no "wipe every
schedule" form, because that makes each schedule reprocess its whole backlog with
no undo. Prefer `set` to a known-good value when you can.

Full flags: [`crewship routine state`](/cli/routine#crewship-routine-state).
...
- **No-op fast path.** An empty / absent `credentials_required` does zero
  probe work.
- **Unavailable is not available.** A credential probe error (including an
  unavailable database) blocks dispatch with HTTP **503**. Retry after the
  store recovers. A confirmed missing credential remains HTTP **422**.
  In-process child calls and scheduled dispatch use the same blocking gate.
...
A whole input or step-output reference preserves the target input's declared
`integer`, `number`, `boolean`, `array` or `object` type. For example,
`"count": "{{ inputs.count }}"` forwards numeric `0` to a child declaring an
integer `count`; it does not forward text `"0"`. Missing references, incompatible
types and invalid choices fail before the child dispatches work. Optional null
stays null. JSON step outputs may include the same markdown fences accepted by
transform steps.

String and undeclared target inputs keep text interpolation. Mixed text such as
`"count={{ inputs.count }}"` remains text. Literal objects and lists pass through
unchanged; templates inside them are not recursively evaluated. Use Code for
advanced bindings; the visual editor can replace the source of an existing
child input without changing its other bindings.

A called routine faces the **same dispatch gates** as one run directly: its
author crew must have the integrations, datastores/tools and credentials the
called routine declares. If it does not, the `call_pipeline` step fails with
the same sentence the direct run would have returned (`routine "x": routine
requires credential of type "stripe" not present in the vault for crew "y"`)
— calling a routine is not a way around a precondition it does not meet.

`dry_run` is exempt, as it is on the direct path: it touches no vault.

#### Chain depth — the composition budget

Nesting depth is not the only way a routine can call itself. A routine can act
on an issue, that change can fire an automation, and the automation can run the
routine again — a loop that never appears on any one run's call stack, because
each hop is a *new* top-level run.

Every composed edge therefore also spends from a **chain depth** budget carried
on the run itself (`chain_depth` on `pipeline_runs`, alongside `chain_origin` —
the id of whatever started the chain):

- A run a person started is depth **0**.
- A routine called by `call_pipeline` is parent + 1.
- A run fired by an automation reacting to an event is that event's run + 1.

**The cap is 8.** The ninth hop is refused before it dispatches: the step fails,
and an `automation.depth_exceeded` entry lands in the activity feed naming the
edge that was cut, the depth, and the chain origin — so a loop shows up as a
readable event rather than as a routine that "sometimes doesn't run".

The number is deliberately small. It can be raised later; it cannot be lowered
once people have built chains that depend on it.
...
are invisible. Crew grants come from `credential_crews`, including a second
or later crew; the legacy `crew_id` field alone grants no access. When several match, the authoring crew's own credential wins
...
the journal, or step output. An explicit `credential_ref` is required: no
matching credential, a missing resolver, an empty value or a resolver error
fails the step **before sending the request**. For a public endpoint that
requires no authentication, omit `credential_ref`. This also prevents a vault
failure between preflight and execution from sending an anonymous write.
Injection schemes: `bearer`
...
```json
{
  ...
  "wait": {
    "kind": "approval",
    "approval_title": "Approve: {{ inputs.action }}",
...
**Name what the approval would do.** `approval_title` is the one line the
inbox row shows. It is templated like `approval_prompt`, and that is the
point: `"Approve: {{ inputs.action }}"` renders a different row per run,
where the prompt's first line is authored boilerplate and therefore
identical on every invocation. Without it, three pending approvals for a
credential rotation, a bucket deletion and a scale-out all read
`"Approve this production action?"` and cannot be told apart in the list.

Omitting it keeps the old behaviour — the row falls back to the prompt's
first line — so routines written before the field exists are unaffected.
The rendered title is secret-redacted and truncated to 80 characters on
the same path the fallback is.

**Declare the blast radius.** `risk_level` is `normal` (the default when
unset) or `destructive`. `destructive` marks an approval whose action
cannot be undone by re-running it, and the inbox marks that row and its
decision card. It is author-declared and never inferred: a heuristic that
looks for "delete" in the prompt is wrong in both directions, and the
direction that matters — reading a destructive action as an ordinary one
— fails silently. An unrecognised value is rejected at save time rather
than being read as `normal`.
...
**Waking an `event` wait: by run, or by topic.** There are two doors, and which one you use depends on whether the sender knows the run id.

| | Endpoint | Use when |
|---|---|---|
| Per run | `POST /api/v1/workspaces/{ws}/pipeline-runs/{runId}/signal` | You triggered the run and kept its id — an external system finishing a job it was handed. |
| Per topic | `POST /api/v1/workspaces/{ws}/signals` | You are announcing that something happened and do not know who cares. Wakes **every** run in the workspace parked on that `event_type`. |

```bash
# one run you already have the id for
crewship routine signal run_cmox... --event deploy.finished --payload "v1.4.2"

# every run parked on the event, whoever they are
crewship signal send --event-type deploy.finished --payload "v1.4.2"
```

The topic form is what makes `wait: event` usable as a subscription rather than a callback: several routines can park on `mission.status_change` and one delivery resumes all of them, each with the payload as its wait step's output. The `event_type` is a free-form string — there is no registry to add to, so a producer and a consumer only have to agree on a name.

Both forms are workspace-fenced: a topic delivery never reaches a run in another workspace, and each parked wait is claimed exactly once, so two producers announcing the same event at the same instant cannot resume one run twice. Delivering a topic nobody is waiting on is a success with `delivered: 0`, not a `404` — an event source emits whether or not anything is listening. (The per-run form still answers `404` when that specific run is not at the wait step, because there the caller named a run and was wrong about it.)
...
**Egress.** A script step runs with `HTTP_PROXY`/`HTTPS_PROXY` pointing at the crew sidecar (`127.0.0.1:9119`), so outbound HTTP from a script obeys the **crew network policy** — a `restricted` crew's script can only reach hosts on its `allowed_domains`. `NO_PROXY` keeps loopback direct.

<Warning>
  Proxy variables are **reserved**. Any `script.env` key ending in `_proxy`
  (any case) plus `no_proxy` is dropped and logged — a routine cannot switch
  off the egress fence that constrains it. The match is on the shape rather
  than an exact list because CPython lowercases every environment name before
  looking for a `_proxy` suffix, so `HtTp_PrOxY` would otherwise reach a Python
  script's proxy configuration. Before this was enforced, script steps ran with
  no proxy at all and the crew allowlist did not apply to them
  ([#1473](https://github.com/crewship-ai/crewship/issues/1473)).
</Warning>

Note this is the **application-layer** fence: a script that deliberately ignores the proxy (`curl --noproxy '*'`, a raw socket) still reaches the network until the L3 fence in [#1368](https://github.com/crewship-ai/crewship/issues/1368) lands. Treat the allowlist as a guardrail for well-behaved tooling, not as containment for hostile code.
...
| `category` | Which [notification category](/guides/notifications) this notice routes under, e.g. `routines.failed`. Optional; see below. |

**Saying what the notice *is*.** Without `category`, every notify step routes as `chat.replies` — the category an inbox `message` maps to. That is fine for a progress ping and wrong for everything else: "the nightly deploy failed" and "here is your weekly digest" arrive under the same label, and the [preference matrix](/guides/notifications#two-layer-preferences-the-%231412-category-system) people tune is invisible to you, the one author who knows what the event actually is.
...
Within an item the body runs like a mini-pipeline: per-step `retry:`, `validation`, `outcomes`, and tier escalation all apply. **Cost is summed across every item** and attributed to the `foreach` step, so `max_cost_usd` still bounds the whole fan-out. The first item that fails cancels the rest and fails the step (fail-fast). `wait`, `call_pipeline`, and a nested `foreach` are **not allowed** inside the body — the fan-out is a bounded, self-contained unit. `items` may hold at most 10,000 elements, and because every item receives its own copy of the run inputs, a run with more than 1,024 inputs fails the step before any item runs.

### `crewship` — act on Crewship's own nouns

Every other step kind reaches **outward**: an `http` step to a third party, an
`agent_run` to a model. `crewship` reaches **inward** — it lets a routine write
back into the product it runs inside, so the loop *"issue changes → routine
runs → routine comments on the issue"* closes without an `http` step aimed at
your own API and a token you have to manage.

```yaml
- id: file_it
  type: crewship
  action: issue.create          # namespaced verb
  args:
    title: "Nightly check failed: {{ steps.probe.output }}"
    priority: high
```

| Field | Notes |
| --- | --- |
| `action` | The verb. Required. See the table below. |
| `args` | Arguments for the verb, template-substituted (including strings nested inside lists and objects). |

The step's output is the API's JSON response, so a later step can pick a field
out of it — `transform` with `.identifier` gives you the new issue's key.

#### Verbs

| verb | does | required `args` | policy action |
| --- | --- | --- | --- |
| `issue.create` | Creates an issue in the routine's author crew | `title` | `mission_create` |
| `issue.update` | Changes an issue's status, priority, assignee, labels or due date | `identifier` | `issue_write` |
| `issue.comment` | Comments on an issue | `identifier`, `body` | `issue_write` |
| `issue.link` | Relates two issues | `identifier`, `target_identifier`, `relation_type` | `issue_write` |
| `assignment.create` | Delegates a task to an agent in the author crew | `target_slug`, `task`, `chat_id` | `assignment_create` |
| `escalation.create` | Raises an escalation to a human | `from_slug`, `reason`, `chat_id` | `escalation_create` |
| `page.write` | Writes a panel's payload onto a page | `page`, `panel`, `data` | `page_write` |

`page.write` is bounded twice over, and the first bound is not the autonomy
level. The routine may only write a panel that **names it as the producer**, or
one a human granted the crew or the acting agent `produce` on — an agent can
never issue that grant itself. On top of that, a **strict** crew's routines do
not write panels at all: `page_write` holds for approval under strict, and on an
unattended run a hold is a refusal. That is deliberate. An unwritten panel goes
`stale` and says so, which is a better outcome than a number nobody approved.
Raise it with `crewship policy set` if a crew should push unattended.

`relation_type` is one of `blocks`, `blocked_by`, `relates_to`, `duplicate_of`,
`sub_issue_of`. A missing required arg is a **save-time** error, not a 400 at
03:00 — as is a verb with no policy action behind it, which is how a new
capability is kept from shipping ungoverned by being forgotten.

The loop the kind exists for, end to end — a probe, a comment on the issue that
triggered the run, and the issue moved on:

```yaml
inputs:
  - name: issue
    type: string          # e.g. ENG-42, passed by the webhook or schedule
steps:
  - id: probe
    type: http
    http: { method: GET, url: "https://status.example.com/api/health" }
  - id: report
    type: crewship
    ...
    args:
      ...
      body: |
        Nightly probe says: {{ steps.probe.output }}
  - id: close_it
    type: crewship
    action: issue.update
...
- **Autonomy.** The verb is gated on the author crew's `autonomy_level`. A crew
  whose level would *hold the action for approval* gets a refusal instead — a
  routine has nobody attached to approve it — so on a `strict` crew, issue
  writes and assignments do not happen. Raise the level with `crewship policy
  set`. The full per-level table, and the reasoning for each cell, is in
  [Autonomy → what a routine may do to the board](/guides/autonomy-and-self-learning#what-a-routine-may-do-to-the-board).
- **Escalation volume.** `escalation.create` is allowed at *every* autonomy
  level, because an approval to authorise an interrupt is itself the interrupt.
  What bounds it is a backlog instead: a routine may not escalate while the crew
  already has `escalation.max_pending_per_crew` (default 10) **unresolved**
  escalations. Resolving them returns the budget.
- **Identity is not yours to set.** `workspace_id`, `crew_id`, `agent_id`,
  `actor_agent_id`, `author_agent_id` and `author_run_id` are injected from the
  run and override anything in `args`. A routine cannot act as another
  workspace, a sibling crew, or a different agent — the last one matters
  because the acting agent is what the delegation depth cap is measured from.
  A routine with no author agent cannot use `issue.comment`, which needs an
  author it can name — so a routine containing that step is **refused at save**
  unless one is set. Set it with
  `crewship routine save --author-agent <slug|id>`
  (the agent must belong to `--author-crew`), or `author_agent_id`
  on the save API. The refusal is deliberate: the same routine used to save
  clean and then fail every run at 03:00 with `400: agent_id is required`. The
  other five verbs have a documented no-agent fallback and are unaffected.
- **Chain depth.** Whatever the verb creates records the run that created it,
  so an automation reacting to it inherits this run's chain budget rather than
  starting a new one. See [Chain depth](#chain-depth-the-composition-budget).
- **`dry_run` does not write.** A preview that files a real issue is not a
  preview; the step reports what it would have called.
...
Iterating on one step's prompt shouldn't cost a whole pipeline run. `crewship routine step-run` executes a **single step against an input fixture**, in isolation — no upstream steps, no DAG, no persisted run record — and prints the step output, validation verdict, and (for `agent_run`) cost.
...
- **Save time** — validation rejects `agent_run` (direct LLM spend), `call_pipeline` (the target resolves by slug at runtime, so a nested routine could gain an agent step later and silently break the guarantee), `crewship` (an @mention in an issue body wakes an agent, an assignment dispatches one — whether it happens depends on rendered content, not on anything provable at save time), and `eval.online` with `sample_rate > 0` (online grading runs a grader agent against the routine's runs).
...
The economic value-prop: an Opus-class authoring model designs the routine, a Haiku-class executor model runs each invocation. Workspace `execution_tiers_json` maps complexity classes to `(adapter, model)`. This is the map every workspace is seeded with (migration v78; nothing rewrites it later, so it is what a workspace runs until an operator edits the column):
...
A workspace whose map is empty (`""` or `{}`) or unreadable, or whose map
does not contain the requested tier, resolves that tier through the model
catalog's Anthropic roles instead: `trivial` and `fast`
run the `cheap` role, `moderate` the provider default and `smart` the `top`
role — today `claude-haiku-4-5`, `claude-sonnet-5` and `claude-opus-5`
(`config/models.json`). An unchanged seeded map contains all four named tiers
and uses its stored model IDs for them.
...
### Draft trigger activation — a second, independent approval

The maker-checker gate above is about the routine's **definition**: is this
DSL safe to run at all. Atomic trigger authoring (below) adds a second,
orthogonal question that applies even to a routine that is `active` and not
risky: is this trigger safe to fire **unattended**, right now. Saving with
`"activation": "draft"` (`--draft` on the CLI) answers "not yet" — the
trigger is created disabled and one `escalation` inbox item asks a
MANAGER+ to turn it on. A routine's `status` and its trigger's activation
state can disagree: a safe, `active` routine can still have a trigger sitting
in draft, and a `proposed` routine's trigger — created disabled either way,
since nothing runs before the routine itself is approved — waits on both.
Activate with `crewship routine schedules activate <schedule_id>` (see
[Triggers](#triggers) below), which resolves that one inbox item.

## Triggers

### Atomic authoring — routine, version, and trigger together

`crewship routine schedules create` (below) is the general-purpose way to
attach, edit, or replace a trigger on an existing routine. `crewship routine
save --cron ...` (and an agent's `save_routine` tool call with a `trigger`
block) is the other path: it creates the routine, its first version, AND the
schedule **in one transaction** — either all three exist afterward, or none
do. This closes the gap where a routine authored by an agent could be saved
with no way to make it fire on its own; the trigger is no longer an optional
second step an authoring agent can forget.

```bash
crewship routine save --name daily-report --definition daily-report.json \
    --author-crew crew_marketing --cron "0 9 * * 1-5" --timezone "Europe/Prague"
```

Every save with a trigger reports **when it first runs** — this is the
sentence the routine-author skill's final message to the user states too:

```
Saved routine daily-report (id=..., hash=...)
Trigger: schedule 0 9 * * 1-5 — first run 2026-09-08T07:00:00+02:00
Invoke with: crewship routine run daily-report
```

Add `--draft` (or `"activation": "draft"` in the agent tool call) to create
the trigger disabled and raise one approval item instead of firing on that
schedule — see [Draft trigger activation](#draft-trigger-activation-a-second-independent-approval)
above. A routine authored with no trigger and no explicit
`--trigger-manual` (`{"kind": "manual"}` for an agent) is flagged on the
routine page as a gap, not a silent success — say so on purpose if the
routine really is meant to run only on demand.

A bad `--cron` or unknown `--timezone` rolls the **entire save** back: the
routine and its version are not persisted either, exactly as if the whole
call had failed.
...
A freshly-seeded workspace ships this pattern against the real public GitHub
Status unresolved-incidents feed: `feed-watch-probe` is agentless and
`feed-change-report` is the agent-written incident brief. They remain
unscheduled until you wire them together:
...
The default baseline is zero unresolved incidents. To use another provider,
change both routines' `url`, update the probe's transform for that response
shape, and add the hostname to each routine's `egress_targets`.
...
### The reliability editor

The routine detail page's **Plan** view is an in-place
editor for every field above, not just a read-only list (B9, #2362,
fixing F18 — the create form used to offer name/cron/timezone/inputs only,
even though the backend already accepted the rest). Opening a schedule's
edit dialog exposes:

- **When** — cron expression + timezone, with a **live preview of the next
  five fire times** computed server-side as you type (see below).
- **After downtime** — the [catch-up policy](#missed-occurrences-on-recovery).
- **Only run if** — the [wake gate](#wake-gates), including fail-open/closed.
- **On repeated failure** — the [circuit breaker](#circuit-breaker) threshold.
- **Version** — the pin, or track head.
- **Health** — last run, next run, consecutive failures, disabled reason,
  missed count and wake stats stay read-only on the list row; they are
  telemetry, not settings.

**If it overlaps** (`concurrency_key` / `max_concurrent`) is shown read-only
here too, but for a different reason: those fields live on the routine's
DSL, shared by every schedule/webhook/manual run of it, not on any one
schedule — so the dialog links to the routine's recipe editor (Edit) instead of
offering a control that would silently change every trigger's behaviour at
once.

Every field the dialog can set is backed by the same `PATCH
.../pipeline-schedules/{id}` endpoint `crewship routine schedules update`
already used — nothing new server-side for this half.

#### A plan's preset is checked when the plan is written

Creating a plan, editing one, and the `trigger` block a routine save can
carry all validate the preset against the recipe it would run, with the same
rules the run itself applies: an unanswered required question, a choice
outside the declared `options`, a value of the wrong declared type or outside
declared bounds is a `400` (a `422` for the trigger block) naming the input,
and nothing is stored. Before this the preset was only checked when the
*recipe* later changed, so a plan could be born carrying values its routine
already rejects, sit enabled in the calendar looking healthy, and fail for the
first time at its next firing — with no run to inspect, because the executor
refuses before a run row exists.

A pinned plan is judged against the version it names, and that version has
to exist: pinning a plan — on create, on repin, or when retargeting it onto
another routine — to a version the routine never archived is a `400` naming
the version, and the plan is left as it was. A plan that already carries a
pin to nothing (written before this check) can still be disabled,
rescheduled or renamed, since an edit that mentions neither the pin, the
target nor the inputs is not judged; repairing it means moving the pin onto
a published version. If the archive cannot be read at all the edit is a
`500`, not a pass.

What is deliberately still allowed: `false`, `0` and an empty list are
answers, not absences; inputs the recipe does not declare are passed through;
and an input that declares no form (no `widget`, no `options`, no
`allow_custom`) is not validated here, because it is not validated at
execution either.

On an edit, what is judged is the plan the edit **produces**: the target
routine, the effective pin and the effective inputs together. Changing the
inputs, repinning to another version, unpinning onto HEAD, or retargeting the
plan all trigger the check against the recipe the plan will actually run;
disabling a plan, changing its cron or renaming it do not, so a plan that
predates this check can still be switched off or rescheduled without first
repairing a preset the edit never mentions.

#### Next-fire-times preview

`GET .../pipeline-schedules/preview?cron_expr=&timezone=&count=` (`crewship
routine schedules preview --cron ... --timezone ... --count 5`) computes
upcoming fire times WITHOUT requiring a saved schedule, using real IANA
tzdata — so it reflects DST transitions exactly as the scheduler will, not
an approximation:
...
A spring-forward day with no valid `02:30` local time is skipped entirely
(the day the clock jumps 02:00→03:00 has none); a fall-back day's ambiguous
local time can appear twice, exactly one hour apart in UTC. Both are what a
real cron daemon does against real tzdata, not a client-side guess — which
is the whole reason this is a server endpoint and not a JS cron library
...
replayable for as long as nothing else rejects it. Every accepted delivery
leaves a receipt keyed on its identifier (`Idempotency-Key`,
`X-Crewship-Event-ID`, the GitHub delivery id, or failing those a hash of the
token, signature and body) that answers a resend with the original receipt
for **30 days** from acceptance, whatever the run then did — a failed run
does not reopen the window. The gap is a captured request resent under a
fresh identifier: to close it, a sender
...
whatever identifier it is resent under. Omitting
...
**Fire failure visibility.** A fire that never becomes a run — governance/pin/concurrency rejected it, or the dispatch itself errored before a run existed — records a `pipeline.webhook.fire_failed` journal entry **every time**. A single failure is not surfaced beyond that: it is treated as noise-tolerant (a momentary DB busy, a governance load hiccup). Once the **same webhook** fails **3 times in a row**, a MANAGER inbox card is raised; further failures past the third do not raise additional cards, and a later successful fire resets the streak so a future run of failures can alert again. This does not cover a run that started and whose routine then genuinely failed — that already has full per-run visibility via the routine's own failed-run journal entry and the Runs view.

#### Editing a webhook in place (F21)

Before `PATCH .../pipeline-webhooks/{id}` existed, changing anything about a
webhook meant delete + recreate — which mints a new token and therefore a
new public URL, breaking every sender already configured against the old
one. `crewship routine webhooks update` (B9, #2362) fixes that: name, rate
limit, inputs template and enabled state all edit in place, and the
token/URL **never** rotates through this command, under any combination of
flags:

```bash
crewship routine webhooks update wh_abc123 \
    --name "github-pr-reviews-v2" --rate-limit 120 \
    --inputs-template '{"source":"github"}'
```

Rotating the HMAC signing secret is still supported, but as one explicit,
opt-in flag rather than the only way to change anything:

```bash
crewship routine webhooks update wh_abc123 --rotate-secret
```

<Warning>
The signing secret is still shown **once** per reveal — on create, or on an
update that passes `--rotate-secret`. `--rotate-secret` invalidates the
OLD secret for every sender still signing with it (the URL keeps working;
senders need the new secret to keep passing HMAC verification). Rotating
the **URL** itself still requires delete + recreate — that is the one
thing `update` deliberately does not do.
</Warning>
...
After **Run**, the run detail shows recorded progress, decisions and results.
**History** retains prior runs. **Test routine** creates no real run: the public,
JWT-authenticated `POST /api/v1/workspaces/{workspaceId}/pipelines/test_run` checks
the draft and returns a workspace/user/recipe-bound `save_token` on success.
Publication verifies that proof. Sample step tests use the separate `fixture_test`
contract and explicitly supplied replacement data.
...
**A deferred run is pinned to the version it was accepted against.** The receipt carries `pinned_version`, and publishing an edit while the trigger waits does not change what fires — the run executes the recipe that passed its preflight, not whatever HEAD has become by the time the dispatcher reaches it. A debounced burst keeps the first trigger's version, because coalescing makes it one trigger — and a later trigger in the window is judged against that version as well as the current one, so inputs written for a recipe published after the window opened are refused with `409` and the pending run is left as it was accepted; let it fire or cancel it first. `pinned_version` can only be named on an immediate or one-time (`fire_at`) start; a delayed or debounced start answers `400` to it. A routine without an archived version is refused with 409. Publish a version before deferring it; every accepted manual deferred start must have a durable recipe.

That is the opposite of a schedule, which deliberately runs current HEAD unless its `target_pipeline_version` says otherwise: a plan is a standing instruction, a deferred run is one start that a person already made.
...
In the UI, use **Test routine** inside the selected step's Test panel for recipe
checks. The saved-recipe `/dry_run` report remains available through the API and
CLI. Its technical report contains the step ID/type, resolved execution tier,
estimated cost and declared agent or child-routine target.
...
The rubric is the contract; write it like a brief to a colleague. Grader and optimizer are ordinary crew agents — a dedicated "reviewer" persona works well as the grader. Full flag reference in the [CLI docs](/cli/routine#crewship-routine-iterate-%3Cslug%3E).
...
# UI: the run's Activity panel (History → run) or /inbox → Approve / Reject
...
you is visible whether or not you're looking at the routine. `/inbox` remains
the place to act on approvals for runs
you didn't just start; the routine's **History** view opens each parked run.
...
**Only a person decides.** A waitpoint is answered by a workspace member with MANAGER+, or by whoever holds the token on the public callback. An agent cannot satisfy one: a peer agent saying "GO" — through its sidecar token, a comment, or any agent-facing route — is refused by the resolve door before the row is touched, the waitpoint stays pending for you, and the attempt is recorded in the audit log as `waitpoint.decision_refused` with the actor named. The inbox card states the audience as `who_can_act: ["role:MANAGER"]`.
...
A run resumes on the recipe it was executing, not on today's head. Every live run captures its executed definition (`executed_definition_json`) before dispatch, and resume re-enters that snapshot — so publishing an edit while a run is in flight, or rolling the routine back, does not interrupt it. Runs recorded before the snapshot existed fall back to the pinned version if the run had one, else to the current definition, and there the legacy drift gate still applies: a content-hash mismatch with what the run started on stamps the run `interrupted` (this catches in-place edits even when every step id survives, not just renamed/removed steps).

When persisted state is insufficient to resume safely, the run is stamped `interrupted` instead — never silently dropped, never wrongly resumed. Fallback triggers: the pipeline row is gone, the executed recipe is unreadable or no longer parses, the legacy hash drift above, unreadable persisted inputs/outputs, or a non-resumable mode (only live `run` rows resume; a `dry_run` preview row from a previous lifetime is never re-run). The reason lands in the run's `error_message`.
...
`cancelled` covers every way a run's context is CANCELLED, not just the Cancel button. The default synchronous `POST .../run` hands the executor the HTTP request's own context, and Go cancels that context when the client connection goes away — so a closed tab, a proxy hanging up, or a CLI whose own per-call deadline fired and closed the socket all reach the server as `context.Canceled` and land a `cancelled` row reading `run cancelled at step <id>`. That matters beyond the label: cancelled runs mint no error fingerprint, page no failure notification and run no `on_failure` hook.

A DEADLINE is not a cancellation. A step that blew its own `timeout_seconds`, a run whose own context carried a deadline and exceeded it, and an approval nobody answered before its `timeout_seconds` all stay `failed`, keep their fingerprint and page as before: the work ran out of the time it was given, which is a result, not an interruption. The distinction is `context.Canceled` versus `context.DeadlineExceeded`, and the run row follows it.

If you need a run to outlive the request that started it, send `Prefer: respond-async` and poll the returned `run_id`; a run parked on a `wait` step already survives, because it releases the request before it parks.
...
Three fields on a run answer three different questions, and the names are close enough to
confuse (`docs/prd/PRD-ISSUES-AND-ROUTINES-2026.md` §9.6/§21 — "Outcome becomes a third
confusing status field" is a named risk of that document, and this section is how the PR
that added `outcome` discharges it):

| Field | Answers | Set by | Vocabulary |
|---|---|---|---|
| `status` | Did the process complete, error, or get cancelled? | The executor, mechanically | `queued`, `running`, `completed`, `failed`, `cancelled`, `interrupted`, `dry_run`, `waiting` |
| `outcome` | What should happen next with this result? | The runner, from the agent's own structured hand-off (work package B6, #2349) | `NO_CHANGE`, `SUCCEEDED`, `WORK_CREATED`, `PARTIAL`, `NEEDS_HUMAN`, `FAILED`, `CANCELLED` |
| `runverdict` (`summary.generated`) | Did an LLM judge, reading the run after the fact, think it accomplished its goal? | An aux LLM call, best-effort, gated by the `run_verdict_summaries` flag | Free-form judgment text, not a closed enum |

`status` is mechanical and never lies about whether the process ran. `runverdict` is an
**advisory opinion**, generated after the fact by a model that did not do the work — useful
as a second opinion, never authoritative, and silently absent for agentless/dry-run
routines. `outcome` is the one **deterministic routing decision**: today it drives the
inbox (`NEEDS_HUMAN` → exactly one action-required card) and an issue session's state
(`NEEDS_HUMAN` → `awaiting_input`); a run with an `agent_run` step that reports none of
the seven values still gets one — `FAILED`, with `error_message` set to `"no outcome
reported"` — rather than being read as a silent success. A routine with **no** agent step
(an `agentless: true` probe, or one built only from `script` / `transform` / `notify` /
`crewship` steps) has nobody who could report a hand-off, so a clean run there is
`SUCCEEDED` with no error — the strict default flags a missing report, not a missing
agent. §9.6 also names an issue-comment lane for
`WORK_CREATED`/`PARTIAL`, which is not yet gated on `outcome` specifically — see the note
under [API reference → Issues → Outcome](/api-reference/issues#outcome) for exactly what
is and isn't wired.

The practical consequence for authors: every `agent_run` prompt should end by
asking for the line — for example *"End your reply with exactly one line:
`outcome: SUCCEEDED` if you did the work, `outcome: NO_CHANGE` if there was
nothing to do, `outcome: NEEDS_HUMAN` if you are blocked."* A routine written
before this rule will show `FAILED` on every run until its prompt says so; the
first scheduled dogfood run on dev1 did exactly that.

### The step's chat is a durable transcript

Every `agent_run` step gets its own chat (titled `Pipeline <id> · step <id>`, one per attempt), and that chat keeps **both** halves of the turn: the rendered prompt the step sent, and the agent's answer with its reasoning and tool activity in the order they streamed. Read it with `crewship chat <chat-id>`, or open the chat in the dashboard — the transcript is the same one [`crewship chat stream`](/guides/watching-agent-runs) shows live, so a browser left open on a running step still has the text after a reload. A step that failed **after** the agent said something keeps what it said, which is usually the most useful thing on the screen.

These chats raise **no** "agent replied" inbox item, by design: nobody asked the step a question, and a routine on a fifteen-minute cron would otherwise bell you ninety-six times a day per step. Use a [`notify` step](#notify) when a run genuinely needs to reach someone.
...
- **`/routines` list** — a routine with an active run shows a live *Running* pill, and a parked run *Waiting for you* with its own banner above the list.
...
```bash
crewship routine budget set my-routine --amount 50    # cap at $50/month
crewship routine budget get my-routine
# my-routine (2026-07): [#####---------------]
#   $12.50 of $50.00 (25%)
crewship routine budget set my-routine --amount 0      # clear the cap
crewship routine budget summary                        # workspace roll-up
```

This is **not** the same knob as DSL `max_cost_usd` (above) — that's a per-RUN hard gate authored into the definition and enforced mid-run by the executor (a run aborts if it would breach it). The monthly budget is external operator config, set out-of-band from the DSL (never touched by `routine save`, never bumps the version history), and is a **view + soft cap indicator**, not an enforcement gate — a routine over its monthly budget keeps running; nothing currently blocks the next invocation. A routine can have neither, either, or both knobs.

`routine budget summary` rolls up every routine that has a budget set OR spend this month (a routine with neither is excluded — noise reduction for workspaces with dozens of zero-cost deterministic routines):

```bash
$ crewship routine budget summary
ROUTINE          BUDGET   SPENT   PCT   STATUS
my-routine       $50.00   $12.50  25%
cost-report      $20.00   $22.10  111%  OVER
summarize-text   —        $3.40   —

2026-07 total: $37.99 spent of $70.00 budgeted
```

The routine detail page shows the same meter (spent-vs-cap bar) when a budget is set, and the workspace roll-up appears on the routines list page. API: `GET/PATCH /api/v1/workspaces/{ws}/pipelines/{slug}/budget`, `GET /api/v1/workspaces/{ws}/pipelines/budget-summary` — see the [CLI reference](/cli/routine#crewship-routine-budget) for the full flag/response shape.
...
For variance characterisation across many runs (is this routine production-ready at this tier?), use [`crewship routine bench`](/cli/routine#crewship-routine-bench-%3Cslug%3E). For matrix-level cross-tier consistency (which scenarios diverge between Haiku and Opus?), use [`crewship eval scenarios`](/cli/eval#crewship-eval-scenarios) and [`crewship eval baseline diff`](/cli/eval#baseline-diff-%3Cname%3E) for CI regression gates.
...
A second request for the same `account_id` while the first is still running is rejected immediately with `429` and a `Retry-After` header — it does not queue or wait; requests for different `account_id`s run in parallel. If you want a burst of same-key triggers to collapse into one run instead of erroring, that's a different mechanism — [deferred dispatch's debounce](#deferred-dispatch-delay-ttl-debounce-priority) — not `concurrency_key`. Pair `concurrency_key` with the `Idempotency-Key` header for webhook handlers (see the [Concurrency + idempotency recipe](/guides/routines-cookbook#recipe-5-—-idempotent-routine-with-concurrency-key)).

Also note: the concurrency gate is enforced per-process, not cluster-wide — a multi-replica deployment has no cross-replica coordination, so the effective cap is `max_concurrent` × replica count (see [`runs/active` is single-replica scope](/api-reference/workspaces#list-active-runs-in-memory-registry) for the same limitation on the adjacent endpoint).
...
| `routine budget get/set/summary` | Monthly spend cap vs actual (budget-vs-actual view, not an enforcement gate) |
...
**Instrumentation for the mention → wake → reply loop is real, not aspirational.** The Issues & Routines PRD's §19.3 service levels — comment-persisted-to-ack latency, first-agent-acknowledgement latency, lost/duplicate deliveries, context-pack size and compaction, checkpoint compliance, and outcome routing — are computed series on `GET /metrics`, not declared targets: see the [Prometheus metrics reference](/observability/metrics) for which series answers which of those questions (grouped as delivery, continuation, duplication, and human comprehension, per §24.1). A percentile with zero samples in its window is absent from the scrape, never reported as `0`.

## Limitations (current MVP)

These are known gaps in the current MVP — none block production use, but they shape what you can rely on today.

- **Resume is at-least-once, step-granular** — restart recovery re-enters runs from the last persisted step boundary (see [Durability and restart recovery](#durability-and-restart-recovery)). The step that was in flight at the kill re-executes from scratch; there is no sub-step checkpointing, nested `call_pipeline` runs re-execute in full, and DAG runs recover at wave granularity. A run resumes on its captured recipe snapshot, so a publish during the run does not interrupt it; only runs recorded before the snapshot existed fall back to `interrupted` on a content-hash mismatch. `max_cost_usd` under-counts true spend after a crash — see [Production notes](#production-notes).
...
- **Routine runs' fleet-wide aggregates stay ad-hoc-only** — `GET /api/v1/runs` and the Runs tab's recent-runs table now include routine runs (`kind: "pipeline"`, keyed on `actor_id` since a routine's `pipeline.run.*` entries carry no `trace_id` — see [`/api-reference/runs`](/api-reference/runs), #2284). `GET /api/v1/runs/insights` (the KPI/breakdown row above that table) and the `stats` tile embedded in `GET /api/v1/runs` itself were deliberately left ad-hoc-only — unifying cost roll-ups and correlation across the two engines is a separate decision. A routine's own run history is unaffected either way: `crewship routine runs-all` / `routine records <slug>` / the routine's own Runs tab always had it.
- **No cross-run continuity** — each run starts with an empty context; the previous run's conversation is not carried forward, and `pipeline_routine_state` is a string key/value store for watermarks and counters, not memory. Give the agent what it needs in the routine's brief or in state you write explicitly.
- **A trigger that has matched nothing is not flagged** — an automation whose event type or payload filter can never fire is now refused at save time, but one that is valid and has simply never matched is not surfaced after any interval. Check `crewship automation list` for rules with no fires.
