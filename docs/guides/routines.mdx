...
| verb | does | available |
| --- | --- | --- |
| `issue.create` | Creates an issue in the routine's author crew. Requires `title`. | **yes** |
| `issue.update` | Changes an issue's status/fields. | not yet |
| `issue.comment` | Comments on an issue. | not yet |
| `issue.link` | Relates two issues. | not yet |
| `assignment.create` | Delegates work to an agent. | not yet |
| `escalation.create` | Raises an escalation to a human. | not yet |

**"Not yet" means the routine will not save.** Every `crewship` action is
bounded by the author crew's **autonomy level**, and five of the six verbs
describe capabilities Crewship has no autonomy action for yet — deciding what
a `guided` crew may do unattended is a governance decision, not something a
step kind gets to assume. Rather than ship them ungoverned, they are declared
and refused at **save time**, with an error that says why. A routine that saves
clean and then refuses at 03:00 is worse than one that never saved.

#### What bounds it

- **Autonomy.** The verb is gated on the author crew's `autonomy_level`, the
  same gate an agent doing this by hand passes. A crew whose level would *hold
  the action for approval* gets a refusal instead — a routine has nobody
  attached to approve it. Raise the level with `crewship policy set`.
- **Identity is not yours to set.** `workspace_id`, `crew_id`,
  `author_agent_id` and `author_run_id` are injected from the run and override
  anything in `args`. A routine cannot act as another workspace or a sibling
  crew.
