...
| Field | Notes |
| --- | --- |
| `to` | Recipient. `workspace` (default — anyone in the workspace), `trigger` (the user who triggered the run; falls back to a workspace-wide notice for unattended triggers like schedules), `user:<id>`, `role:OWNER`, `role:MANAGER`. Template-substitutable, e.g. `user:{{ inputs.assignee }}`. |
