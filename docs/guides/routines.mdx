...
Only a workspace whose map is empty (`""` or `{}`) or unreadable resolves each
tier through the model catalog's Anthropic roles instead: `trivial` and `fast`
...
(`config/models.json`). A seeded workspace never takes that path.
