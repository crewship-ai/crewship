...
A `code` step has a `runtime`. Two runtimes are wired today, both deterministic and token-zero: **`expr`** and **`cel`**.
...
`cel` evaluates a [Google CEL](https://github.com/google/cel-go) expression — pure-Go, non-Turing-complete (every expression provably terminates), no loops, no I/O. It reaches further than `expr`: boolean operators, arithmetic, string ops, list/map membership, ternaries, and field access, while keeping the same token-zero / no-RCE guarantees. Inputs are exposed as a typed `inputs` map (no `{{ }}` templating needed). Reach for `cel` when `expr`'s single comparison isn't enough.
...
<Warning>
`runtime: bash`, `python`, and `go` are **not executable**: a routine that uses one is **rejected at author time** (save / apply / test_run), with a `crewship apply` plan-time warning for anything that bypasses the validator. Use `runtime: expr` or `runtime: cel` for agentless logic, or convert the step to `http` + `transform`, or an `agent_run` step with a shell-tool-enabled agent. See the "Code steps" section of the [routine manifest reference](/manifest/routine) for the full conversion recipe.
</Warning>
