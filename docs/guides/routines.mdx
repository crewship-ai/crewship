...
| **Triggers** | Cron schedules and inbound webhooks. **Manage** reveals the full editors. |
| **Access** | Integrations, credentials, agents and egress — everything the routine can reach if it misbehaves. |
| **Versions** | Head version and metadata; **History** reveals the full version list and rollback. |
| **Runs** | Recent invocations, each linking into Activity filtered to this routine. **Manage** reveals per-run controls, including cancelling an active run. |
