...
The main **Routines** list shows each recipe's name, purpose and step count.
Search and status filters apply to both this list and the sidebar. **Health**,
**Calendar** and **Recent runs** are separate views. Missing descriptions are
summarized from actual step actions; an unreadable recipe is shown as unavailable.
On a phone, the sidebar opens over the main view and closes after selection.
...
| --- | --- |
| **Recipe** | What the routine does: purpose, inputs, results, readable steps, access and scheduled starts. List and Map show the same steps. **Edit** opens the authoring surface. |
...
| **Schedule** | Scheduled starts and their saved inputs, shared with Calendar. |
...
schedule — so the dialog links to the routine's own Editor tab instead of
