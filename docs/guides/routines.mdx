...
<Warning>
Default production builds do not yet wire the Docker-backed CodeRunner — a `code` step currently fails at runtime with a clear "no CodeRunner wired" error (and `apply` warns at plan time). Until [#618](https://github.com/crewship-ai/crewship/issues/618) lands, express the logic with `http` + `transform` steps, or run it through an `agent_run` step.
</Warning>
...
```json
{
  ...
  "steps": [
    { "id": "fetch", "type": "http",
      "http": { "method": "GET", "url": "https://billing.example.com/today?threshold=100" } },
    { "id": "judge", "type": "transform",
      "transform": { "input": "{{ steps.fetch.output }}", "expression": ".over_threshold" } }
  ]
}
```

The probe's verdict comes from data reshaping, not scripting: the `http` step fetches a JSON status and the `transform` step projects the boolean out of it. (`code` steps are also agentless-eligible, but see the [CodeRunner caveat](#code) before relying on them.)
