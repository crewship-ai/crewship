...
<Note>
The public draft **test_run** endpoint has been removed (you can't run an agent "dry"), so a real run is just **Run**. To preview a routine before running it, use **Dry run** — it walks the saved definition, renders templates, and returns the declared **manifest** (the blast radius) without invoking anything. Drafts are validated by the internal save gate (dry-run) at save time.
</Note>
