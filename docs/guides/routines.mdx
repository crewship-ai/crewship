...
```json
{
  "trivial":  { "primary": {"adapter":"claude","model":"claude-haiku-4-5"} },
  "fast":     { "primary": {"adapter":"claude","model":"claude-haiku-4-5"},
                "fallback":[{"adapter":"claude","model":"claude-sonnet-5"}] },
  "moderate": { "primary": {"adapter":"claude","model":"claude-sonnet-5"} },
  "smart":    { "primary": {"adapter":"claude","model":"claude-opus-5"} }
}
```

A workspace with no map (or an unreadable one) resolves each tier through the
model catalog's Anthropic roles rather than a frozen id: `trivial` and `fast`
...
(`config/models.json`), the same generation every other model picker offers.
