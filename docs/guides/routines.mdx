...
| `resources` | object | Agent-declared capability manifest — `datastores[]` and `tools[]` the routine touches that **can't** be inferred from the step graph. Advisory only (never gates a run). See [Capability manifest](#capability-manifest) below. |
...
Validation is **lenient**: declaring a resource is always allowed (it never
gates a run). Save only rejects malformed entries — a blank or non-slug `type`,
or more than 32 of either kind.
