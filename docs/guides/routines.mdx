...
`on_fail` lives at the step level (not inside `validation`) and is one of `escalate_tier | abort` — `escalate_tier` walks the fallback chain (e.g., Haiku → Sonnet → Opus) until validation passes or the chain exhausts. (`retry_step` is reserved but **not yet implemented** — it is rejected at validation; use `escalate_tier`.)
