...
(`NEEDS_HUMAN` → `awaiting_input`); a run that reports none of the seven values still
gets one — `FAILED`, with `error_message` set to `"no outcome reported"` — rather than
being read as a silent success. §9.6 also names an issue-comment lane for
