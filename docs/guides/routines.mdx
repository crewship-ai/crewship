...
- **`run` and `test_run` are gated; `dry_run` is not.** A `test_run` really
  executes against the author crew's agents, so it's gated too (fail fast
  rather than burn a token-spending run the agent can't complete). `dry_run`
  is a preview that invokes nothing, so it's left ungated.
...
- `datastores[].type` — `redis | postgres | mysql | mongodb | other`
- `tools[].type` — `ansible | terraform | kubectl | bash | python | other`
...
- **`run` and `test_run` are gated; `dry_run` is not.**
...
A `code` step has a `runtime`. Exactly one runtime is wired today: **`expr`**.
...
`expr` is an in-process, pure-Go evaluator — **no container, no LLM, no filesystem, no network**, so it costs zero tokens and runs in microseconds. It evaluates a **single comparison** and emits `true` or `false`:
...
#### `runtime: bash | python | go` (not wired)
...
The container-backed runtimes are accepted by the schema but have **no sandboxed runner wired** in current builds.

<Warning>
`runtime: bash`, `python`, and `go` are **not executable today**: a step using them **fails closed at runtime**, and `crewship apply` emits a plan-time warning. Use `runtime: expr` for boolean probes, or express richer logic with `http` + `transform` steps (or an `agent_run` step). Only `expr` is wired.
</Warning>
...
Routines have a lifecycle `status` (migration v128): **`active`** (live + runnable), **`proposed`** (awaiting approval), or **`disabled`** (admin airbag). The test-run gate still applies on top — `status` is an *additional* gate.
...
A `proposed` (or `disabled`) routine refuses `run` / `run_batch` with `409 Conflict` — `"routine is awaiting approval"` or `"routine is disabled"`. `dry_run` always previews, and `test_run` runs an unsaved draft, so neither is blocked.
...
<Note>
**Test run** in the detail panel runs the routine's current definition as a draft against the execution tier (the same draft endpoint the save gate uses), so it requires an **author crew**. For routines without one, the button is disabled with a tooltip — use **Run** instead. (Run / Dry run address the saved routine by slug and have no such requirement.)
</Note>
...
Alternatives:
- `crewship apply --skip-test-gate` (CLI — the flag lives on `apply`, not `routine save`) / `"skip_test_gate": true` (API) if your role is OWNER/ADMIN and you trust the DSL — bypasses the gate without running the test_run.
