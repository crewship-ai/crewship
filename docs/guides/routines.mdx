...
Any step can carry `"if": "{{ inputs.skip == false }}"`. False → step skipped, marked `<skipped>` in StepOutputs.
...
For terminal-side observability: `crewship routine watch <slug>` polls the runs endpoint and prints events as ANSI-coloured timeline; `--json` for JSON Lines piping into `jq`; `--once` exits after the first run terminates (CI-friendly).
...
The save endpoint requires `last_test_run_passed = true` AND a `last_test_run_at` within 5 minutes of the server clock. Run `/test_run` first, then save with the returned timestamp — or `--skip-test-gate` if your role is OWNER/ADMIN and you trust the DSL.
...
| `routine run <slug>` | Invoke against execution tier |
...
The `pipeline` alias is preserved for back-compat: every `crewship routine X` invocation also works as `crewship pipeline X`.
