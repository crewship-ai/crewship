...
`cancelled` covers every way a run's context ends early, not just the Cancel button. The default synchronous `POST .../run` hands the executor the HTTP request's own context, so a caller hanging up — a closed tab, a proxy timeout, a CLI deadline — ends the run the same way, and the row reads `cancelled` with `run cancelled at step <id>` rather than a failure. That matters beyond the label: cancelled runs mint no error fingerprint, page no failure notification and run no `on_failure` hook. If you need a run to outlive the request that started it, send `Prefer: respond-async` and poll the returned `run_id`; a run parked on a `wait` step already survives, because it releases the request before it parks.
