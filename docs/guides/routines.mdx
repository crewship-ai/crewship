...
| Field | Notes |
| --- | --- |
| `to` | Recipient. `workspace` (default — anyone in the workspace), `trigger` (the user who triggered the run — including **deferred** runs started with `--delay`/`--debounce-key`; falls back to a workspace-wide notice only for genuinely unattended triggers like cron schedules and service-token calls that carry no user), `user:<id>`, `role:OWNER`, `role:MANAGER`. Template-substitutable, e.g. `user:{{ inputs.assignee }}`. `crew:<slug>` is **not yet supported** — it's rejected at save/validate (and at run time for a template that renders to `crew:…`) and tracked for a later release. |
...
The message lands **live** (WebSocket broadcast to members) and **scrubbed** (secrets in mid-run outputs are redacted before insert). Delivery is **best-effort**: a routine whose notification can't be written never fails on that account — the run's real work is already done. Retries are **idempotent** — each `notify` is keyed on `(run, step)`, so a re-run doesn't double-post. Routine updates carry a `routine_update` subkind so the inbox can keep them in their own lane, separate from approvals and escalations.
