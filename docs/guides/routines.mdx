...
crewship routine step-run parse-invoice extract --input @sample.json -f json | jq -r .output
```

- **`--input`** is the step's inputs fixture — a JSON object, inline or `@file.json`. The step's `{{ inputs.X }}` refs resolve against it; `{{ steps.Y.output }}` refs resolve to empty (there are no upstream steps in an isolated step-run).
