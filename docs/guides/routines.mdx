...
Save endpoints (sidecar `/pipelines/save`, user `/api/v1/workspaces/{ws}/pipelines/save`, internal `/api/v1/internal/pipelines/save`) require the routine to clear a validation gate before it persists. The gate is a **dry-run validation** of the draft, not a real execution — there is no "test run" mode (you cannot run an agent dry). The sidecar agent-authoring flow forwards the draft to `/api/v1/internal/pipelines/test_run`, which parses, schema-validates, and dry-runs it (rendering every template, invoking no agent); on success it mints an HMAC **`save_token`** bound to (workspace, definition hash, author crew), and the sidecar forwards **that token** to save.

<Info>
**Autonomous ≥ interactive: the unattended path is gated at least as strictly as the interactive one.** Both save paths now clear the gate the same way — with a `save_token` the server mints on a passing dry-run and re-verifies, never a body-supplied "it passed" claim. The user path binds the token to the saving **user**; the internal (agent/sidecar) path binds it to the author **crew**. The old `last_test_run_at` / `last_test_run_passed` body fields were forgeable and are gone from `InternalSave` — an agent cannot activate a routine that never passed a dry-run, and there is **no** agent-side skip escape hatch (`skip_test_gate` is OWNER/ADMIN, user path only). This closes the gap where autonomous authoring was weaker than the interactive flow (#1371).
</Info>
...
- **Probe errors fail open by default**: a broken or deleted probe wakes the main routine instead of going silently blind, and records `last_wake_status: ERROR` so you can see the probe needs fixing. That's the right default for a *monitoring* probe (a little wasted token spend beats a blind spot).
- **Fail-closed (opt-in) for gates that must hold** (#1372): when the probe is a genuine safety gate rather than a monitor, pass `--fail-closed` on create (or `--fail-closed` / `--fail-closed=false` on update). A probe error then **holds** the run instead of firing it, and records the distinct `last_wake_status: HELD`. A held tick advances `next_run_at` (so the schedule isn't wedged) but counts as a check, not a fire. `--fail-closed` requires a wake probe; a schedule with no gate can't be fail-closed. Choose it when a broken or tampered probe firing the routine anyway is worse than the routine not firing.

    ```bash
    crewship routine schedules create --slug deploy-canary \
        --cron "*/5 * * * *" --wake-slug canary-healthy-probe --fail-closed
    ```
