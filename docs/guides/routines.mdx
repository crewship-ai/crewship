...
- **Routine runs are not in the Runs view** — a routine's runs are recorded in the journal and reachable through the routine's own run history (`crewship routine runs-all`), but `GET /api/v1/runs` and the Runs tab only aggregate `run.*` entries that carry a `trace_id`, which a routine's `pipeline.run.*` entries do not. Tracked as #2284.
