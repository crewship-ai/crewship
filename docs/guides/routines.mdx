...
| Field | Meaning |
| --- | --- |
| `max_attempts` | Total attempts including the first. Capped at 10. |
...
`retry_on` sees two variables: `error` (the message string) and `transient` (a bool from the built-in 429/5xx/timeout/net-blip classifier). It must evaluate to `bool`. Examples: `'transient'` (shorthand), `'error.contains("429")'`, `'error.matches("(?i)timeout|5\\d\\d")'`. A predicate that doesn't compile — or doesn't return a bool — is rejected at save time.

The loop is **cost-aware**: if the next attempt would push the run's accumulated cost past `max_cost_usd`, the retry loop stops mid-way and fails with a cost-cap error rather than burning the rest of the budget. Each retry emits a `pipeline.step.retrying` event (`attempt N/M`) so the recovery is visible in the run trace without the run going red.
