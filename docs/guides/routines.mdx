...
routines. `outcome` is the one **deterministic routing decision**: it is what the inbox
(`NEEDS_HUMAN` → exactly one action-required card), the issue comment thread
(`WORK_CREATED`/`PARTIAL`), and an issue session's state (`NEEDS_HUMAN` → `awaiting_input`)
all key off, and a run that reports none of the seven values still gets one —
`FAILED`, with `error_message` set to `"no outcome reported"` — rather than being read as
a silent success. Full routing table and the action-contract shape: [API reference →
Issues → Outcome](/api-reference/issues#outcome).
