...
| `CURSOR_CLI` | `result` event with `is_error: true` | `{"type":"result","subtype":"error","is_error":true,"result":"…"}` |
| `FACTORY_DROID` | `result` event with `is_error: true` (snake_case), or an `error` event | `{"type":"result","is_error":true,…}` / `{"type":"error","error":"…"}` |
| `CODEX_CLI` | `turn.failed` envelope, or an `error` event | `{"type":"turn.failed","error":"…"}` |
| `GEMINI_CLI` | `result` event with `status:"error"` (or a non-empty `error`), or an `error` event at `severity != "warning"` | `{"type":"result","status":"error",…}` |
...
- **Missions** — the assignment is `FAILED` and the mission task fails, instead of the mission advancing on an empty answer.
