...
`BuildCLICommand` only ships adapters for CLI agents that meet four
criteria:
...
CLI agents that don't meet all four are skipped today. Adding a new
adapter is a one-line change once the upstream surface stabilises;
open a GitHub issue to discuss before opening a PR.
