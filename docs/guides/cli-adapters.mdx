...
2. **Point Crewship at it** (server-level opt-in; the path is off until you set this):

   ```bash
   CREWSHIP_LOCAL_MODEL_BASE_URL=http://host.docker.internal:11434/v1
   ```
...
3. **Pick a local model** on an OpenCode agent: `ollama/qwen3-coder:30b` or `ollama/devstral:24b`. The onboarding wizard and agent form make the API key optional for these. At run time the orchestrator injects the generated provider block via `OPENCODE_CONFIG_CONTENT`; in restricted network mode the endpoint's host is auto-allowlisted for that run only.
