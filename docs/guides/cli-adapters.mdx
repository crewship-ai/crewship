...
The token and headers are **encrypted and never displayed** — `crewship credential list/get` shows only the base URL. They travel to OpenCode inside the generated `OPENCODE_CONFIG_CONTENT` (`options.apiKey` / `options.headers`). That value is itself an environment variable in the agent container, so — like any BYO credential dialed directly rather than through the sidecar reverse-proxy — the token is readable by the agent process; it is reported by the agent's credential-exposure audit. Existing bare-URL credentials keep working unchanged (no auth).

**HTTPS is required when a token is attached.** Because a bearer token / header sent over plaintext `http://` leaks on the wire, attaching `--auth-token`/`--header` to an `http://` endpoint is rejected — use `https`. The one exception is an `http://` endpoint on a **private/loopback IP literal** (e.g. `http://192.168.1.10:11434/v1`), the on-prem-LAN case where the auth proxy is on a trusted network.

**Rotating the token.** `crewship credential rotate <name> --auth-token <new>` rotates just the token and preserves the base URL and headers. (Rotating an `ENDPOINT_URL` credential with a bare non-URL `--value` is rejected, so a token can't silently overwrite — and erase — the endpoint's base URL.)
...
<Warning>
**Multi-tenant / cloud operators:** the per-crew `--allow-private-endpoints` flag is set by a workspace ADMIN, so on a shared instance it is not sufficient on its own — the effective decision is `instance-cap AND crew-flag`. Private-endpoint egress stays **off for the whole instance** unless the operator enables it at deploy time:

```bash
CREWSHIP_ALLOW_PRIVATE_ENDPOINTS=true   # (or local_models.allow_private_endpoints in config)
```

Default is **off**, so a hosted instance is safe out of the box: a tenant admin cannot self-grant egress into the host's private network (other tenants' containers, host services) by toggling a crew flag. A single-tenant self-host operator sets this once. Link-local/metadata stay hard-blocked regardless. This ceiling lives in operator-controlled deploy config, which tenants cannot reach — unlike workspace settings.
</Warning>
