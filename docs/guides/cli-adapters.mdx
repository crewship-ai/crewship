...
|---|---|---|
| `MINIMAL` | `Read, Glob, Grep` | Read-only — inspection, review, grading. |
| `CODING` (default) | `+ Write, Edit, Bash, WebFetch, WebSearch` | Write + execute + network. The workhorse. |
| `FULL` | `+ NotebookEdit` | Everything useful. |

This governs **only built-in tools**. MCP tools (`crewship-memory`, Composio apps, …) come from the agent's `.mcp.json` via `--mcp-config` and are unaffected by `--tools`, so they always resolve. The real Crewship capabilities (missions, issues, pipelines, keeper, peer messaging) are sidecar HTTP endpoints advertised in the system prompt — not CLI tools — and are gated by the agent's role capabilities.
