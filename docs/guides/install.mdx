...
sudo dnf install ./crewship-<version>.linux_amd64.rpm
```

The package **creates a locked `crewship` service account** and installs the
unit, but deliberately **does not enable or start it** — you opt in after
setting the required secrets:

```bash
# 1. Generate the two required secrets into the env file
...
# Or try it in the foreground without Docker (dashboard only):
...
| `/lib/systemd/system/crewship.service` | the service unit |
| `/etc/crewship/crewship.env` | secrets + overrides (**config file — survives upgrades**) |
| `/var/lib/crewship` | database, snapshots, memory blobs |
