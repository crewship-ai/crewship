---
title: "Install"
description: "Three supported ways to install Crewship: Homebrew tap, signed curl|sh installer, or Docker Compose."
...
<CardGroup cols={3}>
  ...
  <Card title="curl | sh" icon="terminal" href="#curl--sh">
...
```bash
CREWSHIP_VERSION=v0.1.0-beta.1 curl -fsSL https://crewship.ai/install | sh
```
...
```bash
CREWSHIP_INSTALL_DIR=$HOME/.crewship/bin curl -fsSL https://crewship.ai/install | sh
```
...
`ENCRYPTION_KEY` and `NEXTAUTH_SECRET` are **required** — the compose
file uses `${VAR:?...}` syntax that fails fast on `docker compose up`
if they're empty.
...
`crewship doctor` prints what it detected:

- Container runtime (`docker` / `podman` / `colima` / …) and whether
  the socket responds
- Data directory location and write permissions
- Database file present + readable
- Whether [telemetry](/guides/telemetry) is on or off
