...
-- Compaction sink. Aged rows from journal_entries land here with the payload
-- copied into compressed_payload (truncated to 400 chars) before being
-- deleted from the base table; original_size_bytes records the pre-truncation size.
CREATE TABLE journal_entries_archived (
  ...
  compressed_payload  TEXT NOT NULL DEFAULT '{}',  -- payload truncated to 400 chars
  original_size_bytes INTEGER NOT NULL DEFAULT 0
);

-- embedding relation graph: similarity + curated edges linking related entries.
CREATE TABLE memory_relations (
  entry_id         TEXT NOT NULL,
  related_entry_id TEXT NOT NULL,
  relation_kind    TEXT NOT NULL,               -- 'similar' | 'supports' | 'refutes' | 'duplicates'
  score            REAL NOT NULL DEFAULT 0,      -- cosine for 'similar', 1.0 for curated kinds
...
CREATE TABLE memory_health_snapshots (
  ...
  overall      REAL NOT NULL,                   -- weighted sum (see below)
...
<Note>
  ...
  Measured against this repository's own docs tree, `se*` expanded to 160 index
  ...
  therefore searched exactly, never as a prefix.
</Note>

Note the journal's FTS5 index is tokenized `porter ascii`, which stems English
...
- **`similar`** — `LinkSimilarOnIndex` runs at insert time: for the freshly-indexed entry, query the existing embedding pool, take the **top-3 cosine matches above 0.80**, and write `similar` rows. Edges are **symmetric** — both `(entry_id=new, related_entry_id=match)` and the reverse `(entry_id=match, related_entry_id=new)` are inserted with `relation_kind='similar', score=cosine`, so a cosine relation reads the same from either end.
- **`supports`** — `LinkSupports` is called by the consolidator when it derives a rule from one or more journal entries. The rule entry points (directionally) to every supporting evidence entry: `relation_kind='supports', score=1.0`. This is what makes consolidated rules traceable back to their evidence.

The `relation_kind` CHECK also reserves `refutes` and `duplicates`; neither is populated automatically today.
...
The composite `overall` score (exposed as `overall` in the API/DB row) is reported with three colour bands:
