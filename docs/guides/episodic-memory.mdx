...
<Note>
**Roadmap (v0.2).** The episodic package ships in v0.1 but the embed-on-write hook into the journal writer is not yet wired. Vector recall queries return empty results until v0.2. Manual indexing via API works for evaluation purposes.
</Note>
...
Health is computed by `internal/episodic/health.go`'s `ComputeSnapshot`. The daily consolidator job calls it after `IndexNew`; operators can call it on demand via:
