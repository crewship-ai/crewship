...
Every POST accepts an optional `trace_id`. When the orchestrator wires the active OTel trace onto the assistant turn (`turn.metadata.trace_id`), the frontend includes it in the payload and the row lands indexed for `WHERE trace_id = ?` queries.

```sql
CREATE INDEX idx_feedback_trace ON message_feedback(trace_id) WHERE trace_id IS NOT NULL;
```

This is what powers the "show me every signal for this routine run" eval-mining query: collector trace ID → `message_feedback.trace_id` → all signals filed against that conversation.
