...
The `message_feedback` table (migration v95) gives consumers a typed `signal` column with a CHECK constraint, a free-form `reason`, and a `trace_id` link back to the [OTel trace](/guides/tracing) of the run that produced the message. Eval datasets join on `trace_id` to recover the (prompt, original answer, preferred answer, ts) tuple.
...
Adding a new signal requires both a UI patch and a migration that widens the v95 CHECK clause — the handler validates the enum before any DB work so a renamed value fails fast with a readable 400 instead of a SQLite constraint violation.
...
<Note>
  **Current state (PR #447):** the schema, API, frontend store, and `ChatTurn.metadata.trace_id` interface are wired and consume the field when present. The orchestrator → WebSocket → `ChatTurn` propagation is **not yet shipped** — feedback rows for `assistant` turns produced after this PR will land with `trace_id = NULL` until the follow-up lands. The partial index `WHERE trace_id IS NOT NULL` keeps the lookup path cheap regardless. Direct API callers (CLI, eval widgets) that already have the trace id in hand can pass it on POST today.
</Note>
