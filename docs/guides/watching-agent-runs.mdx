...
<Warning>
**Only chat-initiated runs stream today.** A run started from the chat UI, `crewship run`, or `crewship ask` emits frames on the session channel. A run started by a **routine, a webhook, a pipeline step, or the internal agent-start IPC** does not — those paths execute the agent without publishing to the session channel at all, so attaching to one yields `stream.open` with `active:false`, then `stream.end` / `no_active_run`, and no output.

The run is really executing; there is simply nothing on the channel to watch. This is not specific to the HTTP stream — the WebSocket path has the same blind spot — and [issue #1823](https://github.com/crewship-ai/crewship/issues/1823) tracks making those runs watchable. Until it lands, use the [Crew Journal](/guides/crew-journal) to follow automated runs.
</Warning>

## When to use it

- **Watch a run somebody else started.** `crewship run` and `crewship ask` already follow the run they launch. This follows a chat-initiated run started by the web UI or another shell — you only need the chat id. (Routine, webhook and pipeline runs are not yet covered; see the warning above.)
