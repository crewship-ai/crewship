...
<Warning>
Accounts created by the old flow have no password — Google was their only credential. They cannot sign in until an admin adds them again (Settings → Members → Add member) with an email that has no account yet, or until they use **Forgot password**, which works because the address is on file.
</Warning>
...
<Tabs>
  <Tab title="UI">
    **Settings → Members → Add member.** Enter their email and role. Crewship creates the account if the email is new, adds it to the workspace, and shows a setup link. Copy it and send it however you like — Slack, SMS, in person.
    ...
    The link is shown **once**. The token is stored hashed, so it cannot be displayed again; if it goes astray, add the person again to issue a fresh one (which invalidates the old).
  </Tab>
  <Tab title="CLI">
    ```bash
...
If the email already belongs to an account, that person is added to the workspace and **no setup link is issued** — they sign in with the password they already have. Issuing one would hand whoever holds the link the ability to change that password, and creating a workspace is self-serve.

Requires **ADMIN** or **OWNER**. `CREWSHIP_PUBLIC_URL` must be set — without it there is no link to build, and Crewship refuses rather than creating an account nobody can reach.
