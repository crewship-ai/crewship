...
- **Signup's collision is handled by email, not by the response.** When the address is taken, the owner gets a "someone tried to sign up with your email" notice (only if a mailer is configured; otherwise it is logged with the address hashed) — exactly the split `/forgot` uses. Both signup paths also burn one bcrypt at the same cost so the *timing* doesn't answer what the body won't.
