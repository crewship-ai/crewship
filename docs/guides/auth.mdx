...
Pass `scopes` when minting to restrict what the token can do — a CI token that only triggers agent runs never needs to delete crews or read credentials:

```bash
curl -sX POST https://crewship.example.com/api/v1/auth/cli-token \
  -H "Cookie: $SESSION_COOKIE" \
  -d '{"name":"ci-agents","scopes":["agents:write","crews:read"]}'
```

Scopes use the shape `<resource>:<action>`. The recognised vocabulary:

| Resource | Scopes |
|----------|--------|
| `agents` | `agents:read`, `agents:write`, `agents:run`, `agents:*` |
| `crews` | `crews:read`, `crews:write`, `crews:*` |
| `credentials` | `credentials:read`, `credentials:write`, `credentials:*` |
| `skills` | `skills:read`, `skills:write`, `skills:*` |
| `webhooks` | `webhooks:read`, `webhooks:write`, `webhooks:*` (outbound notification channels) |
| `workspace` | `workspace:read`, `workspace:admin`, `workspace:*` (workspace administration) |
| _all_ | `*` |
...
- **No read⇒write implication:** an `agents:read` token cannot call an `agents:write` route.
...
- **Reads are not scope-gated yet** — a scoped token can still read; scoping applies to mutations. Read scoping is a planned follow-up.
