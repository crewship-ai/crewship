...
{/* docs-inventory: ignore-api /api/v1/auth/google/redirect reason=historical-removed-route */}
{/* docs-inventory: ignore-api /api/v1/auth/google/callback reason=historical-removed-route */}
# Authentication
...
| **Bcrypt hash** | The on-disk form of every **password**. Cost factor is **12**, declared once as `api.ProductionBcryptCost` (`internal/api/bcrypt_cost.go`) and read by signup, bootstrap, password reset, profile password change, public-page tokens, the signin timing equaliser, and the admin CLI's `reset-password` — so it is held constant across origins by construction and the verifier doesn't have to special-case any of them. The test binary lowers its own copy (see the guard tests in `bcrypt_cost_test.go`); a server never does. (CLI tokens are *not* bcrypt — see **CLI token** below.) |
...
The endpoint is idempotent — a second call against a database that already has any user returns 410 (`Already initialized — please log in at /login instead`), so re-running provisioning is safe.
...
crewship login --server https://crewship.example.com \
  --email demo@crewship.ai --password-stdin <<< "$LOGIN_PASSWORD"

# Or via an env var — no stdin involved at all:
CREWSHIP_PASSWORD="$LOGIN_PASSWORD" \
...
|--------|------|-------------|
| `POST` | `/api/v1/bootstrap` | Create the first `OWNER` on a fresh database. Idempotent — 410 if any user exists. |
