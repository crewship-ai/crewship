...
An account with **none** of them is unclaimed — that is exactly what this endpoint creates. Re-adding an unclaimed person issues a fresh link and invalidates the previous one, which is the supported way to recover a link that went astray before it was used.
