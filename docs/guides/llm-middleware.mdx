...
Every constructor in `internal/llm` returns something the middleware accepts, so
which backend you wrap is a construction detail the stack never sees:

```go
base := llm.NewAnthropic(apiKey)                       // hosted Anthropic
base = llm.NewAnthropicWith(llm.AnthropicConfig{...})  // configurable: endpoint, version, betas
base = llm.NewOpenAI(apiKey)                           // hosted OpenAI
base = llm.NewOpenAICompat(cfg)                        // any OpenAI-compatible backend
base = llm.NewOllama(baseURL, model)                   // local daemon, native protocol
```

`llm.BuildAuxProvider` does the same resolution from a provider **id** rather
than a constructor, via the registry — see
...
- **Anthropic** — `internal/llm/anthropic.go` ships `anthropic-beta: prompt-caching-2024-07-31` by default and stamps `cache_control: ephemeral` on the system prompt and the last tool definition (tool schemas are usually large and stable across turns — single highest-leverage breakpoint). Response usage parses `cache_read_input_tokens` + `cache_creation_input_tokens`. The header set is no longer fixed: `AnthropicConfig.Version` and `.Beta` are configurable, and a **non-nil empty** `Beta` slice sends no beta header at all — for a proxy that rejects unknown betas. A nil slice keeps the default set, so the zero config is byte-identical to the old hard-coded client.
- **OpenAI** — auto-activates for prompts ≥1024 tokens (Sept 2025). Response usage parses `prompt_tokens_details.cached_tokens` (no separate creation counter), and the codec **subtracts** it from `prompt_tokens` before storing `Response.InputToks`, because OpenAI's count includes the cached read. `InputToks` therefore means *fresh* input on every provider — see [Paymaster](/guides/paymaster#prompt-cache-token-flow).
...
- [Multi-provider LLM configuration](/guides/multi-provider-llm) -- layer 5. Which provider you are wrapping, and how it is built.
