...
The `chat.user_message` entry beside it follows the same rule: `payload.content`
is scrubbed and capped to the same 240 characters as the entry summary, with
`length_chars` recording the message's full length in characters and
`truncated` saying whether the cap applied. One consequence worth knowing:
