...
The LLM call itself goes through a new `run_summary` aux-model slot (`internal/llm/aux.go`, same PR-B F3 auxiliary-model machinery Keeper's F4 evaluators use) — defaults to `claude-haiku-4-5`, resolved and the provider built once at boot (`Router.RunVerdictProvider()`), shared by every executor construction site (HTTP, boot-resume, cron scheduler) rather than re-resolved per run.
