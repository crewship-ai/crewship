...
| chevron | 14px | Disclosure state; the row carries `aria-expanded` and `aria-controls`. |
...
- Agent resolved → DiceBear avatar from `avatar_seed` + `avatar_style`.
...
- No `agent_id` at all (`system`, `sidecar`, `orchestrator`, `keeper`, `user`) → a labelled glyph, never a blank seat.
...
Rows are memoized and the list's `itemContent` is stable, so a keystroke in the search box does not re-render every mounted row. Anything added to the row must keep both properties or virtualization regresses under load.
