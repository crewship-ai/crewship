...
Returns a workspace-scoped map of `crews`, `agents`, and `missions` so the UI can render entry cards with palette-coloured chips and lucide icons without joining on the streaming path. The provider is fetched once on page mount and invalidated by realtime events (new crew, renamed agent, ...). `JournalLookupProvider` / `useJournalLookup` are the React hooks; backend handler is `internal/api/journal_lookup.go`.
