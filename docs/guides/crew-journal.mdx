...
Cross-tenant existence is never leaked: unknown IDs return 404 with the same shape as "not in your workspace". See [`paymaster_handler.go`](/api-reference/paymaster) for the shared `crewBelongsToWorkspace` / `missionBelongsToWorkspace` helpers used across every read handler.
