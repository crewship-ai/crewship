...
| Bucket | Entry types |
|---|---|
| Communication | `peer.conversation`, `peer.escalation` (one type per escalation lifecycle transition — `payload.state` is `pending`, `resolved`, `expired` or `cancelled`), `message.broadcast`, `agent.mentioned` |
...
| Security | `keeper.request`, `keeper.decision`, `keeper.rule_auto_tuned`, `guardrail.input_blocked`, `guardrail.output_blocked`, `approval.requested/granted/denied/timeout/cancelled`, `approval.trust_granted/trust_revoked` |
