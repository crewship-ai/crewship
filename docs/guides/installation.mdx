...
The crewship-prod VM uses a similar but separate flow — `crewship-deploy.timer` polls every 5 minutes, fast-forwards `release`, and rebuilds if the SHA changed. To deploy to prod, push `main:release` rather than running deploy directly on the VM.
...
```bash
ssh crewship-dev                    # Connect to dev VM
cd /opt/crewship && ./dev.sh start  # Start services (run inside tmux)
```

| Resource | Value |
|----------|-------|
| Backend URL | `http://192.168.1.201:8080` |
| Frontend URL | `http://192.168.1.201:3001` |
| vCPU | 12 |
| RAM | 64 GB |
| Storage | 200 GB NVMe |
...
<Tip>
  ...
  ```bash
  code --remote ssh-remote+crewship-dev /opt/crewship
  ```
</Tip>
