...
<CardGroup cols={2}>
  ...
  <Card title="Node.js 22+ & pnpm" icon="node-js">
...
<Steps>
  ...
  <Step title="Open the web UI and complete onboarding">
    ...
    for the initial workspace, LLM credential, and your first crew.

    Prefer the terminal? `crewship init --email you@example.com --name "You"`
    hits the same bootstrap endpoint, and `crewship setup` mirrors the
    onboarding wizard — see the [quickstart](/quickstart) for the CLI-first
    path. (Manual DB seeding is *not* supported.)
  </Step>
</Steps>
...
pnpm build --> out/ (static HTML/JS/CSS)
    |
    v
embed-web-out.sh sync --> web/out/  (Go embed source)
    |
    v
...
# Add ?workspace_id=<ws> to resolve as ADMIN and get the full inventory:
# every runtime present, each with its own version and socket, and which
# one is in use. Without it the response is just {"available": true}.
curl -H "Authorization: Bearer <token>" \
  http://localhost:8080/api/v1/system/runtime
```

See [Which container runtime is Crewship actually
using?](/guides/troubleshooting) for the payload and the admin console
