...
```json
{
  ...
  "steps": [
    ...
    },
    {
      ...
      "wait": {
        "kind": "approval",
        "approval_title": "Publish triage for {{ inputs.ticket_id }}",
...
| Every pending approval looks the same in the inbox | No `approval_title`, so the row falls back to the prompt's first line — the same string on every run | Set `approval_title` to something templated, e.g. `"Publish triage for {{ inputs.ticket_id }}"` |
