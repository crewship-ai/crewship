...
|---|---|---|
| GET | [`/api/v1/workflow-templates`](#get-api-v1-workflow-templates) | List templates |
| POST | [`/api/v1/workflow-templates`](#post-api-v1-workflow-templates) | Create a template |
| GET | [`/api/v1/workflow-templates/{id}`](#get-api-v1-workflow-templates-id) | Get a single template |
| PATCH | [`/api/v1/workflow-templates/{id}`](#patch-api-v1-workflow-templates-id) | Update a template |
| DELETE | [`/api/v1/workflow-templates/{id}`](#delete-api-v1-workflow-templates-id) | Delete a template |
...
| `template_json` | string | JSON-encoded stage array. See [Stage shape](#stage-shape). The server validates and re-marshals it on every write, so reads use canonical JSON formatting. |
| `icon` | string \| null | Optional icon value returned to the client. |
| `color` | string \| null | Optional template color. On update, a non-empty value must be a 3- or 6-digit hex string such as `#3B82F6`. |
...
| `template_json` | string | Yes | Stage array (see [Stage shape](#stage-shape)). The handler validates and canonicalizes this value before storing it. |
| `icon` | string \| null | No | |
| `color` | string \| null | No | Optional template color. The create handler stores this value without applying the update handler's top-level hex validation. |

**Response:** `201 Created` with the full template object.

| Status | Condition |
|---|---|
| `400` | Invalid JSON, missing `name`, missing/invalid `template_json` (any stage rule above). |
...
**Request:** No request body. The `{id}` path parameter identifies the workflow template.
...
Hard delete. The template row is removed outright; the handler does not
soft-delete it.

**Request:** No request body. The `{id}` path parameter identifies the workflow template to delete.
