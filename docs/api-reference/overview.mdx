...
# Connect, then send the token as the first message (not a URL query
# param — that would leak into proxy/access logs and Referer headers)
...
These are the five action gates checked by `canRole` (`internal/api/helpers.go:438`). `create`/`update` share the same tier, as do `manage`/`delete`.
...
| `404` | Not Found |
| `405` | Method Not Allowed -- the path exists but not for this method; the `Allow` response header lists the ones it does support |
| `409` | Conflict -- duplicate slug, relation already exists |
| `429` | Too Many Requests -- per-IP rate limit; `Retry-After` and `X-RateLimit-*` say how long to back off |
...
All system rows below are `GET` requests with no body. **Response:** the named
...
| `GET /api/v1/system/resources` | [Measured host CPU and RAM history](/api-reference/system-resources) for dashboard windows |
| `GET /api/v1/system/resources/latest` | Latest host CPU and RAM reading without scanning chart history |
...
| `GET /api/v1/system/aux-status` | Auxiliary-model assignment per subsystem — `{"subsystems": […]}` with provider, model, timeout, `explicit`/`fallback`/`keeper_config` source, plus `healthy` (provider builds) and `reachable` (the model server answered; omitted when not probed) |

### Activity

| Endpoint | Description |
|---|---|
| `GET /api/v1/journal?entry_type=peer.conversation,peer.escalation,assignment.created,assignment.running` | Cross-crew activity feed, sourced from the journal (the standalone `/api/v1/activity` aggregator was removed). Filter by `crew_id`/`agent_id`, bound with `since=`. See also the `crewship activity` CLI command. |

### Auth / Signup

**Request:** JSON for `POST` rows and no body for `GET`; **response:** the
...
**Request:** CSRF/session cookies as required by the browser flow; otherwise no
JSON body. **Response:** NextAuth JSON or redirect. **Statuses:** `200` success;
...
state or completion JSON. **Statuses:** `200` success; `400` invalid step;
...
**Request:** no body for these `GET` routes. **Response:** the resource JSON
...
| `GET /api/v1/admin/health` | Process, database, disk, encryption-key, and log-level health snapshot (ADMIN/OWNER) |
| `GET /api/v1/admin/log-level` | Read live and baseline log levels (ADMIN/OWNER) |
| `PUT /api/v1/admin/log-level` | Set a live log level with optional TTL (ADMIN/OWNER) |
| `GET /api/v1/admin/legacy-resources` | Detect orphaned pre-C1 Docker resources (ADMIN/OWNER) |
| `GET /api/v1/admin/rate-limits` | List instance-wide limiter states (ADMIN/OWNER) |
| `PUT /api/v1/admin/rate-limits/{key}` | Override one limiter (ADMIN/OWNER) |
| `DELETE /api/v1/admin/rate-limits/{key}` | Reset one limiter to its default (ADMIN/OWNER) |

All rows above take no request body and return JSON. The health, legacy, and
rate-limit reads return `200` on success; admin authorization failures return
...
`503` when its configuration store is unavailable. See the [Admin API](/api-reference/admin)
for complete response shapes.

### Audit

**Request:** optional query filters, no body. **Response:** paginated audit JSON.
**Statuses:** `200` success; `403` missing `manage` permission.
...
**Request:** optional query filters, no body. **Response:** workspace-scoped
run list. **Statuses:** `200` success; `401` unauthenticated; `403` forbidden.
...
**Request:** JSON for mutation rows and no body for `GET`; **response:** workspace
