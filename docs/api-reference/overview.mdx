...
<CardGroup cols={2}>
  ...
  <Card title="Agents" href="/api-reference/agents">
    CRUD for agents, skills, credentials, chats, runs, fleet status.
  </Card>
...
### Captain (Workspace AI Assistant) — deprecated

<Warning>
  **Deprecated (2026-04-16):** Captain endpoints remain registered for
  backward compatibility but receive no new features. See the
  [Captain guide](/guides/captain) for migration path.
</Warning>

| Endpoint | Method | Description |
|---|---|---|
| `/api/v1/captain/chat` | POST | *(deprecated)* Send message to Captain (SSE streaming response) |
| `/api/v1/captain/context` | GET | *(deprecated)* Get workspace context (phase, counts) |
| `/api/v1/captain/history` | GET | *(deprecated)* Load Captain conversation history |
| `/api/v1/captain/history` | DELETE | *(deprecated)* Clear Captain conversation history |

See the [Captain guide](/guides/captain) for detailed documentation on tools, streaming protocol, and onboarding phases.
