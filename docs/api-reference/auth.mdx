...
<Note>
  Need the new user's id (to add them to a workspace, say)? Use `POST
  /api/v1/workspaces/{workspaceId}/members` with `{"email": "..."}` —
  it resolves the address behind the workspace-manage role gate.
</Note>
