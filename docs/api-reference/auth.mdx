...
**Response:** `201 Created` with the new user + workspace summary and
a `Set-Cookie: __Secure-authjs.session-token=...` header. The caller
is signed in — no follow-up login call needed.
