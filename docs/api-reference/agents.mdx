...
The blob is written to `<crewID>/<agentSlug>/attachments/<chatId>/<filename>`,
which the agent sees at `/output/<agentSlug>/attachments/<chatId>/<filename>`.
A provisioned crew owns that tree as uid 1001, so the server cannot create the
...
message. The `attachments` metadata row is written only after the bytes land —
...
**Response:** `200 OK` or `201 Created` with attachment metadata.

**Status:** `400` invalid upload, `401` unauthenticated, `403` denied, `404`
unknown or mis-nested agent/chat, `409` the crew container is not running
(start the crew and retry), `413` too large to write through the crew
container, `503` no container runtime is configured, and `502` sidecar
failure.
