...
| `cost_usd_this_month` | `float` | Sum of `paymaster_events.cost_usd` for runs attributed to this agent in the current calendar month. |
| `llm_calls_this_month` | `int` | Count of `agent_run` step completions in the current month. |
| `tokens_used_this_month` | `int64` | Sum of input + output tokens reported by the journal in the current month. |
