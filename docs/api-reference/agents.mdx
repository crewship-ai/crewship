...
the next generator upgrade will change its face again.

<Note>
  A stored render depicts one specific `(avatar_seed, avatar_style)` pair,
  so it is dropped automatically whenever either changes: via `PATCH
  /api/v1/agents/{agentId}`, or in bulk via [`POST
  /api/v1/crews/{crewId}/apply-avatar-style`](/api-reference/crews). An
  unrelated edit (a rename, a model change) leaves it intact.
</Note>
