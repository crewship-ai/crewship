...
| GET | [/api/v1/agents/{agentId}/credential-readiness](#credential-readiness) | Report whether a delivered credential authenticates the agent's model |
...
Provider-account metadata is permission-dependent. Callers without account
access receive `pays_with: { "provider": "OPENAI", "restricted": true }`
(with the applicable provider), not an unfunded-agent verdict. Account name,
credential id, owner, plan and refresh details are withheld.

**Response:** `200 OK` -- full agent object, plus `pays_with`: the provider login paying for this agent's model — `{ "credential_id", "name", "login" }` with the same `login` object the credentials API returns (see [Credentials → Response Fields](/api-reference/credentials#response-fields)), or `null` when nothing in the agent's resolved delivery is a login of its adapter's provider (`CLAUDE_CODE` → `ANTHROPIC`, `CODEX_CLI` → `OPENAI`, `GEMINI_CLI` → `GOOGLE`, `CURSOR_CLI` → `CURSOR`, `FACTORY_DROID` → `FACTORY`, `OPENCODE` → the agent's `llm_provider`). Derived on this route only; list rows carry `null`.
...
| `unavailable` | `string[]`, optional | Component reads that failed (`approvals`, `assignments`, `escalations`, `peers`, `cost`). Do not present their default values as measured zero. |
...
Per-`(agent, user)` markdown notes — the agent's own "what I know about this person" file.
Automatic `PeerCardSync` generation is currently unavailable; existing notes
remain readable. These endpoints are **self-only**: the cards live on disk
...
primitives. The agent-specific list returns only the signed-in user's row.
Read and delete require `{userId}` to equal that user's ID, including for
workspace owners and administrators; another subject returns `403`.
Administrator compliance operations use their explicit admin endpoints.
...
```
GET /api/v1/agents/{agentId}/credential-readiness?workspace_id={workspaceId}
```

Reports whether a credential the runtime will actually deliver to this agent
authenticates its adapter's model provider. The agent-scoped member of the
readiness family ([Crew Credential Readiness](/api-reference/crews#credential-readiness)
is the other).

It exists because [List Credentials](#list-credentials) answers a different
question. That listing is the union of everything that reaches the agent, so one
crew-scoped `GH_TOKEN` binding makes it non-empty for every agent in the crew —
including an agent with no model credential, whose first run then fails at the
first model call. Nor is "the model variable in the env is unfilled" the right
test: for `CLAUDE_CODE`, `CODEX_CLI` and `GEMINI_CLI` the sidecar reverse proxy
injects the key mid-flight and the env deliberately holds a dummy.

The verdict is computed by the runtime's own delivery rules — the same
classification that decides what the sidecar CredStore holds, which login is
written to `CLAUDE_CODE_OAUTH_TOKEN` or rendered to a login file, and which key
an OpenCode or Cursor run gets in its env — over the same resolved delivery set
the run boots with (explicit grants, bindings by scope, crew links; lease-gated).
No credential value is decrypted to answer.

The endpoint is **read-only and advisory**. It grants nothing. A provider or
adapter the runtime has no opinion about reports `unknown` rather than guessing,
for the same reason the crew report does: a false "missing" on a healthy agent
teaches the operator to ignore the report.

**Request:** `agentId` is a required path parameter and `workspace_id` selects the workspace. No request body.

**Auth:** any workspace member. Below the `manage` role the matched credential's
`credential_name` is withheld — the same rule that hides provider accounts from
[List Credentials](#list-credentials) — while the verdict itself is not.

**Response:** `200 OK`

```json
{
  "agent_id": "cagent0123456789abcdefghi",
  "agent_slug": "parity-probe",
  "adapter": "CLAUDE_CODE",
  "model_credential": {
    "state": "missing",
    "provider": "ANTHROPIC"
...
```json
{
  "agent_id": "cagent0123456789abcdefghj",
  "agent_slug": "casey",
  "adapter": "CLAUDE_CODE",
  "model_credential": {
    "state": "ready",
...
| `agent_slug` | string | The agent's slug |
| `adapter` | string | The agent's `cli_adapter` |
| `model_credential.state` | string | `ready` — a delivered credential authenticates the provider. `missing` — the provider is known and nothing delivered authenticates it. `unknown` — no opinion: an unrecognised adapter, an `OPENCODE` agent naming no provider, or a model on the operator's local endpoint |
| `model_credential.provider` | string | The LLM provider the adapter pays through (`CLAUDE_CODE` → `ANTHROPIC`, `CODEX_CLI` → `OPENAI`, `GEMINI_CLI` → `GOOGLE`, `CURSOR_CLI` → `CURSOR`, `FACTORY_DROID` → `FACTORY`, `OPENCODE` → the model's provider prefix, else `llm_provider`). Omitted when unknown for want of one |
| `model_credential.credential_name` | string | The matched credential's name. Only when `ready`; withheld below the `manage` role |
| `model_credential.credential_id` | string | The matched credential's ID. Only when `ready` |
| `model_credential.source` | string | Where the match comes from, in the vocabulary of `crewship credential resolve`: `agent_grant`, `agent_binding`, `crew_binding`, `workspace_binding`, `crew_link`. Only when `ready` |
| `model_credential.delivery` | string | The channel the runtime uses: `sidecar` (the proxy injects it; the env holds a dummy), `login_env` (`CLAUDE_CODE_OAUTH_TOKEN`), `login_file` (Codex `auth.json`, Gemini `oauth_creds.json`), `env` (the real key in the env, for a CLI the proxy cannot inject into). Only when `ready` |
| `notes` | string[] | Operator-facing reasons: why an unknown is unknown, what a missing is missing. Never `null` |
...
```
crewship agent credential-readiness <slug-or-id>
```
...
| `kind` | string | Optional. `all` (must be supplied alone), `direct`, `routine`, `issue`, `agent` — comma-separated; filters before the page is cut. |
| `q` | string | Optional. Server-side substring search on the session title, Unicode case-folded, applied before pagination — so it matches sessions beyond the page a caller holds. Composes with `kind`, `limit` and `offset`. |
| `chat_id` | string | Optional. Restricts the list to that one chat id. |
| `routine_id` | string | Optional. Only chats whose persisted pipeline run resolves to that routine (pipeline) in the workspace. |
| `source` | `1` | Optional. Attaches a `source` object to each row naming the chat's persisted issue/routine provenance (see below). |
...
**Response:** `200 OK` -- array of chat session objects, ordered by
`last_activity_at` descending:

| Field | Type | Meaning |
|---|---|---|
| `id`, `agent_id`, `workspace_id` | string | Identity of the session and where it lives. |
| `title` | string or null | The session's name, if it was given one (see [Rename Chat](#rename-chat)). |
| `mode` | string | `CHAT` for a conversation, `MISSION` for the synthetic chat an issue runs its work in. |
| `status` | string | Session status (`ACTIVE` until ended). |
| `origin` | string or null | Raw provenance token as stored: `UI`, `CLI`, `WEBHOOK`, `CRON`, `AGENT` or `ROUTINE`; `null` when the writer sent none or an unknown value. |
| `kind` | string | The partition the row falls in, decided from `mode` and `origin` on the server: `direct` (a person talking to the agent — the catch-all), `routine` (a routine step, cron or webhook dispatch), `issue` (mode `MISSION`), `agent` (agent-to-agent delegation). Never null; always the same rule the `kind` filter uses. |
| `message_count` | int | Messages in the session. |
| `unread_count` | int | Messages the caller has not seen and did not write. |
| `source` | object or null | Only with `source=1`: the chat's persisted provenance, never guessed from the title. `{kind, id, name, slug?, run_id?, step_id?}` — `kind` is `routine` (with the pipeline's id/name/slug and, when known, the `run_id` and `step_id` the chat was minted for) or `issue` (with the issue's id, title and identifier). `null` when the chat has no persisted source; the object is omitted entirely without `source=1`. |
| `started_at`, `ended_at`, `created_at`, `last_activity_at` | string | Timestamps; `ended_at` is null while the session is open. |
...
**Status:** `400` for invalid request/workspace context, or for a `kind`
value that names no kind (`?kind=,`, `?kind=chat`) or combines `all` with
another kind — the filter fails closed rather than silently listing
everything; `401` when unauthenticated; `403` when the caller cannot access
the workspace.
...
`origin` may be `UI`, `CLI`, `WEBHOOK`, `CRON`, `AGENT` or `ROUTINE`. It is
provenance, not input the caller is entitled to invent: any other value is
**stored as null, not rejected** — the chat is created and lists with
`origin: null` and `kind: direct`. The response is `{ "id": "<chatId>" }`.
...
one of `UI`, `CLI`, `WEBHOOK`, `CRON`, `AGENT` or `ROUTINE`.

**Status:** `201` with the created chat id; `400` invalid JSON,
`401` unauthenticated, `403` denied, `404` agent not found, and `409`
when the requested session conflicts. An unknown `origin` is not a `400`.
...
`POST /api/v1/agents/{agentId}/stop` targets the active invocations owned by
this daemon when it receives the request. A `200` / `STOPPED` response follows
confirmed process termination and completion of those invocations. Direct
execs use an isolated process group; tmux runs use their per-run session.

If the daemon is unavailable, cannot identify the runtime (including an
unowned run after restart), or cannot confirm termination within its bounded
wait, the API returns `502` and leaves the stored agent status unchanged.
The request does not disable the agent or prevent a separate new submission.
