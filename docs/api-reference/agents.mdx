...
# (IDLE/RUNNING/ERROR/STOPPED). Drives the 'Crews idle/online' badge
# in the top toolbar.
```
...
All four status fields (`running`, `error`, `idle`, `stopped`) are always present in the response — zero counts are not omitted.
