...
<Note>Early: `issue_agent_sessions` is work package B1 of the Issues & Routines architecture (`docs/prd/PRD-ISSUES-AND-ROUTINES-2026.md`) — the event-log and session foundations. A mention resolves or creates a session; nothing yet reads it back into an agent's context, and the state machine (`pending` → `active` → …) has only its `pending` starting state wired. That is later work (B2–B5), not a gap in this endpoint.</Note>

The durable, per-(issue, agent) cursor a mention resolves or creates. `UNIQUE(mission_id, agent_id)` on the underlying table means mentioning the same agent on the same issue twice never creates a second session — the second mention finds and reuses the first's row.
