...
<Note>Early: `issue_agent_sessions` is work package B1 of the Issues & Routines architecture (`docs/prd/PRD-ISSUES-AND-ROUTINES-2026.md`) — the event-log and session foundations. A mention resolves or creates a session; the state machine (`pending` → `active` → …) still has only its `pending` starting state wired — that transition is later work (B4). What B3 (#2339) added is the run-admission guarantee this session exists to carry: at most one `assignments` row may be `PENDING`/`QUEUED`/`RUNNING` for a given session at a time, enforced by a partial unique index rather than a read-then-write check. `active_run_id` itself is not yet populated by any writer — read `assignments.session_id` to find a session's current or most recent run.</Note>
...
| `state` | string | One of `pending`, `active`, `awaiting_input`, `idle`, `error`, `stale`, `closed`. Every session created today is `pending` — the transitions between the other states are later work (B2/B4) |
