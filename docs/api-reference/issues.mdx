...
<Note>Session lifecycle: `issue_agent_sessions` is work package B1 (event log + session foundations) of the Issues & Routines architecture (`docs/prd/PRD-ISSUES-AND-ROUTINES-2026.md`), extended by B3 (#2339, the run-admission guarantee: at most one `assignments` row may be `PENDING`/`QUEUED`/`RUNNING` for a given session at a time, a partial unique index rather than a read-then-write check) and B4 (#2343): `state` genuinely moves with its runs — `pending → active` when a run wins the admission CAS, `active → idle` when that run completes or is cancelled, `active → error` when it fails or its lease expires with no live driver — and `active_run_id` is populated for as long as a run is live, cleared once it settles. What is still NOT wired: `awaiting_input` (needs the `outcome` contract, B6) and the 14-day `idle → stale` sweep. An ephemeral agent that expires mid-session is reconciled onto `error` (was active) or `closed` (otherwise) rather than left rendered `active` over a ghost.</Note>
