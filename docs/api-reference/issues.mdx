...
Lists every run attributed to an issue — not just mission tasks joined to their `assignments` row, but also mention-dispatched runs (an agent tagged in a comment) and delegation-hop runs (a sub-agent assigned further work via `/assign` mid-mission). Newest-first, capped at 100 rows.
