...
`200 OK` instead of `201` means these exact bytes were already attached to this
issue: no second record was created, no second timeline entry was written, and
the returned object is the one that already existed.

| Status | Condition |
|---|---|
| `201` | Attached |
| `200` | Already attached — the same bytes, the same issue |
