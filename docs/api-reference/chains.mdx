---
title: "Chains"
description: "Reconstruct the causal graph around one issue, run, routine, automation or assignment in a single call."
icon: "share-nodes"
---

A chain answers "what caused what". Given one anchor, it returns the connected
graph around it — the issue, the routine it is bound to, the runs that fired,
the nested runs those called, the assignments agents were given, who executed
them, and the inbox items the whole thing produced.
...
| Param | Description |
|---|---|
| `anchor` | An issue identifier (`ENG-4`), an issue id, a run id, a routine id, a routine slug, an assignment id, an automation id, or an inbox item id. Resolved in that order; the first match wins. |
...
| `nodes[].kind` | string | `issue`, `routine`, `run`, `automation`, `assignment`, `agent`, `inbox`. Open set — treat an unknown kind as opaque rather than an error. |
| `nodes[].ref` | string | Primary key of the underlying row. |
| `nodes[].key` | string | Human handle. Unique per workspace for `issue` (identifier), `routine` and `agent` (slug); for `run` it is the routine slug and for `inbox` the item kind, so it is **not** an identifier for those. |
...
| `nodes[].depth` | integer | Shortest distance from the anchor, in hops. |
...
| automation → run | `triggers` | `triggered_via='automation'`, `triggered_by_id` = `automations.id` |
| automation → routine | `triggers` | `automations.action_config_json` → `$.routine_slug`, joined to `pipelines.slug` in the same workspace |
...
id, an automation id, or an issue identifier depending on `triggered_via` — so it is only ever
...
| 404 | No issue, run, routine, automation, assignment or inbox item matches the anchor **in this workspace**. Deliberately the same answer as an anchor that exists in another workspace — otherwise this endpoint becomes an oracle for identifiers in a tenant the caller cannot read. |
| 500 | Query failed. |
