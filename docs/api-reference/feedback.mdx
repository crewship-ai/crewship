...
Idempotent at the `(workspace_id, message_id, user_id, signal)` tuple. A re-submit in the same workspace replaces `reason`, `trace_id`, and `chat_id` on the existing row rather than producing a duplicate. A forked workspace can retain its own feedback row for the same source message id.

**Request body:**

| Field | Type | Required | Notes |
|---|---|---|---|
| `message_id` | string | yes | Turn id from the chat UI. Must already exist in the `conversation_messages` search mirror — see below. Max 256 chars. |
| `signal` | enum | yes | One of: `helpful`, `not_helpful`, `inaccurate`, `unsafe`, `edit`, `regenerate`. |
| `chat_id` | string | no | Accepted for older clients but not trusted for authorization. Workspace and chat are always resolved from `message_id`'s own `session_id`. Max 256 chars. |
...
<Note>
  `message_id` must resolve to a real, visible message — not merely be well-formed. The handler looks it up in `conversation_messages` (the search mirror, migration v111) and derives `workspace_id`/`chat_id` from that row's own `session_id`, then confirms the caller is a member of that workspace. A `message_id` that was never persisted there (nothing has been sent in that chat yet, or it predates the mirror) gets the same `404` as a message in a chat the caller can't see — the two aren't distinguishable from outside, closing a cross-tenant message-existence oracle (#1208). `chat_id` in the body is accepted but no longer authoritative.
</Note>
...
| `404` | `message_id` doesn't resolve to a row in `conversation_messages`, or it does but the caller isn't a member of that message's workspace. |
...
|---|---|---|---|
| `message_id` | string | one of `message_id` / `trace_id` | Return feedback for one chat message. |
| `trace_id` | string | one of `message_id` / `trace_id` | Return feedback joined to one OTel trace. |

The request has no body. Supplying both filters is rejected with `400`.
...
Removes the caller's row for the given `(message_id, signal)` tuple in one workspace. Other users' rows and copies in other workspaces are untouched. Existing clients can omit `workspace_id` when exactly one visible row matches. If a fork leaves matching rows in two workspaces, the server returns `409`; use the `workspace_id` from GET to choose which one to clear.
...
| `workspace_id` | string | no | Required when matching feedback exists in multiple workspaces. The caller must still be a member of this workspace. |
...
| `401` | No authenticated user. |
| `409` | Matching feedback exists in multiple visible workspaces; supply `workspace_id`. |
...
`message_feedback` table (added in migration v96; unique key scoped to a workspace by #2274):
