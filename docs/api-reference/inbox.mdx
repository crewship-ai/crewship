...
- `skipped` / `skipped_ids` — decision items the server refused to flip; the UI can surface "22 resolved, 3 need the source endpoint" and link each skipped id to its [source endpoint](#source-managed-kinds--409-conflict).
