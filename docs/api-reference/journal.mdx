...
| `run_id` | string | Everything that happened during one run, **whichever engine ran it**: matches `trace_id` (ad-hoc agent runs), `actor_id` (routine runs stamp their own id there) and `payload.run_id` (indexed generated column, v120), OR-ed together. Prefer this over `trace_id` unless you specifically want the agent-run spans. Accepted by list, stream and count; `crewship journal --run-id` sends it. |
