...
| GET | [`/api/v1/journal/spend`](#spend-rollup) | Cost rollup — by agent, by routine, top spenders |
...
|---|---|---|
| `crew_id` | string | Filter by a single crew — id, slug or display name (see [References](#references)). |
| `agent_id` | string | Filter by a single agent — id, slug or display name (see [References](#references)). |
| `mission_id` | string | Only one issue — its mission id or its identifier (`ENG-4`); resolved per workspace, an unknown reference matches nothing. |
| `trace_id` | string | Narrow to one run's spans (`trace_id` == run id). Reaches **ad-hoc agent runs only** — a routine run never sets `trace_id`. |
| `run_id` | string | Everything that happened during one run, whichever engine ran it: matches `trace_id` (ad-hoc agent runs), `actor_id` and `payload.run_id` (routine runs), OR-combined. Prefer this over `trace_id`, which reaches agent runs only. AND-combines with every other filter. |
| `crew_ids` | string | CSV multi-crew filter — takes precedence over `crew_id` when non-empty. Each value is resolved the same way. |
| `agent_ids` | string | CSV multi-agent filter — takes precedence over `agent_id` when non-empty. Each value is resolved the same way. |
...
| `q` | string | FTS5 search across `summary` + `payload` (migration 55). Bounded to 200 chars; tokenised before MATCH (see [Free-text search](#free-text-search)); AND-combined with structural filters. |
...
`q` is split on whitespace and the terms are AND-combined, so word order does
not change the result set: `q=session%20morgan` returns exactly what
`q=morgan%20session` returns. The **last** term is a prefix match — `q=morg`
reaches `morgan` — while earlier terms match whole words.

Each term is wrapped in an FTS5 phrase literal before it reaches SQLite, so
operators inside the input (`AND`, `OR`, `NOT`, `NEAR(...)`, `*`, `column:`, a
bare `"`) are literal search text. They can neither restructure the query nor
produce a 500. Terms containing no letter or digit are dropped; input made up
entirely of such terms matches nothing rather than everything.

The index covers `summary` and `payload` only — never `entry_type`.

### References

`crew_id`, `agent_id`, `crew_ids` and `agent_ids` accept an id, a slug, or a
display name (case-insensitive), resolved within the caller's workspace. A
value that resolves to nothing is used as-is against the id column, so an id
whose row has since been deleted still reaches that entity's history and a typo
returns zero rows instead of silently widening the filter. A display name shared
by several agents matches all of them.

---

## Stream entries (SSE)

Live tail of the journal over Server-Sent Events, seeded with a recent batch then driven by the emitter's flush notifications.
...
- **Seed:** sends the most recent 50 entries matching filters, then goes live.
- **Wake-up:** notification-driven. The writer signals the stream as soon as a
  batch commits, so a new entry is on the wire within milliseconds of the
  flush rather than on a fixed clock. A **20 s** fallback ticker re-polls
  regardless, covering the case where a notification is missed or the wired
  emitter does not support them. (This used to be a flat 1 s poll per
  connected client, which is what older revisions of this page described.)
...
- **Reconnect:** clients send the SSE `Last-Event-ID` header; the server looks the entry up, treats its `ts` as the lower bound, and pages through the gap (up to **500 entries**, 10 × 50-row pages) before going live. A disconnect that produced more than 500 matching entries truncates the older end of the gap and the server logs a `warn` — clients should reconcile by re-fetching `/api/v1/journal?since=<last-known-ts>` if they need a longer window.

  Any client that can set request headers should resume this way, and
  `crewship journal --follow` does: `internal/cli/sse.go` sets `Last-Event-ID`
  and `followJournal` threads it across its own bounded-backoff reconnects.

  A **browser** `EventSource` cannot set request headers. It sends
  `Last-Event-ID` only on its built-in automatic retry — a web client that
  closes the connection and constructs a new `EventSource` (which is what the
  web UI does, so it can control the backoff and stop polling on success)
  starts a fresh stream with no `Last-Event-ID` and must close its own gap
  with `/api/v1/journal?since=<last-known-ts>`. The UI's journal hook does
  exactly that: while the stream is down it polls `since` on a rolling
  watermark, walking the cursor up to 4 pages per tick, and reports
  "Entries missing" if the backlog was deeper than that.
...
**Path parameters:** `id` is the journal entry ID. The request has no body;
...
**Request:** no body. The workspace is derived from the authenticated session;
there are no query parameters.
The authenticated workspace context determines the lookup scope.
...
| `missions[].status` | string | A mission status: `PLANNING`, `IN_PROGRESS`, `REVIEW`, `DONE`, `FAILED`, `CANCELLED` — or, for an issue row, an issue status (`BACKLOG`, `TODO`, `DUPLICATE` as well). `COMPLETED` is retired (B13, #2383): the column holds `DONE`. |
...
Cost rollup over `cost.incurred` journal entries and `pipeline_runs.cost_usd` (#1404) — a separate, journal-native surface from [Paymaster](/api-reference/paymaster)'s `/api/v1/paymaster/spend/*`.

```
GET /api/v1/journal/spend
```
...
| Param | Type | Description |
|---|---|---|
| `window` | string | `24h`, `7d`, or `30d`. Default `24h`. Unknown values → `400` (not silently defaulted). |
| `top` | integer | Bounds `top_routines`/`top_runs`. 1-50, default 5. |
...
```json
{
  ...
  "total_cost_usd": 12.5,
  "by_agent": [
    {"date": "2026-07-24", "crew_id": "crw_backend", "agent_id": "agt_viktor", "cost_usd": 10.0, "call_count": 5}
  ],
  "by_routine": [
    {"date": "2026-07-24", "pipeline_id": "pln_abc", "pipeline_slug": "nightly-report", "cost_usd": 2.5, "run_count": 2}
  ],
  "top_routines": [
    {"kind": "routine", "id": "pln_abc", "label": "nightly-report", "cost_usd": 2.5}
  ],
  "top_runs": [
    {"kind": "run", "id": "run_xyz", "label": "nightly-report", "cost_usd": 1.5}
  ],
  "truncated": false
}
```
...
| Field | Type | Description |
|---|---|---|
| `total_cost_usd` | number | Sum of `cost.incurred` entries only — the ledger's source of truth. Not the sum of `by_routine`, which reads a denormalized copy of the same spend. Summed by a dedicated unbounded query, so it stays **exact even when `truncated` is set** (it is not affected by the `by_agent` row cap). |
| `by_agent` | array | Day × crew × agent buckets, keyed on the **UTC** calendar day. |
| `by_routine` | array | Day × routine buckets, keyed on the **UTC** calendar day. |
| `top_routines` | array | Top-N routines by total spend across their runs in the window. |
| `top_runs` | array | Top-N individual runs by cost in the window. |
| `truncated` | boolean | Set when the window held more rows than the aggregation cap (20000) — the `by_agent`/`by_routine` breakdowns cover the most-recent rows only. `total_cost_usd` is unaffected. |

**Errors:**

| Status | Condition |
|---|---|
| 400 | Bad `window` or `top`. |
| 401 | No workspace context. |
| 500 | DB error. |

CLI: `crewship spend --window 7d --top 10` — see [`crewship spend`](/cli/spend).
