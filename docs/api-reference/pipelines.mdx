...
cleanly but reports no recognised outcome is recorded `FAILED` with `error_message` set
to `"no outcome reported"`; every other outcome (`NO_CHANGE`, `SUCCEEDED`,
...
one of the run's steps is an `agent_run` step whose prompt asks for one — a routine with
no agent step, or one that never reports an outcome, gets the `FAILED`/"no outcome
reported" default like any other run. Empty on a non-terminal run, or one that predates
the `outcome` column.
