...
| POST | `/api/v1/workspaces/{workspaceId}/pipeline-schedules` | create | Create a cron schedule |
| PATCH | `/api/v1/workspaces/{workspaceId}/pipeline-schedules/{scheduleId}` | manage | Replace a schedule |
| DELETE | `/api/v1/workspaces/{workspaceId}/pipeline-schedules/{scheduleId}` | delete | Soft-delete a schedule |
...
| `DRY_RUN_OK` | A dry-run preview completed; no agent was invoked and no run was persisted. |
...
the schedule object. `PATCH .../pipeline-schedules/{scheduleId}` replaces the
schedule row and returns `200` with the schedule object. Both return `400` for
...
is `503`. Update requires `manage`; create requires `create`.

`DELETE .../pipeline-schedules/{scheduleId}` requires `delete`, returns `204`
