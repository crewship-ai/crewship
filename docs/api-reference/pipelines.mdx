...
panel uses. When the crew, its container or the file is unavailable the entry
is `present: false` with the size, time and description omitted or empty —
a file can never fail the detail request. At most 25 files are checked per
request.
