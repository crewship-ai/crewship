...
or a non-empty `debounce_key`) returns `202 Accepted` with `{ "status":
"SCHEDULED", "pending_id", "fire_at", "coalesced", "priority" }` and does
...
does) or `"manual"` (an explicit, on-purpose "no trigger" — distinct from
omitting `trigger` entirely). Webhook and automation-binding trigger kinds
are not yet supported here; create those separately via their own endpoints.
...
it (or send anything else) to activate the trigger immediately. Approve via
`POST .../pipeline-schedules/{scheduleId}/activate` (see
...
or an unsupported `kind`) is `422` and — per the atomicity guarantee above —
the routine is not saved either.
