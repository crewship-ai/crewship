...
under [Issues → Outcome](/api-reference/issues#outcome). In short: `NO_CHANGE` and
`SUCCEEDED` stay in history; `WORK_CREATED`/`PARTIAL` also get an issue comment when the
run is issue-triggered; `NEEDS_HUMAN` raises exactly one `run_needs_human` inbox item
with an action contract; a run that ends cleanly but reports no recognised outcome is
recorded `FAILED` with `error_message` set to `"no outcome reported"`. `outcome` is
parsed from a `---CHECKPOINT---` or `---HANDOFF---` block in the run's own output when
