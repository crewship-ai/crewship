...
`routines/schedules` and the two read routes are registered only when the `PipelineHandler` is wired; `skills/generate` only when the `SkillGenerateHandler` is wired. `pipelines/test_run` / `save` / `run` are always registered on the `PipelineHandler`.
