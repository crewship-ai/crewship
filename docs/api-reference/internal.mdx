...
| agent | `status = PENDING_REVIEW`; the chat bridge refuses to start the agent | approval flips it to `IDLE` — also reachable via `POST /api/v1/agents/{agentId}/approve-hire` |
