...
| `GET` | `/api/v1/internal/pipelines?workspace_id=…` | List the workspace's routines — the sidecar→main forward for the `list_routines` MCP tool. `workspace_id` is **required**: without it the public `List` has no tenant and fails, and defaulting it would answer a malformed call with an empty list, which reads to a model as "this workspace has no routines". |
| `GET` | `/api/v1/internal/crews/{crewId}/capabilities?workspace_id=…` | The crew's authoring surface — the forward for `discover_capabilities`. The public handler does its own cross-tenant isolation check, so a foreign crew is refused there rather than here. |
