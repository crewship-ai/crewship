...
| `GET` | [/api/v1/internal/workspace/overview](#agents-and-crews) | Read-only inventory of the workspace (the Guide's `workspace_overview` MCP tool) |
...
| `POST` | [/api/v1/internal/pipelines/drafts/get](#pipelines-routines-and-skills) | Read a crew's unpublished routine draft (the `get_routine_draft` MCP tool) |
| `POST` | [/api/v1/internal/pipelines/drafts/save](#pipelines-routines-and-skills) | Save a routine draft without publishing (the `save_routine_draft` MCP tool) |
| `GET` | [/api/v1/internal/pipelines](#pipelines-routines-and-skills) | List the workspace's routines (the `list_routines` MCP tool) |
| `GET` | [/api/v1/internal/crews/{crewId}/capabilities](#pipelines-routines-and-skills) | The crew's authoring surface (the `discover_capabilities` MCP tool) |
...
| `GET` | [/api/v1/internal/runs/\{runId\}/status](#runs) | Is this run still the live attempt of live work? (sidecar run authority) |
| `POST` | [/api/v1/internal/memory/mutation](#memory) | Guaranteed-profile memory write against the host ledger |
| `GET` | [/api/v1/internal/memory/canonical](#memory) | Read a memory file with its current revision |
| `POST` | [/api/v1/internal/memory/search/hybrid](#memory) | Hybrid memory search scoped to the acting agent |
...
| `PATCH` | [/api/v1/internal/issues/\{identifier\}](#missions-and-issues) | Update an issue (status, priority, assignee, labels, estimate, due date) |
| `POST` | [/api/v1/internal/issues/\{identifier\}/work](#missions-and-issues) | Hand issue work off to a person or another agent |
| `POST` | [/api/v1/internal/issues/\{identifier\}/comments](#missions-and-issues) | Comment on issue |
| `GET` | [/api/v1/internal/issues/\{identifier\}/comments](#missions-and-issues) | Read the issue's comment thread |
| `POST` | [/api/v1/internal/issues/\{identifier\}/relations](#missions-and-issues) | Relate or re-parent an issue |
| `GET` | [/api/v1/internal/issues/\{identifier\}/attachments](#missions-and-issues) | List the issue's attachments (metadata only) |
| `GET` | [/api/v1/internal/issues/\{identifier\}/attachments/\{attachmentId\}](#missions-and-issues) | Read one attachment, fenced and size-capped |
| `POST` | [/api/v1/internal/issues/\{identifier\}/attachments](#missions-and-issues) | Attach a file to the issue |
...
| `GET` | [/api/v1/internal/notifications/channels](#agent-notifications) | List channels the calling agent may post to |
| `POST` | [/api/v1/internal/notifications/send](#agent-notifications) | Agent sends a message to a channel it is paired with |
| `PUT` | [/api/v1/internal/pages/\{page\}/data](#pages) | Write one panel's payload on behalf of a routine run or a container agent |
| `POST` | [/api/v1/internal/pages/save](#pages) | Agent-authored page creation (name + panels) |
| `POST` | [/api/v1/internal/pages/project](#pages) | Draft-only source edits and builds of a custom Page application (the `page_project` MCP tool) |
...
<Warning>
  **One-time upgrade step.** The deploy that first introduces the stable master rotates it once — from the old per-boot random to the `ENCRYPTION_KEY`-derived value. A per-crew container that outlived *that* deploy still holds a token minted under the old master and is orphaned exactly once (it does not self-heal). Recreate the surviving containers (a normal redeploy / re-dispatch re-mints), or run [`crewship admin reap-orphan-containers --apply`](/cli/admin#crewship-admin-reap-orphan-containers) to clear just the orphaned ones. Every restart after this is a no-op for live containers. A running sidecar advertises a `token_fp` fingerprint of the crew-bound token it holds on its `/health` endpoint so the server can tell an orphaned container from a healthy one without ever seeing the token itself.
</Warning>
...
| `POST` | `/api/v1/internal/crews` | Create a crew programmatically. Gated on the caller's crew `autonomy_level` — see [Autonomy gate](#autonomy-gate-on-agent-driven-creation). |
| `POST` | `/api/v1/internal/agents` | Create an agent programmatically. Gated on the caller's crew `autonomy_level` — see [Autonomy gate](#autonomy-gate-on-agent-driven-creation). |
...
| `GET` | `/api/v1/internal/workspace/overview?workspace_id=…` | One read-only bundle of what the workspace holds — the forward for the Guide's `workspace_overview` MCP tool, so it can advise about the workspace it can see rather than one it remembers. Returns `{crews[], routines[], pages[], open_issues, credentials[]}`: each crew carries `id`, `name`, `slug`, `icon`, `color` and its `agents[]` (`name`, `slug`, `role_title`, `cli_adapter`, `llm_model`, `status`); routines and pages are `name` + `slug` (+ routine `status`); credentials are `name`, `provider`, `status` — **names only, never a value**. The reserved setup crew is filtered out. `workspace_id` is required (`400`). |
...
| `POST` | `/api/v1/internal/pipelines/drafts/get` | Read the crew's unpublished draft of one routine — the forward for the `get_routine_draft` MCP tool. Body: `workspace_id`, `slug` (both required, `400`), `author_crew_id`, `author_agent_id`, optional `target_crew_slug` (setup-crew delegation only). The same draft CAS the user editor uses, scoped to the crew the token is bound to: a draft or published recipe owned by another crew is `403` ("draft belongs to another crew" / "published recipe belongs to another crew"). Returns `{"draft": {id, workspace_id, slug, revision, base_pipeline_id, base_revision, document, updated_by, created_at, updated_at}, "editor_url": "/routines?draft=…&draft_id=…&workspace=…", "published": false}`. |
| `POST` | `/api/v1/internal/pipelines/drafts/save` | Save that draft — the forward for `save_routine_draft`. Same body plus `draft` (`slug` must equal the top-level `slug`, `revision` ≥ 0 is the CAS base, `document` must carry the same `slug` and an object `definition`). `save_token`, `skip_test_gate`, `skip_governance_gate`, `last_test_run_at` and `last_test_run_passed` are stripped from the document; `author_crew_id` / `author_agent_id` are stamped from the proven identity and `updated_by` is `agent:<id>` (or `crew:<id>`). **Never publishes** and mints no publication proof — a person publishes from the editor. `409` when the revision is stale or `draft.id` names a different draft; `400` on a malformed draft. Same response shape as `drafts/get`. |
| `GET` | `/api/v1/internal/pipelines?workspace_id=…` | List the workspace's routines — the sidecar→main forward for the `list_routines` MCP tool. Wrapped in `internalWsCtx`, so `workspace_id` is **required** (`400`) and must match the token's binding (`403`). |
| `GET` | `/api/v1/internal/crews/{crewId}/capabilities?workspace_id=…` | The crew's authoring surface — the forward for `discover_capabilities`. Same `internalWsCtx` wrapping, **plus** `assertBoundCrewWorkspaceDB` on the path's `crewId`: a crew-bound (`crwv1`) sidecar may introspect only its own crew. Without that a sidecar could pull a **sibling crew's** integrations, agents and runtimes — same workspace, so the workspace check alone waves it through. |
...
| `GET` | `/api/v1/internal/runs/{runId}/status` | Answers whether `runId` is the current, open attempt of live work in the durable work ledger: `200 {"run_id", "active": bool, "reason"}`. A run absent from the ledger is `200 active:false`, deliberately **not** `404` — on this surface a 404 is byte-identical to "unregistered path or refused caller". `503` when the ledger cannot answer. This is the host half of the sidecar's opt-in run authority (`CREWSHIP_SIDECAR_RUN_AUTHORITY`, see [Environment Reference](/configuration/environment#read-by-the-sidecar-inside-the-container)). |
...
The host side of the agent memory contract. `X-Internal-Token` authenticates the sidecar; `X-Acting-Agent-Slug` narrows it to one agent and is re-proved against the token's workspace/crew binding, so the header can only narrow the token's authority, never widen it.

| Method | Path | Purpose |
|---|---|---|
| `POST` | `/api/v1/internal/memory/mutation` | Perform a memory write against the real ledger on the host side of the crew bind mount — the only route that can satisfy the **guaranteed** profile (caller-supplied `operation_id`, `expected_revision` + declared removals on a replace, durable ledger). Also requires the original per-run bearer capability, matched against workspace, agent and run; a live sibling run of the same agent does not authorize the caller. |
| `GET` | `/api/v1/internal/memory/canonical?scope=agent\|crew&file=…` | Read one memory file with its current revision — the other half of the same contract, since a client cannot supply `expected_revision` without having been told the revision. |
| `POST` | `/api/v1/internal/memory/search/hybrid` | Hybrid (lexical + embedding) memory search resolved strictly inside the acting agent's workspace/crew. Registered only when the hybrid search handler is wired. |

See [Agent memory](/guides/agent-memory) for the profiles and conflict vocabulary.
...
| `POST` | `/api/v1/internal/queries` | Free-form sub-query (e.g. agent asking another agent). Synchronous queries return `409` when the target agent or server execution capacity is busy, rather than waiting for a slot that may belong to the waiting parent. Retry after the active run finishes. |
...
| Method | Path | Purpose |
|---|---|---|
| `POST` | `/api/v1/internal/missions` | Create a mission. Gated on the crew's `autonomy_level`; a held mission cannot be started. See [Autonomy gate](#autonomy-gate-on-agent-driven-creation). Also bounded by two instance settings: `mission.max_tasks` (default `16`) limits the task list of one mission, and `mission.max_active_per_crew` (default `6`) limits how many agent-created missions a crew may have in a non-terminal state. Past either, the response is `403` with `{"error","reason","crew_id","setting","limit"}` — `setting` names the key an operator would change — and no row is written. Missions created through the JWT API are neither capped nor counted. |
...
| `GET` | `/api/v1/internal/issues` | List issues. Optional filters: `status` (comma-separated), `assignee_id`, `mission_type`, and `q` — a free-text search over title and description, with `%` and `_` escaped so they match literally. A crew-bound (`crwv1`) token constrains the listing to its own crew even when `crew_id` is omitted. |
| `GET` | `/api/v1/internal/issues/{identifier}` | Get a specific issue. |
| `POST` | `/api/v1/internal/issues` | Create issue. `author_agent_id` (validated against the crew/workspace) is persisted as the creator with `authored_via = agent_tool_call`; optional `author_chat_id` / `author_run_id` record provenance. If `assignee_id` is set, `assignee_type` must be `user` or `agent` and the assignee must belong to `workspace_id` — otherwise `400` (`assignee_type must be 'user' or 'agent' when assignee_id is set` / `assignee_id does not exist in this workspace`). |
| `PATCH` | `/api/v1/internal/issues/{identifier}` | Update an issue: `status`, `priority`, `assignee_type`/`assignee_id`, `labels`, `estimate`, `due_date`, plus an optional `comment`. Writes a `mission_activity` audit row **and a Crew Journal entry** attributed to `agent_id` (or `system` when absent) — the journal entry is what notification routing keys off, so an agent's change is subscribable exactly like a human's. A `comment` requires `agent_id`. `assignee_id` is validated against `workspace_id` (`400` when it names another tenant's user/agent); `labels` is a full replacement whose insert is workspace-scoped, so a foreign label id attaches nothing. An empty-string `assignee_id` / `due_date` clears the field. `title`, `description`, `project_id`, `milestone_id`, `routine_id`, `sort_order` and `parent_issue_id` are **not** accepted here. |
| `POST` | `/api/v1/internal/issues/{identifier}/work` | Hand issue work off from inside the container — the forward for the sidecar's `POST /issue/{identifier}/work` verb. Body: `workspace_id`, `agent_id` (both required, `400`; the agent must belong to the workspace and the issue's crew to the token binding), then the same fields as the public [Transfer Issue Work](/api-reference/issues#transfer-issue-work) route: `operation_id`, `revision` (the issue's current `work_revision`), `action`, `target_id`, `note`. **Agents may only `handoff_agent` or `handoff_human`** — `take_over` and `submit` answer `403` ("human takeover and submission require a person"), and only the issue's current agent delegate may hand off (`403` otherwise). `200 {"revision", "status"}`; a replay of the same `operation_id` returns `{"revision", "replayed": true}`. `404` unknown issue; `409` stale revision, reused `operation_id` with different content, closed issue, held target agent, or active runs still stopping. |
...
| `GET` | `/api/v1/internal/issues/{identifier}/comments?workspace_id=…` | Read the issue's comment thread, oldest first — the forward for the sidecar's `GET /issue/{identifier}/comments` verb, so an agent that wants history no longer has to be handed all of it in the brief. Each row is `{id, mission_id, author_type, author_id, author_name, body, created_at, updated_at}`. Scoped exactly like `GET .../issues/{identifier}` (workspace only). `400` without `workspace_id`, `404` unknown issue. |
| `GET` | `/api/v1/internal/issues/{identifier}/attachments` | List the issue's attachments as **metadata only** — `{id, filename, content_type, size_bytes, sha256, created_at, uploaded_by}` where `uploaded_by` is the kind (`user` / `agent`), not an identity, and `filename` is already fenced as untrusted content. Listing is cheap; reading is a decision the agent makes per file. |
| `GET` | `/api/v1/internal/issues/{identifier}/attachments/{attachmentId}` | Read one attachment: the metadata row plus `encoding` (`text` / `base64`), `content` (fenced text, or raw base64) and `truncated`, which is always present. |
| `POST` | `/api/v1/internal/issues/{identifier}/attachments` | Attach a file. Body: `workspace_id` (required and proven against the token), `agent_id` (set by the sidecar from the per-agent token), `filename`, `content_base64`. `413` past the upload cap. Wraps the public attachment handler, so filename sanitisation, the content-type allowlist and de-duplication are the same on both doors; the three routes are registered only when that handler is wired. |
| `POST` | `/api/v1/internal/issues/{identifier}/relations` | Relate two issues. Body: `target_identifier`, `relation_type` ∈ `blocks` \| `blocked_by` \| `relates_to` \| `duplicate_of` \| `sub_issue_of`. `blocked_by` is normalised to a swapped `blocks` row; `sub_issue_of` writes `parent_issue_id` on the issue named in the path instead of a `mission_relations` row. The path issue is crew-gated like every other mutation; the target only has to live in the bound workspace, and a foreign-tenant target is a `404` rather than a `403` so the endpoint is not an existence oracle. Duplicate relation → `409`. |
...
| Method | Path | Purpose |
|---|---|---|
| `PUT` | `/api/v1/internal/pages/{page}/data` | Write one panel's payload on behalf of a routine run or a container agent. |
| `POST` | `/api/v1/internal/pages/save` | Agent-authored page **creation** — build a new page's metadata + panels from inside a container. See [Pages: agent creation](#pages-agent-creation) below. |
| `POST` | `/api/v1/internal/pages/project` | Draft-only source edits and builds of a custom Page application — the forward for the `page_project` MCP tool. See [Pages: project source](#pages-project-source) below. |

The body of `PUT .../data` is an **envelope**, not the payload: `workspace_id`, `crew_id`,
...
notifies the page owner. See [Pages](/guides/pages).

### Agent notifications

| Method | Path | Purpose |
|---|---|---|
| `GET` | `/api/v1/internal/notifications/channels?workspace_id=…&agent_id=…&crew_id=…` | Discovery — the channels the calling agent may post to. `workspace_id` and `agent_id` are required; `crew_id` is optional. |
| `POST` | `/api/v1/internal/notifications/send` | Send a message to a channel the agent is paired with. See [Agent notifications: send](#agent-notifications-send) below. |

An agent posting a status update to a Slack channel a human explicitly wired
it to — distinct from the outbound routing the [category preference
matrix](/api-reference/preferences#notification-preferences) governs
(`agents.message` is deliberately excluded from that matrix; the pairing
itself is the authorization, see below). Implementation:
`internal/api/internal_notifications.go` (`AgentNotifyHandler`).

Four gates, checked in order, so a rejected call is told which one it failed:

1. **Identity** — the agent exists in the claimed workspace, and (for a
   crew-bound token) belongs to that crew. An agent cannot send as another
   agent.
2. **Pairing** — the channel must be paired to *this* agent. Default-deny; a
   workspace admin grants it with `crewship notifychannel agents allow
   <channel-id> --agent <agent-id>`.
3. **Rate** — a per-agent token bucket (burst 3, refill 1/60s per
   agent×channel), tighter than the human-facing preference matrix's, because
   an agent in a loop can call a tool far faster than a human clicks.
4. **Scrub** — the title and body are redacted by the same secret scrubber
   that guards memory and tool output before the message leaves the instance.

#### Agent notifications: send

```
POST /api/v1/internal/notifications/send
```

**Request body:** capped at 64 KiB; `body` itself is capped at 8 KiB.
...
```json
{
  ...
  "body": "staging is green, 12 tests skipped"
}
```

| Field | Type | Notes |
|---|---|---|
| `workspace_id`, `agent_id`, `channel_id` | string | **Required.** |
| `crew_id` | string | Optional; when the token is crew-bound it must agree with the token's own crew. |
| `title` | string | **Required** (trimmed, must be non-empty). |
| `body` | string | Optional; max 8 KiB. |

**Response:** `200 OK`

```json
{ "ok": true, "channel_id": "nch_…" }
```

Delivery is journaled either way — `notification.delivered` on success,
`notification.failed` (with the dispatch error) otherwise — attributed to the
agent as actor, so "who sent this to Slack?" has an answer on the Activity
timeline.
...
| Status | Condition |
|---|---|
| 400 | Invalid JSON, missing `workspace_id`/`agent_id`/`channel_id`, empty `title`, `body` over 8 KiB, or the channel is disabled. |
| 403 | Unknown agent, agent not in the claimed workspace/crew, or the channel is not paired with this agent. |
| 404 | `channel_id` does not resolve in this workspace. |
| 429 | The per-agent rate gate tripped. |
| 502 | The channel's dispatcher failed to deliver (e.g. the webhook/Slack call itself failed). |
...
| `description` | string | Optional. On re-save of an existing slug, omitting the key preserves the stored description; an explicit `""` clears it. |
...
| `target_crew_slug` | string | Optional. Re-points ownership at the crew the routine is built FOR — accepted only from the onboarding setup crew (see `internal_delegated_crew.go`). |
| `save_token` | string | The HMAC-signed proof minted by `POST /api/v1/internal/pipelines/test_run` that **this** crew just dry-ran **this** `definition_hash`. It is the only thing that clears the test-run gate on this path; there is no OWNER/ADMIN skip here because an agent has no such role. The former `last_test_run_at` / `last_test_run_passed` body fields are no longer read — a caller claiming "it passed" is not proof it did. |
| `change_summary` | string | Optional one-line provenance note recorded on the version row. |
| `trigger` | object | Optional. `{kind: "schedule" \| "manual" \| "once", cron, timezone, catchup_policy, max_consecutive_failures, fire_at, inputs}` — created in the **same transaction** as the routine and its version, so a bad cron or timezone rolls the whole save back. Same block as the public save; see [Atomic trigger authoring](/api-reference/pipelines#atomic-trigger-authoring). |
| `activation` | string | Optional, top-level next to `trigger`. `"draft"` creates the trigger disabled and raises exactly one inbox item asking a MANAGER+ to activate it; omitted means live at once. Requires `trigger`; sending it with `kind: "manual"` or `"once"` is refused as `422`, the same as any other trigger validation failure. |

**Response:** `201 Created` — the saved pipeline object (`toPipelineResponse`), plus a `trigger` block (`kind`, `schedule_id`, `cron_expr`, `timezone`, `enabled`, `first_fire_at`, `approval_required`, `inbox_item_id`) when the save requested one. A risky definition lands as `proposed` with an inbox review item, exactly as on the user path.

**Errors:** `400` (invalid JSON, or missing `workspace_id` / `slug` / `definition`), `409` — either `slug already exists in workspace`, or the **schedule-preset gate**: when the new definition renames or adds a required input that an active plan's preset no longer satisfies, the answer is `409 {"error", "hint", "schedule_conflict"}` where `schedule_conflict` is `{schedule_id, name, reason}` so a client can open the named plan and repair its preset, and `hint` spells out the way out (send a `trigger` block with the plan's inputs in the same save, repair the plan first, or disable it, publish, then re-enable). `422` (DSL parse / validation / cycle-detection failure, an invalid `save_token`, the test-run gate not satisfied — `save requires a fresh, passing test_run (within 5 minutes)` — or a `trigger` / `activation` that fails validation, which also rolls back the routine), `500` (DB error).

## Pages: agent creation

```
POST /api/v1/internal/pages/save
```

The trusted endpoint the sidecar's `save_page` MCP tool forwards to — the
route that lets an agent build a Page's metadata + panels from scratch,
distinct from `PUT /api/v1/internal/pages/{page}/data`, which only pushes one
panel's payload onto a page a **human** already created. Mirrors
`pipelines/save`'s trust model as closely as the two domains allow: identity
is proven against the token binding (never trusted from the body beyond
that), the request is gated on the authoring crew's `autonomy_level`, and it
reuses the exact validation/insert machinery `POST /api/v1/pages` (the public
create route) uses. (`internal/api/pages_internal_save.go:90`)

**Request body:** capped at `pages.MaxSpecBytes` + 8 KiB envelope slack.

| Field | Type | Notes |
|---|---|---|
| `workspace_id` | string | **Required.** |
| `crew_id` | string | **Required** — an agent-created page is always owned by its authoring crew; there is no user fallback on this route. |
| `agent_id` | string | Optional. Must name a live agent of `crew_id`, or `400`. |
| `name` | string | **Required.** Slugified to the page's URL slug. |
| `description` | string | Optional. |
| `panels` | array | **Required, non-empty** — a page needs at least one panel. Same shape as `POST /api/v1/pages`. |
| `target_crew_slug` | string | Optional. Re-points ownership at the crew this page is being built FOR — accepted only from the onboarding setup crew (see `internal_delegated_crew.go`); when set, `agent_id` is cleared before the author-agent check. |

**Response:** `201 Created` — the created page document (same shape as
`GET /api/v1/pages/{slug}`).

**Autonomy gate.** `page_create` is evaluated against the *acting* crew (the
authoring crew, before any `target_crew_slug` delegation) before validation
runs, so a held request never has to pass full spec validation to learn it
will not be created:

- **Held** (autonomy below the level `page_create` requires): unlike
  `crew_create`/`agent_create`, a page has no "created but inert" staging
  state — no status column, no `.proposed` directory — so a held request
  creates **nothing at all**. The response is `403` with `{"error",
  "reason", "crew_id", "autonomy_level", "policy_action", "pending_review":
  true, "requested_name", "requested_slug"}`, and a blocking, ADMIN-addressed
  inbox item records what was attempted.
- **Allowed:** the page is created; at `full` autonomy a **non-blocking**
  inbox notice is also left ("worth a glance" even though nothing needs
  approval).

See [Autonomy gate on agent-driven creation](#autonomy-gate-on-agent-driven-creation)
for the general model this follows.

**Errors:**

| Status | Condition |
|---|---|
| 400 | Invalid JSON, missing `workspace_id`/`name`, empty `panels`, `crew_id` empty after delegation resolution, `agent_id` not in the authoring crew, or `name` slugifies to nothing. |
| 403 | Cross-tenant `workspace_id`/`crew_id` against the token binding, or the autonomy gate held the request (see above). |
| 409 | A page with the resolved slug already exists in the workspace. |
| 422 | Panel spec validation, reference resolution, or gate-expression resolution failed, or the workspace's page cap (`pages.MaxPagesPerWorkspace`) was reached. |
| 500 | DB error. |

## Pages: project source

```
POST /api/v1/internal/pages/project
```

The trusted endpoint the sidecar's `page_project` MCP tool forwards to: an
agent edits the **source project** of a custom Page application and builds it,
without ever publishing. The route is draft-only by construction — its private
principal is minted only after the internal token, the acting agent, the exact
owner crew and the `page_create` autonomy gate all agree, and no header or body
field can produce a user identity or a publication. A person publishes the
reviewed build through the editor or `crewship page project publish`.
(`internal/api/pages_project_internal.go`)

**Request body:** capped at 1 MiB plus the identity envelope.

| Field | Type | Notes |
|---|---|---|
| `workspace_id`, `crew_id`, `agent_id` | string | **Required.** Injected by the sidecar from IPC; proven against the token binding and the agent must be a live member of `crew_id`. |
| `target_crew_slug` | string | Optional. Setup-crew delegation, as on `pages/save`. |
| `operation` | string | **Required.** One of `init`, `read`, `save`, `build`, `status`, `check`. Anything else — including any form of publish — is `400`. |
| `slug` | string | **Required.** The Page whose project is addressed; the crew must be its exact owner. |
| `expected_revision` | integer | CAS base. `init` requires `0`; `save`, `build` and `check` require the revision last read. |
| `files` | array | `save` only: whole-file replacements `{path, encoding, content}`. At most `pages.MaxProjectFiles`, no duplicate paths. |
| `delete_paths` | array | `save` only: paths to remove; deleting a missing file is `400`. |
| `definition` | object | `save` only: optionally replaces the complete portable Page definition in the draft. |
| `path` | string | `read` only: return one file instead of the file list. |
| `build_id` | string | `check` only: the build to verify. |

A field sent with an operation it does not apply to is `400` ("fields do not apply to this project operation").

| Operation | Effect | Response |
|---|---|---|
| `init` | Creates the supported React/TypeScript starter as revision 1. `409` on an existing project — never use it to reset one. | The project write result. |
| `read` | Inspect the draft. | `{revision, digest, git_commit, files[{path, encoding, bytes}], profile, sdk{module, source}, pages_theme, definition, page_path}`; with `path`, `{revision, file}`. `404` when the Page has no project yet or the file does not exist. |
| `save` | Apply replacements and deletions atomically as a new revision, with a Git checkpoint. | The project write result. `409` when `expected_revision` is stale. |
| `build` | Start the offline compiler on the current revision. One build runs per instance; a busy worker refuses. | Build identity. |
| `status` | Poll the build: `running`, `ready`, `failed` or `interrupted`, with diagnostics but without the compiled JavaScript. | The preview/status document. |
| `check` | Static verification of `build_id` against the source revision. A pass does not replace browser or security review. | The check result. |

`init`, `save` and `build` are gated on the acting crew's `autonomy_level` for `page_create`; a held request is `403` and changes no draft and starts no build. `read`, `status` and `check` need the owning crew and a current agent in it. Every operation takes the Page's lease, so an agent and the editor cannot write the same draft concurrently. The operator-side prerequisites (`page_projects_path`, Git, the pinned `page_build_image`) are in the [YAML configuration](/configuration/yaml-config#storage) and [Pages apps operations](/guides/pages-apps-operations).
...
- **Autonomous-agent** (`X-Caller-User-Id` absent): skips the capability gate — a capability belongs to a user and this path has none — and is instead gated on the calling crew's `autonomy_level`. See [Autonomy gate on agent-driven creation](#autonomy-gate-on-agent-driven-creation).
...
Six internal routes let an agent create something that keeps acting after the request ends. Each one now consults the calling crew's `autonomy_level` (see [Policies](/api-reference/policies)) before writing, using the same decision matrix `POST /api/v1/agents/hire` already follows.
...
| Route | Policy action | `strict` | `guided` | `trusted` | `full` |
|---|---|---|---|---|---|
| `POST /api/v1/internal/crews` | `crew_create` | `403` | held | held | live |
| `POST /api/v1/internal/agents` | `agent_create` | `403` | held | held | live |
| `POST /api/v1/internal/routines/schedules` | `routine_schedule_create` | `403` | live | live | live |
| `POST /api/v1/internal/missions` | `mission_create` | held | live | live | live |
| `POST /api/v1/internal/skills/generate` | `skill_create` | staged | staged | staged | live |
| `POST /api/v1/internal/skills/author` | `skill_create` | staged | staged | staged | staged |

**Refused (`403`)** returns a structured body — `error`, `reason`, `crew_id`, `autonomy_level`, `policy_action` — so the CLI can name the `crewship policy set` that would unblock it.
...
| Route | Sentinel | Released by |
|---|---|---|
| crew | the new crew is pinned to `autonomy_level=strict`, so `agent_create` and `routine_schedule_create` inside it are themselves refused | approval restores the *creating* crew's level |
| agent | `status = PENDING_REVIEW`. Refused by a chat message, `/assign` (`409`), an `@mention` on an issue, and a mission task — but **not** by every door: see the note below | approval flips it to `IDLE`. **Not** `POST /api/v1/agents/{agentId}/approve-hire`, which answers `409` for a gate-held agent |
| schedule | `enabled = 0`; the scheduler only picks up `enabled = 1` rows | approval enables it — also `PATCH /api/v1/workspaces/{workspaceId}/pipeline-schedules/{scheduleId}` |
| mission | stays `PLANNING`; `POST /api/v1/internal/missions/{missionId}/start` **and** the public `POST /api/v1/crews/{crewId}/missions/{missionId}/start` both answer `403` — the two routes share one helper (`refuseUnlessAutonomyGateApproved`) so a held mission cannot be started from either | approval lets `start` proceed |

<Warning>
**"Inert" is true for the doors an agent can reach by being named, not for every door in the product.** Three surfaces still resolve an agent and run it without reading `agents.status`: `/query` (peer questions), the webhook trigger, and a routine's agent step. All three predate the autonomy gate and none of them can be reached by mentioning an agent or assigning it work, which is why they are listed rather than claimed closed. The authoritative list lives beside the code, in `internal/api/internal_status.go`.

`POST /api/v1/agents/{agentId}/approve-hire` is **not** a release path for a gate-held agent and answers `409`. It decides *ephemeral hire* rows only. It was briefly widened to decide autonomy-gate rows too, which let a `MANAGER` release a hold the gate addresses to OWNER/ADMIN and let a terminal (denied or timed-out) row be released anyway; narrowing it back is what closed that. The one release surface is `POST /api/v1/approvals/{id}/decide` — `crewship approvals approve <id>`.
</Warning>
