...
| agent | `status = PENDING_REVIEW`; every door that starts an agent refuses it — a chat message, `/assign` (`409`), an `@mention` on an issue, and a mission task | approval flips it to `IDLE` — also reachable via `POST /api/v1/agents/{agentId}/approve-hire` |
