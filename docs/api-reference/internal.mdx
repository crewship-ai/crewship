...
| `activation` | string | Optional, top-level next to `trigger`. `"draft"` creates the trigger disabled and raises exactly one inbox item asking a MANAGER+ to activate it; omitted means live at once. Requires `trigger`, and has no meaning for `kind: "manual"` or `"once"`. |
