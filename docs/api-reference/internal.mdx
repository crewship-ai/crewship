...
The body is an **envelope**, not the payload: `workspace_id`, `crew_id`,
`agent_id` and `author_run_id` are injected by the caller (the dispatcher for
the `page.write` verb, the sidecar for an agent) and the payload rides in
`data`. `author_run_id` is optional — an agent in a container has no run, and
