...
| `GET` | [`/api/v1/mcp-tool-calls`](#mcp-tool-call-audit) | Audit trail of MCP tool invocations |
...
Each endpoint below has **Auth**, **Request**, **Response**, and **Status**
details. Workspace routes require a session or CLI token plus membership;
OAuth provider/discovery and registry list/search routes explicitly do not.
Requests and responses are JSON unless noted. Workspace reads return `200`,
...
Every MCP server — workspace- or crew-scoped — carries a `default_access`
column that says who may use it:

| `default_access` | Who resolves the server |
|---|---|
| `all` (default) | every agent in the server's scope, whether or not it has a binding |
| `bound-only` | only agents with an explicit agent binding |

An **agent binding** adds things to one agent's use of a server: a credential,
a config override, or an opt-out (`enabled: false`). It does **not** change the
audience, and creating one for one agent never changes what any other agent
resolves.

<Warning>
Before Crewship 2026.08 this was inferred rather than stored: a server with
**zero** bindings resolved for every agent, and the first binding anywhere in
the workspace silently flipped it to opt-in, revoking it from every other
agent. Existing servers were migrated to whichever audience they effectively
had at upgrade time — servers that already carried a binding became
`bound-only`, everything else became `all` — so nobody's access moved. Narrowing
a server is now a deliberate act: `PATCH` with `default_access`, or
...
**Auth:** Authenticated workspace member (`workspace_id` is required).
...
| `default_access` | string | Who may use it: `all` (every agent in scope) or `bound-only` (only agents with an explicit binding). See [Who can use an integration](#who-can-use-an-integration). |
| `agent_binding_count` | integer | Number of agent bindings. Informational only — it does **not** affect who resolves the server. |
...
| `default_access` | string | No | `"all"` | `all` or `bound-only`. A new integration is available to every agent unless you say otherwise. |
...
| Status | Condition |
|---|---|
| `400` | Missing `name`, invalid `transport`, `endpoint`/`command` missing for the chosen transport, or `default_access` outside `all` / `bound-only` |
...
| `404` | Integration not found in the workspace |
...
All fields optional: `display_name`, `transport`, `endpoint`, `command`, `args_json`, `env_json`, `config_json`, `icon`, `enabled`, `default_access`.

Sending `default_access` is the only way to change who may use the integration —
creating or deleting agent bindings never does. CLI: `crewship integration access <id-or-name> <all|bound-only>`.

**Response:** `200 OK`

| Status | Condition |
|---|---|
| `400` | Invalid JSON or invalid merged transport configuration |
...
|---|---|
| `400` | Missing workspace context |
| `500` | Database failure while loading the overview |
...
|---|---|
| `400` | Missing workspace context |
| `404` | Crew not found in the workspace |
...
| Status | Condition |
|---|---|
| `400` | Missing `name`, invalid `transport`, missing endpoint/command for transport, `default_access` outside `all` / `bound-only`, or referenced workspace integration not in same workspace |
| `404` | Crew not found |
...
Sending `default_access` is the only way to change who may use the integration —
creating or deleting agent bindings never does. CLI: `crewship integration crew update <crew-slug> <integration-id> --access <all|bound-only>`. Changing `transport` re-validates against the merged final endpoint/command.
...
**Auth:** Authenticated member with the `create` capability; workspace context
...
path, `enabled` only decides whether a tool's name is listed in the
agent's `[CONNECTED INTEGRATIONS]` prompt block (a hint of what to
call) — the sidecar gateway that actually dispatches tool calls does
no per-tool check, so a disabled tool can still be invoked directly.
Composio-routed integrations are the exception: those enforce their
own `allowed_tools` scope server-side, independent of this table.
...
| `enabled` | boolean | Per-crew enabled state — advisory only, see caveat above |
...
|---|---|
| `400` | Missing workspace context |
| `404` | Agent not found or outside the workspace |
...
Returns the effective set of enabled MCP servers for an agent after cascading
workspace and crew servers and applying the agent's bindings. Credential IDs
and names may be present, but secret values are never returned. The response
is a JSON array; a missing agent returns `404`.

A server is in the answer when it is enabled **and** either its `default_access`
is `all` or this agent holds a binding for it — see
[Who can use an integration](#who-can-use-an-integration). Another agent's
binding never removes a server from this list.
...
[
  {
    ...
    "credential_name": "github-primary",
    "default_access": "all"
  }
]
```
...
**Request:** No body. `workspace_id` is required; the agent must belong to that
workspace.
...
| `400` | Missing workspace context |
| `401` | Missing or invalid authentication |
| `403` | Caller is not an authorized workspace member |
| `404` | Agent not found in the workspace |
| `500` | Database failure while resolving integrations |
...
```
GET /api/v1/mcp-tool-calls?workspace_id={workspaceId}
```

Returns the workspace's MCP tool-invocation audit trail — one row per tool call an agent made through an MCP server — newest first, capped at **200** rows (`internal/api/mcp_audit.go:69`). Use the filters to scope to a single agent, server, status, or time window.
...
|---|---|---|
| `workspace_id` | string | Required. Workspace whose tool calls are returned. |
| `agent_id` | string | Optional. Filter to one agent (`internal/api/mcp_audit.go:48`). |
| `server_id` | string | Optional. Filter to one MCP server (`mcp_server_id`) (`internal/api/mcp_audit.go:52`). |
| `status` | string | Optional. Filter by call status (`internal/api/mcp_audit.go:56`). |
| `since` | string | Optional. Lower bound on `created_at` (`datetime()`-parsed, inclusive) (`internal/api/mcp_audit.go:60`). |
| `until` | string | Optional. Upper bound on `created_at` (inclusive) (`internal/api/mcp_audit.go:64`). |

**Response:** `200 OK` — a JSON array (empty `[]` when nothing matches — `internal/api/mcp_audit.go:96-99`).
...
| Field | Type | Description |
|---|---|---|
| `crew_id` | string? | Owning crew; `null` for workspace-scoped servers (`internal/api/mcp_audit.go:23`). |
| `mcp_server_scope` | string | Scope the server was bound at, e.g. `workspace` / `crew` (`internal/api/mcp_audit.go:26`). |
| `input_hash` | string? | Hash of the tool input (arguments are not stored in cleartext) (`internal/api/mcp_audit.go:28`). |
| `status` | string | Call outcome (e.g. `success`, `error`) (`internal/api/mcp_audit.go:29`). |
| `duration_ms` | integer? | Wall-clock duration; `null` when not recorded (`internal/api/mcp_audit.go:30`). |
| `error_message` | string? | Failure detail; `null` on success (`internal/api/mcp_audit.go:31`). |

| Status | Condition |
|---|---|
| `200` | Audit rows returned (possibly empty). |
| `400` | `workspace_id` missing (`internal/api/mcp_audit.go:38-41`). |
...
<Note>
  Every endpoint below except `/callback` — which is a browser redirect target and has no CLI form — is driveable headlessly with [`crewship oauth`](/cli/oauth). `crewship oauth connect` runs the loopback leg and waits for the credential to reach `ACTIVE`, since none of these endpoints reports completion on its own.
</Note>
...
```json
{ "credential_id": "cred_oauth", "redirect_uri": "https://app.example/callback" }
```
...
```json
{ "auth_url": "https://provider.example/authorize?...", "state": "..." }
```
...
**Request:** `code` and `state` query parameters are required; `error` may be
returned by the provider.

**Response:** `200 OK` is an HTML success page that closes the authorization
window — the success path is not JSON. Every **error** status below, however,
is a JSON body in the standard `{"error": "<message>"}` shape (e.g.
`{"error": "Invalid or expired state token"}`), so a client that lands on a
non-200 can read the reason instead of parsing HTML.

| Status | Condition |
|---|---|
| `400` | Missing/invalid/expired state, missing code, or provider error |
| `404` | Credential referenced by a valid state no longer exists |
| `500` | State, credential, or token-storage failure |
...
```json
{ "credential_id": "cred_oauth", "code": "provider-code", "state": "..." }
```
...
```json
{ "auth_url": "https://provider.example/authorize?...", "loopback_port": 43127, "state": "..." }
```
...
**Request Body:** `{ "mcp_url": "https://mcp.example.com" }`.

**Response:** `200 OK` -- `{ "auth_url", "token_url", "registration_endpoint?",
"scopes?", "supports_pkce", "supports_dcr" }`.

| Status | Condition |
|---|---|
| `400` | Missing/invalid `mcp_url` |
| `404` | Discovery failed and the URL does not match a known provider |
...
```json
{ "mcp_url": "https://mcp.example.com", "server_name": "linear", "provider_hint": "linear" }
```
...
| Status | Condition |
|---|---|
| `400` | Missing `mcp_url`, or OAuth discovery/provider resolution failed |
| `403` | Caller lacks workspace manage permission |
| `500` | Credential/state creation failed |
...
**Request:** no body. **Response:** `{servers, total, limit, offset}`.
...
| Status | Condition |
|---|---|
| `200` | Registry page returned. |
| `400` | Invalid `limit`, `offset`, `trust_tier`, or `featured` filter. |
| `401` | Missing or invalid authentication. |
| `500` | Registry database query failed. |
...
**Request:** no body. **Response:** `{servers, total, limit, offset, query}`.

**Query Parameters:** `q`, plus the same `limit`, `offset`, `trust_tier`, and
`featured` filters as List. An empty `q` behaves like List.

**Response:** `200 OK` — `{ "servers": [], "total": 0, "limit": 50,
"offset": 0, "query": "github" }`.

| Status | Condition |
|---|---|
| `200` | Search results returned; an empty `q` delegates to List. |
| `400` | Invalid pagination or registry filter. |
| `401` | Missing or invalid authentication. |
| `500` | Registry database query failed. |
...
**Auth:** `OWNER` or `ADMIN` role on the workspace.
...
| `401` | Missing or invalid authentication. |
| `403` | Caller lacks `manage` permission. |
| `429` | A sync was triggered within the last hour. |
