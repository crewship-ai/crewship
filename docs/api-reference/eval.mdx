...
Fetches a single run by id, scoped to the caller's workspace. Same shape as one row of [List runs](#list-runs) — this is the endpoint behind `crewship eval get <id>` ([CLI docs](/cli/eval#crewship-eval-get-run-id)).
