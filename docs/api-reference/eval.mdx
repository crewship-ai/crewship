...
Quartermaster performs observational replay — it rehydrates a mission's trajectory from the journal and recomputes metrics, without re-executing the agents — and diffs two missions to detect regressions in tool success rate, steps to goal, cost, and hallucination count. Both mutating endpoints return `202 Accepted` immediately and run the work in a background goroutine; poll [List runs](#list-runs) for results. See the [Quartermaster guide](/guides/quartermaster).
...
| GET | [`/api/v1/eval/runs/{id}`](#get-a-run) | Fetch one eval run's full record |
...
|---|---|---|---|
| `limit` | integer | 50 | Positive integer up to 200. Missing, malformed, zero, or out-of-range values use the default `50`. |
...
Fetches a single run by id, scoped to the caller's workspace. Same shape as one row of [List runs](#list-runs) — this is the endpoint behind `crewship eval get <id>` ([CLI docs](/cli/eval#crewship-eval-get-%3Crun-id%3E)).
...
| 404 | `id` doesn't exist, or belongs to a different workspace (same shape either way — existence isn't leaked cross-tenant). |
| 500 | DB failure. |
...
gate. The API returns its standard JSON error object for `401`, `400`, and
`500` responses.
