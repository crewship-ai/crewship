...
override state (`override_enabled`); the field is omitted when no override row
...
**Request:** No request body. The `{key}` path parameter is the feature-flag key.
...
```
PUT /api/v1/feature-flags/{key}/override
```
...
```
DELETE /api/v1/feature-flags/{key}/override
```
...
All six operations require an authenticated request. The API resolves the
current workspace from the authenticated workspace context; `workspace_id` is
not a query parameter on these routes. `GET` requires a workspace member with
the read permission. Every mutation requires `OWNER` or `ADMIN` and returns
`401` if unauthenticated or `403` when the role is insufficient. JSON errors
use the API's standard error shape; successful delete operations have no body.
