...
<Note>
  Each of these has a CLI form: [`crewship consolidate run`](/cli/consolidate) and [`crewship consolidate proposed explain|diff|approve|reject`](/cli/consolidate#crewship-consolidate-proposed). Note that no endpoint *lists* proposals — the id comes from `crewship inbox list --kind memory_consolidation`.
</Note>
...
**Request:** `id` is the required proposal path parameter. Workspace context is
