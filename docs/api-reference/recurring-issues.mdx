...
description: "Cron-scheduled issue templates that produce a fresh backlog row on every tick. CRUD plus server-side cron parsing and next_run recomputation."
icon: "repeat"
---
...
**Creator attribution.** The user who creates the template is recorded on the row (`created_by`). Every fired issue is then attributed to that user (`created_by_user_id`), so the [Issues](/api-reference/issues) API surfaces the creator instead of omitting it. Templates created before attribution was captured leave the creator unset and the fired issue omits it.

**Fire idempotency.** Each occurrence reserves a durable key (`sha256(kind‖id‖next_run-bucket)`) in the shared `pipeline_run_idempotency` table before inserting, so two dispatcher replicas racing the same due row create the issue exactly once — the same scheme the pipeline scheduler uses.
...
| `crew_name` | string | Pre-joined from `crews.name` for display. Omitted from the response (`omitempty`) when the crew was deleted (the join yields an empty string). |
...
| `assignee_type` | string \| null | `agent` or `user` when an assignee is set. |
...
| `assignee_type` | string | `agent` or `user` when setting an assignee; validated with `assignee_id`. |
| `assignee_id` | string | |
| `labels_json` | string | |
...
| `500` | Database or response-loading failure. |
...
**Request:** No request body. The `{recurringId}` path parameter identifies the recurring-issue template.
