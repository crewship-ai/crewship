...
| `idle` | integer (seconds) | `300` | Close after this long with no run frame. Heartbeats do not reset it. `0` disables; values above `3600` are clamped. |
...
| `stream.reset` | The stream has a gap it cannot fill — `reason` is `replay_truncated` (the server's buffer overflowed during the run) or `slow_consumer` (this reader fell too far behind). |
| `stream.end` | Always last. `reason` is one of `run_complete`, `no_active_run`, `idle_timeout`, `replay_truncated`, `slow_consumer`, `stream_closed`; `last_seq` is the resume watermark. Without `follow`, a chat with nothing generating gets `no_active_run` immediately — use `follow=1` (bounded by `idle`) to wait for a run to start. |
