...
Replaces the policy. Requires `OWNER` / `ADMIN` (`canRole "manage"`)
— the same gate as [`/api/v1/agents/{id}/learning`](/api-reference/agents#set-self-learning).
...
| `reason` | yes | Non-empty after trim — persisted on the audit triple. |
...
| Status | Condition |
|--------|-----------|
| `400` | Invalid enum value, or missing `reason` |
