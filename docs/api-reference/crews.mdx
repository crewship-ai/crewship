...
| GET | [`/api/v1/activity`](#activity-feed) | Unified cross-crew activity feed |
...
```
GET /api/v1/activity?workspace_id={workspaceId}
```

A unified, time-sorted feed that merges three activity sources — task **assignments**, agent **peer conversations**, and **escalations** — into one workspace-wide stream, newest first (`internal/api/activity_handler.go:214`). Each source is queried for `limit + offset` rows and the merged result is sorted by `created_at` and paged in memory, so the correct page is present even when one activity type dominates the head of the feed.

**Query Parameters:**

| Parameter | Type | Default | Description |
|---|---|---|---|
| `workspace_id` | string | -- | Required. Workspace to read. |
| `agent_id` | string | -- | Optional. Narrow the feed to one agent (`internal/api/activity_handler.go:217`). |
| `crew_id` | string | -- | Optional. Narrow the feed to one crew (`internal/api/activity_handler.go:218`). |
| `limit` | integer | `30` | Page size; clamped to a max of `100` (`internal/api/activity_handler.go:221`). |
| `offset` | integer | `0` | Rows to skip in the merged feed. |

**Response:** `200 OK` — a JSON array of activity items (empty `[]` when nothing matches or the page is past the end).
...
[
  {
    ...
    "type": "escalation",
    "status": "open",
    "summary": "Escalated: deploy script keeps failing",
    "detail": "Third retry failed on the same step.",
    "from_name": "Martin",
    "from_slug": "martin",
    "to_name": "Alena",
    "to_slug": "alena",
...
| Field | Type | Description |
|---|---|---|
| `type` | string | Activity source: `assignment`, `peer_conversation`, or `escalation` (`internal/api/activity_handler.go:13`). |
| `detail` | string? | Longer body text; `null` when the source has none (`internal/api/activity_handler.go:16`). |
| `from_name` / `from_slug` | string | Originating agent (`internal/api/activity_handler.go:17-18`). |
| `to_name` / `to_slug` | string? | Target agent, when the activity has one (`internal/api/activity_handler.go:19-20`). |
| `crew_name` / `crew_slug` | string | Crew the activity belongs to (`internal/api/activity_handler.go:21-22`). |
| `crew_color` | string? | Crew color tag, when set (`internal/api/activity_handler.go:23`). |
