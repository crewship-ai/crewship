...
| `container_memory_mb` | integer | No | `4096` | Container memory limit in MB; must be between `6` and `262144` (`0` uses the server default) |
| `container_cpus` | number | No | `2.0` | Container CPU limit; must be between `0.01` and `512` (`0` uses the server default) |
...
| `container_memory_mb` | integer | Container memory limit in MB; must be between `6` and `262144` (`0` resets to the server default) |
| `container_cpus` | number | Container CPU limit; must be between `0.01` and `512` (`0` resets to the server default) |
