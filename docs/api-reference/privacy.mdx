...
the raw file plus each `- key: value` bullet split into a field, and for each
field where it came from. A read is recorded in the peer-card audit log with
`action='read'`.
  ...
  "facts": [
    {
      ...
      "provenance": {
        ...
        "message_id": "msg_1757912400123_9f2c1a4e7b3d0518",
        "source_type": "stated",
    ...
    },
    {
      ...
      "provenance": {
        "quote": "I'm on UTC+1",
        "message_id": "msg_1757912455771_0a1b2c3d4e5f6071",
        "source_type": "stated",
        "at": "2026-08-02T05:00:00.000Z"
      }
    },
...
`provenance` is where the fact came from: `quote` is the verbatim span of the
person's own words the extraction verified it against, `message_id` the id of
the message it was found in, `source_type` the origin the extraction profile
admitted (`stated` under every shipped profile), and `at` when it was recorded
(ISO 8601, UTC, milliseconds). Provenance is stored beside the model file, never
inside it, and every sync appends: a field changed or restated in a later
message gets a new row and the response shows the newest (the same sentence
re-found by the next day's sweep is not recorded twice). A fact recorded before provenance
was kept has no `provenance` key at all — never `null`, never an empty object.
...
`facts` array above; matching is case-insensitive. The field's provenance rows
go with it — every one, not only the newest.
...
Forgets the whole operator model and every provenance row behind it. Does
**not** opt the caller out: new facts may be recorded again from things they
state later. Use `PUT /users/me/peer-consent` with `opted_out: true` to stop
recording altogether — that purges the model and its provenance too.
