...
<Note>
  ...
  601 of 624 operations return a named 2xx schema, 0 fall back to an unconstrained `object`, and 23 have no success body at all. 268 operations carry a request body: 264 with a named JSON schema, 4 with a non-JSON media type, and 0 with a generic JSON fallback.
