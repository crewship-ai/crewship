...
<Note>
  ...
  586 of 601 operations return a named 2xx schema, 0 fall back to an unconstrained `object`, and 15 have no success body at all. 258 operations carry a request body: 254 with a named JSON schema, 4 with a non-JSON media type, and 0 with a generic JSON fallback.
