...
<Note>
  ...
  {/* These five numbers are derived from internal/api/openapi.gen.json, not typed. TestOpenAPIReferenceQuotesTheCurrentSpec in scripts/docs-inventory re-derives them on every run and prints the replacement sentence when they drift — they had gone 52 operations stale before that test existed. */}
