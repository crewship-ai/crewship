...
<Note>
  ...
  635 of 659 operations return a named 2xx schema, 0 fall back to an unconstrained `object`, and 24 have no success body at all. 281 operations carry a request body: 277 with a named JSON schema, 4 with a non-JSON media type, and 0 with a generic JSON fallback.
  640 of 664 operations return a named 2xx schema, 0 fall back to an unconstrained `object`, and 24 have no success body at all. 284 operations carry a request body: 280 with a named JSON schema, 4 with a non-JSON media type, and 0 with a generic JSON fallback.
  647 of 671 operations return a named 2xx schema, 0 fall back to an unconstrained `object`, and 24 have no success body at all. 286 operations carry a request body: 282 with a named JSON schema, 4 with a non-JSON media type, and 0 with a generic JSON fallback.
