...
The model's answer is validated before it is returned, so a suggestion is
always one this API can go on to create: 2–6 agents, exactly one with
`agent_role` `LEAD` and the rest `AGENT` — the same two values
[`POST /api/v1/agents`](/api-reference/agents) accepts. Any other role is a
rejected suggestion (`502`), not a crew you are offered and then cannot
