...
Every endpoint identifies **Auth**, **Request**, **Response**, and **Status**.
Read routes require a session or CLI token and workspace membership. Import,
generation, bulk import, proposed-skill review, and template writes use the
role gate shown beside the endpoint; crew-template deployment and the AI wizard
require membership only. Requests are JSON; successful reads return `200`,
...
For the five workflow-template endpoints below, **Auth** is workspace member
for reads and `OWNER`/`ADMIN`/`MANAGER` for writes; **Request** is the JSON
...
| Field | Type | Required | Description |
|---|---|---|---|
| `description` | string | Yes | Natural-language description (10–2000 bytes of UTF-8, not characters — the check is `len(string)`) |
...
{
  "crew_name": "Release Crew",
  "crew_slug": "release-crew",
  "description": "Triages issues and drafts release notes.",
  "agents": [
    {
      "name": "Rita",
      "slug": "rita",
      "role_title": "Release lead",
      "agent_role": "LEAD",
      "system_prompt": "You coordinate the release."
    }
  ]
}
```

The model's answer is validated before it is returned. What that validation
guarantees is the shape and every agent's create-time bounds: 2–6 agents,
exactly one with `agent_role` `LEAD` and the rest `AGENT` — the same two
values [`POST /api/v1/agents`](/api-reference/agents) accepts — and each
agent's `name` (2–100 bytes) and derived `slug` (2–50 bytes) within the
same limits the create endpoint enforces, judged against its actual
constants rather than restated numbers. Any agent outside those bounds
makes the whole suggestion a `502`, rather than a crew you are offered and
then cannot create. Roles are matched case-insensitively and returned in
upper case; an omitted `agent_role` is `AGENT`. Slugs are normalised, and
derived from the agent's name when missing or unusable.

It does **not** guarantee that `crew_name` / `crew_slug` clear the bounds
[`POST /api/v1/crews`](/api-reference/crews) enforces — only the
per-agent fields above are checked. Treat a suggestion as a draft to
review, not as a validated payload.

| Status | Condition |
|---|---|
| `400` | Invalid JSON, or `description` shorter than 10 / longer than 2000 bytes of UTF-8 |
| `422` | No active Anthropic `API_KEY` credential in the workspace |
| `502` | AI suggestion call failed, or the model's answer failed validation |
...
**Auth:** Authenticated workspace member with the `OWNER`, `ADMIN`, or `MANAGER` role.
...
| `404` | Agent or skill assignment not found |
