...
All endpoints require authentication and are workspace-scoped. There is **no register / create / delete endpoint** -- hook registration is a config-time operation (via `hooks.Register` in Go). See the [Hooks guide](/guides/hooks).
...
Shell hooks additionally require the caller's role to be `OWNER` -- `hooks.ErrShellHookNotAllowed` otherwise.
