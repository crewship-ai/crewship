...
| `crew_icon` | string | no | Icon for the crew card and row — a name from the web icon set (`lib/crew-icons.ts`, mirrored server-side). Overrides the template's default. Checked against that vocabulary and **dropped when unknown** — never stored as typed, and never a `400`: a runnable crew with the default look beats a refused proposal mid-onboarding. |
| `crew_color` | string | no | Colour for the crew card: a palette id (`blue`, `emerald`, `violet`, `amber`, `rose`, `cyan`, `lime`, `fuchsia`) or a 6-digit hex, with or without `#` (stored with it). Same drop-when-unknown rule. |
...
`payload.crew_icon` / `payload.crew_color` are present only when the proposal
carries a look — from the request or from the template — and are what
`apply` writes onto the crew row. `crewship onboarding proposal create
--crew-icon --crew-color` sets them.
