...
A member with no explicit set falls back to a role-derived default (`OWNER`/`ADMIN` → the admin set **plus `credentials:reveal`**, `MANAGER` → chat + `routine.create` + `issue.create` + `memory.write`, others → `chat`). An explicit set is always authoritative, so revoking `credentials:reveal` from an ADMIN sticks.
