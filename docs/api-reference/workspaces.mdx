...
A **workspace** is the top-level tenant in Crewship. Every crew, agent, credential, skill, and pipeline belongs to exactly one workspace, and a user joins a workspace with a role (`OWNER`, `ADMIN`, `MANAGER`, `MEMBER`, `VIEWER`).
...
| `_count_crews` | integer | Number of non-deleted crews |
| `_count_agents` | integer | Number of non-deleted agents |
| `_count_members` | integer | Number of workspace members |
...
> **Note:** There is no `DELETE /workspaces/{workspaceId}` endpoint and no `crewship workspace delete` command. Workspace deletion is handled out-of-band (a direct operation on the host database); deleting the last workspace would orphan its owner, so it is intentionally not exposed through the API or CLI.
...
Adds an existing user to the workspace by user ID. Use this when the user already has a Crewship account; for unknown email addresses, use [Create invitation](#create-invitation) instead.
...
| `last_test_run_at` | string | conditional | RFC 3339 timestamp from a recent `/test_run`; required unless `save_token` or `skip_test_gate` is used |
| `last_test_run_passed` | boolean | conditional | Must be `true` if relying on body-trust |
| `save_token` | string | no | HMAC-signed proof returned by `/test_run`; supersedes the body-trust on `last_test_run_at` |
| `skip_test_gate` | boolean | no | OWNER/ADMIN-only escape hatch -- bypass the 5-minute test-run gate entirely |
...
| `422` | DSL parse / validate / cycle-detect failure, expired `save_token`, or test-run gate not satisfied |
...
Executes an **unsaved** DSL against the runner so the author can confirm it works before calling `/save`. On success the response includes a `save_token` HMAC-bound to `(workspace_id, definition_hash, user_id)` that can be passed to `/save` instead of a body-trust timestamp.
...
```json
{
  "definition": { /* DSL JSON */ },
  "author_crew_id": "crew_eng",
  "sample_inputs": { "issue": "ENG-9" }
}
```

**Response:** `200 OK` -- a [`RunResult`](#runresult-shape) with an additional `save_token` field when the run completed successfully and a token signing secret is wired.

| Status | Condition |
|---|---|
| `400` | Missing `definition` or `author_crew_id`, invalid JSON |
| `422` | DSL parse/validate failure |
| `503` | Runner not wired (orchestrator hasn't booted, or build excluded the runner) |
...
Returns the structured **WouldExecute** report for the supplied inputs -- no agent invocations, no journal entries.

**Request body:** Same shape as [Run](#run-pipeline). All fields optional.

**Response:** `200 OK` -- `RunResult` with `mode="dry_run"`.
...
|---|---|---|---|
| `limit` | integer | 50 | Hard cap 500 |
| `status` | string | -- | Filter to a single `RunStatus` (`running`, `completed`, `failed`, `paused`, `queued`, `cancelled`) |
...
**Response:** `200 OK` -- single row with the same fields as the workspace-runs list plus `workspace_id`, `inputs` (parsed), and `idempotency_key`.
...
Imports **skip the test-run gate** by design -- a marketplace bundle is presumed to have passed test-run in its source workspace.
...
| `signing_secret` | string | no | Used for HMAC-SHA256 verification of the `X-Crewship-Signature` header |
| `inputs_template` | object | no | Merged on top of the default `{event, raw, headers}` envelope when the webhook fires |
| `enabled` | boolean | no | Default `true` |
| `rate_limit_per_min` | integer | no | Per-token rate limit; 0 disables |

**Response:** `201 Created` -- webhook object **including** `signing_secret` (this is the only time it is returned; subsequent reads return only `signing_secret_set: true`).
