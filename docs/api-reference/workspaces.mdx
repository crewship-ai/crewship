...
| `GET` | [`/api/v1/workspaces/{workspaceId}/pipeline-runs/{runId}/files`](#run-produced-files) | Files a run produced (mtime-windowed) |
...
<Note>
The mtime-window heuristic can cross-attribute a file when two runs on the same crew have overlapping windows. It intentionally does **not** modify the pipeline executor — see the `routine result` [CLI docs](/cli/routine#crewship-routine-result-run-id).
</Note>
