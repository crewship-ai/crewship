...
| `approvals_retention_days` | integer? | Days of decided `approvals_queue` history to keep. `null` **or `0`** use the 90-day default — unlike the audit pair above, there is no keep-forever sentinel. Present on Update responses. |
...
| `approvals_retention_days` | integer | Days of decided `approvals_queue` history to keep. Must be positive — `0` is rejected, same as `run_retention_days`. |

<Note>
`0` behaves differently on the two audit windows than on `run_retention_days`
and `approvals_retention_days`, both of which reject it. For an audit table,
"keep this forever" is a retention decision an operator has to be able to
express — so `null` means "use the product default" and `0` means an
explicit never-delete. `approvals_queue` is not a compliance record (every
terminal decision it holds is also captured durably in the Crew Journal), so
it follows `run_retention_days`'s simpler rule instead: `null` uses the
90-day default, and there is no way to ask for "keep forever" through this
column. See [Migrations — 20260810170000](/guides/migrations) and
