...
| `POST` | [`/api/v1/workspaces/{workspaceId}/members`](#add-member) | Add an existing user by user ID or email |
...
Adds an existing user to the workspace by user ID -- or by email, when
that is the only handle the caller has (`POST /api/v1/auth/signup` no
longer returns the account's id; see [Auth](/api-reference/auth)). The
email is resolved server-side, case-insensitively, behind the same
manage-role gate.
...
|---|---|---|---|---|
| `user_id` | string | yes* | -- | Target user's CUID. *Optional when `email` is given. |
| `email` | string | yes* | -- | Target user's email address, as an alternative to `user_id`. Ignored when `user_id` is set. |
