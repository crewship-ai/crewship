...
an account via [`POST /api/v1/auth/signup`](/api-reference/auth#signup),
...
A run with no resolvable/in-workspace crew returns `{ "crew_id": "", "files": [] }`. Download a listed file via [`crew files download`](/api-reference/workspaces#file-endpoints) / `crewship crew files get <crew_id> <path>`.
