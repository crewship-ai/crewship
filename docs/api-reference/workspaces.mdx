...
A member with no explicit set falls back to a role-derived default (`OWNER`/`ADMIN` → the admin set, `MANAGER` → chat + `routine.create` + `issue.create` + `memory.write`, others → `chat`).
