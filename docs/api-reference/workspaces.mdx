...
```json
{
  "error": "publication blocked by schedule Nightly: input \"region\" is required",
  ...
  "schedule_conflict": { "schedule_id": "psched_...", "name": "Nightly", "reason": "input \"region\" is required" }
}
```

Only enabled plans that follow head (no version pin) are judged; disabled and
pinned plans never block a save. The same envelope comes back from
