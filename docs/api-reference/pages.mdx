...
contains `revision`, `digest`, `git_commit`, `actor`, `created_at`, `restorable`.
...
expected_publication, reviewed_code:true}`. Use publication `0` for the first
publication. For rollback, send `{rollback_version, expected_publication,
reviewed_code:true}` and omit `build_id` and `expected_revision`.
...
`409` changed draft, publication or declaration, `413` oversized body,
`422` invalid bindings, `507` publication quota, plus common errors.
