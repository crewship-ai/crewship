...
the response is scoped to what the caller can see.
...
declared `actions`, `wake` gates and `on_failure` — because that is what an edit
form has to round-trip. A reader does not, and an absent authored half is
...
| 401 | No session or token. |
| 403 | Caller is not a member of the workspace. |
| 404 | No page with that slug in this workspace. |
