...
no draft or the draft is already live.
...
from its archived Page definition. That is exactly the key set a
...
`routines[].state` is `unchanged`, `changed` or `unknown`; `unknown` covers a
publication that recorded no digest for a routine and a routine that no longer
resolves, and is never rendered as agreement. `blockers[].code` is one of
`no_candidate`, `candidate_matches_live`, `build_missing`, `build_failed`,
`build_stale`, `baseline_unavailable`, `definition_moved`, `not_permitted` and
`storage_unavailable`. Missing build storage or a missing runtime origin is a
