...
source_unavailable_reason}` and `candidate` is
...
`definition_moved`, `not_permitted` and `storage_unavailable`.
...
or a malformed fence, `409` changed draft, publication or declaration, `413`
