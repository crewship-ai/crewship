...
| `limit` | integer | `20` | Rows per page (1-200; each row can carry its task list) |
| `offset` | integer | `0` | Pagination offset |

The body stays a bare array. The total the filter matched — before `limit`/`offset` — is in the `X-Total-Count` response header, with `X-Limit` and `X-Offset` echoing the values applied.
...
| `q` | string | -- | Search title and identifier on the server (substring match) |
| `limit` | integer | `20` | Max items (1-100) |
| `offset` | integer | `0` | Pagination offset |

**Response:** `200 OK` -- array of mission objects with `task_stats`. The same
`X-Total-Count`, `X-Limit` and `X-Offset` headers as the workspace list, so a
crew-scoped page knows how many rows the crew has.
...
When status changes to `DONE`, `FAILED`, or `CANCELLED`, `completed_at` is automatically set.

<Note>
  Before this release, a mission approved out of `REVIEW` moved to
  `COMPLETED` while an *issue* approved the same way moved to `DONE` — two
  words for the identical operator action, sharing one column
  (`missions.status`). `COMPLETED` is now retired: this endpoint writes
  `DONE` for both. For backward compatibility, `{"status":"COMPLETED"}` is
  still accepted and silently normalized to `DONE` — it is never written or
  returned again. Any script or dashboard still matching the literal string
  `COMPLETED` on a mission's `status` field should switch to `DONE`.
</Note>
...
| `403` | Mission was created under an [autonomy hold](/api-reference/internal#autonomy-gate-on-agent-driven-creation) and the hold is undecided or denied. Body: `{"error": "Mission start rejected by policy", "reason", "mission_id", "remedy"}` — an OWNER/ADMIN approves the `autonomy_gate` row (`crewship approvals approve <id>`) and the start is retried. Same helper as the internal start route, so a held mission cannot be started from either. |
| `404` | Mission not found |
...
Restarts a `DONE`, `FAILED`, or `CANCELLED` mission. Resets non-COMPLETED tasks (PENDING if dependencies allow, BLOCKED otherwise), increments their `iteration`, clears errors, and moves the mission back to `PLANNING` (call `start` afterwards to dispatch). (`internal/api/task_state.go:14`)
...
|---|---|
| `404` | Mission not found |
| `409` | Mission is not in a terminal state (`DONE`/`FAILED`/`CANCELLED`) |
...
| `max_iterations` | integer | Max retry iterations (0–`maxTaskIterations`) |
...
**Auth:** Authenticated workspace request. **Request:** required `workspace_id` query parameter; no body. **Response:** JSON aggregate metrics object. **Statuses:** `200 OK`; `400` for missing or invalid workspace context, `401` when unauthenticated, and `500` for an aggregation failure.
...
`total_tokens_24h` and `total_cost_24h` are **mission** totals: they sum the tokens and estimated cost of mission tasks completed in the window, and nothing else. Spend that happens outside a mission — chats, ad-hoc agent runs, routine runs — is not counted here, so a workspace can report `0` on both while it is genuinely spending money. That spend lives in the cost ledger, which the paymaster endpoints report (and `crewship cost` prints). A `0` here means "no mission task was recorded in the window", not "no spend".
...
| `DONE` | Successfully finished and approved (formerly `COMPLETED` — still accepted as `PATCH` input, normalized) |
