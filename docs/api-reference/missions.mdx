...
<Note>
  Mission proposals are a low-level primitive retained for future
  human-in-the-loop approval flows. The high-level approval gating today
  is handled by [Harbormaster](/guides/harbormaster).
</Note>

Agents can propose missions that require human approval before execution.
