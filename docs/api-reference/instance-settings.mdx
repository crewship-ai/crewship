...
**Auth:** Authenticated workspace member with `OWNER`, `ADMIN`, or `MANAGER` role (`create` tier)
...
**Auth:** Authenticated workspace member with `OWNER`, `ADMIN`, or `MANAGER` role
...
```
PUT /api/v1/instance/settings/{key}
```
...
```
DELETE /api/v1/instance/settings/{key}
```
...
All routes require an authenticated request with workspace context; the
workspace selects authorization but does not filter the instance-global rows,
and `workspace_id` is not a query parameter. `GET` requires `MANAGER`,
`ADMIN`, or `OWNER`; `PUT` and `DELETE` require `ADMIN` or `OWNER`. The API
returns `401` for missing authentication and `403` for an insufficient role.
