...
Round-trip self-test: create a synthetic backup, verify its MANIFEST + payload, and clean up. Used by health checks and CI to confirm encryption keys, storage paths, and the restore-side decrypt path are all wired correctly.
...
- `dry_run`: when `true`, validate configuration and storage permissions without writing a bundle. Default `false`.
...
{
  "ok": true,
  "created_path": "/var/lib/crewship/backups/self-test-2026-05-11T07-30-00Z.crewbackup",
  "verified": true
}
```

If `dry_run: true`, `created_path` is empty and `verified` reflects only configuration checks.
...
| `401 Unauthorized` | No authenticated session. |
| `403 Forbidden` | Authenticated, but email does not match `CREWSHIP_OWNER_EMAIL`. |
| `409 Conflict` | Another backup operation holds the workspace lock. |
| `500 Internal Server Error` | Storage path unwritable, encryption key missing, or verify-decrypt failed. |

Audit action: `backup.self_test`.
