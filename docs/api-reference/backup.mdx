...
<Warning>
  `contents.crews[].memory_included` means "the crew and per-agent memory trees under `/crew` are in this bundle" **only from `format_version` 3**. At v1 and v2 the flag was set unconditionally for any crew that had a container, and the section behind it held `/output`, not memory. Read it together with `format_version`, or use the CLI, which already does. From v3 every per-crew inclusion flag is derived from what the collector observed writing: `memory_included` (`/crew`), `output_included` (`/output`), `workspace_included`, `volumes_included`, `system_included`.
</Warning>
