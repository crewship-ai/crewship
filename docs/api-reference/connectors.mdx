...
**Request:** `connectorId` is the required path parameter. No request body.
...
| `500` | Connector catalog lookup failed |
...
| `next_step` | string? | Omitted for a complete install, or `"oauth"` (open `oauth_url`) / `"mcp_oauth"` (hand off to MCP-OAuth/DCR) |
