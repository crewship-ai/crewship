...
The `rows[]` objects serialize with snake_case keys, matching the envelope. (Before #1209 they carried Go field names — `ID`, `CrewID`, `CreatedAt` — which neither the CLI's struct tags nor the dashboard's schema could fully decode; treat the PascalCase shape as a historical bug, not a contract.) `decided_at` and `timeout_at` are nullable — they serialize as `null` when unset rather than being omitted.
...
|---|---|---|
| `rows[].kind` | string | `tool_call`, `cost_threshold`, `destructive_op`, `target_environment`, `custom`, or the synthetic `agent_hire` (see below). |
...
#### Staged hires (`kind: agent_hire`)

Staged guided-autonomy hires — ephemeral agents parked in `PENDING_REVIEW` — are projected into the pending listing as synthetic rows (they are never persisted to `approvals_queue`). Their `id` is the **agent id**, `payload` carries `{"type": "hire", "agent_name": …, "expires_at": …, "decide_via": "crewship hire approve <agent-id>"}`, and `timeout_at` mirrors the hire TTL. They appear only while pending (`?status=pending` or `all`); once approved or ghosted they leave the projection — the decision history lives in the journal (`agent.hire_approved`). See [Staged hires in the queue](/guides/harbormaster#staged-hires-in-the-queue).
...
Deciding a staged hire (`kind: agent_hire`, id = agent id) with `status: approved` delegates to the same state machine as `POST /api/v1/agents/{agentId}/approve-hire` — the agent flips to `IDLE` and its blocking inbox waitpoint resolves. `status: denied` on a hire returns 409: staged hires have no deny lifecycle — an unapproved hire ghosts when its TTL expires.
