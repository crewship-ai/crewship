...
| `rows[].payload` | object | Tool call context (`tool`, `args`, plus any rule-specific fields). For `ephemeral_hire` it carries `inbox_item_id`, the inbox waitpoint projecting the same decision. |
