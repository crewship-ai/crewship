...
**Auth:** authenticated session + workspace context (normally supplied by the
`X-Workspace-ID` header or the authenticated session).

**Request:** `slug` is the required recipe path parameter. Workspace context is
...
| Status | Condition |
|---|---|
| `400` | Workspace context is missing or malformed. |
