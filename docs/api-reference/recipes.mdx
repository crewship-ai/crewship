...
The implementation lives in `internal/api/recipes.go`. Install is **atomic**: credentials, the new crew, and its MCP servers are all created in a single transaction — any failure rolls back everything, so a half-installed recipe (orphan crew without its credentials, etc.) never exists.
...
**Auth:** authenticated session + workspace context + `OWNER`, `ADMIN`, or `MANAGER` role (`canRole(role, "manage")`).
