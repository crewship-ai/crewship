...
<Note>
**Revocation reaches running agents within ~60s.** A revoked credential is filtered out of every fresh delivery path immediately, but each agent's sidecar also holds an in-memory snapshot of its provider tokens (loaded at container start). The sidecar reconciles that snapshot against the live set on a ~60s interval and drops any credential the server no longer lists (revoked or deleted), so a revoked key stops being injected into new outbound requests within one interval — without waiting for the agent container to restart.
</Note>
