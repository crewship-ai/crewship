...
| `PUT` | [`/api/v1/credentials/{credentialId}/fields/{key}`](#update-custom-field) | Update a field |
| `DELETE` | [`/api/v1/credentials/{credentialId}/fields/{key}`](#delete-custom-field) | Delete a field |
...
```
PUT /api/v1/credentials/{credentialId}/fields/{key}
```

**Auth:** `MANAGER+` role. The body uses `value`, optional `is_secret`, and
optional `ordinal`; the path key cannot be renamed. Returns `200 OK` with the
field object, or `400` for invalid input and `404` for a missing/invisible
credential or field.
...
**Auth:** `MANAGER+` role. Returns `200 OK` with `{"success":true}`; a
missing/invisible credential or field returns `404`.
