...
`supported` reports whether a real upstream probe ran. Crewship maintains probes for `ANTHROPIC`, `OPENAI`, `OPENAI_COMPAT`, `GOOGLE`, `OPENROUTER`, `CURSOR`, `FACTORY`, `GITHUB`, `GITLAB` and `VERCEL` (`probeSupportedProviders` in `internal/api/credentials_test_endpoint.go`), plus any credential of type `ENDPOINT_URL` whatever its provider — for that type the stored value *is* the target being dialled. Every other provider falls through to a no-op that answers `valid: true` — literally true, since nothing failed, but nothing was attempted either.

`OPENAI_COMPAT` is probeable only here, on the stored-credential route, not on the unauthenticated-body `POST /api/v1/credentials/test` above: its endpoint is operator-supplied, so it is dialled only from this role-gated path, the dialer refuses link-local and metadata addresses after DNS resolution, redirects are never followed, and the probe offers no credential at all — it asks for a model list, because reachability is the thing an operator cannot otherwise find out.
