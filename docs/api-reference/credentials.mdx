...
| `GET` | `/api/v1/credentials/{credentialId}/fields` | List a credential's custom fields (secret values omitted) |
| `POST` | `/api/v1/credentials/{credentialId}/fields` | Add a field |
| `PUT` | `/api/v1/credentials/{credentialId}/fields/{key}` | Update a field |
| `DELETE` | `/api/v1/credentials/{credentialId}/fields/{key}` | Delete a field |
...
`source`, most specific first: `agent_grant` (an explicit per-agent assignment), `agent_binding`, `crew_binding`, `workspace_binding`, `crew_link` (no binding — delivered under the credential's own name).
