...
<Warning>
This ~60s reaper covers only the **in-memory provider-token store** (the sidecar proxy's injected LLM keys). **File-based secrets** written to `/secrets/{agent-slug}/` (DB passwords, `GH_TOKEN`, SSH keys, etc.) are materialized on disk at container start and are **not** re-reaped — a revoked file-based secret remains readable to the agent until the container is recreated. Revoke-and-recreate the crew container to fully cut off a file-based secret.
</Warning>
</Note>
