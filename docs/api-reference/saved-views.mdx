---
title: "Saved Views"
description: "Per-user saved issue views with optional workspace sharing — store filters_json + sort_json so a user (or the workspace) can replay a saved filter from one click."
icon: "bookmark"
...
<Note>
  **Two surfaces share this table.** The issue tracker and the
  [Crew Journal](/guides/crew-journal#saved-views) both store their bookmarks
  here, and `List` returns every view in the workspace, so each client filters
  the list by a `surface` key inside its own `filters_json`. Do **not** reach
  for `view_type` as the discriminator: the column is
  `CHECK (view_type IN ('board','list'))`, so a third value fails the insert
  with a 500 rather than creating a new category.
</Note>
...
| Status | Condition |
|---|---|
| `400` | Malformed JSON body or no fields provided (`No fields to update`). In `PATCH`, a JSON `null` for `sort_json` is decoded as an omitted field; it does not clear the stored value. |
| `401` | Not authenticated. |
...
**Request:** No request body. The `{viewId}` path parameter identifies the saved view.
