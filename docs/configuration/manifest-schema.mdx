...
<Warning>
  **Private once applied.** A service that sets `env`, `command`, `healthcheck` or `auto_credentials` is never returned on a read: `services_json` on every crew response becomes the literal `[service configuration withheld: contains private runtime settings]`, for every role. `crewship export` then refuses the crew, a later `apply` of a document that carries a `services:` key is refused (`cannot be diffed or replaced from a redacted snapshot`), and a `PATCH` with any other `services_json` is `409`. Keep the applied manifest — it is the only complete copy. A service that names only `name`, `image`, `ports`, `env_refs` and `volumes` stays readable and round-trips. See [Crew manifest → Private service settings](/manifest/crew#private-service-settings).
</Warning>
