...
<Note>
  This page documents the two workspace-bundle kinds, **`Crew`** and
  **`Workspace`**. The apply pipeline also accepts additional
  ...
  `metadata` envelope but are out of scope for this schema reference.
</Note>
...
| `memory_mb` | int | Container memory cap. 6–262144 (Docker's own minimum); `0` or omitted = server default (`4096`). Below `2048` — one agent CLI's working set — applies with a server warning rather than an error; that floor is the `runtime.agent_min_memory_mb` instance setting. |
| `cpus` | float | CPU cores. 0.01–512 (Docker's own minimum); `0` or omitted = server default (`2.0`). Below `0.5` applies with a server warning; that floor is the `runtime.agent_min_cpus` instance setting. |
| `ttl_hours` | int | Idle hours before the crew container is stopped. Omit it for the server default (4 hours); `0` = never stop. |
...
| `suggested_prompts` | string | — | The agent's own chat suggestions — the chips under an empty composer — **one per line**, written as a YAML block scalar. At most 8 lines, at most 120 characters each; blank lines are ignored. Omit it to leave the server's value alone and fall back to the role packs. |
...
| `auto_credentials` | `[]AutoCredential` | — | Secrets Crewship generates and manages for the sidecar. Each entry produces an `AUTO_MANAGED` credential row (attributed to the crew's lead agent), is injected into this sidecar's env, and is appended to every crew agent's `env_refs`. Known images (`postgres:*`, `mysql:*`, `mongo:*`, …) get a sugar default merged in. **Refused when the service publishes a port to the host** (external attack surface needs an operator-chosen credential). |

<Warning>
  **Private once applied — and a bundle cannot re-apply it.** A service whose stored configuration carries `env`, `command`, `healthcheck` or `auto_credentials` is never returned on a read: `services_json` on every crew response becomes the literal `[service configuration withheld: contains private runtime settings]`, for every role. In a Workspace bundle that includes every catalog datastore image, however bare the declaration: `{name: postgres, image: postgres:16}` gets `POSTGRES_USER` plus a generated `POSTGRES_PASSWORD` written into its `env` on the first apply (the sugar defaults above), so it is private from then on. Only a non-catalog image declared with nothing beyond `name`, `image`, `ports`, `env_refs` and `volumes` stays readable.

  Once a crew's services are private, **any** later apply of the bundle that names that crew is refused — with or without a `services:` key — because the apply regenerates auto-managed credentials from the stored snapshot and there is none to read: `crew "<slug>": auto-managed credentials: service configuration is private; refusing to regenerate credentials from a redacted snapshot`. `crewship export` refuses the crew for the same reason, and a `PATCH` with any other `services_json` is `409`. To change such a crew, delete and re-create it from the bundle, or manage it through a top-level [`kind: Crew`](/manifest/crew#private-service-settings) document, which has no sugar and accepts metadata-only edits. Keep the applied manifest — it is the only complete copy.
</Warning>
...
| --- | --- | --- | --- |
| `name` | string | (required) | Workspace-unique credential name **and** the default env-var name on the sidecar + every crew agent. Use `SCREAMING_SNAKE_CASE`. |
| `inject_as_env` | string | `name` | Overrides the env-var name the sidecar receives (e.g. `MARIADB_ROOT_PASSWORD`). |
| `inject_to_agents` | bool | `true` | When `false`, the sidecar uses the secret internally but no agent picks it up. |
| `length` | int | `32` bytes (64 hex chars) | Random bytes generated before hex-encoding. Minimum `16`, maximum `512`; outside that range the validator refuses the row. |
| `description` | string | — | Shown in the UI on the "Created by \<agent\>" row. |

### `ServiceVolume`

| Field | Type | Notes |
| --- | --- | --- |
| `name` | string | Per-crew namespaced internally as `crewship-svc-{crew-slug}-{crew-id}-vol-{name}`. Two crews can both declare `pg-data` and get isolated stores — including two crews with the *same slug* in different workspaces, which is why the crew id is in the name (#1732). |
...
| `suggested_prompts` holds at most 8 non-blank lines | `<agent>: suggested_prompts has N entries (at most 8 are allowed)` |
| Each `suggested_prompts` line is at most 120 characters | `<agent>: suggested_prompts entry N exceeds 120 characters (it has M)` |
