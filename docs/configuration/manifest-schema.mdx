...
| Field | Type | Required | Notes |
| --- | --- | --- | --- |
| `name` | string | ✓ | DNS-label (`[a-z][a-z0-9-]{0,30}[a-z0-9]`). Becomes the bridge-network alias. |
...
The validator runs the following cross-checks at apply time. Errors abort before any mutation lands.

| Rule | Failing message |
| --- | --- |
| Every `agent.skills[]` slug must be declared in the merged workspace + crew skill scope | `agent "X" references unknown skill "Y"` |
| Every `agent.env_refs[]` env name must be declared in the merged credential scope | `agent "X" references unknown credential env "Y"` |
| Every `mcp.env_mapping[]` value must be declared in the merged credential scope | `mcp "X": env_mapping[KEY] → "Y" references unknown credential` |
| Every `service.env_refs[]` env name must be declared in the merged credential scope | `service "X": env_refs[Y] references unknown credential` |
| Each skill must have exactly one source (`path` / `source` / `inline`) | `skill "X": must have one of path/source/inline` |
| At most one agent per crew may have `agent_role: LEAD` | `crew "X" has more than one LEAD` |
| Service names must be unique within a crew | `crew "X": duplicate service name "Y"` |
