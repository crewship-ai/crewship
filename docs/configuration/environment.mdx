...
| `CREWSHIP_ALLOW_PRIVATE_ENDPOINTS` | (unset — private endpoints blocked) | Instance-level ceiling for a crew's `--allow-private-endpoints` opt-in on a BYO local-model `ENDPOINT_URL`. Set to `1`/`true`/`yes`/`on` to let crews (that also set their flag) point agents at loopback/RFC1918/LAN endpoints — e.g. an on-prem Ollama or `host.docker.internal`. Off by default so a hosted/multi-tenant instance is safe out of the box: a workspace admin cannot self-grant egress into the host's private network. The effective decision is `instance-cap AND crew-flag`. Link-local/metadata stay hard-blocked regardless. Read by `internal/config/config.go` (`local_models.allow_private_endpoints`). |
