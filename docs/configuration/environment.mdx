...
| Variable | Default | Description |
|----------|---------|-------------|
| `CREWSHIP_SOCKET_PATH` | `/tmp/crewship.sock` — or `$CREWSHIP_DATA_DIR/crewship.sock` for a relocated data dir | Unix socket path for internal IPC. The `/tmp` literal is the default only when `CREWSHIP_DATA_DIR` is unset or is the packaged root `/var/lib/crewship` (what the deb/rpm unit pins); any other data dir derives the socket from it, so two instances on one host cannot collide on a single socket. If the derived path would exceed the ~100-byte `sockaddr_un` limit, the default falls back to `<tmpdir>/crewship-<hash-of-data-dir>.sock`. Starting the server refuses outright if something is already listening on the socket path, rather than unlinking a running instance's socket. |
