...
| `CREWSHIP_BOLT_PATH` | `/var/lib/crewship/state.db` — or `$CREWSHIP_DATA_DIR/state.db` for a relocated data dir | BoltDB file path. Same rule as `CREWSHIP_SOCKET_PATH`: the literal stands when `CREWSHIP_DATA_DIR` is unset or is `/var/lib/crewship`, otherwise the file lives under the data dir. The bbolt file is exclusively locked by whichever process opens it, so instances must not share one. |
