...
# Container provider
container:
  provider: "docker"            # "docker" | "apple" | "auto" | "k8s"
  runtime_image: "ghcr.io/crewship-ai/agent-runtime:latest"
...
BoltDB is the default state store for agent run tracking. For high-throughput deployments, switch to PostgreSQL:
...
The minimum viable configuration requires only:

```yaml
# No YAML file needed -- just environment variables:
# ENCRYPTION_KEY=<64-hex-chars>
# NEXTAUTH_SECRET=<random-secret>
```

All other values have sensible defaults. The server will:
- Listen on `0.0.0.0:8080`
- Use Docker as container provider
- Store data in `/var/lib/crewship`
- Use BoltDB for state
- Auto-generate the internal token
- Auto-derive the NextJS URL
...
container:
  provider: "docker"
  runtime_image: "ghcr.io/crewship-ai/agent-runtime:v1.0.0"
