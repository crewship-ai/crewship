...
Cache-image deletion is opt-in via [`CREWSHIP_CACHE_GC_AUTODELETE`](/configuration/environment#devcontainer-cache-gc).
Default is log-only — operators see the orphan list and can decide. The
sweeper logs a single line per pass with `removed`, `total_orphans`, and
`skipped_too_young` so dashboards can alert on growth.

The sweeper shares a 10-second image-list cache with `GET /api/v1/admin/cache`,
so admin UI polling does not pay the O(n) Docker `ImageList` cost on every
request.
