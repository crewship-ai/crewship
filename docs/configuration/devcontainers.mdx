...
- **Temp containers** — those *named* `crewship-provision-*` **and** labelled
  ...
  container's labels into the resulting `crewship-cache:*` image — so every
  crew container started from it inherits `crewship.temp=provision`. Crew
  containers set no labels of their own, making it the only label they carry.
  A label-only sweeper therefore force-removes healthy crews an hour into
  their lives; this happened on a dev host on 2026-07-20. Container names are
  never copied into images, which is why the name is the marker that holds.

  Consequence for `docker ps`: filtering by `label=crewship.temp=provision`
  lists your running crews too. Filter by `name=crewship-provision-` to see
  only genuine scratch containers.
  </Warning>
