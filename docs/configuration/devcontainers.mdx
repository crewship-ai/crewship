...
| `features` | `map[featureRef]options` | — | Community devcontainer features. |
| `postCreateCommand` | `string` \| `[]string` \| `map[string]string` | — | Commands run as `agent` user (UID 1001) after feature install. |
| `containerEnv` | `map[string]string` | — | Env vars baked into the cached image (via `ENV` on commit). |
| `remoteUser` | string | — | Default user for `postCreateCommand`. Crewship forces `agent` regardless. |
