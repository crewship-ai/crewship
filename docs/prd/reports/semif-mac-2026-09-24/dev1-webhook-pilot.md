...
Archived manual development experiment from closed, unmerged PR #2630. Both
test webhook triggers and their target routines were disabled during archival
closeout on 2026-09-25. Their receipts and run history remain available.
...
when the Mac is unavailable. The closed PR's server `decision` step only
wired hosted providers; Crewship `main` has no such step.
