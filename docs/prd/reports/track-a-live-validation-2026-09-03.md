...
| 4 | Correction during an active run reflected in the next step | PASS (B3b, #2350) | A comment arriving during an active run is now classified `correction`, folded ahead of normal follow-ups into the next run on the SAME session, and labelled `CORRECTION` ("read and apply them first"). Proven by `TestDispatchQueuedFollowUps_CorrectionLeadsAndIsLabelled`, `TestDeliverAndDispatch_SessionBusyMarksCorrection` (both fail to compile on `main` — the priority constant and behaviour do not exist). Live on dev1 (`main`+B3b): a mention started a run, a `Correction:` comment posted while it was live was folded into the next run on the same session with the header "Any marked CORRECTION are steering the work you just did — read and apply them first" and the entry `--- CORRECTION 1, from Demo User ---` leading the brief. **Boundary:** "next step" = next exec on the session; mid-token injection of a live turn is N1 (non-goal, stateless adapters) and `Bridge.Steer` into a session-bearing run stays follow-on. |
