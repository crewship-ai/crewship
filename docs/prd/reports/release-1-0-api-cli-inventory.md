...
|---|---:|---|---|
| API operations | 553 | 553 exact, 0 resource-level, 0 missing | 418 with a test signal |
| CLI commands | 752 | 752 exact mentions, 752 root pages present, 0 root pages missing | 737 with a test signal |

Response schema quality: 539 API operations have a concrete 2xx schema; 0 still use the generic object fallback.

Request schema quality: 233 operations have request bodies; 229 have concrete JSON schemas, 4 use non-JSON media types, and 0 still use a generic JSON fallback.

CLI flag quality: 355 commands define flags; 355 document all of their flags and 0 still have undocumented flag(s).

Environment variables: 101 discovered, 0 missing documentation. Manifest kinds: 20 discovered, 0 missing documentation.

Docs → code references: 4089 commands, 2049 API paths, 420 environment variables, 209 manifest kinds, and 1902 flags; missing symbols: 0, 0, 0, 0, and 0 respectively.
