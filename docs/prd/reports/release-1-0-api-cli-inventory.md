...
|---|---:|---|---|
| API operations | 536 | 536 exact, 0 resource-level, 0 missing | 404 with a test signal |
| CLI commands | 730 | 730 exact mentions, 730 root pages present, 0 root pages missing | 716 with a test signal |

Response schema quality: 522 API operations have a concrete 2xx schema; 0 still use the generic object fallback.

Request schema quality: 225 operations have request bodies; 222 have concrete JSON schemas, 3 use non-JSON media types, and 0 still use a generic JSON fallback.

CLI flag quality: 343 commands define flags; 343 document all of their flags and 0 still have undocumented flag(s).

Environment variables: 96 discovered, 0 missing documentation. Manifest kinds: 20 discovered, 0 missing documentation.

Docs → code references: 3921 commands, 2126 API paths, 405 environment variables, 212 manifest kinds, and 1825 flags; missing symbols: 15, 153, 10, 3, and 6 respectively.
