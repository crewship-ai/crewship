...
|---|---:|---|---|
| API operations | 557 | 557 exact, 0 resource-level, 0 missing | 420 with a test signal |
| CLI commands | 759 | 759 exact mentions, 759 root pages present, 0 root pages missing | 744 with a test signal |

Response schema quality: 542 API operations have a concrete 2xx schema; 0 still use the generic object fallback.

Request schema quality: 235 operations have request bodies; 231 have concrete JSON schemas, 4 use non-JSON media types, and 0 still use a generic JSON fallback.

CLI flag quality: 359 commands define flags; 359 document all of their flags and 0 still have undocumented flag(s).

Environment variables: 101 discovered, 0 missing documentation. Manifest kinds: 20 discovered, 0 missing documentation.

Docs → code references: 4158 commands, 2076 API paths, 421 environment variables, 209 manifest kinds, and 1927 flags; missing symbols: 0, 1, 0, 0, and 0 respectively.
