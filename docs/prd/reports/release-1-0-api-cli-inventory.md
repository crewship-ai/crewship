...
|---|---:|---|---|
| API operations | 538 | 538 exact, 0 resource-level, 0 missing | 406 with a test signal |
| CLI commands | 732 | 732 exact mentions, 732 root pages present, 0 root pages missing | 718 with a test signal |

Response schema quality: 524 API operations have a concrete 2xx schema; 0 still use the generic object fallback.

Request schema quality: 225 operations have request bodies; 221 have concrete JSON schemas, 4 use non-JSON media types, and 0 still use a generic JSON fallback.
...
Docs → code references: 3931 commands, 1988 API paths, 401 environment variables, 209 manifest kinds, and 1817 flags; missing symbols: 0, 0, 0, 0, and 0 respectively.
