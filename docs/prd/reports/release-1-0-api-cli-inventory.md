...
|---|---:|---|---|
| API operations | 549 | 549 exact, 0 resource-level, 0 missing | 416 with a test signal |
| CLI commands | 747 | 747 exact mentions, 747 root pages present, 0 root pages missing | 732 with a test signal |

Response schema quality: 535 API operations have a concrete 2xx schema; 0 still use the generic object fallback.

Request schema quality: 231 operations have request bodies; 227 have concrete JSON schemas, 4 use non-JSON media types, and 0 still use a generic JSON fallback.

CLI flag quality: 352 commands define flags; 352 document all of their flags and 0 still have undocumented flag(s).

Environment variables: 100 discovered, 0 missing documentation. Manifest kinds: 20 discovered, 0 missing documentation.

Docs → code references: 4046 commands, 2034 API paths, 401 environment variables, 209 manifest kinds, and 1878 flags; missing symbols: 0, 0, 0, 0, and 0 respectively.
