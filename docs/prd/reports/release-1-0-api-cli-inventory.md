...
|---|---:|---|---|
| API operations | 552 | 552 exact, 0 resource-level, 0 missing | 416 with a test signal |
| CLI commands | 751 | 751 exact mentions, 751 root pages present, 0 root pages missing | 736 with a test signal |

Response schema quality: 538 API operations have a concrete 2xx schema; 0 still use the generic object fallback.

Request schema quality: 232 operations have request bodies; 228 have concrete JSON schemas, 4 use non-JSON media types, and 0 still use a generic JSON fallback.

CLI flag quality: 354 commands define flags; 354 document all of their flags and 0 still have undocumented flag(s).

Environment variables: 100 discovered, 0 missing documentation. Manifest kinds: 20 discovered, 0 missing documentation.

Docs → code references: 4073 commands, 2046 API paths, 402 environment variables, 209 manifest kinds, and 1892 flags; missing symbols: 0, 0, 0, 0, and 0 respectively.
