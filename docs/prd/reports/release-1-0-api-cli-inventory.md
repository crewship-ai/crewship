...
|---|---:|---|---|
| API operations | 548 | 548 exact, 0 resource-level, 0 missing | 415 with a test signal |
| CLI commands | 746 | 746 exact mentions, 746 root pages present, 0 root pages missing | 731 with a test signal |

Response schema quality: 534 API operations have a concrete 2xx schema; 0 still use the generic object fallback.
...
Docs → code references: 4032 commands, 2032 API paths, 401 environment variables, 209 manifest kinds, and 1872 flags; missing symbols: 0, 0, 0, 0, and 0 respectively.
