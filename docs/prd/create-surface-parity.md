...
- **Workspace-wide (crewless) agent** (closed 2026-09-05) — `requiresCrew` is `draft.agentRole === "LEAD"` rather than a literal `true`, which is the rule `agents_create.go:137` has always enforced: only a LEAD is rejected without a `crew_id`. The Crew field stays a picker for both roles — an Agent gets a "Workspace-wide (no crew)" row that also clears a crew preselected by the canvas you opened the dialog from, and the empty-workspace banner tells an Agent author their agent will be workspace-wide instead of sending them to go create a crew. Nothing new had to be built to hold the result: `/crews` already groups crewless agents under **Unassigned** (`explorer-groups.ts:58`). The “Coordinator” half of this row was already gone — #2166 removed that copy; what was left was the behaviour it had been describing.
