...
Final verification: desktop and 390×844 touch-browser checks passed. Initial `go test ./... -count=1`: 144 packages passed; API/database hit their 10-minute package timeouts. Both tests active at timeout passed separately on ordinary disk (API curated fallback 2.084s; WAL bound 143.400s). Full API/database package retries passed with isolated temporary databases on tmpfs (195.964s / 490.742s). This is not a claim of one clean full-suite run or a disk-performance benchmark. Private operational logs and machine-readable evidence live under `/srv/crewship/zai-acceptance/sidebar-*`.
