...
**B3 · One active turn per session.** *(merged #2342; live-checked on dev1: two mentions two seconds apart → one run, then one follow-up run carrying the second, both answered. §16.1: no new table; the partial unique index on `assignments(session_id)` is remediation-first.)* The partial unique index — **and the insert-path rewrite it depends on**: `cappedAssignment` and `insertCappedAssignment` (`internal/api/delegation_limits.go:509-577`) gain `session_id`, with session resolve-or-create inside the same transaction as the fan-out guard (§3.1). Follow-ups become queued deliveries consumed at the next step boundary via the existing steering queue.
