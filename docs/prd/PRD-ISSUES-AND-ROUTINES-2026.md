...
**B5 · Checkpoints and the context pack.** *(Also owns F3 as left open by B3: delivering a queued follow-up to the live run at its next step boundary through the steering queue, instead of only at run end. If B5 cannot take it, it becomes its own package B3b — it must not vanish between the lines.)* `agent_session_checkpoints`; §11.1 assembly; the sidecar comment-read verb; compaction path recorded per run; lookout scanning for replayed content (F40).
