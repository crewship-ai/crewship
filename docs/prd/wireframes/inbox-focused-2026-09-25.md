...
| Search and crew selection serve different tasks. | `filterInboxEntries` searches title/summary/subject/category plus resolved agent and crew names/slugs; the old placeholder mentions all of these. | Keep Inbox search broad, with the shorter “Search inbox” label; use the separate crew picker when the person wants to browse a known crew. |
...
4. Move complete source-type filtering under Advanced and align it with `internal/inbox.AllKinds` plus approval/mission signals. Keep the current roster-aware search. Test empty, loading, degraded, long-title, keyboard and phone states.
