...
Channel membership refinement: Members now supports creator-managed add/remove for public channels as well as private groups. Public access remains workspace-wide. The roster contains stored members, not the whole workspace. A new is_member flag separates roster membership from read cursors/mute preferences; passive reads, sending and muting do not enroll users. Existing stored participant rows are retained by migration (historically these can include readers); other workspace users are not automatically added. Owners cannot be removed and private-group access revocation is unchanged. The open Work/Artifacts rail now uses the same card base and accent overlay as the panel content.

Review follow-up: new-chat picker requests are consumed on first opening so remounting the sidebar does not reopen the picker. The editor can be locked during a save without rebuilding its document; cancel/back controls are disabled while saving. File downloads in bearer-auth desktop mode go through apiFetch and are not limited by the preview cap. Binary artifact previews reuse loaded bytes. Follow checks directory metadata every five seconds and reads bytes on metadata changes, with a periodic one-minute byte check for preserved timestamps or missing metadata. Pause/resume performs an immediate fresh read. Focused regressions cover metadata polling, byte reuse, pending saves and authenticated large downloads.

Latest validation: full Go suite passed using a temporary in-memory filesystem for test data (the earlier disk-bound timeout reports above are historical), go vet passed, and 708 Chat/conversation tests passed. TypeScript and production build checks accompany the PR; browser smoke verified channel add/remove persistence and matching Work/rail surfaces. No main merge is authorized before valid review and required CI.
