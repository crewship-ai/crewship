...
reports the *working* link and blesses the dead `#crewship-routine-result-run-id`
written on another page. On top of that, API-reference anchors are generated by
Mintlify rather than written as headings, so heading extraction alone does not
describe the target's anchor namespace. Path-level enforcement lands green and
is worth having today; anchor verification is a follow-up that needs the
slugger settled first, with the false-negative direction proven, not assumed.
