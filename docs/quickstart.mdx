...
<Steps>
  ...
  <Step title="Check system health">
    ...
    crewship doctor
    ```
    Verifies Docker is running and all runtime dependencies are met. The `crewship-sidecar` binary and `entrypoint.sh` ship **alongside** `crewship` in the release archive (and the Homebrew formula), and `crewship start` autodetects them from the binary's own directory — so no separate build step is required.
  </Step>
