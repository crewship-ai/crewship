...
- See your crews and agents on **Crews** — an explorer of crews on the left, the selected crew's canvas (Overview / Team / Work / Memory) on the right
- Watch what agents are doing, live, in the [Crew Journal](/guides/crew-journal) and the bottom dock on the Crews page
- Manage credentials and skills
- Create and track work as **Issues**
- Chat with agents directly, and message the people in your workspace

The **Dashboard** (`/`) is one screen, top to bottom: a **Needs your attention** strip (what is waiting on you, each row with its verb; "All clear" when nothing is), **Results & review** beside **Routines running now**, **Up next** (the next scheduled runs) and **Your crews** (one row per crew: state, agents, runs); then an **Agent run summary** for the chosen window (24h / 7d / 30d — completed, success rate, p95 duration, spend) with **Run volume by crew**; a **Pages** strip; and a collapsed **System details** foot for capacity, memory and services. **New issue** and **Chat** sit in its header.
