  ...
  <Step title="Create the first admin user">
    ...
    <Tabs>
      <Tab title="Browser">
        ...
        **Deploy-race safety.** The server arms a short startup window (default 5 minutes) during which the bootstrap endpoint accepts requests; afterwards it refuses with 410 until restart. An unbootstrapped instance reachable on the public internet doesn't sit indefinitely open for the first scanner. If you miss the window, restart `crewship start` to arm a new one.
      </Tab>
    ...
    <Note>
      **The 5-minute deploy-race window applies to both paths.** `crewship init` POSTs to the same `/api/v1/bootstrap` endpoint that the browser form does, so it's bound by the same window — after the deadline, both paths return `410 Bootstrap window expired — restart the server to open a new one` until you restart. For unattended provisioning where the bootstrap call might land minutes-to-hours after the server boots, restart the server immediately before the CLI call so the window is fresh.
    </Note>
  </Step>
