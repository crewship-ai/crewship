---
title: "Architecture"
description: "How Crewship runs AI coding agents — one Linux container per crew, sidecar proxy, Unix socket IPC, and a single-binary deployment model that runs entirely on your hardware."
icon: "building"
...
2. **Embedding:** `scripts/embed-web-out.sh sync` stages `out/` into `web/out/`, which is embedded via `//go:embed all:out`. `web/out/` also holds one tracked file, `.placeholder.html`, so the embed resolves in a fresh clone or worktree; a binary built over it serves 503 on every UI route instead of a blank page.
...
**Stage 1** — Static prompt (`internal/api/agent_config.go`):
```
...
7. [CREW CONTEXT] (Lead) OR [PEER CONTEXT]
...
| Mode | Behavior |
|------|----------|
| **restricted** (default for new crews) | Only allow connections to allowlisted domains (the default allowed domains below + any the crew adds) |
| **free** | Allow all outbound connections (opt in per crew) |

New crews default to **restricted** with an empty allowlist (fail-safe defaults) — the default allowed domains below still make LLM/CLI providers reachable, so agents work out of the box while arbitrary egress is denied. This default applies to **every** creation path (dashboard wizard, API/CLI, recipes, crew-template deploy, onboarding, internal) via a single shared default, so no path silently creates a `free` crew. A denied request emits a `network.egress` journal entry (visible in Crow's Nest) naming the policy, so the operator can add the host to `allowed_domains`. Crews created before this change keep their existing `free` mode (grandfathered).

**Externally-triggered runs are always confined:** a webhook-initiated run executes under `restricted` egress regardless of the crew's own mode (the payload comes from outside the trust boundary), while the crew's `allowed_domains` still apply.

Default allowed domains (from `internal/egressallow/allowlist.go`, re-exported by `internal/sidecar` — each entry exists for a specific CLI):
...
### Sidecar Endpoints (45 path patterns, 58 method+path arms)

The sidecar exposes a local API on port 9119 (`internal/sidecar/server.go`). The
two numbers differ because several patterns are prefixes carrying more than one
verb — `/issue/{identifier}` alone answers GET, PATCH, comment, link and
attachments. Both are derived from the `buildHandler` switch; the authoritative
list is that switch, and `TestSidecarRoutes_IdentityCoverage` fails the build
when a route registered there is not classified by whose identity guards it.

<Accordion title="Full endpoint catalog (45 path patterns)">
| Category | Endpoints |
|----------|-----------|
| **Memory** | `/memory/read`, `/memory/write`, `/memory/search`, `/memory/status`, `/memory/reindex` |
| **In-container MCP servers** | `/mcp/memory`, `/mcp/memory/{slug}`, `/mcp/routines`, `/mcp/notify` |
| **MCP gateway** | `/mcp/tools`, `/mcp/call`, `/mcp/status` |
| **Assignment** | `/assign`, `/results/{id}`, `/query`, `/standup`, `/escalate`, `/report-confidence` |
| **Missions** | `/mission/create`, `/mission/templates`, `/mission/{id}` (GET status, POST `/start`) |
| **Issues** | `/issue/create`, `/issues` (GET), `/issue/{identifier}` (GET/PATCH, plus POST `/comment`, GET `/comments`, `/link`, `/work`, `/attachments`, `/attachments/{id}`) |
| **Pipelines** | `/pipelines` (GET), `/pipelines/save`, `/pipelines/{slug}` (GET, POST `/run`, POST `/dry_run`) |
| **Pages** | `/pages/{page}/{panel}` (PUT — the producer door, see [Pages](/guides/pages)) |
| **Connections** | `/connections`, `/connections/{id}` (`/message`, `/messages`, `/files`), `/crew-connections` |
| **Keeper** | `/keeper/request`, `/keeper/execute` |
| **Credentials** | `/credentials` (GET), `/credentials/create`, `/credentials/{name}/rotate` |
| **Authoring** | `/skills/generate`, `/skills/author`, `/routines/schedules/create` |
| **Management** | `/crews`, `/crew/create`, `/agent/create`, `/spawn` |
| **Other** | `/manifest` (GET/PATCH), `/expose-port` |
</Accordion>

`POST /agent-credentials` and `POST /crew-connections` were listed here until
#1768 and never worked: both forwarded to an internal endpoint that is
registered nowhere, so the agent got a 502 every time. They were removed rather
than repaired, because a route that *looks* like it grants credential
assignment invites someone to add the backend half without the gate.

## Keeper Request Lifecycle

Keeper is **off by default** (`KEEPER_ENABLED=false`), and when it is on it withholds only `type: SECRET` credentials from boot delivery (`internal/credpolicy`) — every other type still reaches the container as a file or env var. A withheld credential is reachable through exactly two endpoints on the sidecar, and neither ever returns the value:

| Endpoint | What the agent gets back | When to use |
|----------|--------------------------|-------------|
| `POST /keeper/request` | A verdict — `ALLOW` / `DENY` / `ESCALATE` with a reason and risk score. **Never the credential value.** | To learn whether a longer piece of work will be allowed before starting it; the judge's answer is not memoized, so it does not pre-authorise a later `execute` |
| `POST /keeper/execute` | Stdout / stderr from a command, scrubbed | The only way an agent *uses* a Keeper-gated credential — the value is injected into that one command's environment inside the crew container; the response contains scrubbed output |

### Decision flow

The sidecar forwards both endpoints over `/tmp/crewship.sock` to crewshipd, which calls into `internal/keeper/gatekeeper`:

1. **L1 fast-path** — for `request` only (never `execute`), an L1 credential plus an intent ≥ 10 non-whitespace characters AND ≥ 5 distinct non-whitespace characters, not injection-shaped, is auto-allowed without an LLM round-trip — unless the workspace has an active watch policy, which sends every tier to the judge. Single-character, whitespace-only, or trivial-filler intents like `"aaaaaaaaaa"` are rejected.
2. **Deterministic refusals** — an intent shorter than the tier's minimum (10 / 15 / 25 / 35 characters for L1–L4) is denied before any model is called, on both routes.
3. **LLM evaluation** — every other request is scored by `Gatekeeper.Evaluate`, which prompts the judge — a local Ollama model by default, or the workspace's governance model — with the credential's tier block, the operator's watch policy, computed facts, the agent's recent conversation, and the request. The model returns a structured `{ decision, reason, risk }` JSON object, after which the tier floor is applied: in the default profile an `ALLOW` at L4 becomes an `ESCALATE` (`--escalate-from 5` removes that step).
4. **Fail-closed default** — if no LLM provider is configured, the Gatekeeper returns `DENY`. The same applies if the LLM response cannot be parsed as JSON or does not arrive inside the judge's time budget.
5. **Execute branch only** — on `ALLOW`, crewshipd runs the command via `docker exec` inside the crew container with the credential injected as an environment variable. The exec runs as the container's resolved non-root run-as user and fails closed if that user is empty, undeterminable, or root. Stdout and stderr pass through the [scrubber](#output-processing) before being returned. Execution uses the agent's own non-root identity; scrubbing the reply does not create a separate UID boundary around the injected environment. An `ESCALATE` returns without executing; a person's later ruling records the verdict and does not run the command.
...
sequenceDiagram
    ...
    alt L1 request, no command, intent ≥ 10 non-whitespace chars AND ≥ 5 distinct non-whitespace chars, no watch policy
...
- **Agent leaking a credential into chat or logs.** `/keeper/execute` has crewshipd run the command inside the crew container with the value injected for that one process, scrubs stdout/stderr, and returns the redacted result through the API — for `SECRET` credentials, which are the ones Keeper withholds. Other types are in the container from boot, and the general output scrubber is what stands between them and the chat.
...
- **Credential dump from the agent process.** Credentials are held in crewshipd's process memory; UID 1001 can talk to UID 1002 over loopback but cannot read the sidecar's process memory or files. Filesystem credentials at `/secrets/{slug}/` live on the crew's writable tmpfs, owned by UID 1001 with restrictive modes (`0400` files, `0700` directories) — that keeps them from any non-1001 process, but not from sibling agents in the same crew, which share UID 1001 (see below).
...
- **Trivial fast-path bypasses.** L1 auto-allow is gated to `request` only — an attacker cannot smuggle a command through the L1 path because `execute` always pays the LLM round-trip. Empty, whitespace-only, filler and injection-shaped intents are rejected before they reach the LLM.
...
- **A malicious or misconfigured Keeper LLM.** If the model used by the Gatekeeper has been swapped for an adversarial one, `ALLOW` decisions cannot be trusted — the tier floor still turns an L4 `ALLOW` into an escalation in the default profile, but L1–L3 are the model's call. Pin model digests, audit the model out-of-band, and treat the Keeper as a defense-in-depth layer rather than a primary control.
- **Side channels in scrubbed output.** The scrubber redacts the credential's literal value and a fixed set of encodings of it (base64 in four alphabets, base32, URL-escaped, hex in both cases, reversed) — not every transform. A sufficiently creative agent can still leak partial information through timing, exit codes, chunking, or format-string tricks. The scrubber raises the cost of the common one-liners; the crew's egress policy is the containment.
- **A hostile sibling agent inside the same crew.** All agents in a crew container run as one principal (UID 1001), so there is no kernel-level isolation between them: each can read and write every sibling's `/secrets/{slug}/` files, home directory, and output, and signal every sibling's processes. The enforced isolation boundary is the **crew** (one container, tmpfs, home volume, and sidecar per crew), not the individual agent. Agents that must not read each other's credentials belong in separate crews — see [Credentials > The trust boundary is the crew](/guides/credentials#the-trust-boundary-is-the-crew-not-the-agent).
...
| `CODEX_CLI` | `codex exec --json` | Folded into user message (no flag) | The Rust port has no `--quiet`/`--system-prompt`; non-interactive runs go through `exec` |
| `GEMINI_CLI` | `gemini -p <message> --output-format stream-json` | Folded into user message (no flag) | `--system-instruction` is not documented in headless mode, so the system prompt is prepended to the message |
...
- [Harbormaster](/guides/harbormaster) queues approvals; transitions emit `approval.*`.
...
- **Migration 55** added `journal_entries_fts` (FTS5 mirror of `summary` + `payload`), `journal_entries_archived` (compaction sink with truncated payloads), `memory_relations` (embedding relation graph), and `memory_health_snapshots` (daily 5-metric scoring) — see [Episodic memory](/guides/episodic-memory).
...
The `/journal` UI exposes this via a tab strip — `Timeline | Runs | Spend` — backed by the same SSE stream and the same FTS5 index (`?q=` does a server-side `MATCH` against `summary` + `payload`). Card enrichment (palette colours and lucide icons for crew/agent/mission chips) comes from a workspace-scoped lookup map at `GET /api/v1/journal/lookup`, fetched once and invalidated by realtime events. See [Crew Journal](/guides/crew-journal).
