...
### Sidecar Endpoints (40 paths, 44 routes)

The sidecar exposes a local API on port 9119 (`internal/sidecar/server.go`). The path count below is 40; the 44-route total reflects four paths that register multiple HTTP verbs (`/manifest` GET+PATCH, `/crew-connections` GET+POST, `/connections/{id}/files` GET+POST, `/issue/{identifier}` GET+PATCH):

<Accordion title="Full endpoint catalog (40 paths)">
| Category | Endpoints |
|----------|-----------|
...
| **Issues** | `/issue/create`, `/issues` (GET), `/issue/{identifier}` (GET/PATCH), `/issue/{identifier}/comment`, `/issue/{identifier}/link`, `/issue/{identifier}/attachments` (GET/POST), `/issue/{identifier}/attachments/{id}` (GET) |
