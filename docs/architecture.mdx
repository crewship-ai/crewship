...
| `POST /keeper/execute` | Stdout / stderr from a command, scrubbed | The only way an agent *uses* a Keeper-gated credential — the value is injected into that one command's environment inside the crew container and never crosses back into UID 1001 |
...
5. **Execute branch only** — on `ALLOW`, crewshipd runs the command via `docker exec` inside the crew container with the credential injected as an environment variable. The exec runs as the container's resolved non-root run-as user and fails closed if that user is empty, undeterminable, or root. Stdout and stderr pass through the [scrubber](#output-processing) before being returned. The credential value never crosses back into UID 1001. An `ESCALATE` returns without executing; a person's later ruling records the verdict and does not run the command.
