...
New crews default to **restricted** with an empty allowlist (fail-safe defaults) — the default allowed domains below still make LLM/CLI providers reachable, so agents work out of the box while arbitrary egress is denied. A denied request emits a `network.egress` journal entry (visible in Crow's Nest) naming the policy, so the operator can add the host to `allowed_domains`. Crews created before this change keep their existing `free` mode (grandfathered).
