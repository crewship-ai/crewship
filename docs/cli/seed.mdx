...
| --- | --- | --- |
| `--nuke` | `false` | Delete all workspace contents before seeding. Runs *after* auth, so a missing/invalid token won't wipe the wrong workspace. |
...
Wipes the active workspace's contents first, then seeds. The auth step runs before `--nuke` so a typo in `--server` or a stale token won't trash a different workspace.
