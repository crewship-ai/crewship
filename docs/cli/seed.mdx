...
Replaces the old `prisma/seed.ts` script. Creates a release-operations demo environment — admin user, workspace, connected crews, agents with system prompts, credentials, integrations, focused routines, Pages, and executable issues — entirely through the REST API so business logic (validation, encryption, audit logging) is exercised end-to-end. The default content uses real read-only public sources (`crewship.ai`, `docs.crewship.ai`, the Crewship GitHub release feed, and GitHub Status) and keeps synthetic model-regression routines out of the normal product demo.
