...
<Note>
  **`diff` compares agent runs** — `msg_…` (legacy `r_…`) IDs from
  [`crewship history`](/cli/history). The `run_…` / `prn_…` IDs printed by
  `crewship routine runs <slug>` are routine (pipeline) runs and live in a
  different namespace; there is no routine-run equivalent of `diff`, so
  inspect them individually with `crewship routine report <run-id>`.
  Passing a `run_…` ID on either side fails fast (exit code `3`) instead of
