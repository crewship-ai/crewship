...
a folder, the folder's slug and its sharing label (`none`, `crew`,
`workspace`) are printed too: the label, never the names. Anyone who can open
...
sees a label (`none`, `crew`, `workspace`) in `folder list` and `folder show`,
