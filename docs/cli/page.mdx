...
webhook id, the version seq — so a listing can be piped into the command that
consumes it. `page export` is the exception in the other direction: it prints
YAML unless you ask for `--format json`.
...
<Note>
  ...
  ```bash
  curl -s -X PUT http://localhost:9119/pages/fleet-201/sluzby \
    -H "Content-Type: application/json" \
    -d '{"items":[{"name":"api","state":"ok"}]}' \
    -K /dev/fd/3 3<<AUTH
