...
`publish` and `rollback` also send the two values the reviewer attested to.
...
routine definition behind each `call` action. Omit them and the CLI reads them
from `project review` itself — `rollback` asks it for the retained version it
is restoring — and prints, on stderr, every digest it is fencing
on before it sends the request — nobody has to copy hex by hand, and nobody
attests to a value they were not shown. Pass either flag and that half is sent
verbatim, unread. If the live definition or a routine script moved since the
review, the server refuses with 409 and names which one; re-read `review` and
publish again.
