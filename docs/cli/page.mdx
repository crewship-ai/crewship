...
*unfiled*. Folders arrange and do not share: filing a page in one changes
nobody's access to it, and a folder shows you only the pages in it you already
reach.
...
explicitly. The page's owner or a workspace admin may; the folder is not
consulted.
...
| Flag | Description |
|---|---|
| `--folder <slug>` | File the page in this folder, moving it out of any other. |
| `--unfiled` | Take the page out of whichever folder it is in. The folder is read from the page, so you name none. |

Exactly one of the two. Moving changes nobody's access to the page.
...
as an admin or a MANAGER (or higher) of its owning crew. Lacking either, the
refusal names who can. Unfiling is the page owner's or an admin's alone —
taking your page back never waits for anyone.

**The fences.** The command reads the page's `pages_version` and the folder's
`grants_version` right before it writes and sends exactly what it read. A
**409** therefore means somebody else moved the page, or changed the folder,
between that read and the write; the error prints the current pair — look
again and re-run. There is deliberately no flag to type a fence by hand: a
