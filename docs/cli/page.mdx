...
`owner`, `role`, `crew:<slug>`, `panel_crew:<slug>`, `grant`, comma-joined
when there is more than one path. A page whose only path is `grant` is one
somebody shared with you. `--format quiet` prints one slug per line, which is
...
| `4` | You are not the panel's declared producer and hold no `produce` grant — answered only when you can reach the page; otherwise it is exit 3 — or you are not authenticated | A producer 403 is journalled and notifies the page owner |
...
# Review sources without requesting the complete panel declaration:
...
`withheld_change`. `--reviewed-code` attests that the whole change was
...
live definition too. Publishing the Page is still yours to do; this particular
...
`--acknowledge-unavailable-baseline` is separate from the digests and is never
filled in for you. When the live publication's retained source cannot be read
back — history compacted, a checkpoint lost — there is nothing to compare the
candidate against, and the server refuses the publication with 409 until
somebody says so explicitly. The CLI prints what is missing and what
acknowledging it would mean, and then stops; passing the flag records that
nobody made that comparison. It proves nothing about the candidate, which is
verified in full either way, and the publication's `checks` keep the answer as
`baseline_source`.
