...
| `--crew-icon <name>` | template's, or none | Icon for the crew card and row — a name from the web icon set (`shield`, `rocket`, `code`, `chart`, `users`, …). Sent as `crew_icon`. Checked against the server's vocabulary; an unknown name is **dropped, not refused**, so the proposal is still created with the default look. |
| `--crew-color <color>` | template's, or none | Colour for the crew card: a palette id (`blue`, `emerald`, `violet`, `amber`, `rose`, `cyan`, `lime`, `fuchsia`) or a 6-digit hex (`1a2b3c` or `#1a2b3c`, stored with the `#`). Sent as `crew_color`; anything else is dropped the same way. |
...
Prints the proposal id, its status (`PENDING`) and the resolved roster, plus a
`Crew Icon` / `Crew Color` row for each the proposal carries. Add `--format json`
for the full payload, where they are `payload.crew_icon` / `payload.crew_color`.
