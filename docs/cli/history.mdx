...
| `--issue <identifier-or-id>` | string | (unset) | Only runs that worked on this issue, e.g. `ENG-4`. Sent as `mission_id`; the server resolves the identifier. Rows show the issue they worked on in the ISSUE column. |
