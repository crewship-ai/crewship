...
| `admin` | full set, including `credential.create`, `credential.rotate` and `credentials:reveal` (everything but `page.create`). An OWNER/ADMIN with no explicit set already holds this. |
