...
<Note>
**Auth:** `list`, `get`, and `create` require `crewship login` (`create` clears the workspace context server-side — it makes a new one). `update`, all `member` operations, and `invite` additionally require an active workspace. `use` only writes the local config and validates the slug against the server when you're already logged in. Role gates apply server-side — `OWNER`/`ADMIN` to update or invite, `OWNER` only to `delete`, `MEMBER+` to read.
</Note>
