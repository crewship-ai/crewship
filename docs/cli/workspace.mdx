...
| `workspace member add <user-id>` | Add an existing user to the workspace. Takes a **user** id. |
| `workspace member role <member-id-or-user-id> <ROLE>` | Change a member's role (`MANAGER+`, ladder-guarded). |
| `workspace member remove <member-id-or-user-id>` | Remove a member (prompts unless `-y`). |
...
`Language`, `Allow privileged credentials`, `Pages theme`, `ID`, and
`Created`; machine formats use the corresponding API fields `id`, `name`,
`slug`, `created_at`, `preferred_language`, `allow_privileged_credentials`
and `pages_theme`. `Pages theme` lists the shared Page colours as
`key=#RRGGBB` pairs, or `(SDK defaults)` when none is set — see
[`workspace update --pages-theme`](#workspace-update).
...
| `--credential-audit-retention-days` | Days of `credential_audit` history to keep. `0` means keep forever. Leave unset to use the 90-day default. |
| `--audit-log-retention-days` | Days of `audit_logs` history to keep. `0` means keep forever, which is the default — `audit_logs` is the compliance trail and its retention obligation is yours to declare. |
| `--approvals-retention-days` | Days of decided `approvals_queue` history (approved/denied/timeout/cancelled) to keep. `0` means keep forever. Leave unset to use the 90-day default. A pending approval is never swept regardless of age. Rejected above `106751` days (~292 years) — past that the day count overflows the retention sweep's `int64`-nanosecond cutoff arithmetic. |
| `--pages-theme <json\|@file>` | The shared colours every custom [Page application](/guides/pages#shared-company-colors) reads through the `--crewship-page-*` design tokens: a JSON object whose keys are `accent`, `background`, `surface`, `text`, `muted` and `border` and whose values are `#RRGGBB` colours. Inline (`'{"accent":"#0f766e"}'`) or `@theme.json`. The object **replaces** the stored one — omitted keys fall back to the SDK defaults — and `{}` resets it. Any other key, any other colour form, or more than 1 KiB is refused by the server. This is the same setting as **Settings → General → Pages appearance**; `OWNER`/`ADMIN` only, and audited. |

<Warning>
Only flags you actually pass are sent, and that matters here: `0` is a
meaningful value on all three retention flags (an explicit "keep forever"),
so an unset flag must not be transmitted as zero. `crewship workspace update
--name x` never touches your retention windows.
</Warning>
...
| Column | What it is | Consumed by |
| --- | --- | --- |
| `MEMBER ID` | The `workspace_members` row | `PATCH` / `DELETE /api/v1/workspaces/{id}/members/{memberId}` |
| `USER ID` | The person | `POST /api/v1/workspaces/{id}/members` |

So `member add` takes a **user** id while the endpoints behind `member role`
...
```
no workspace member matches "nobody": it is neither a MEMBER ID nor a USER ID
in 'crewship workspace member list'
```

Resolution is a convenience, not a new dependency: if the member roster cannot
be read (permissions, an unreachable server), the argument is sent through
untouched and the server answers as it always did.
...
```bash
crewship workspace member role <member-id-or-user-id> <ROLE>
```
...
```bash
crewship workspace member remove <member-id-or-user-id> --yes
```
...
Removal also drops every **crew** membership the person holds in this
workspace, and first transfers any page they own to a crew — a page that
resolves to no crew refuses the whole removal. Adding them back later
restores neither: crew membership and any per-crew role override have to
be granted again. See
[Remove member](/api-reference/workspaces#remove-member).
...
| `--create-only` | `false` | Require a new account. If the email already exists, return `409` before changing membership, role or setup tokens. |

Use `--create-only` for isolated demo provisioning when an existing person must
remain untouched. The default provisioning behavior is unchanged when omitted;
the flag does not reset an existing account’s password.
...
# Keep two years of audit_logs; keep credential reads forever.
crewship workspace update --audit-log-retention-days 730 --credential-audit-retention-days 0
```
...
crewship workspace member remove usr_abc123 --yes   # user id or MEMBER ID both work
...
Valid capability strings: `chat`, `routine.create`, `routine.run`, `skill.create`, `credential.create`, `credential.rotate`, `credentials:reveal`, `issue.create`, `page.create`, `memory.write`. Note the colon in `credentials:reveal` — it is the one capability spelled the way the security document names it.
...
| `power` | chat + `routine.create` + `routine.run` + `issue.create` + `memory.write` (trusted team members). |
| `admin` | full set, including `credential.create`, `credential.rotate` and `credentials:reveal` (everything but `page.create`). An OWNER/ADMIN with no explicit set holds the admin preset minus `routine.run` (the role clears that gate on its own). |
...
- **`no fields to update`** — `update` was called without any of `--name` / `--slug` / `--language` / `--allow-privileged-credentials` / `--credential-audit-retention-days` / `--audit-log-retention-days` / `--approvals-retention-days` / `--pages-theme`.
- **`--pages-theme must be a JSON object …`** — the value (or the `@file`) did not parse as a JSON object of strings. The colour vocabulary itself is checked by the server: `pages_theme supports accent, background, surface, text, muted and border as #RRGGBB colors` is its answer to an unknown key or a named colour.
