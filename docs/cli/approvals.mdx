...
description: "List and decide Harbormaster human-in-the-loop approval requests."
icon: "anchor"
stability: stable
tag: Stable
---
...
<Note>
  Every subcommand requires `OWNER` or `ADMIN` role server-side (403
  otherwise) — including `list` and `get`, as of #2233. Before then the
  read endpoints had no role floor and any workspace member could read the
  full queue, including request payloads; the API reference's
  [Approvals](/api-reference/approvals) page has the endpoint-level detail.
</Note>
...
crewship approvals list --status all --all      # the whole queue, every page
crewship approvals list --status all            # first page of history only
```
...
| `--offset` | `int` | `0` | Skip this many rows (server-side offset pagination). |
| `--all` | `bool` | `false` | Keep requesting pages until the server reports `has_more: false`. Exact only on a quiet queue: a row decided mid-walk shifts the window and is never returned. |
...
Text mode prints canonical fields (`id`, `status`, `kind`, `reason`, `crew_id`, `agent_id`, `mission_id`, `requested_by`, `decided_by`, `decided_at`, `decision_comment`, `timeout_at`, `routine_version` when set, `created_at`) up top, then any extra keys verbatim so an evolving server payload still renders something useful. The names are the row's own (see the [API reference](/api-reference/approvals#list-approvals)): the decision comment is `decision_comment`, not `comment`, and there is no `updated_at`.
