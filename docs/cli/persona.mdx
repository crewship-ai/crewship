...
mirrors the [agent persona API](/api-reference/agents#agent-persona) and the [crew persona API](/api-reference/crews#persona).
...
Same verbs against the crew-default layer at [`/api/v1/crews/{crewId}/persona`](/api-reference/crews#persona). There is **no `suggest`** equivalent — agents propose only into their own layer, and the operator authors crew defaults directly.
...
- [Crew Persona API](/api-reference/crews#persona) — the default layer this CLI's `crew` sub-tree edits.
