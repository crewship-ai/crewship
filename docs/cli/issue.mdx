...
<Note>Early: `issue_agent_sessions` is work package B1 of the Issues & Routines architecture (`docs/prd/PRD-ISSUES-AND-ROUTINES-2026.md`). A mention resolves or creates a session — mentioning the same agent on the same issue twice reuses it rather than creating a second — but nothing yet reads a session back into an agent's context, and every session created today sits in `pending`. That is later work (B2–B5).</Note>
