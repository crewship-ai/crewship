...
**Output columns:** RUN (the journal run id — what `crewship journal --run-id` and the Activity page take; `—` when the assignment never reached a run), AGENT (slug), TASK (truncated to 28), STATUS, STARTED (relative time), DURATION, SOURCE, RESULT (error message or result summary; ANSI-sanitised, truncated to 50 chars).
