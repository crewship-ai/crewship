...
`/-/merge_requests/` → GitLab), so self-hosted instances of either work with no
configuration. Crewship then fetches the request through that provider's API
using a stored `GITHUB` / `GITLAB` credential in the workspace.
