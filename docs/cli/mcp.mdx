...
| `--set-file <path>` | (unset) | Replace from a file. Mutually exclusive with `--set`. |
| `--resolved` | `false` | Print the full effective MCP surface: the merged blob config (crew + agent; agent keys win) plus the integration-table bindings (workspace/crew servers + agent bindings). Cannot be combined with `--set` / `--set-file`. |
...
`--resolved` prints two labeled sections. The first, `# Blob config
(crews/agents.mcp_config_json):`, merges by `mcpServers.<name>` — agent keys win over
crew keys with the same name, additive otherwise. The second, `# Integration bindings
(workspace/crew servers + agent bindings):`, folds in the integration-table cascade,
one line per server, e.g.:

```
filesystem [crew, stdio] stdio — no credential
github [agent, http] https://api.github.com/mcp — credential: gh-token
```

An agent with neither source prints
