...
| `--allow-private-endpoints` | `bool` | `false` | Allow agents to reach a private/LAN model endpoint (RFC1918/loopback). Link-local and cloud-metadata addresses stay blocked. Subject to the instance ceiling `CREWSHIP_ALLOW_PRIVATE_ENDPOINTS`. |
| `--issue-prefix` | `string` | | Prefix for this crew's issue identifiers — `ENG` gives `ENG-42`. Pass an empty string to clear it, after which identifiers use the first three letters of the crew slug upper-cased. See [Issue prefixes](#issue-prefixes). |
