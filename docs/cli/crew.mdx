...
Check provisioning status for a crew. Polls the provision job; exits non-zero if the latest build failed.
...
**Output columns:** TAG, SIZE, CREATED, REFERENCED_BY (number of crews), AGE.
...
- [`crewship features`](/cli/system#crewship-features) — browse the devcontainer feature catalog before editing `devcontainer.json`.
- [`crewship runtimes`](/cli/system#crewship-runtimes) — browse the mise runtime catalog before editing the mise config.
