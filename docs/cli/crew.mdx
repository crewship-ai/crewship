...
| `--memory-mb` | `int` | `0` | Container memory limit in MB. 6–262144; `0` uses the server default. |
| `--cpus` | `float64` | `0` | Container CPU limit. 0.01–512; `0` uses the server default. |
...
`--memory-mb` and `--cpus` are range-checked by the server and a value
outside the range comes back as a `400` naming it. The lower bounds are
Docker's own, so a crew sized below them is refused here rather than
accepted and then rejected by the daemon at wake time — which wedges
every run of that crew until the value is corrected. Note the `--cpus`
ceiling cannot be fully validated server-side, because the daemon's real
upper bound is the host's core count.
...
| `--icon` | `string` | | Lucide icon name (e.g. `code`, `rocket`, `terminal`). |
| `--memory-mb` | `int` | | Container memory limit in MB. 6–262144; `0` resets to the server default. |
| `--cpus` | `float64` | | Container CPU limit. 0.01–512; `0` resets to the server default. |
