...
crewship mission list --search Harborlight --limit 20 --offset 20
```

| Flag | Type | Default | Description |
|------|------|---------|-------------|
| `--crew` | `string` | | Filter by crew slug or ID. |
| `--search` | `string` | | Search missions by title or identifier, on the server (sent as `q`). |
| `--limit` | `int` | `20` | Rows per page (the server caps it at 200). |
| `--offset` | `int` | `0` | Skip this many rows. A human table ends with a footer such as `showing 21–40 of N · next page: --offset 40` (that one for `--offset 20`); JSON and quiet output carry no footer. |
...
crewship mission update <id> --status DONE
```
...
| `--description` | `string` | | Mission description. |
| `--status` | `string` | | Status: `PLANNING`, `IN_PROGRESS`, `DONE`, `FAILED`. `COMPLETED` is still accepted and silently normalized to `DONE` (a retired word kept only for backward compatibility — see [API reference → Missions → Update Mission](/api-reference/missions#update-mission)). |
...
crewship mission start <id> --wait
crewship mission start <id> --wait --wait-timeout 1h
```

The DAG engine evaluates task dependencies and dispatches ready tasks to agents.

If the mission was created under an autonomy hold (see [Autonomy gate on agent-driven creation](/api-reference/internal#autonomy-gate-on-agent-driven-creation)) and its `autonomy_gate` approval is still pending, denied, cancelled, or timed out, `start` answers `403` regardless of who is calling — an OWNER/ADMIN must decide the hold with `crewship approvals approve <id>` first.

| Flag | Default | Effect |
| --- | --- | --- |
| `--wait` | `false` | Block and poll `GET /api/v1/crews/{crewId}/missions/{missionId}` (the same route `mission get` uses) every 2s until the mission reaches `DONE` or `FAILED`, instead of returning as soon as the start request is acknowledged. Exits non-zero on `FAILED`. |
| `--wait-timeout` | `30m` | Max time to block with `--wait` (`0` = forever). |
