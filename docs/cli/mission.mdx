...
| Flag | Default | Effect |
| --- | --- | --- |
| `--wait` | `false` | Block and poll `GET /api/v1/crews/{crewId}/missions/{missionId}` (the same route `mission get` uses) every 2s until the mission reaches `COMPLETED` or `FAILED`, instead of returning as soon as the start request is acknowledged. Exits non-zero on `FAILED`. |
