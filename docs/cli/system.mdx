...
| `crewshipd` | Probe the crewshipd sidecar daemon's own health (uptime, connections) |
...
Probe the crewshipd sidecar daemon's own health — uptime and connection count. Backs `GET /api/v1/crewshipd`, a thin proxy to the sidecar's `/health` over its Unix socket.
...
<Note>
This is a **different process** from the one `system health` reports on. `system health` is the crewship API server's own health (uptime, log level, disk headroom); `system crewshipd` is the sidecar daemon that runs crew containers, reached over IPC. An unreachable sidecar answers `{"status":"unreachable"}` with **HTTP 200**, not a request failure — check the `status` field rather than the exit code.
</Note>
