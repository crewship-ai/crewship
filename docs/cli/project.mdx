...
Auth: every subcommand requires `crewship login` plus an active workspace. The PATCH/DELETE surface mirrors `internal/api/project_handler.go` — fields you don't set with `Changed()` aren't sent, so partial updates won't clobber unrelated columns.
...
The delete unlinks every issue and mission (sets `project_id = NULL` in the same transaction) so the destroy doesn't cascade into issue loss.
...
- [Projects API](/api-reference/projects/list) — `GET /api/v1/projects`.
