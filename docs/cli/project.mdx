...
| `project milestone list <id-or-slug>` | List milestones for a project. |
| `project milestone create <id-or-slug> --name <n>` | Create a milestone. |
...
The three verbs written `<id-or-slug>` accept either. `/api/v1/projects/{projectId}` keys on the **id** alone, so the CLI resolves a slug against `project list` before it builds the URL — `update`, `delete` and `stats`, written `<id>`, take the id only.
