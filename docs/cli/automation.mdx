...
| `--event` | `string` | Journal entry type to watch, e.g. `mission.status_change`. Exactly one. Required. Checked against the journal's closed registry at save time — an unregistered value is refused, naming real alternatives. |
...
| `--payload-equals` | `stringSlice` | Only match when a journal payload field equals this, as `key=value`. Repeatable. Checked at save time against a curated (not exhaustive) list of known keys per event type — see below. |
| `--debounce-seconds` | `int` | Hold the enqueued run open this long for further events to coalesce into. Default 10. |
| `--max-per-hour` | `int` | Burst brake: cap runs this automation may cause per hour. Default 60. Counted per server process and cleared by a restart — see [Automations → Burst control](/guides/automations#burst-control). |

A predicate flag you omit is "don't care"; omit all of them and every entry of
the event type matches.

<Warning>
  Check the payload before you match on it, even now that `--payload-equals`
  is checked at save time. The check only covers a curated set of event
  types (`mission.status_change`, `mission.created`, `mission.assigned`,
  `automation.throttled`, `automation.depth_exceeded`,
  `pipeline.schedule.circuit_breaker_tripped`,
  `pipeline.schedule.missed_occurrences`, `page.wake.fired` — see
  [Automations → Payload key validation](/guides/automations#payload-key-validation));
  `mission.comment` is deliberately not in that set (its two emitters write
  disjoint payloads), and for it or any other event type a key no emitter
  writes is still accepted, saved,
  listed — and matches nothing, forever, with no error anywhere:

  ```bash
  crewship journal --type mission.status_change --lines 1 --format json
  ```

  `mission.status_change` carries `action` (a closed set — `status_changed`,
  `review_approved`, `task_failed`, …), `details` (prose, e.g.
  `"BACKLOG → TODO"`), and `from`/`to` (only on `action=status_changed`).
  Match on `action` for anything but a specific transition; read `details`
  inside the routine via `{{ event.payload.details }}` rather than matching
  on it — it's a sentence, not a field. `--payload-equals to=DONE` IS a valid
  predicate for "moved to DONE". See
...
Standard [CLI exit codes](/cli/overview#exit-codes-&-machine-readable-errors). A routine slug that does
