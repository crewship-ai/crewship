...
- **`bootstrap request failed: HTTP 410: Already initialized — …`** — the database already has a user. `init` is a one-shot. Use `crewship login` and (if you've lost UI access) `crewship admin reset-password` instead.
