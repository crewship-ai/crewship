...
| `--save` | | `string` | | Tee the agent's text response to this path (no ANSI). |
...
Low-friction one-shot prompt against a default agent. Inherits the same `--with-*`, `--save`, and `--markdown` flags as `run`. The agent is resolved from `--agent` flag → `default-agent` config key → interactive picker (TTY) → error.
