...
| `--profile` | | `string` | | Server profile to target (manage with `crewship server`). Also via `CREWSHIP_PROFILE` env var. |
| `--format` | `-f` | `string` | `table` | Output format: `table`, `json`, `yaml`, `ndjson`, or `quiet`. |
...
| `2` | Validation failure (HTTP 400 / 422, or a missing precondition such as no workspace set) |
| `3` | Not found (HTTP 404, or a client-side lookup miss — an agent/crew/workspace slug or resource name that doesn't resolve) |
...
All list and detail commands support `--format` (`-f`), and so does every
mutation that hands you back something the next command needs (see
[Mutations that return their result](#mutations-that-return-their-result)):
...
| `yaml` | YAML output. |
| `ndjson` | Newline-delimited JSON (one record per line), for streaming pipelines. |
...
The guarantee is uniform: commands that emit structured output honor all three machine
formats (`json`, `yaml`, `ndjson`) equivalently on stdout, and under a machine format
errors go to stderr as a structured envelope in that same format (see
[Exit codes & machine-readable errors](#exit-codes-machine-readable-errors) above).
Keys are identical across formats: a field named `created_at` under `-f json` is
`created_at` under `-f yaml` too, so `yq .created_at` on the YAML output and
`jq .created_at` on the JSON output read the same value. A
few commands have a different *default* than `table`:

- `journal` and its subcommands default to `text`.
- Structured-record commands with no canonical table — `agent debug` and
  `system onboarding status` — default to `json` (with `yaml`/`ndjson` honoured), and
  `metrics` defaults to `json` (with `yaml` honoured).

### An empty result is `[]`, never `null`

Any command whose answer is a list emits an empty **array** when it finds nothing —
never a bare `null`. This matters more than it looks: Go marshals an unfilled slice as
`null`, so before this was fixed centrally a list command answered `[]` on a populated
workspace and `null` on an empty one, which meant `crewship prompt list -f json | jq '.[]'`
worked while you were developing against seeded data and failed with
`Cannot iterate over null` on a fresh install.

```bash
crewship prompt list -f json        # []   — not null, even with no prompts saved
crewship routine active -f json     # []   — not "No active runs."
```

An empty *list* is `[]`; an absent *object* is still `null`, because `{}` and `null`
mean different things for a record and nothing iterates an object the way `.[]`
iterates an array.

### Where stdout is not a document

`--format` describes how a **result** is rendered. Four kinds of command put something
else on stdout, and for them the flag has nothing to convert:

| Kind | Commands | What stdout is |
|------|----------|----------------|
| Downloads | `crew files get`, `agent avatar show`, `memory show`, `memory versions show` | The file's raw bytes, streamed. Use `--out` to write to disk. |
| Manifests | `export crew`, `export page`, `export workspace` | A YAML manifest you check into git and feed back to `crewship apply`. Always YAML, by design. |
| Generated documents | `routine schema`, `completion <shell>` | The schema / shell script itself. |
| Live streams | `ask`, `explain`, `repl`, `logs --follow` | A token or event stream with no end to close. `logs --follow` emits NDJSON under a machine format, which is the streaming-native shape. |

The one documented exception on the *error* side is `crewship wait`: it owns its
run-outcome exit codes and prints `[wait] error:` diagnostics to stderr as plain text
even under a machine `--format` (no structured error envelope). See the
[Wait page](/cli/wait).

### Success receipts belong on stderr

A command that *changes* something prints its confirmation (`✓ Integration enabled`) to
**stderr**, so stdout stays clean for whatever the command returns. The answer to "did
it work" is the exit code. Some mutation commands still print their receipt on stdout;
that is a known cleanup, and `TestMutationReceiptsOnStdoutDoNotGrow` in
`cmd/crewship` keeps the population from growing.

### Mutations that return their result

A mutation whose result the caller has to *read* — the identifier the server just
minted, or the new state of the thing that changed — renders that result on stdout
under `-f json` / `yaml` / `ndjson`, in the same shape the matching read command emits,
so one parser handles both. Under the default `table` the human receipt on stderr is
unchanged. The population is closed and guarded
(`TestMutatingCommandsReturnTheirResultMachineReadably` in `cmd/crewship`):

| Commands | What `-f json` prints |
|---|---|
| Every `create` subcommand in the tree (`agent create`, `crew create`, `issue create`, `page create`, `project create`, `workspace create`, …) | The created object, as the corresponding `get` / `list` row shows it (`issue create` emits the [`issue get`](/cli/issue) object). |
| `token create` | `{"id", "name", "token"}` — the bearer itself, once. The sensitivity warning goes to stderr; `--output-file` and `--quiet` take precedence and keep the token off stdout. |
| `issue update`, `issue comment`, `issue start`, `issue stop`, `issue review` | The updated issue (or the comment), same shape as `issue get`. |
| `issue delete` | `{"identifier", "id", "deleted": true}` — synthesised, because the server answers `204` with no body. |

Mutations outside this table are not on the guarded contract. Most are a bare receipt
whose real answer is the exit code and print nothing on stdout under `-f json`; a few
(`admin gdpr delete`, `backup self-test`, `backup rotate`, …) already render a
receipt object. Do not script against the machine output of a
mutation that is not in the table — check the command's own page first.

## Resolving resource references

Anywhere a command takes an agent, crew, skill, or credential reference, you can pass
either the record id or the human identifier — the slug for agents, crews, and skills
(display names don't resolve), and the name for credentials and integrations (which
have no slug) — including slugs that happen to
look like ids. CUID-shaped input is first verified with a single lookup; on a miss the
CLI falls back to slug/name resolution, so a slug like `customersuccessemea42` resolves
...
## Trace and automation workflows

The orchestration surfaces answer four related operational questions:

- **What caused this?** `crewship chain <anchor>` follows the causal graph around
  an issue, run, routine, assignment, or journal entry. Use `--format json` when a
  script needs the graph. See the [`chain` reference](/cli/chain).
- **Why did this run start?** `crewship routine records <slug>` exposes run
  provenance such as `chain_depth`, `chain_origin`, and the automation that fired
  the run. See the [routine reference](/cli/routine#crewship-routine-records-%3Cslug%3E).
- **What will start future work?** `crewship automation list|create|update|enable|disable|delete`
  manages workspace rules that enqueue routines from journal events. Automations
  enqueue deferred work; they do not execute routines inline. See the
  [`automation` reference](/cli/automation) and [automations guide](/guides/automations).
- **Does the crew match its declared runtime?**
  `crewship crew container-status <slug-or-id>`
  compares configured and effective container limits and explains
  whether an operator needs to act. Use `--format json` for scripts. See the
  [crew reference](/cli/crew#crewship-crew-container-status).

Start with `chain` when the origin is unknown, inspect `routine records` to identify
the trigger, then use the journal or run detail command for the complete evidence.
...
| `crewship notify` | Manage desktop notifications (`enable` / `disable` / `status` / `test` / `send`) | — |
| `crewship routine` | Manage workspace routines (declarative DSL workflows; alias: `pipeline`) | See the [routine CLI reference](/cli/routine) and [Routines guide](/guides/routines) |
| `crewship plan` | Ask the default agent for a plan; tool use is not technically restricted | — |
...
| `crewship signal` | Wake every routine run parked on a named event (`signal send --event-type ...`) | See the [signal CLI](/cli/signal) |
