...
| **(clean) / (uncommitted changes)** | From the build driver's own `-X …/buildinfo.buildDirty` stamp when present, otherwise the embedded `vcs.modified`. **Printed only when it is known** — a build that neither source answered for cannot tell, and says nothing rather than claiming "clean". |
...
<Warning>
  ...
  **parent clone**: a commit that was never built, and a dirty flag from a
  ...
  `make build` and `./dev.sh` ask git directly (`scripts/build-stamp.sh`) and
