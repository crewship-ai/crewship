...
`crewship version` itself does **not** hit the network — it only prints
the compiled-in metadata above. The update-availability banner is
printed by [`crewship doctor`](/cli/doctor) and logged at boot by
