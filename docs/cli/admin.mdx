...
reads the server's `/healthz` `legacy_resources` field and **WARN**s with this
command as the remediation when orphaned resources are present.

The prune removes **only** the legacy names; the id-scoped resources the live
runtime uses are never touched. It scopes to the crew slugs in your workspace
and reports each resource removed:

```text
Pruned 4 legacy resource(s):
