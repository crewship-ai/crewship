...
| `--email <email>` | string | (unset) | Skips the interactive `Email:` prompt for the default email+password flow. Combine with `--password-stdin` or `CREWSHIP_PASSWORD` for a fully non-interactive login. |
| `--password-stdin` | bool | `false` | Reads the password as a line from stdin instead of prompting on the TTY (preferred for CI / scripts — avoids argv leak). Mutually exclusive with `CREWSHIP_PASSWORD`. |
...
The default email+password flow prompts for the password via `term.ReadPassword`,
...
```bash
# stdin (docker-login style) — preferred:
crewship login --server https://crewship.example.com \
  --email demo@crewship.ai --password-stdin <<< "$PASSWORD"

# env var — no stdin involved at all:
CREWSHIP_PASSWORD="$PASSWORD" \
...
| Flag | Type | Default | Effect |
|---|---|---|---|
| `--json` | bool | `false` | Deprecated alias for `--format json`; emit machine-readable JSON to stdout. |
