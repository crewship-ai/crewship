...
| Mission | `get <run-id>` (alias `show`) | Show one run's full result — verdict/diff summary, seed signature, token/cost totals. |
...
Aliased as `crewship eval show <run-id>`.

Sample output (replay):

```
Run er_a1b2c3d4e5f60718: COMPLETED (replay)
Mission: MIS-42
Seed: 42
Signature: 7c1b3a9f...
Tokens: 184251  Cost: $0.8421
Created: 2026-04-17T10:00:00Z  Completed: 2026-04-17T10:02:41Z
...
```
Run er_b2c3d4e5f6071829: COMPLETED (regression)
Baseline: MIS-41  Candidate: MIS-42
Regressed: true
Tokens: 201044  Cost: $0.9133
Created: 2026-04-17T10:15:00Z

Result:
  regressed: tool success dropped 8.0% (> 5% threshold); cost rose 22.0% (> 15% threshold)
```

`--format json`/`yaml`/`ndjson` return the full `quartermaster.RunRecord` — the same shape `eval runs` rows now include (see [Eval API](/api-reference/eval)).

<Note>
  A run id that doesn't exist, or belongs to another workspace, 404s with
  the same "eval run not found" message either way — existence isn't
  leaked cross-tenant.
</Note>
...
All `eval` subcommands (`runs`, `scenarios`, `compare`, `baseline list|show|diff`) also
honour `--format yaml` and `--format ndjson` alongside `json`; the YAML form of
`eval scenarios` carries the same `generated` metadata as the JSON form.
...
| `AMBIGUOUS` | At least one side had a transport error or remains WAITING, RUNNING, or QUEUED — a final verdict is not available. |
...
Both sides now pin the same archived recipe version, resolved once before either
run. `--version N` selects an explicit archive; otherwise the server's `is_head`
marker selects the published version, including after rollback. Missing archive
evidence stops the comparison before any run. Publication between A and B does
not change the selected version.

This is **live execution**, with real effects and costs. JSON/YAML/NDJSON reports
include `execution_mode`, `pipeline_version`, `definition_hash`, and
`agreement_basis: execution_status`. Agreement is about execution status, not
proof of semantic quality or identical model behavior. WAITING/RUNNING/QUEUED
results are `AMBIGUOUS`; finish the underlying run before drawing conclusions.
...
## Comparing from the routine editor

The Test view also offers a live comparison over published recipe versions.
Load the archive list, select a version and worker tier for each side, and
provide a JSON dataset of 1–20 `{id, inputs, expected_output?}` cases. Import
and export preserve typed values. Draft edits are not used until published.

Each accepted run uses the regular execution engine and appears in History.
The comparison shows cost, duration, run links and an exact text check when
`expected_output` is supplied; otherwise it explicitly leaves quality ungraded.
Results export includes the pinned versions/hashes and input snapshots.

If a run waits for a human, resolve it in the run detail or Inbox, then Continue.
Continue reads the accepted run instead of starting it again. After an unclear
start response it retains the same request key within the server's idempotency
window. Leaving the Test view stops future client-side starts, while accepted
runs continue independently. The comparison queue itself is not a durable job.
