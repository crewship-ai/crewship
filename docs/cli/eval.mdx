...
Mission-level subcommands enqueue work asynchronously and return a `run_id` (poll completion via `crewship eval runs`). Routine-level subcommands run synchronously and emit results inline.
...
The routine-level commands target the **eval suite** seeded under the `eval-` slug prefix (see [Routines guide §18](/guides/routines#eval-framework) for the framework concept and [cookbook recipe 6](/guides/routines-cookbook) for the eval-driven promotion workflow).
