...
The web surfaces tell their views apart by a `surface` key **inside** `filters_json`, not by `--view-type` (that column is constrained to `board` or `list`). A view for the [Crew Journal](/guides/crew-journal#saved-views) wraps the journal's URL parameters:
...
A view without `surface` belongs to the issue tracker, whose filter keys are the flat shape in the first example.
