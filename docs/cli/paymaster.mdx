...
Paymaster metering **only captures calls that flow through `internal/llm`** (Keeper, summaries, consolidation, quartermaster). Calls made by the agent's own CLI adapter (`claude`, `gemini`) talking directly to the provider API **are not recorded in `cost_ledger`**. See [Paymaster guide](/guides/paymaster) for details.
