...
in [Telemetry — Default by build flavour](/guides/telemetry#default-by-build-flavour).
...
Telemetry: ENABLED
  ...
  database:   /Users/you/.crewship/crewship.db (the default data directory)
```

Consent is a row in a database, so the answer is only as meaningful as the file
it came from. The `database` line names that file and how it was resolved —
`DATABASE_URL` → `CREWSHIP_DATA_DIR` → `~/.crewship`, the same order the server
uses. It prints on every branch, including "not yet configured", where *which*
file is missing is the useful half of the answer.

<Note>
Until [#2109](https://github.com/crewship-ai/crewship/pull/2109) this command
ignored `DATABASE_URL` and always read `~/.crewship/crewship.db`, so on a host
whose `crewshipd` runs with its own database — every development clone uses
`file:./crewship.db` — it reported a consent row the running server had never
seen.
</Note>
...
Or when no explicit choice has been recorded yet (never-booted DB, or a
stable build that hasn't been opted in). The message reflects the
build's default — prerelease/dev builds settle to ENABLED on the next
`crewship start`, stable builds stay DISABLED until you opt in:

```
$ crewship telemetry status
Telemetry: not yet configured. This prerelease/dev build defaults to ENABLED on the next `crewship start` — opt out now with `crewship telemetry off`.
```
