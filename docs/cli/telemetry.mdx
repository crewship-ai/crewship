...
- `vendor default (compiled in)` — DSN baked into the binary at
  ...
  maintainer's Sentry (`your-sentry-org/crewship-backend`).
- `CREWSHIP_SENTRY_DSN env override` — operator set the env var to
  redirect events to their own Sentry instance. See
  [Telemetry — Routing override](/guides/telemetry#routing-override).
...
[Telemetry — What gets sent](/guides/telemetry#what-gets-sent) and the
