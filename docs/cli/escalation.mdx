...
If the workspace has [segregation of duties](/guides/credentials#segregation-of-duties-second-approver)
enabled (`crewship keeper second-approver enable`), resolving a `CREDENTIAL`
escalation is refused with **403** when the caller is recorded as the owner of
the agent that raised it — including a workspace OWNER, which is not exempt.
