...
Note that only **chat-initiated** runs currently stream (chat UI, `crewship run`,
`crewship ask`). Routine, webhook and pipeline runs publish nothing on the session
channel yet — see [issue #1823](https://github.com/crewship-ai/crewship/issues/1823).
