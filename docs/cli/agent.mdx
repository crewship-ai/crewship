...
| `credential-readiness` | Report whether a delivered credential authenticates the agent's model |
...
| `--q` | `string` | | Server-side search on name, slug and role title (case-insensitive substring); the total narrows with it. |
| `--include-setup` | `bool` | `false` | Also list the onboarding Guide — agents of the `kind=setup` crew, which the server hides from every roster by default (`?include_setup=1`). |

When the table is a window on a longer list, the human formats end with
`showing 1–100 of 308 · next page: --offset 100`; machine formats print the
rows only.
...
Provider account name, id, owner, plan and refresh details are shown only when
permitted. Otherwise `Pays with` shows the provider and `(restricted)`; this
does not mean the agent has no paying account.

**Output fields:** Name, Slug, ID, Role, Role Title (only when set), Crew, Status, CLI Adapter, Tool Profile, Memory, Timeout, Created, Skills count, Credentials count, Pays with (the provider login paying for the agent's adapter — name, id, plan, owner and refresh state, or a dash when nothing in its delivery pays for it; see [Provider logins](/guides/credentials#provider-logins)). When a cron schedule is configured, additional rows appear: Schedule (`<cron> (enabled|disabled)`), Schedule Prompt, Next Run, and Last Run. When `--webhook-require-timestamp` is set on the agent, a `Webhook Auth: timestamped signature required` row appears.
...
The command reports `stopped` only after the runtime has **confirmed** the
stop. When the sidecar cannot be reached, or answers without confirming that
this agent is stopped, the server answers `502` (`runtime stop unavailable` /
`runtime stop not confirmed`), the agent's status is left as it was, and the
command fails with that error and a non-zero exit — it never prints a stop
that did not happen. Check `crewship agent get <slug-or-id>` and retry once
the crew's runtime is reachable.
...
This listing answers "what reaches the agent?", not "can the agent call its model?". One crew-scoped `GH_TOKEN` binding makes it non-empty for every agent in the crew, including one with no model credential at all. For that question use `credential-readiness`.

---

## `crewship agent credential-readiness`

Report whether a credential the runtime will actually deliver authenticates this agent's model provider.

The verdict is computed by the runtime's own delivery rules, not by looking at the credential list: a Claude Code agent is served by the sidecar proxy (its env holds a dummy key by design), a Codex or Gemini login lands in a file under `HOME`, an OpenCode or Cursor key is written to the env. `ready` means the run's own selector finds the credential; `missing` means it will not, and the first run fails at the first model call; `unknown` means the server has no opinion (an adapter it does not recognise, an OpenCode agent naming no provider, a local endpoint) and says nothing rather than guess.

```bash
crewship agent credential-readiness <slug-or-id>
crewship agent credential-readiness <slug-or-id> --format json
```

```
Agent casey (CLAUDE_CODE): model credential ready.

PROVIDER    CREDENTIAL                SOURCE        DELIVERY
ANTHROPIC   CLAUDE_CODE_OAUTH_TOKEN   agent_grant   login_env
```

```
Agent parity-probe (CLAUDE_CODE): NO model credential reaches this agent for ANTHROPIC.
Its first run will fail at the first model call.

no credential in this agent's delivery authenticates ANTHROPIC for CLAUDE_CODE

Grant one, then re-check:
  crewship credential list
  crewship credential assign <credential> parity-probe
```

`SOURCE` uses the vocabulary of `crewship credential resolve` (`agent_grant`, `agent_binding`, `crew_binding`, `workspace_binding`, `crew_link`). `DELIVERY` is the channel: `sidecar` (the proxy injects it, the env holds a dummy), `login_env`, `login_file`, `env`. Below the `manage` role the credential's name is withheld, as it is from `agent credentials`; the verdict is not.

This command only reports. It assigns nothing.
...
crewship agent files viktor --recursive                  # the whole tree, not one level
crewship agent files viktor --subdir reports/2026-09     # one subdirectory of the namespace
...
| `--recursive` | bool | `false` | List the whole tree under the agent's namespace instead of one level. |
| `--subdir <path>` | string | | List this subdirectory of the agent's namespace instead of its root. |
