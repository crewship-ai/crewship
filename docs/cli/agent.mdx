...
| `--role` | `string` | `AGENT` | Agent role: `AGENT`, `LEAD`, or `COORDINATOR` *(deprecated 2026-04-16 — see [Coordinator guide](/guides/coordinator))*. |
| `--role-title` | `string` | | Human-readable role title. |
| `--cli-adapter` | `string` | `CLAUDE_CODE` | CLI adapter: `CLAUDE_CODE`, `CODEX_CLI`, `GEMINI_CLI`, `OPENCODE`. |
| `--system-prompt` | `string` | | System prompt text or `@file.txt` to read from file. |
| `--tool-profile` | `string` | `CODING` | Tool profile: `MINIMAL`, `CODING`, `MESSAGING`, `FULL`. |
