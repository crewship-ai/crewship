...
Inspect and repair a routine's **durable cross-run state** — the key/value bucket behind `{{ routine.state.<key> }}` and a step's `state_write` binding. See [Cross-run state](/guides/routines#cross-run-state-routine-state-key) for how a routine reads and writes it.
