...
| `--tiers` | comma-separated tier overrides. Empty = use authored complexity. |
...
| Flag | Description |
|------|-------------|
| `--tier-a` | tier override for side A (default `fast`) |
| `--tier-b` | tier override for side B (default `smart`) |
| `--inputs` | JSON inputs forwarded to both sides |
