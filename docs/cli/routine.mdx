...
| Runs | `result <run_id>` | Print a run's final deliverable — output **and the files it produced** |
...
**Files produced:** section listing the artefacts the run wrote (e.g.
`report.pdf`, `summary.md`) with a fetch hint. For a document-processing
routine the files *are* the deliverable, so this is the one view that
surfaces them.
...
Files are associated with the run by matching each file's modification
time against the run's `[started_at, ended_at]` window under the run's
crew (agent `/output` + `/crew/shared`) — no executor changes, so it works
for any past run. Two runs on the same crew with overlapping windows can
cross-attribute a file. The `--client` view also lists produced files
after the redacted deliverable. Backed by
[`GET …/pipeline-runs/{runId}/files`](/api-reference/workspaces#run-produced-files).
