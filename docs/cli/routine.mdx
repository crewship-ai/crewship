...
| `--fail-closed` | | Wake gate fails **closed**: a probe error **holds** the run instead of firing it (default is fail-open). Requires `--wake-slug`. Use for gates that must hold, not monitors. |
