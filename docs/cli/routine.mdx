...
process completed cleanly but never reported a recognised outcome shows `FAILED` here
(`error_message` names it `"no outcome reported"`), and `NEEDS_HUMAN` is the one value
