...
```bash
crewship connector verify slack --field bot_token=xoxb-… --field team_id=T123 && echo ok
```

Field names come from the connector manifest. Slack currently requires
...
```bash
crewship connector install slack --field bot_token=xoxb-… --field team_id=T123 --crew backend
```
