...
Command aliases: `notifications`, `notif`.
...
Delete a single notification. Prompts for confirmation unless `--yes`
is passed; a non-interactive session refuses to proceed without it.

```bash
crewship notification delete <id>
crewship notification delete <id> --yes   # skip the prompt
crewship notification rm <id>             # alias
```
