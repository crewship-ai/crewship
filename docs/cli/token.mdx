...
| Flag | Type | Default | Effect |
|---|---|---|---|
| `--output-file <path>` | string | (unset) | Write the bare token to this file (mode `0600`) instead of stdout. Stdout gets only a confirmation line. |
| `--quiet` | bool | `false` | Print **only** the bare token to stdout (for `$(...)` capture); the "sensitive" advisory rides stderr. |
...
Store this token securely — it won't be shown again. (Use --output-file to write it straight to a 0600 file, or --quiet for capture.)
```
...
| `--output-file <path>` | string | (unset) | Write the bare new token to this file (mode `0600`) instead of stdout. |
| `--quiet` | bool | `false` | Print only the bare new token to stdout (for `$(...)` capture); advisory goes to stderr. |

```bash
crewship token rotate tok_abc12345
crewship token rotate tok_abc12345 --name "ci-runner v2"
crewship token rotate tok_abc12345 --output-file ~/.crewship-ci-token
```
