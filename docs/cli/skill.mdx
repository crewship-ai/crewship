...
| `--model` | (catalog default) | LLM model override. Unset, the server drafts with the Anthropic default from the model catalog (`config/models.json`; `claude-sonnet-5` at the time of writing). |
...
```bash
crewship skill proposed list --crew engineering          # slug
crewship skill proposed list --crew <crew-id>            # or id
crewship skill proposed list --crew engineering --format json
```

| Flag | Required | Description |
|---|---|---|
| `--crew` | yes | Crew slug or id whose `.proposed/` dir to scan. A slug is resolved against `crewship crew list` before the request; the resolved id is URL-encoded so any future id-scheme change with special chars stays safe. |
