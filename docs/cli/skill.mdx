...
| `--description` | _(empty)_ | One-line trigger; should start with `Use when...`, `Useful for...`, or `To <verb>...`. The scaffold leaves the frontmatter field blank if omitted, but the skill won't pass `crewship skill validate` until you fill it in. |
