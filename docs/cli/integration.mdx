...
the Composio account id shown by `integration composio inventory --format json`.
...
Prints a "not configured" notice when neither is present. Add `--format json`
(or `yaml`/`ndjson`) for the raw payload — all read-only `composio` subcommands
(`inventory`, `toolkits`, `tools`, `triggers types`, `triggers active`,
`default show`, `key show`, `bindings`) honour the global `--format` flag.
See the [Integrations guide](/guides/integrations#managed-integrations-composio).

When a command that reaches Composio's live API fails, the `API error (502)`
message classifies the failure — rejected API key (HTTP 401/403), rate limit
(429), Composio outage (5xx), rejected request (other 4xx, with Composio's own
message), or `Composio API unreachable` for network-level failures. See
[Troubleshooting Composio errors](/guides/integrations#troubleshooting-composio-errors).
...
| `integration test <id-or-name>` | Hit `POST /integrations/{id}/test` and print the result. For `streamable-http`/`sse` it probes the endpoint; for `stdio` it statically validates the command config (the live launch happens at runtime inside the container). |
...
<Note>
Pass the whole launch line to `--command` — the CLI splits it into the bare
executable (`npx`) and its arguments (stored in `args_json`) for you. Storing
the full line verbatim would make the runtime look for an executable literally
named `npx @modelcontextprotocol/server-github`, which never exists, and the
server would silently never start. `integration test` catches that same mistake
statically for stdio servers before you wonder why no tools appeared.

The split is quote-aware, not a naive whitespace split: quoted substrings stay
one token, so an executable path that contains spaces must be quoted —
`--command '"/opt/my app/bin/server" --port 3000'` yields the executable
`/opt/my app/bin/server` plus one argument. Single quotes are fully literal
inside; inside double quotes a backslash escapes only `"` and `\`, so Windows
paths like `"C:\Program Files\nodejs\npx.exe"` survive intact. There is no
shell involved — no variable expansion, globbing, or `|`/`&&` operators. See
the [MCP guide](/guides/mcp-multi-cli#stdio-servers-local-subprocess) for the full
tokenizer rules.
</Note>
