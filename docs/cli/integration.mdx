...
| `integration composio bind <agent>` | Bind a Composio `user_id` to an agent (per-user MCP server at runtime). Requires `--user <id>`; optional `--toolkits a,b` to restrict apps. |
| `integration composio unbind <agent>` | Remove an agent's Composio user binding. Requires `--user <id>`. |
| `integration composio bindings <agent>` | Show an agent's Composio user binding(s). |
