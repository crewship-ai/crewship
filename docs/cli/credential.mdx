...
| `--auth-token` | `string` | | `ENDPOINT_URL` only: bearer token sent to the endpoint; stored encrypted. |
| `--header` | `KEY=VALUE` (repeatable) | | `ENDPOINT_URL` only: extra request header. |
...
| `--auth-token` | `string` | | `ENDPOINT_URL` only: new bearer token; omitting it drops the stored token. |
| `--header` | `KEY=VALUE` (repeatable) | | `ENDPOINT_URL` only: extra request header. |
