...
| `--security-level` | `int` | `0` | Keeper security level: `0` (leave at the server default), `1` (low), `2` (medium), `3` (high), `4` (critical). L4 requires a human to approve every read. |
| `--crews` | `stringSlice` | | Crew slugs or IDs to scope this credential to (repeatable or comma-separated). Setting it implies `--scope CREW`; omit for a workspace-wide credential. |
| `--scope` | `string` | `WORKSPACE` | Visibility scope: `WORKSPACE` or `CREW`. Usually inferred from `--crews`; set it explicitly only to override. |
...
| `--security-level` | `int` | `0` | Keeper security level: `1` (low) – `4` (critical). L4 requires a human to approve every read. |
| `--crews` | `stringSlice` | | Replace the crew scoping with these crew slugs or IDs (repeatable or comma-separated). Pass an empty value to clear crews and make the credential workspace-wide. |
| `--scope` | `string` | | Visibility scope: `WORKSPACE` or `CREW`. Usually inferred from `--crews`; set it explicitly only to override. |
