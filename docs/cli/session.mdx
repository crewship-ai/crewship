...
Auth: every subcommand requires `crewship login`. No workspace context required.
...
Neither subcommand defines local flags. Use the global `--format json` / `--format yaml` for scripting.
...
`CURRENT` flags the session that owns the token you're using right now — careful with `revoke` on this row.
...
- [Auth Sessions API](/api-reference/auth/list-sessions) — `GET /api/v1/auth/sessions`.
