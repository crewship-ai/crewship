...
| `--oauth-client-secret` | `string` | Client secret; stored encrypted. Omit for a public (PKCE-only) client. |
...
the server would silently drop them, which is worse than refusing them.
...
|---|---|---|---|
| `--timeout` | `duration` | `2m` | How long to wait for the credential to reach `ACTIVE`. |
...
```
timed out after 2m waiting for credential cmt…c1a to connect; it is still
...
The server's listener tears itself down after 120 s, so a `--timeout` shorter
than that gives up while the window is still open, and one longer than it
reports the window closing rather than racing it.
...
crewship oauth auto-connect https://mcp.example/sse --name example --provider linear
```

| Flag | Type | Description |
|---|---|---|
| `--name` | `string` | Name for the MCP server. Server default: `mcp-server`. |
| `--provider` | `string` | Skip discovery and use a catalogue provider's endpoints. |
