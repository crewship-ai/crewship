...
crewship oauth auto-connect https://mcp.example/sse --name example \
  --oauth-client-id "$CLIENT_ID" --oauth-client-secret-stdin
```

| Flag | Type | Description |
|---|---|---|
| `--name` | `string` | Name for the MCP server. Server default: `mcp-server`. |
| `--oauth-client-id` | `string` | Existing OAuth app client ID when the provider does not support DCR. |
| `--oauth-client-secret` | `string` | Existing app client secret. Prefer the stdin form so it does not reach shell history or the process table. |
| `--oauth-client-secret-stdin` | `boolean` | Read the existing app client secret from stdin. Public PKCE clients can omit both secret flags. |
...
Discovers the endpoints, performs RFC 7591 Dynamic Client Registration when the
provider supports it, creates an `OAUTH2` credential in `PENDING`, and prints
the URL to authorize. Finish with `crewship oauth connect <credential-id>`.
...
`--oauth-client-id` retry to run instead — reporting it as success would leave
you looking for a credential that does not exist. The retry repeats discovery
on the server; it does not ask you to copy the protected-resource identifier or
issuer through the command line. The failure output includes the exact redirect
URI to register on the OAuth app before retrying.
</Warning>
