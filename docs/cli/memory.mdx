...
Imported documents pass through the same per-file size ceilings and secret scanning as an agent's own writes; rejections are listed per document and the command exits non-zero. Full mapping table and limits: [Memory Portability](/guides/memory-portability).
