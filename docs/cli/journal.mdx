...
There are no subcommands -- the top-level command is the list view. Live tail (`--follow`) will require SSE client support and is not yet implemented; use the web UI `/journal` page for live view.
...
| `--follow` | `bool` | `false` | Live tail via SSE (not yet implemented; use the web UI). |
