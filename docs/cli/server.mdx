...
<Note>
Because a directory match (layer 3) outranks the persisted `current` default
...
`crewship server current` surfaces this explicitly when it happens (`directory
override for <dir> — persisted default (\`server use\`) is "<name>"`), and
...
`directory_profiles` in the config) rather than repeating `server use`.
</Note>
