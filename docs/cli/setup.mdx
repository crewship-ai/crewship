...
| `--model <model>` | (catalog default) | LLM model. Defaults to the adapter's default in the model catalog (`config/models.json`) — the same model the web wizard starts on. At the time of writing: `claude-sonnet-5` for Claude Code and OpenCode, `gpt-5.5` for Codex, `gemini-2.5-pro` for Gemini, `composer` for Cursor. See [Model discovery](/guides/model-discovery). |
...
| `--telemetry` | (server default) | Enable anonymous crash reporting; `--telemetry=false` opts out. Omitted, the server's version-based default is kept (prerelease on, stable off); interactive runs always ask. |
...
# Send anonymous crash reports to help improve Crewship? (change anytime: crewship telemetry on|off) [Y/n]: y
# ✓ Workspace ready — crew "software-development" deployed with 4 agents.
```

Each prompt has a sensible default; bare Enter accepts the first option. The
telemetry prompt's default follows the server's current state (prerelease
builds default yes, stable builds default no), and any explicit `--telemetry`
flag skips it entirely.
...
Open it in the browser: https://crewship.example.com/crews
```

The link lands on the crew & agent roster rather than on the new agent directly. Every per-agent surface in the console is addressed by the agent's **slug** — `/chat/<slug>` for the conversation, `/crews?agent=<slug>` for the canvas — and the setup response returns ids only, so there is no slug to interpolate. The `First agent ID` line above identifies which agent on the roster is the new one.
