---
title: "Apply"
description: "Apply a workspace manifest. Idempotent, plan-then-confirm."
icon: "play"
...
| `--skip-governance-gate` | `false` | Forward `skip_governance_gate: true` so risky routines become active instead of waiting for approval. Requires OWNER/ADMIN; use only when the governance decision is intentional and auditable. |
| `--no-delete` | `false` | Refuse the run outright if the plan contains any delete. Outranks `--yes` and fails `--dry-run` too. |
...
Apply is **fail-fast**: the first error aborts the run, and every plan
item behind it is left untouched. That makes the counters a *prefix*, so
a failed run never prints `Applied:` — it prints what it managed, says
it failed, and then names what it did not do:

```
$ crewship apply --file workspace.yaml --yes

Plan:
  + crew uctarna
  ~ crew-file uctarna/shared/scripts/archiv.py
  ~ crew-file uctarna/shared/scripts/verify.py
  + routine msn-etn-podklady

FAILED after 1 created, 0 updated, 0 unchanged, 0 deleted — the manifest was NOT fully applied.

NOT APPLIED:
  x ~ crew-file uctarna/shared/scripts/archiv.py  <- failed here
    ~ crew-file uctarna/shared/scripts/verify.py  (never attempted)
    + routine msn-etn-podklady  (never attempted)
  Apply stops at the first failure and is idempotent — fix the cause and re-run.
```

The `NOT APPLIED` block exists because the counters cannot answer the
...
Sync mode makes deletion the default for anything that fell out of the
manifest, and some deletions are not a rollback away: removing an agent
takes its memory and its Composio OAuth binding with it, and that
binding is a browser consent no manifest can replay.

`--no-delete` turns "this run deletes nothing" from a claim a human
makes by reading the plan carefully into one the machine checks:

```bash
crewship apply --file workspace.yaml --no-delete --yes
```

```
REFUSED (--no-delete): the plan would destroy 1 resource:
  - agent uctarna/sberac
```

Nothing is mutated — the refusal happens after the plan is built and
before the first request. It deliberately outranks `--yes` (the flag
...
<Warning>
`crew provision` builds an **image**. It does not start a container, and
on a cache hit it finishes in seconds having started nothing — the
container is created lazily on the crew's first agent run.

This matters when a manifest ships `crew_files` into `shared/`: those
paths are owned by the container user, so overwriting one on a stopped
crew returns `409 file is owned by the crew runtime`. Start the crew,
then re-apply:
...
`crew start` is idempotent, so it is safe to put in front of every
apply that ships crew files.
</Warning>
...
- [CLI → crew provision](/cli/crew) — chained step for crews with devcontainers
