...
| `user_models` (DB) | row WHERE `user_id = $USER_ID` in this workspace | Hard delete |
| `user_models` (disk file) | `/crew/shared/.memory/users/{user_slug}.md` **and its `.lock` sentinel** in the subject's most-active crew | Best-effort — `outputBasePath` must be configured. Failures logged but do not block the DB delete. |
