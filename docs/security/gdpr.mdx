...
The response confirms what landed. `rows_deleted` is the single total across
every table the cascade touched; the per-table breakdown — including
`approvals_queue` (#2233) — is in `scope`:
