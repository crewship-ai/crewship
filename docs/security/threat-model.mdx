...
prompt ran* without storing it. `chat.user_message` is capped and
scrubbed the same way, and the process log line for a run takes the same
sanitised argv. Payload shape:
