...
**BYO model-endpoint SSRF fence (#961)** — a workspace-configured `ENDPOINT_URL` credential is a tenant-supplied destination the sandbox dials, so it is guarded against reaching internal infrastructure. `internal/httpsafe` splits the blocked ranges into a *hard* tier (link-local incl. cloud metadata `169.254.169.254`, multicast, reserved, unspecified — and their IPv4-mapped-IPv6 forms) and a *private* tier (RFC1918, loopback, ULA). `validateEndpointURL` rejects a hard-tier literal at credential-create; `localModelExtraDomains` gates a literal private host at run-setup; and the sidecar proxy's `ssrfDialContext` does the authoritative **resolve-then-pin** — it re-resolves the host at dial and connects to the exact validated IP, so a name that resolves internal (or DNS-rebinds after the allowlist string-match) is refused. The hard tier is blocked unconditionally; the private tier opens only for a crew that set `allow_private_endpoints` (a legitimate on-prem/LAN Ollama). `free` network mode retains its explicit no-limits semantics. Covered by `TestEndpointTiers`, `TestValidateEndpointURL_SSRF`, `TestLocalModelExtraDomains_SSRF`, `TestSSRFDialContext`.

**Instance-level ceiling (#974-S5)** — the per-crew opt-in is set by a *workspace* ADMIN, so on a shared/multi-tenant instance it is not the security boundary on its own: a tenant admin must not be able to reach the host's private network (other tenants' containers, host services, the Docker daemon subnet) by toggling their own crew flag. The effective decision is therefore `instanceCap AND crewFlag` (`effectiveAllowPrivateEndpoints`, collapsed into the run before either the host-side literal-IP gate or the sidecar dial guard reads it). The instance cap `CREWSHIP_ALLOW_PRIVATE_ENDPOINTS` (`local_models.allow_private_endpoints`) defaults **false** and lives in operator-controlled deploy config — tenant-inaccessible, unlike `instance_settings` (which the audit found is only workspace-role gated). As defense-in-depth the L3 sandbox firewall (`init-firewall.sh`) also filters resolved IPs before seeding its allowset, **unconditionally** dropping both hard (link-local/metadata/loopback/reserved) and private (RFC1918/CGNAT) ranges — so a domain that resolves internal at startup can't open an L3 hole. The L3 layer only ever resolves the hardcoded public allowlist (Debian/GitHub/Docker/Anthropic/Sentry), so a private resolution there is always an anomaly; it does **not** honor the private-endpoint opt-in (a legitimate LAN model endpoint is dialed at the sidecar layer, gated by the effective allow-private value — not at L3). Metadata/link-local stay hard-blocked in every layer regardless of the cap.
