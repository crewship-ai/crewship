...
- **Workspace-bound tokens** (`wsv1.<workspace_id>.<HMAC-SHA256(master,
  ...
  master secret, and rejects (403) any request whose `?workspace_id`
  disagrees with the token's binding — so a token captured inside a
  container only ever authorizes the workspace it was issued for.
  Derivation is stateless; tokens roll with the master on every boot.
- **The master token** (`CREWSHIP_INTERNAL_TOKEN`) never enters a
  container. It remains valid for host-side trusted callers (the chat
  bridge resolver and the LLM proxy cost monitor) that talk to the
  internal API from inside the trust boundary.
...
`POST /internal/pipelines/save`) enforce the same binding in-handler
(`assertInternalTokenWorkspace`), since the auth middleware cannot
inspect bodies. Residual: the binding derives its trust from the
sidecar process boundary (UID 1002 vs. agent UID 1001) — an attacker
who fully compromises the sidecar process itself still acts with that
workspace's authority, which is the intended blast-radius cap, not a
bypass.
...
| Body-workspace internal routes (cost / journal / pipeline save) | ✅ closed — in-handler `assertInternalTokenWorkspace` binding check |
