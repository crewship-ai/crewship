...
The gate exempts `/mcp/memory*` from the *cross-agent* refusal that the legacy
routes carry, because that transport resolves each caller's own tier from its
token rather than the boot agent's. That exemption is keyed on the path prefix,
so it is test-enforced: every route registered under `/mcp/memory` must reach
`memoryAgentContextFor`, and a future route under that prefix which does not
resolve per-agent context fails the build
...
- **A sibling's own valid token.** The five legacy handlers resolve the agent
  tier from the sidecar's boot agent, so *any* authenticated member read and
  overwrote the boot agent's tier. Agent-scoped requests from a non-boot member
  are now refused, pointing at `/mcp/memory/<slug>`, which resolves the acting
  agent properly.

  The refusal is scoped to the **agent** tier. `scope=crew` resolves to one
  directory shared by the whole crew by construction, so there is no per-agent
  crew tier to cross and refusing it would protect nothing while breaking
  sibling access to shared memory. The first cut of this fix did exactly that —
  a functional regression wearing a security fix's clothes, caught in review
  before it shipped. `scope=both` reads the agent tier and is refused with it.
...
cannot enforce anything. Serving each agent its own tier on the legacy routes —
rather than refusing non-boot agents — is the better end state and is tracked
separately; `search`/`status`/`reindex` run through a single indexed engine
bound to one tier, so it is a subsystem change, not a guard change.
