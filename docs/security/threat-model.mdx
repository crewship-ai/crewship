...
- **A sibling's own valid token.** The five legacy handlers resolve their tier
  from the sidecar's boot agent, so *any* authenticated member read and
  overwrote the boot agent's tier. They are now refused there, pointing at
  `/mcp/memory/<slug>`, which resolves the acting agent properly.
