...
- with **no** token the route falls back to the prior behaviour —
  membership-validated `from` for escalate/query, URL slug for the memory
  path — so legacy callers keep working. Because the master never enters a
  container, the token cannot be forged from inside the agent process.
