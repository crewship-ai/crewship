...
skips.
Every argv element is scrubbed *before* the entry is written, against
the built-in patterns **and** the run's own credential values; there is
no after-the-fact redaction for a journal entry.
