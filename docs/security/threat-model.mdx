...
| `POST /api/v1/auth/signup` | Always `202` with one fixed body, no account id, no session cookie, and one bcrypt on both paths. The collision is reported to the address itself (mailer) or to the log with the address hashed. Previously `409 Email already registered`. |
