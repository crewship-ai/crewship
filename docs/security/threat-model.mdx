...
router** (`refuseTokenlessMemory`, `internal/sidecar/memory_guard.go`), before
the route switch chooses a handler. Both `/memory/*` and `/mcp/memory*` are
covered, a memory route added later inherits the check from its registration,
and the refusal is a `403` on every one of them — the MCP transport used to
answer `200` with a JSON-RPC error, which made downgrade attempts read as
successes in access logs.
