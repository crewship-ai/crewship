...
than per-user JWTs. The token authenticates the caller as "the
sidecar" but is NOT bound to a specific workspace in the current
implementation — the `internalWsCtx` middleware reads `?workspace_id`
from the query string and trusts whatever value is passed.

For most internal routes, the IPC layer projects `IPCConfig.workspace_id`
from the orchestrator's sidecar contract before the request leaves the
container, so a captured token can't be aimed at another workspace
even with crafted query parameters. This invariant is the basis of
the claim in `docs/api-reference/internal.mdx`.

### Known exception — Keeper Phase 2 family
...
`assertBodyWorkspaceMatchesCtx` helper in `internal/api/keeper_phase2.go`,
which compares both values and returns 400 on mismatch.

Symmetric forgery — caller picks one workspace consistently across
both query and body — is **not yet closed**. It would require binding
the `X-Internal-Token` to a workspace at issue time and validating
that binding in `internalWsCtx`. Tracked as `PR-F24` in
PRD-AGENT-EVOLUTION-2026.md §6.1. A second path (`PR-F25`) eliminates
`body.workspace_id` from these four handlers and derives workspace
from context only, restoring the original invariant. Either close
retires this "Known exception".
...
| F4 handler body / ctx workspace symmetric forgery | ⏳ PR-F24 (token-to-workspace binding) |
| F4 handler body-workspace drop | ⏳ PR-F25 (eliminate body workspace param) |
