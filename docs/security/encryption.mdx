...
Upgrading to the release that introduced webhook-secret encryption runs a one-time migration (v140) that encrypts any webhook/signing secrets previously stored in plaintext. If no usable `ENCRYPTION_KEY` is configured when that migration runs, it **skips instead of failing** — a migration must never break boot — and logs a `WARN` stating that the backfill is deferred. The migration does not run again later, so after configuring a key, run:
