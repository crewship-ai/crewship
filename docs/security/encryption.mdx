...
<Note>
**Webhook secrets are fail-open at rest.** `agents.webhook_secret` and `pipeline_webhooks.signing_secret` are encrypted only when `ENCRYPTION_KEY` is configured; with no key they are stored plaintext (with a startup/rotate warning) so key-less webhook deployments keep working. During master-key rotation a bare (non-enveloped) webhook value is therefore reported as **skipped**, not failed — so the `failed=0 ⇒ retire the old key` criterion stays honest. Migration `v140` backfills existing plaintext webhook secrets to envelopes when a key is present.
</Note>
