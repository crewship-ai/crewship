...
<Note>
**`escalations.resolution` is not in the walk.** It used to be, for `CREDENTIAL` escalations, when a human's typed answer was encrypted into it. Since the answer to a credential ask is a grant into the vault rather than a value (see [Credentials → What a `CREDENTIAL` escalation never carries](/guides/credentials#what-a-credential-escalation-never-carries)), that column only ever holds plaintext — the historical marker `[credential submitted]`, or the prose an expiry, cancel or auto-resolve leaves. Walking it could only ever produce `failed` rows, which made the `failed=0` criterion unreachable on any database that had answered a credential ask; it was removed from the inventory in `#2408`.
</Note>
