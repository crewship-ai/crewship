...
- **tmpfs /tmp (500M):** Writable temp directory (`rw,noexec,nosuid,size=500m`), not persistent across restarts. `noexec`/`nosuid` block staging and executing dropped binaries; scripts still run via an explicit interpreter (`sh`, `bash`, …), which reads the file rather than exec'ing it.
