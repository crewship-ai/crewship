...
Everything on this page fences the *agent* out of the host. The Crewship
process is on the other side of that fence by necessity: it creates the
containers, so it holds Docker API access, which is root-equivalent.

You can narrow that to the endpoints Crewship actually calls with a socket
proxy — [Docker Socket Proxy](/guides/docker-socket-proxy) publishes the exact
list and ships it wired in `docker/docker-compose.prod.yml`. Read its ceiling
...
- **UI:** **Settings → General → Privileged credentials → "Load credentials
  into privileged crews."** (It is workspace-wide, not per-crew — a crew's own
  Settings tab configures that crew, this switch governs the whole workspace.)
...
- **Shared network, restricted egress:** every crew's container joins one
  Docker bridge network (`crewship-agents` by default) — crews are not
  network-isolated from each other on that bridge. Outbound access is
  controlled per crew by egress policy enforced by the sidecar proxy
  (`127.0.0.1:9119`, via `HTTP_PROXY`/`HTTPS_PROXY`) — it filters only
  traffic actually routed through it. A raw socket, or a tool pointed past
  the proxy (`curl --noproxy '*'`), reaches the bridge directly: the shared
  bridge itself carries no network-layer isolation to fall back on.
...
| **Init process (PID 1)** | `init` | **Ignored — a tiny init always runs as PID 1.** The crew entrypoint ends in `exec sleep infinity`, which never reaps, and every sidecar process is reparented onto it, so a crew without an init leaks zombies until it can no longer fork. The runtime therefore sets docker `--init` unconditionally and neither the stored key nor `--init` changes anything. |
...
<Note>
`crewship crew config --init` is a deprecated no-op — the PID 1 reaper is
always on. The flag still parses, warns, and changes nothing.
</Note>
...
1. **Sidecar validation:**
   ...
   - Null byte rejection
   - Shell metacharacter blocking outside single quotes (`;`, `|`, `>`, `&`, `<`, `` ` ``, `$(`, newlines)
   - Content inside single quotes is exempt
   - Container ID and requesting agent always from IPC config and the agent's bearer token (agent cannot override)

2. **crewshipd validation:**
   - A stricter second denylist (defense in depth): the operators above plus `${`, `<(`, `<<`, ANSI-C `$'…'`, every control character including U+2028/U+2029, interpreter re-invocation (`sh -c`, `python -c`, `node -e`, …) and `awk`/`sed`, with real quote-state tracking — see [Shell injection protection](/guides/keeper#shell-injection-protection)
   - The credential must be `ACTIVE`, granted to the requesting agent and inside its lease, or the request is `404` before the judge runs
   - The Keeper judge evaluates the command; the L1 fast path never applies to execute

3. **Execution:**
   - Credential injected as environment variable for the command only, run as the container's resolved non-root user (refused if that user is empty, undeterminable or root)
   - Output scrubbed of the credential value and a fixed set of its encodings (base64 variants, base32, URL-escaped, hex, reversed) before returning to agent — a denylist of transforms, not a boundary
...
<Warning>
  **containerd and nerdctl are not supported.** `/run/containerd/containerd.sock`
  used to appear in this list. It was removed because containerd serves its own
  gRPC API over HTTP/2, not the Docker REST API — the client Crewship uses can
  never talk to it, on any version. If a containerd socket is present when
  detection fails, the error now names it and says so. Rancher Desktop counts
  here too: it works in **dockerd (moby)** mode and not in containerd mode,
  where it leaves a `~/.rd/docker.sock` behind that accepts a connection and
  then closes it.
</Warning>
