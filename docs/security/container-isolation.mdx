...
| Routine `http` steps | ✓ | ✓ |
| **Routine `script` steps** (in-container) | ✓ | ✓ |
...
The two **in-container** rows are enforced differently from the rest: nothing on the server intercepts them, so the boundary is the sidecar proxy the process points at. That makes the proxy environment itself the security control — a process in a crew container with no `HTTP_PROXY` does not bypass the allowlist, it never meets it. Both paths now take those variables from one exported source (`orchestrator.SidecarProxyEnv`), and a routine cannot override them from `script.env`.
