...
so the crew settings UI (**Container image & features → Security** tab) now
exposes them as labeled, validated controls with inline warnings instead of a
raw JSON blob. Each control serializes to the exact top-level
`devcontainer_config` key the runtime reads, and a raw-JSON "advanced" editor
remains available for anything the structured UI does not model (it is preserved
verbatim on save).

| Control | JSON key | What it costs you |
|---------|----------|-------------------|
| **Privileged mode** | `privileged` | Nulls `no-new-privileges`, drops the read-only rootfs, grants ~all Linux capabilities and host device access. An agent can escape to the host. Admin-only in the UI, and refused on save unless the workspace has `allow_privileged_credentials` set. Prefer adding a single capability instead. |
| **Added capabilities** | `capAdd` | Re-adds specific Linux capabilities on top of `--cap-drop=ALL`. Only `NET_BIND_SERVICE` is auto-allowed for community features; the rest (`SYS_ADMIN`, `NET_RAW`, `DAC_OVERRIDE`, …) are operator-only and flagged **high-risk** in the picker. |
| **Extra mounts** | `mounts` | Bind/volume mounts into the container. The allowlist mirrors `internal/devcontainer/mount_validate.go`: only `/dev/fuse` and valid named volumes are permitted. The Docker socket (`/var/run/docker.sock`), `/var/lib/docker`, `/tmp` and arbitrary host paths are rejected — each is a container-escape primitive. A `ro` toggle marks a mount read-only. |
