...
- **tmpfs /tmp (500M):** Writable temp directory (`rw,size=500m`), not persistent across restarts
...
- Reverse proxy mode — Claude Code via `ANTHROPIC_BASE_URL=http://127.0.0.1:9119` (→ `api.anthropic.com`) and, since #1030, Codex via `OPENAI_BASE_URL=http://127.0.0.1:9119/openai/v1` (→ `api.openai.com`; the `/openai` routing prefix is stripped before forwarding). For both, the real key stays in the sidecar CredStore and only a dummy placeholder reaches the agent env.
