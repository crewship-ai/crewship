...
export function RoutineRunArtifacts({ workspaceId, runId, active, compact = false }: { workspaceId: string; runId: string; active: boolean; compact?: boolean }) {
  ...
  if (compact && !loading && !error && !artifacts.length) return <div className="flex items-center gap-3 rounded-2xl border border-border/60 bg-card px-4 py-3 text-xs text-muted-foreground"><Files className="h-4 w-4 shrink-0" />No saved files or declared outputs{active ? " yet" : ""}.</div>
  const content = <section className="space-y-3">{!compact && <h2 className="text-sm font-medium">Declared outputs</h2>}{error && <p role="alert" className="text-sm text-destructive">{error} <button onClick={() => void refresh(new AbortController().signal)}>Retry</button></p>}{loading ? <p className="text-sm text-muted-foreground">Loading outputs…</p> : !error && !artifacts.length && <p className="text-sm text-muted-foreground">No declared outputs recorded. Step responses are shown below.</p>}{artifacts.map(artifact => <article key={artifact.id} className="space-y-2 rounded-xl border p-4"><div className="flex flex-wrap items-center justify-between gap-2"><h3 className="text-sm font-medium">{artifact.label}</h3><span className="text-xs text-muted-foreground">{artifact.kind.replaceAll("_", " ")} · {artifact.state}</span></div><p className="break-all text-xs text-muted-foreground">{artifact.execution_path} · Attempt {artifact.attempt}</p>{artifact.error && <p className="text-sm text-destructive">{artifact.error}</p>}{artifact.kind === "file" ? artifact.sha256 && <Button size="sm" variant="outline" onClick={() => download(artifact)}>Download saved version</Button> : <details onToggle={e => { if (e.currentTarget.open) void loadContent(artifact) }}><summary className="cursor-pointer text-sm">View content</summary><pre className="mt-2 max-h-80 overflow-auto whitespace-pre-wrap break-words text-xs">{contents[artifact.id] ?? "Loading content…"}</pre></details>}<details className="text-xs text-muted-foreground"><summary className="cursor-pointer">Origin</summary><p className="break-all">{artifact.source}</p>{artifact.sha256 && <p className="break-all">SHA-256: {artifact.sha256}</p>}</details></article>)}<div className="flex gap-2">{pages.length > 0 && <Button variant="outline" size="sm" onClick={() => { setPage(pages[pages.length - 1]); setPages(pages.slice(0, -1)) }}>Previous outputs</Button>}{cursor && <Button variant="outline" size="sm" onClick={() => { setPages([...pages, page]); setPage(cursor) }}>More outputs</Button>}</div></section>
  return compact ? <DetailCard title="Saved files and outputs" icon={Files}>{content}</DetailCard> : content
}
