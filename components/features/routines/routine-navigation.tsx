      ...
      {items.map((v) =>
        v === "run" ? (
          ...
          <button
            key={v}
            className={tabClass(view === v)}
            onClick={() => onChange(v as RoutineView)}
            aria-pressed={view === v}
          >
            {labels[v as RoutineView]}
          </button>
        ) : (
          <Link
            key={v}
            className={tabClass(view === v)}
            href={routineViewHref(slug, v as RoutineView)}
            aria-current={view === v ? "page" : undefined}
          >
            {labels[v as RoutineView]}
          </Link>
...
export function RoutineSubNavigation({
  items,
  value,
  onChange,
  label,
  ...
  return (
    <nav aria-label={label} className="flex flex-wrap gap-2 border-b border-border pb-2">
      {items.map((t) => {
        const Icon = icons?.[t]
        return (
          <button
            key={t}
            className={cn(tabClass(value === t), "inline-flex items-center gap-2")}
            onClick={() => onChange(t)}
