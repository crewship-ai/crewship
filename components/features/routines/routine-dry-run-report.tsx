...
export function RoutineDryRunReport({ result, onClose }: Props) {
  ...
  return (
    <div className="border-b border-border bg-purple/5 px-6 py-4">
      <div className="mb-3 flex items-center gap-2">
        <Eye className="h-3.5 w-3.5 text-purple" aria-hidden="true" />
        <span className="text-sm font-semibold text-purple">Plan preview</span>
        <Badge variant="outline" className="px-2 py-0 text-[11px] font-mono">
          {steps.length} {steps.length === 1 ? "step" : "steps"}
        </Badge>
        <div className="flex-1" />
        <span className="font-mono text-[11px] tabular-nums text-purple/80">
      ...
      {/* Honesty note — a dry run is a static plan, not an execution and not a
          ...
          report as "it worked". */}
      <div className="mb-3 flex items-start gap-2 rounded-md border border-purple/20 bg-purple/[0.06] px-3 py-2">
        <Info className="mt-[1px] h-3.5 w-3.5 shrink-0 text-purple" aria-hidden="true" />
        <p className="text-[12px] leading-relaxed text-purple/80">
      ...
      {steps.length === 0 ? (
        ...
        <ol className="space-y-1.5">
          {steps.map((s, i) => (
            <li
              key={`${s.step_id}-${i}`}
              className="flex items-center gap-3 rounded-md border border-purple/10 bg-background/40 px-3 py-2 text-sm"
            >
              ...
              <Badge
                ...
                className={cn(
                  ...
                  s.step_type === "call_pipeline" && "border-success/30 text-success",
                  s.step_type === "http" && "border-warn/30 text-warn",
                  s.step_type === "code" && "border-purple/30 text-purple",
                  s.step_type === "wait" && "border-warn/30 text-warn",
                  s.step_type === "transform" && "border-notice/30 text-notice",
                )}
              >
