  ...
  return <div className="space-y-4">
    <DetailCard title="Version history" subtitle={loading ? "Loading…" : `${versions.length} loaded`}>
      ...
      <ol className="divide-y divide-border/40">{versions.map(v => <li key={v.version} className="flex flex-wrap items-center gap-3 py-3">
        <div className="min-w-0 flex-1"><div className="flex items-center gap-2"><span className="font-medium">Version {v.version}</span>{v.is_head && <Pill tone="blue">Current</Pill>}</div><p className="mt-1 text-xs text-muted-foreground">{new Date(v.created_at).toLocaleString()} · {v.change_summary || "Saved recipe"}</p><details className="mt-1 text-[11px] text-muted-foreground"><summary className="cursor-pointer">Author and metadata</summary><p className="break-all">{v.author_type} · {v.author_id || "Not recorded"}</p><p className="break-all">{v.definition_hash}</p></details></div>
