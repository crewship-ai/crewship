...
vi.mock("@/hooks/use-pipeline-schedules", () => ({ usePipelineSchedules: () => ({ schedules: [{ id: "daily", name: "Daily", target_pipeline_slug: "incident-timeline", cron_expr: "0 9 * * *", timezone: "UTC", enabled: true, inputs: { message: "Daily summary" } }], loading: false, error: null, create: vi.fn(), update: vi.fn(), remove: vi.fn(), preview: vi.fn() }) }))
