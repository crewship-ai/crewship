...
const { mode } = vi.hoisted(() => ({ mode: { recurring: false, message: "Daily summary" } }))
vi.mock("@/hooks/use-pipeline-schedules", () => ({ usePipelineSchedules: () => ({ schedules: mode.recurring ? [{ id: "daily", name: "Daily", target_pipeline_slug: "incident-timeline", cron_expr: "0 9 * * *", timezone: "UTC", enabled: true, inputs: { message: mode.message } }] : [], loading: false, error: null, create: vi.fn(), update: vi.fn(), remove: vi.fn(), preview: vi.fn() }) }))
vi.mock("@/lib/api-fetch", () => ({ apiFetch: vi.fn(async () => ({ ok: true, json: async () => [{ id: "from-calendar", pipeline_slug: "incident-timeline", fire_at: "2026-09-09T08:00:00Z", inputs: { message: "Once summary" } }] })) }))
...
describe("routine Plan", () => {
  ...
  it.each([false, true])("shows one-time presets with recurring plans: %s", async (recurring) => {
    ...
    if (recurring) expect(schedules.getByText("Inputs: message: Daily summary")).toBeInTheDocument()
