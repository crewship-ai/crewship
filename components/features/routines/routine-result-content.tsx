...
export function RoutineResultContent({ output }: { output: string }) {
  // Detect JSON, but render the original bytes: parsing/stringifying would round
  // large integer identifiers and change recorded evidence.
  let structured: unknown
  try { structured = JSON.parse(output) } catch { /* A text result is Markdown. */ }
  if (structured !== null && typeof structured === "object") return <pre className="overflow-auto whitespace-pre-wrap break-words text-sm">{output}</pre>
