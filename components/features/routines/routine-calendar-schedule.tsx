...
export function RoutineCalendarSchedule({
  workspaceId,
  routines,
  date,
  onClose,
    ...
    void (async () => {
      try {
        ...
        if (!controller.signal.aborted) {
          setSpecs(routineInputSpecs(recipe.definition))
          setLoadedSlug(slug)
        }
      } catch (e) {
  ...
  const schedule = async (inputs: Record<string, unknown>) => {
    ...
    if (!at) {
      setError(
        "Choose a valid date and time. This local hour may not exist because of daylight saving time.",
      )
      return
    }
    ...
    try {
      const res = await apiFetch(
        `/api/v1/workspaces/${encodeURIComponent(workspaceId)}/pipelines/${encodeURIComponent(slug)}/run`,
        {
          ...
          body: JSON.stringify({ fire_at: at.toISOString(), inputs }),
        },
      )
    ...
    >
      <DialogContent className="max-h-[90dvh] overflow-y-auto border-border bg-card sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Choose an existing recipe and its inputs for one scheduled start. Repeating schedules
            can be managed in the routine’s Plan.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-2">
          <label htmlFor="calendar-routine" className="text-sm font-medium">
            Routine
          </label>
          <select
            id="calendar-routine"
          ...
          {!eligible.length && (
            <p className="text-xs text-muted-foreground">
              No active routines are available in this selection.
            </p>
          )}
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label htmlFor="calendar-date" className="text-sm">
              Date
            </label>
            <input
              id="calendar-date"
              ...
              className="mt-1 w-full min-w-0 rounded-md border bg-card p-2 text-sm"
            />
          </div>
          <div>
        ...
        <p className="text-xs text-muted-foreground">
          Timezone: {Intl.DateTimeFormat().resolvedOptions().timeZone}. This start will keep the
          published recipe version accepted when you schedule it.
        </p>
        {error && (
          <p role="alert" className="text-sm text-destructive">
            {error}
            {slug && specs === null && (
              <button className="ml-2 underline" onClick={() => setRetry((v) => v + 1)}>
                Retry
              </button>
            )}
          </p>
