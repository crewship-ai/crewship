...
export function RoutineRunDetail({ workspaceId, runId }: { workspaceId: string; runId: string }) {
  ...
  return <div className="mx-auto flex max-w-[1800px] flex-col gap-4 p-4">
    ...
    {tab === "outputs" && <section className="space-y-3 rounded-3xl border border-white/[0.06] bg-card p-4">{resultPanel}<RoutineRunArtifacts key={runId} workspaceId={workspaceId} runId={runId} active={["running", "queued", "waiting", "paused"].includes(run.status)} /><h2 className="pt-3 text-sm font-medium">Step responses</h2>{run.step_outputs_available === false ? <p role="alert">Step outputs could not be loaded. <button onClick={refresh}>Retry</button></p> : outputs.length === 0 ? <p className="text-sm text-muted-foreground">No step outputs recorded yet.</p> : outputs.map(([id, value]) => <details key={id} className="rounded-3xl border border-white/[0.06] bg-card p-4"><summary className="cursor-pointer text-sm font-medium">{id}</summary><pre className="mt-3 max-h-96 overflow-auto whitespace-pre-wrap break-words text-xs">{renderOutput(value)}</pre></details>)}</section>}
