...
export function RoutineRunDetail({ workspaceId, runId }: { workspaceId: string; runId: string }) {
  ...
  useEffect(() => { setTab("result"); setSelectedStep(null); setConfirmStop(false) }, [runId])
  ...
  return <div className="mx-auto flex max-w-[1800px] flex-col gap-4 p-4">
    ...
    <DetailCard title={explanation.title} action={<Link className="inline-flex items-center gap-1.5 text-xs text-primary hover:underline" href={activityHref}><Activity className="h-3.5 w-3.5" />Open in Activity ↗</Link>}>
      <p className="max-w-[85ch] text-sm text-foreground/85">{explanation.detail}</p>
      <div className="mt-3 flex flex-wrap gap-3 text-xs text-muted-foreground"><span>{new Date(run.started_at).toLocaleString("en-GB")} · {run.triggered_via} · {formatDurationMs(run.duration_ms)}</span>{run.pipeline_version != null && <Link className="text-primary" href={`${routineViewHref(run.pipeline_slug, "versions")}&version=${run.pipeline_version}`}>Executed recipe v{run.pipeline_version} ↗</Link>}<Link className="hover:text-foreground" href={routineViewHref(run.pipeline_slug, "history")}>All runs →</Link>{run.issue_identifier && <Link className="text-primary" href={`/issues?issue=${encodeURIComponent(run.issue_identifier)}`}>Issue · {run.issue_identifier} ↗</Link>}</div>
      {run.error_message && <details className="mt-3 text-xs"><summary className="cursor-pointer text-destructive">Recorded error{run.failed_at_step ? ` · ${run.failed_at_step}` : ""}</summary><p className="mt-2 whitespace-pre-wrap break-words" role="alert">{run.error_message}</p></details>}
    </DetailCard>
    ...
    {tab === "result" && <div className="space-y-4">
      {resultPanel}
      <DetailCard title="Files from this run"><RoutineRunArtifacts key={runId} workspaceId={workspaceId} runId={runId} active={active} /></DetailCard>
      ...
      <DetailCard title="Recorded work" action={<Button size="sm" variant="ghost" onClick={() => setTab("workflow")}><Network className="mr-1.5 h-3.5 w-3.5" />Workflow map</Button>}>
        ...
        {run.step_outputs_available === false ? <p role="alert" className="text-sm text-destructive">Step outputs could not be loaded. <button onClick={refresh}>Retry</button></p> : outputs.length === 0 ? <p className="text-sm text-muted-foreground">No step outputs recorded yet.</p> : outputs.map(([id, value]) => <details key={id} className="border-t border-border/60 py-3"><summary className="cursor-pointer text-sm font-medium">{id.replaceAll("_", " ")} <span className="font-normal text-muted-foreground">· recorded response</span></summary><pre className="mt-3 max-h-96 overflow-auto whitespace-pre-wrap break-words text-xs">{renderOutput(value)}</pre></details>)}
        <p className="mt-3 text-xs text-muted-foreground">Responses are recorded evidence, not a count of successful steps. The workflow shows dependencies and the execution history preserves attempts.</p>
      </DetailCard>
    </div>}
    {tab === "activity" && <RunActivityTimeline workspaceId={workspaceId} params={{ run_id: runId }} title="Run activity" card hideWhenEmpty={false} />}
    {tab === "inputs" && <section className="rounded-3xl border border-white/[0.06] bg-card p-4"><h2 className="mb-3 text-sm font-medium">Inputs used for this run</h2><p className="mb-3 text-xs text-muted-foreground">Saved at execution time. Editing the recipe does not change these values.</p><div className="mb-4 flex flex-wrap gap-3 text-xs"><Link className="text-primary" href={activityHref}>Activity ↗</Link><Link className="text-primary" href={`/routines?${new URLSearchParams({ slug: run.pipeline_slug, view: "edit" })}`}>Current access and credentials ↗</Link></div><pre className="overflow-auto whitespace-pre-wrap break-words text-xs">{JSON.stringify(run.inputs ?? {}, null, 2)}</pre></section>}
