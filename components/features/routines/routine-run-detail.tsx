...
export function RoutineRunDetail({ workspaceId, runId }: { workspaceId: string; runId: string }) {
  ...
  const startAgain = async (inputs: Record<string, unknown>) => {
    ...
    try {
      const res = await apiFetch(
        `/api/v1/workspaces/${encodeURIComponent(workspaceId)}/pipelines/${encodeURIComponent(run.pipeline_slug)}/run`,
        {
          method: "POST",
  ...
  return (
    <div className="mx-auto flex max-w-[1800px] flex-col gap-4 p-4">
      ...
      >
        <div className="flex items-start gap-3">
          <span className="mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-muted">
            <StatusIcon
              className={`h-5 w-5 ${presentation.tone === "destructive" ? "text-destructive" : presentation.tone === "warn" ? "text-warn" : active ? "text-primary" : presentation.tone === "success" ? "text-success" : "text-muted-foreground"}`}
            />
          </span>
        ...
        <dl className="mt-4 grid grid-cols-2 gap-3 border-t border-border/60 pt-4 lg:grid-cols-4">
          <div>
            <dt className="mb-1 flex items-center gap-1.5 text-[11px] text-muted-foreground">
          ...
          <div>
            ...
            <dd className="text-xs">{formatDurationMs(run.duration_ms)}</dd>
          </div>
          <div>
            <dt className="mb-1 flex items-center gap-1.5 text-[11px] text-muted-foreground">
            ...
            <dd className="text-xs">{triggerLabel}</dd>
          </div>
          <div>
            <dt className="mb-1 flex items-center gap-1.5 text-[11px] text-muted-foreground">
            ...
            <dd className="text-xs">
              {run.pipeline_version != null ? (
                ...
                >
                  Executed recipe v{run.pipeline_version} ↗
                </Link>
              ) : (
                "Version unavailable"
              )}
        ...
        {run.error_message && (
          <details className="mt-3 rounded-xl border border-destructive/15 bg-destructive/5 px-3 py-2 text-xs">
      ...
      <RoutineSubNavigation
        label="Run detail"
        items={["result", "workflow", "activity", "inputs"]}
        value={tab}
        onChange={setTab}
        icons={RUN_TAB_ICONS}
      />
        ...
        <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_320px]">
          {dsl ? (
            <DetailCard
              ...
              subtitle={
                run.pipeline_version != null
                  ? `Executed version ${run.pipeline_version}`
                  : "Historical recipe"
              }
              bare
            >
              <div className="h-[56vh] min-h-[380px] overflow-hidden">
                <TraceCanvas
                  run={run}
                  dsl={dsl}
                  workspaceId={workspaceId}
          ...
          <aside className="space-y-4">
            <DetailCard title="Step details" icon={ListOrdered}>
              <label
                htmlFor={`run-step-${runId}`}
              ...
              <select
                id={`run-step-${runId}`}
                className="mb-4 w-full rounded-xl border border-border bg-muted/30 px-3 py-2 text-sm"
                value={selectedStepId || ""}
                onChange={(e) => setSelectedStep(e.target.value)}
              >
                {dsl?.steps?.map((item, index) => (
                  <option key={item.id} value={item.id}>
                    {index + 1}. {readableFieldName(item.id)}
                  </option>
                ))}
              </select>
              {step ? (
                <div className="space-y-4">
                  <details className="text-xs">
                    <summary className="cursor-pointer text-primary">Step instructions</summary>
                    <div className="mt-3">
                      <RoutineStepDefinition step={step as unknown as Record<string, unknown>} />
                    </div>
                  </details>
                  <div className="border-t border-border/60 pt-3">
                    ...
                    {run.step_outputs_available === false ? (
                      <p className="text-xs text-destructive">Step outputs could not be loaded.</p>
                    ) : Object.hasOwn(run.step_outputs ?? {}, step.id) ? (
                      <RoutineRecordedValue value={run.step_outputs![step.id]} />
                    ) : (
                  ...
                  <details className="border-t border-border/60 pt-3 text-xs">
                    <summary className="cursor-pointer">
                      Agent and tool activity · {mapSubSpans(run.sub_spans?.[step.id]).length}
                    </summary>
                    <div className="mt-3 space-y-2">
                      {mapSubSpans(run.sub_spans?.[step.id]).map((span, index) => (
                        <details
                          key={`${span.startedAt}-${span.name}-${index}`}
                ...
                <p className="text-xs text-muted-foreground">
                  Select a step in the map to inspect its recorded work.
                </p>
              )}
            </DetailCard>
          </aside>
      ...
      {tab === "workflow" && (
        <RoutineExecutionHistory
          ...
          workspaceId={workspaceId}
          runId={runId}
          active={["running", "queued", "waiting", "paused"].includes(run.status)}
        />
      )}
      {tab === "result" && (
        <div className="space-y-4">
          ...
          />
          {!dsl && (
            <p className="rounded-3xl border border-white/[0.06] bg-card p-4 text-sm text-muted-foreground">
              The historical recipe is unavailable. Recorded outputs and activity remain accessible.
            </p>
          )}
          <DetailCard
            ...
            icon={ListOrdered}
            action={
              <Button size="sm" variant="ghost" onClick={() => setTab("workflow")}>
          ...
          >
            {!run.output && (
              <p className="mb-3 text-xs text-muted-foreground">
                No final response has been recorded{active ? " yet" : ""}. Individual step responses
                and files may still be available.
              </p>
            )}
            {run.step_outputs_available === false ? (
              <p role="alert" className="text-sm text-destructive">
                Step outputs could not be loaded. <button onClick={refresh}>Retry</button>
              </p>
            ) : outputs.length === 0 ? (
              <p className="text-sm text-muted-foreground">No step outputs recorded yet.</p>
            ) : (
              <details className="text-sm">
                <summary className="flex cursor-pointer items-center gap-2 text-primary">
                  <ChevronRight className="h-4 w-4" />
                  Step responses · {outputs.length}
                </summary>
                  ...
                  {outputs.map(([id, value]) => (
                    ...
                    >
                      <summary className="cursor-pointer text-sm font-medium">
                        {readableFieldName(id)}{" "}
                        <span className="font-normal text-muted-foreground">
                          · recorded response
                        </span>
                      </summary>
                      <div className="mt-3">
                        <RoutineRecordedValue value={value} />
                      </div>
                      <button
            ...
            <p className="mt-3 text-xs text-muted-foreground">
              Individual step responses and attempts are available in Workflow.
            </p>
          </DetailCard>
        </div>
      )}
        ...
        <div className="space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-border/60 bg-card px-4 py-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium">Events from this run</span>
              <Pill tone={active ? "blue" : "default"}>
                {active ? "Live updates" : "Recorded history"}
              </Pill>
            </div>
            <Link className="text-xs text-primary" href={activityHref}>
              Open in Activity ↗
            </Link>
          </div>
          <RunActivityTimeline
          ...
          <details
            ...
            className="rounded-2xl border border-border/60 bg-card p-4 text-xs"
          >
