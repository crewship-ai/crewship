...
export function PageApplicationView({ workspaceId, slug, page, fallback }: { workspaceId: string; slug: string; page: WirePage | null; fallback: ReactNode }) {
  ...
  const { query } = usePageApplication(workspaceId, slug, page !== null && page.has_application !== false)
  ...
  useEffect(() => {
    if (!hasApplication || (query.isError && !storageBusy) || (!query.isPending && query.data?.publication === null)) {
      setOpened(null)
