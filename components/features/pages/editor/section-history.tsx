 ...
 *
 * Chrome: every card is a `DetailCard` from the shared detail kit, the same
 * card the issue detail is built from. What each restore DOES is the card's
 * one-line footer, not a paragraph above the list. The kit's card takes no
 * `data-slot`, so each card sits in a plain div that carries the slot the
 * tests and the shell address it by.
 */
...
import { ControlRefusal, PanelVersionsCard, Refusal } from "@/components/features/pages/page-settings"
...
/** One revision or publication in a card's list — the issue detail's row register. */
const ROW = "flex items-center gap-2.5 border-b border-hairline py-2 type-page-meta last:border-b-0"
      ...
      <DetailCard
        ...
        footer={
          <>
            ...
            application does not change until you publish it.
          </>
        }
      >
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex min-w-0 flex-1 flex-col">
                      <span className="truncate text-foreground/85">{when(revision.created_at)}</span>
                      <span className="type-meta truncate text-muted-foreground-soft">
                        ...
                        <span className="type-page-stamp">
                    ...
                    {isCurrent ? (
                      <Pill className="shrink-0">current draft</Pill>
                    ) : (
                      <Button
                        size="sm"
                        variant="ghost"
...
function PublicationsCard({
  ...
  return (
    <div data-slot="history-publications">
      <DetailCard
        title="Application publications"
        icon={Rocket}
        subtitle={
          listFailed
            ? "unavailable"
        ...
        footer={
          <>
            Publishing a retained version makes a <strong>new live publication</strong> of it: the
            version counter goes forward, and completed routine actions are not undone.
          </>
        }
      >
            ...
            <div data-slot="publications">
              {receipts.map((receipt) => (
                ...
                >
                  ...
                  {/* The live one is marked, not merely mentioned: this is the
                      version a reviewer must not republish, and the row's
                      control is disabled on exactly this condition below. */}
                  {state?.published && state.publication_version === receipt.version && (
                    <Pill tone="blue" className="shrink-0" data-testid="publication-live">
                      live
                    </Pill>
                  )}
                  <Button
    ...
    <div data-slot="editor-section-history" className="flex w-full flex-col gap-4">
      ...
      {(capabilities.hasApplicationDraft || capabilities.hasApplication) &&
        (capabilities.mayViewSourceHistory ? (
          ...
          // read means.
          <div data-slot="history-application-refused">
            <DetailCard
              title="Application source and publications"
              icon={History}
              subtitle="not yours to read"
            >
              <ControlRefusal>
      ...
      {/* What none of the three restores covers. Last, not first: each card's
          footer already says what ITS restore does, and this line is the one
          caveat that applies to all three — a reader meets it after the
          cards, once the three effects are in front of them. */}
