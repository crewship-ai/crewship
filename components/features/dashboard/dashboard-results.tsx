...
export function DashboardResults({ review, inProgress, completed, activeAgentRuns, activeRoutineRuns, recentRoutineRuns, agents, crews, workspaceId, loading, error, routineError, routineLoading, onRetry }: {
  ...
  return <DashboardCard title="Results & review" icon={CheckCheck} hint={<span className="hidden sm:inline">{runningCount > 0 ? `${runningCount} live now` : "Live work and outcomes"}</span>} action={<Link href={entityHref({ kind: "issues" })} className="text-primary-hover hover:underline">All issues →</Link>} className="flex h-full min-h-0 flex-col border-primary/20">
    ...
    {loading && counts.all === 0 ? <div className="space-y-3" aria-label="Loading results"><Skeleton className="h-9 rounded-md" /><Skeleton className="h-9 rounded-md" /><Skeleton className="h-9 rounded-md" /></div> : <div className="max-h-[440px] min-h-[210px] overflow-y-auto overscroll-contain pr-1 xl:min-h-0 xl:max-h-none xl:flex-1" tabIndex={0} aria-label="Work list">
