...
export interface CruiseAgentInlineProps {
  agent: AgentData
  workspaceId: string
}

/**
 * Middle-pane agent detail, shown when an agent is selected in /cruise.
 ...
 */
export function CruiseAgentInline({ agent, workspaceId }: CruiseAgentInlineProps) {
  ...
  const agentPath = `/cruise/agents/${agent.id}`
          ...
          <Card>
            <CardContent className="p-4 space-y-3">
              ...
              <dl className="space-y-2 text-label">
                ...
                {agent.crew && (
                  <Row label="Crew">
                    <Link href={`/cruise/crews/${agent.crew_id}`} className="hover:underline">
                      {agent.crew.name}
                    </Link>
                  </Row>
                )}
          ...
          <Card>
            <CardContent className="p-4 space-y-3">
              <div className="flex items-center justify-between">
                ...
                {agent.crew && (
                  <Link
                    href={`/cruise/crews/${agent.crew_id}`}
              ...
              <div className="flex items-center gap-2 flex-wrap">
                {peers.slice(0, 8).map((peer) => (
                  <Link
                    key={peer.id}
                    href={`/cruise?agent=${peer.slug}&crew=${agent.crew?.slug ?? ""}`}
