...
export interface CruiseAgentInboxProps {
  agent: AgentBrief
 ...
 * center CruiseAgentInline pane.
 */
export function CruiseAgentInbox({
  agent, workspaceId: _workspaceId,
  scheduleNextRun, memoryEnabled, leadMode,
  onClose,
}: CruiseAgentInboxProps) {
          ...
          {loading && !inbox ? (
            ...
            <div className="space-y-1">
              ...
              <InboxRow
                ...
                href={
                  agent.crew?.slug
                    ? `/cruise/crews/${agent.crew.slug}?tab=journal`
                    : `/cruise/agents/${agent.id}`
                }
              />
              <InboxRow
                icon={AlertTriangle}
