...
export interface CruiseCrewDetailProps {
  crew: CrewData
  agents: AgentData[]
...
export function CruiseCrewDetail({ crew, agents, missions, onAgentClick }: CruiseCrewDetailProps) {
    ...
    <div className="p-4 sm:p-6 h-full overflow-y-auto space-y-5">
      ...
      <div className="flex items-center gap-2">
        <Button variant="outline" size="sm" asChild>
          <Link href={`/cruise/crews/${crew.id}`}>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          ...
          <Button variant="outline" size="sm" className="h-7 text-[11px]" asChild>
            <Link href={`/cruise/agents/new?crew_id=${crew.id}`}>+ Add</Link>
          </Button>
        </div>
