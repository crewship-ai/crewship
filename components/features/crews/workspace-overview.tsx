...
import { Activity, CheckCircle2, CircleAlert, MessageSquare, Sparkles, Users, Inbox } from "lucide-react"
...
export function RunMetrics({ workspaceId, agentId, crewId, revision = 0 }: { workspaceId: string; agentId?: string; crewId?: string; revision?: number }) {
  ...
  return <section aria-label="Runs in the last 7 days" className="space-y-3">
    <p className="text-xs text-muted-foreground">Agent runs started in the last 7 days{data?.truncated ? " · most recent 20,000 runs only" : ""}</p>
