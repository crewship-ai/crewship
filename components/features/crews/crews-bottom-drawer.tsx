...
import { CrewsActivityFeed } from "@/components/features/crews/crews-activity-feed"
...
interface CrewsBottomDrawerProps {
  crews: CrewExport[]
  agents: AgentExport[]
  isMobile: boolean
  /**
...
export function CrewsBottomDrawer({ crews, agents, isMobile, onAgentsChanged }: CrewsBottomDrawerProps) {
    ...
    >
      ...
      {drawerOpen && (
        <div className="flex-1 min-h-0 border-t border-border overflow-auto">
          {drawerTab === "activity" && (
            <CrewsActivityFeed agents={agents} />
          )}
          {drawerTab === "bulk" && (
            <CrewsBulkActions agents={agents} onSuccessRefresh={onAgentsChanged} />
          )}
          {drawerTab === "export" && (
