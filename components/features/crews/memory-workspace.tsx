...
export function useWorkspaceResource<T>(url: string, revision = 0) {
...
export function MemoryWorkspace({ workspaceId, agentId, agentSlug, crewId, memoryEnabled }: {
  ...
  return <div className="space-y-5 max-w-6xl">
    <div className="flex items-start justify-between gap-4">
      ...
      <div className="flex flex-wrap justify-end gap-2">{crewId && <ScopeExport crewId={crewId} agentSlug={agentSlug} workspaceId={workspaceId} />}<Button variant="outline" size="sm" onClick={() => setRevision((n) => n + 1)}>Refresh</Button></div>
    </div>
    ...
    <div className="flex flex-wrap gap-2" aria-label="Memory views">
      {[['knowledge', 'Knowledge'], ['me', 'About me']].map(([id, label]) => <Button key={id} variant={section === id ? "secondary" : "ghost"} onClick={() => setSection(id)} aria-pressed={section === id}>{label}</Button>)}
      <Button variant="ghost" asChild><Link href="/inbox?kind=memory_consolidation">Knowledge proposals ↗</Link></Button>
    </div>
    {section === "me" ? <PersonalMemory key={workspaceId} workspaceId={workspaceId} /> : <>
      {error ? <p role="alert" className="text-sm text-destructive">{error} Use Refresh to try again.</p> : !data ? <p role="status" className="text-sm text-muted-foreground">Loading knowledge…</p> : <div className="grid gap-4 lg:grid-cols-[280px_minmax(0,1fr)]">
        <div className="space-y-4">
          {Object.entries(data.scopes).map(([scope, state]) => <section key={scope} className="rounded-2xl border border-border bg-card p-4">
            <h3 className="font-medium text-sm">{scope === "agent" ? "Agent knowledge" : scope === "workspace" ? "Shared with workspace" : "Shared with crew"}</h3>
            <p className="text-xs text-muted-foreground mt-1">{scope === "agent" ? "Notes retained by this agent." : scope === "workspace" ? "Knowledge shared across crews." : "Available to agents working in this crew."}</p>
            {state === "unavailable" ? <p className="text-sm mt-3" role="alert">This knowledge could not be read.</p> : state === "empty" ? <p className="text-sm text-muted-foreground mt-3">No saved notes yet.</p> : <ul className="mt-3 space-y-1">{data.documents.filter((d) => d.scope === scope).map((d) => <li key={d.id}><button onClick={() => setSelected(d.id)} className={cn("w-full rounded-lg px-2 py-2 text-left text-sm break-words hover:bg-muted", selected === d.id && "bg-primary/10 text-primary")}>{d.name}</button></li>)}</ul>}
          </section>)}
        </div>
        <section className="min-w-0 rounded-2xl border border-border bg-card p-5">
          {document ? <><h3 className="font-medium break-words">{document.name}</h3><p className="text-xs text-muted-foreground mt-1">Current file · {document.scope === "crew" ? "Crew knowledge" : document.scope === "workspace" ? "Workspace knowledge" : "Agent knowledge"}{document.updated_at && ` · Updated ${new Date(document.updated_at).toLocaleString()}`}</p>
            ...
            {document.state === "available" && <Button className="mt-4" size="sm" variant="outline" onClick={() => {
              ...
              const anchor = window.document.createElement("a"); anchor.href = url; anchor.download = document.name.split('/').pop() ?? "memory.md"; anchor.click(); URL.revokeObjectURL(url)
          ...
          </> : <div className="py-12 text-center text-sm text-muted-foreground">Select a note to read its current content.<p className="mt-2">Instructions and persona belong in Edit. History is shown separately for each note.</p></div>}
        </section>
      </div>}
    </>}
  </div>
...
export function PersonalMemory({ workspaceId }: { workspaceId: string }) {
  const [revision, setRevision] = useState(0)
  ...
  const model = useWorkspaceResource<MyModel>(`/api/v1/users/me/user-model${query}`, revision)
  const cards = useWorkspaceResource<MyCards>(`/api/v1/users/me/peer-cards${query}`, revision)
  const consent = useWorkspaceResource<{ opted_out: boolean }>(`/api/v1/users/me/peer-consent${query}`, revision)
  ...
  return <div className="space-y-4">
    <div className="rounded-2xl border border-border p-5"><h3 className="font-medium">About me in this workspace</h3><p className="text-sm text-muted-foreground mt-1">Your retained preferences and notes from working with agents. This view uses your signed-in identity.</p>
