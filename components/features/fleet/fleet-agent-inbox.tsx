...
export interface FleetAgentInboxProps {
  agent: AgentBrief
 ...
 * center FleetAgentInline pane.
 */
export function FleetAgentInbox({
  agent, workspaceId: _workspaceId,
  scheduleNextRun, memoryEnabled, leadMode,
  onClose,
}: FleetAgentInboxProps) {
          ...
          {loading && !inbox ? (
            ...
            <div className="space-y-1">
              ...
              <InboxRow
                ...
                href={
                  agent.crew?.slug
                    ? `/fleet/crews/${agent.crew.slug}?tab=journal`
                    : `/fleet/agents/${agent.id}`
                }
              />
              <InboxRow
                icon={AlertTriangle}
