...
export function ConversationActivity({ workspaceId, userId, conversationId, canManage }: { workspaceId: string; userId: string; conversationId: string; canManage: boolean }) {
  ...
  return <section className="space-y-3 border-t pt-3"><h3 className="flex items-center gap-2 text-[10px] uppercase tracking-wider text-muted-foreground"><Activity className="size-4" />Workspace activity</h3><p className="text-xs text-muted-foreground">Post new issue updates and routine results here. Earlier activity is not imported. {canManage ? "" : "The channel creator manages these subscriptions."}</p>{settings.isPending && <p className="text-xs">Loading subscriptions…</p>}{settings.data && ([['issues', 'Issue updates'], ['routines', 'Routine results']] as const).map(([field, label]) => <label key={field} className="flex min-h-9 coarse:min-h-12 items-center gap-3 text-xs"><Checkbox checked={settings.data[field]} disabled={!canManage || busy} onCheckedChange={(checked) => { void change(field, checked === true) }} aria-label={label} />{label}</label>)}{settings.error && <p role="alert" className="text-xs">Unable to load activity subscriptions. <button type="button" className="underline" onClick={() => { void settings.refetch() }}>Retry</button></p>}{error && <p role="alert" className="text-xs text-destructive">Unable to save activity subscriptions. Please try again.</p>}</section>
}
