...
var rootCmd = &cobra.Command{
	...
	// Bare `crewship` with no subcommand: show the brand logo, then the
	// usual help. Subcommands are unaffected (cobra only calls Run when no
	// child command matches), and `crewship --help` still prints help alone.
	Run: func(cmd *cobra.Command, args []string) {
		cli.PrintLogo(os.Stderr, version)
		_ = cmd.Help()
	},
	SilenceUsage:  true,
	SilenceErrors: true,
