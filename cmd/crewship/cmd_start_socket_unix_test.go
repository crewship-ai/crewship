//go:build unix && !clionly

// Every case in this file is written in unix path literals — absolute
// /home/... roots, the /tmp/crewship.sock the packaging pins, and the
// sockaddr_un length cap — so the file carries a `unix` build tag instead of
// each test asking runtime.GOOS at run time. A skipped test prints the same
// `ok` as a passing one; excluding the file at compile time is the same guard
// without the false green. The windows half of startSocketPath (%ProgramData%
// paths, no sockaddr_un ceiling) is covered by internal/config's
// defaultPathsFor tests, which are GOOS-parameterised and run everywhere.
