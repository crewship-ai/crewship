...
func pageProjectSchemaCatalog() map[string]DomainSchema {
	...
	reviewCandidate := map[string]any{"type": "object", "nullable": true, "description": "The current draft, or — with ?publication=N — retained publication N as a rollback candidate.", "required": []string{"revision", "git_commit", "source_digest", "created_at", "actor", "build"}, "properties": map[string]any{"revision": revision, "git_commit": checkpoint, "source_digest": digest, "created_at": map[string]any{"type": "string", "format": "date-time", "description": "In draft mode, when the source revision was saved. In ?publication=N mode, when that publication was made."}, "actor": reviewActor, "build": reviewBuild}}
	...
	reviewBlocker := map[string]any{"type": "object", "required": []string{"code", "message"}, "properties": map[string]any{"code": map[string]any{"type": "string", "enum": []string{"no_candidate", "candidate_matches_live", "build_missing", "build_failed", "build_stale", "baseline_unavailable", "routine_unresolved", "definition_moved", "not_permitted", "storage_unavailable"}}, "message": map[string]any{"type": "string"}}}
