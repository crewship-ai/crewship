...
func pageProjectSchemaCatalog() map[string]DomainSchema {
	...
	reviewActor := map[string]any{"type": "object", "required": []string{"kind", "id"}, "properties": map[string]any{"kind": actorKind, "id": map[string]any{"type": "string"}, "label": map[string]any{"type": "string", "description": "Resolved at read time from the current directory; absent when the id no longer resolves."}}}
	...
	reviewCandidate := map[string]any{"type": "object", "nullable": true, "description": "The current draft, or — with ?publication=N — retained publication N as a rollback candidate.", "required": []string{"revision", "git_commit", "source_digest", "created_at", "actor", "build"}, "properties": map[string]any{"revision": revision, "git_commit": checkpoint, "source_digest": digest, "created_at": map[string]any{"type": "string", "format": "date-time"}, "actor": reviewActor, "build": reviewBuild}}
	reviewBaseline := map[string]any{"type": "object", "required": []string{"publication_version", "published", "definition_digest", "source_revision", "git_commit", "source_available", "source_unavailable_reason"}, "properties": map[string]any{
	...
	reviewRoutine := map[string]any{"type": "object", "required": []string{"routine", "published_digest", "current_digest", "state"}, "properties": map[string]any{"routine": map[string]any{"type": "string"}, "published_digest": map[string]any{"type": "string", "nullable": true}, "current_digest": map[string]any{"type": "string", "nullable": true}, "state": map[string]any{"type": "string", "enum": []string{"unchanged", "changed", "unknown"}}}}
	reviewBlocker := map[string]any{"type": "object", "required": []string{"code", "message"}, "properties": map[string]any{"code": map[string]any{"type": "string", "enum": []string{"no_candidate", "candidate_matches_live", "build_missing", "build_failed", "build_stale", "baseline_unavailable", "definition_moved", "not_permitted", "storage_unavailable"}}, "message": map[string]any{"type": "string"}}}
	...
	return map[string]DomainSchema{
		...
		"POST /api/v1/pages/{slug}/project/publish":                          {RequestRequired: true, Request: map[string]any{"type": "object", "additionalProperties": false, "required": []string{"expected_publication", "reviewed_code", "expected_definition_digest", "expected_routine_digests"}, "properties": map[string]any{"build_id": map[string]any{"type": "string"}, "expected_revision": revision, "expected_publication": map[string]any{"type": "integer", "minimum": 0}, "reviewed_code": map[string]any{"type": "boolean", "enum": []bool{true}}, "rollback_version": revision, "expected_definition_digest": map[string]any{"type": "string", "pattern": "^[0-9a-f]{64}$", "description": "sha256 of the Page definition the publisher reviewed; a 409 with conflict:\"definition\" means it moved."}, "expected_routine_digests": map[string]any{"type": "object", "additionalProperties": digest, "description": "Reviewed routine definition digests keyed by routine slug; {} when the candidate declares no call actions. A 409 with conflict:\"routines\" names the ones that moved."}}}, Response: receipt},
