...
func adminSpecialRequestSchemaCatalog() (map[string]DomainSchema, map[string]any) {
	...
	components := map[string]any{
		...
		"AdminMemorySyncRequest": object(map[string]any{
			"workspace_id": map[string]any{"type": "string", "description": "Narrow the run to one workspace, by id or slug. Omitted: every active workspace on the instance, as the scheduled sweep does. Body only — the ?workspace_id= query parameter is the session workspace, not a filter."},
			"dry_run":      map[string]any{"type": "boolean", "description": "Run everything up to the write, including the extractor, and report what would have happened. Nothing is written, indexed or audited. Also accepted as ?dry_run=true."},
		}),
	...
	routes := map[string]DomainSchema{
		...
		"PUT /api/v1/admin/log-level": {Request: ref("AdminLogLevelRequest")}, "PUT /api/v1/admin/rate-limits/{key}": {Request: ref("AdminRateLimitRequest")}, "PATCH /api/v1/admin/memory/config": {Request: ref("AdminMemoryConfigRequest")}, "POST /api/v1/admin/memory/user-model-sync": {Request: ref("AdminMemorySyncRequest")}, "POST /api/v1/admin/memory/peer-card-sync": {Request: ref("AdminMemorySyncRequest")}, "POST /api/v1/admin/prune-crew-runtimes": {Request: ref("EmptyRequest")}, "POST /api/v1/admin/prune-legacy-resources": {Request: ref("EmptyRequest")}, "POST /api/v1/admin/reap-orphan-containers": {Request: ref("EmptyRequest")}, "POST /api/v1/admin/reencrypt": {Request: ref("EmptyRequest")},
