...
func finalAdminPlatformSchemaCatalog() (map[string]DomainSchema, map[string]any) {
	...
	components := map[string]any{
		...
		"FinalAdminPlatformOAuthAutoConnect": object(map[string]any{
			"status": str(), "auth_url": str(), "token_url": str(), "scopes": str(), "redirect_uri": str(),
			"credential_id": str(), "message": str(),
		}),
