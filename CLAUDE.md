...
- **`NEXTAUTH_SECRET` MUST be set on the Crewship server** — otherwise the entire API router AND static UI handler are silently skipped. The conditional registration of routes happens around `internal/server/server.go:266` inside `if deps != nil && deps.DB != nil && cfg.Auth.JWTSecret != ""`, which encloses the API router creation (~line 440) and SPA handler setup (~line 454). Separately, line 155 emits the only user-facing signal of the misconfig: `WARN NEXTAUTH_SECRET not set, WebSocket auth disabled`. Symptom: every route returns 404 with no ERROR log. Healthy startup logs both `Go API routes mounted` and `serving embedded static UI` — confirm via `journalctl -u crewship-prod | grep -E "API routes mounted|serving embedded"`.
