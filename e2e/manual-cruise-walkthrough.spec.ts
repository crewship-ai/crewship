...
test("/cruise renders top tabs + no console errors", async ({ page }) => {
  await login(page)
  ...
  console.log("/cruise issues:", JSON.stringify(issues, null, 2))
  expect(issues.jsErrors).toHaveLength(0)
})

test("legacy /agents redirects to /cruise/agents in browser", async ({ page }) => {
  await login(page)
...
test("agent full page: all 7 tabs reachable without JS errors", async ({ page }) => {
  await login(page)
  const issues = attachCollectors(page)
  await page.goto("/cruise/agents")
  await page.waitForLoadState("networkidle")

  const agentLink = page
    .locator("a[href^='/cruise/agents/']:not([href$='/new']):not([href*='/agents/new'])")
    .first()
...
test("crew full page: all 6 tabs reachable", async ({ page }) => {
  ...
  const crewLink = page
    .locator("a[href^='/cruise/crews/']:not([href$='/new']):not([href*='/crews/new'])")
    .first()
...
test("agent tools sub-strip: skills/credentials/mcp", async ({ page }) => {
  ...
  await page.goto("/cruise/agents")
  await page.waitForLoadState("networkidle")
  const agentLink = page.locator("a[href^='/cruise/agents/']:not([href$='/new']):not([href*='/agents/new'])").first()
  if ((await agentLink.count()) === 0) {
    test.skip(true, "no agents")
    return
  }
...
test("crew overview shows avatar picker + apply button", async ({ page }) => {
  ...
  await page.goto("/cruise/crews")
  await page.waitForLoadState("networkidle")
  const crewLink = page.locator("a[href^='/cruise/crews/']:not([href$='/new']):not([href*='/crews/new'])").first()
  if ((await crewLink.count()) === 0) {
    test.skip(true, "no crews")
    return
  }
  await crewLink.click()
  await page.waitForURL(/\/cruise\/crews\/[^/]+/)
...
test("agent create form → list → delete flow (UI end-to-end)", async ({ page }) => {
  await login(page)
  const slug = `cruise-e2e-${Date.now()}`
  await page.goto("/cruise/agents/new")
  await page.waitForLoadState("networkidle")

  await page.locator('input[id="name"]').fill("Cruise E2E")
  ...
  await page.waitForURL(/\/cruise\/agents\/?(\?|$)/, { timeout: 15_000 })

  // Confirm agent is in the list
  const newAgent = page.locator(`a[href*='/cruise/agents/']`).filter({ hasText: "Cruise E2E" }).first()
...
test("chat session: create via UI and verify in agent Sessions tab", async ({ page }) => {
  ...
  const agentId = overviewUrl.match(/\/cruise\/agents\/([^/?]+)/)?.[1]
  ...
  // the display title.
  await page.goto(`/cruise/agents/${agentId}/sessions`)
  await page.waitForLoadState("networkidle")
  await expect(
