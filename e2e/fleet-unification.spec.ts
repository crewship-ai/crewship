...
test.describe("Fleet Unification", () => {
  test("legacy /agents redirects to /fleet/agents", async ({ page }) => {
  ...
  test("/fleet renders explorer and 3 top-level tabs", async ({ page }) => {
  ...
  test("agent full page has 7-tab rail", async ({ page }) => {
    await page.goto("/fleet")
    await page.waitForLoadState("networkidle")
    const agentLink = page.locator("a[href^='/fleet/agents/']").filter({ hasNotText: "new" }).first()
  ...
  test("crew detail renders 6 tabs", async ({ page }) => {
    ...
    await page.waitForLoadState("networkidle")
    const crewLink = page.locator("a[href^='/fleet/crews/']").filter({ hasNotText: "new" }).first()
