...
try {
  ...
  const copiedEdit = await openEdit(page, copy.slug)
  await copiedEdit.getByLabel(/^Name/).fill("Renamed unpublished copy")
  await copiedEdit.getByRole("button", { name: "Save draft", exact: true }).click()
  await expect(copiedEdit).toHaveCount(0)
  await request("GET", api + "/pipelines/" + copy.slug, undefined, 404)
  const renamed = await request("GET", api + "/pipelines/" + copy.slug + "/draft")
  assert.equal(renamed.document.name, "Renamed unpublished copy")
  report.checks.push("Renaming an unpublished copy remains a draft")
