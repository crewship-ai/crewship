...
(function () {
  document.addEventListener('DOMContentLoaded', function () {
    ...
    toolbar.className = 'h-12 bg-neutral-950 flex items-center justify-between px-4 flex-shrink-0';
    toolbar.innerHTML =
      // Left: Logo
      '<div class="flex items-center gap-2.5">' +
        '<svg class="w-5 h-5 text-primary-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M2 21c.6.5 1.2 1 2.5 1 2.5 0 2.5-2 5-2 1.3 0 1.9.5 2.5 1 .6.5 1.2 1 2.5 1 2.5 0 2.5-2 5-2 1.3 0 1.9.5 2.5 1"/><path d="M19.38 20A11.6 11.6 0 0 0 21 14l-9-4-9 4c0 2.9.94 5.34 2.81 7.76"/><path d="M19 13V7a2 2 0 0 0-2-2H7a2 2 0 0 0-2 2v6"/><path d="M12 10v4"/><path d="M12 2v3"/></svg>' +
        '<span class="text-sm font-semibold text-white">Crewship</span>' +
      '</div>' +
      // Center: Org selector
      '<div class="flex items-center">' +
        '<button class="flex items-center gap-2 px-3 py-1.5 bg-neutral-800/80 rounded-md hover:bg-neutral-800">' +
          '<div class="w-5 h-5 rounded bg-primary-600 flex items-center justify-center text-white text-[9px] font-bold">U</div>' +
          '<span class="text-xs text-neutral-300 font-medium">Unify Technology</span>' +
          '<svg class="w-3.5 h-3.5 text-neutral-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="m6 9 6 6 6-6"/></svg>' +
        '</button>' +
      '</div>' +
      // Right: utilities
      '<div class="flex items-center gap-0.5">' +
        ...
        '<button class="flex items-center gap-2 px-1.5 py-1 rounded-md hover:bg-neutral-800">' +
          ...
          '<svg class="w-3 h-3 text-neutral-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="m6 9 6 6 6-6"/></svg>' +
        '</button>' +
      '</div>';
