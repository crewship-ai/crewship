...
**Kontext:** Leader/Director agenti potřebují posílat delegační příkazy
...
- Go binary (~5MB) běžící v každém team kontejneru na `localhost:9119`
- Standardní REST API: POST /delegate, POST /ask, GET /results
- CLI tools delegují přes `curl` (built-in bash tool, standardní)
- API-direct runtime deleguje přes nativní HTTP call
...
jako message broker pro delegace a crew orchestraci.

**Problém:**
- crewshipd je single-process Go service s goroutinami
- Delegace Leader→Worker je Docker exec ve stejném kontejneru
...
3. PostgreSQL LISTEN/NOTIFY (zváženo pro Phase 2 WS broadcast, ne pro delegace)
...
## ADR-004: Leader modes (active/passive)

**Status:** Accepted
**Datum:** 2026-02-15
**Kontext:** Leader běží po celou dobu Crew execution (long-running).
To je flexibilní ale drahé — Leader konzumuje LLM tokeny po celou dobu.

**Problém:**
- Active leader: 5 delegací = minimálně 6 LLM calls (1 init + 5 responses)
- Každý call = plný context (system prompt + konverzace + worker výsledky)
- Pro rutinní úkoly (denní report) je to zbytečně drahé

**Rozhodnutí:** Dva leader modes — uživatel volí per-leader konfigurací.

**Active mode (default):**
- Leader běží celou dobu, rozhoduje v real-time
- Může měnit strategii za běhu (jiný worker, jiný přístup)
- Dražší, flexibilnější
- Use case: složité úkoly, debugging, iterativní práce

**Passive mode:**
- Leader se spustí 2x: init (task breakdown → task plan) + finalize (agregace)
- Mezi tím crewshipd orchestruje workery deterministicky dle task plan
...
## ADR-005: Worker output compression

**Status:** Accepted
**Datum:** 2026-02-15
**Kontext:** Worker vrací výsledek Leaderovi. Pokud worker output = 50k tokens,
Leader musí celý output přečíst v LLM context window → drahé.

**Problém:**
- 5 workerů × 50k tokens = 250k tokens v Leader contextu
- Claude 200k context: potenciální overflow
- Náklady: 250k input tokens = ~$0.75 per Leader turn

**Rozhodnutí:** crewshipd automaticky sumarizuje worker output před předáním Leaderovi.

**Mechanismus:**
1. Worker output > `worker_output_max_tokens` (default 2000)?
2. Ano → crewshipd zavolá levný LLM (Haiku/GPT-4o-mini) pro sumarizaci
3. Leader dostane: summary (2k tokens) + file reference na plný output
4. Ne → Leader dostane plný output přímo

**Trade-off:**
- Sumarizace 1 worker výsledku: ~$0.002 (Haiku)
- Bez sumarizace, 50k tokens v Leader contextu: ~$0.15 per call
- 5 workerů: $0.01 sumarizace vs $0.75 plný context → 75x levnější

**Důsledky:**
- (+) Dramatická úspora tokenů (Leader context zůstává malý)
- (+) Leader nepotřebuje 200k context window model
- (-) Ztráta detailu při sumarizaci (mitigováno file referencí)
- (-) Extra LLM call per worker (mitigováno levným modelem)
- (-) Konfigurační otázka: jaký model pro sumarizaci?

---

## ADR-006: Circuit breaker pro delegace

**Status:** Accepted
**Datum:** 2026-02-15
**Kontext:** Worker může opakovaně selhávat (bug, špatný prompt, nedostupná služba).
Bez circuit breakeru Leader pořád zkouší delegovat → plýtvá tokeny a časem.

**Rozhodnutí:** Circuit breaker pattern pro každého workera.

**Flow:**
```
Worker fail → increment counter → counter >= 3?
  NO → auto-retry s exponential backoff (1s, 2s, 4s)
  YES → circuit OPEN → eskalace na Leadera
        → po 5 min cooldown → circuit HALF → 1 zkušební delegace
        → úspěch? → circuit CLOSED
        → fail? → circuit OPEN → další cooldown
```

**Backpressure:**
- Max queue depth per Leader: 10 čekajících delegací
- Queue full → Leader dostane error → může čekat nebo informovat uživatele

**Důsledky:**
- (+) Zabraňuje plýtvání tokeny na mrtvého workera
- (+) Leader může reagovat (jiný worker, jiný přístup)
- (+) Standardní distributed systems pattern
- (-) Trochu víc komplexity v DelegationEngine
- (-) Cooldown = čekání (5 min default, konfigurovatelné)

---

## ADR-007: Director bez kontejneru (MVP)

**Status:** Accepted
**Datum:** 2026-02-15
**Kontext:** Virtual Director koordinuje cross-team úkoly. Otázka:
potřebuje vlastní Docker kontejner?

**Rozhodnutí:** MVP Director běží jako přímý LLM API call v crewshipd (Go).
Žádný Docker kontejner.

**Zdůvodnění:**
- Director jen přemýšlí a deleguje — nepíše kód, nepotřebuje tools
...
- Director panic → `recover()` v goroutině, neshodí celý process
- Bez tools → postačující pro MVP (delegace + agregace)

**Phase 3:** Director dostane vlastní lightweight kontejner pokud bude
...
- (-) Director nemůže používat tools (mitigováno: deleguje na leadery co tools mají)
- (-) Žádná izolace — Director goroutina běží v crewshipd procesu
...
**Datum:** 2026-02-15
**Kontext:** Crew operace (Leader + Workers) konzumují víc LLM tokenů
než přímý chat s agentem. Uživatel (BYOK) potřebuje vědět kolik to stojí.

**Problém:**
- Leader + 5 Workers = 6+ LLM calls per Crew operace
- Active Leader = N LLM calls (kde N = počet worker výsledků + init)
- Uživatel netuší, že Leader konzumuje 3-5x víc tokenů než přímý chat

**Rozhodnutí:** Implementovat cost estimation a tracking.

**Plánované features:**
1. **Pre-execution estimate:** "Tato Crew operace bude stát přibližně $2.50"
   - Na základě: počet workerů × average output size × model pricing
2. **Real-time tracking:** Zobrazit aktuální token consumption v delegační timeline
3. **Per-crew budget:** Max $/tokens per crew execution (hard limit, crew se zastaví)
4. **Post-execution report:** Kolik stál každý worker, kolik leader, celkem

**Důsledky:**
- (+) Transparentnost nákladů pro uživatele
- (+) Budget limits zabraňují runaway costs
- (-) Pricing data se mění (nutné aktualizovat)
- (-) Odhad může být nepřesný (worker output variabilní)
...
- **CLI mode:** Docker exec → CLI tool (Claude Code, OpenCode, Codex)
  ...
  - Delegace přes `curl localhost:9119/delegate`
...
**Status:** Accepted
**Datum:** 2026-02-15
**Kontext:** Crew execution zahrnuje Leader + N Workers. Debugging vyžaduje
korelaci sessions, delegačních logů, a JSONL konverzací across agentů.

**Rozhodnutí:** Každá crew execution dostane unikátní `trace_id`.
- Přidán do: DelegationLog, JSONL metadata, Meilisearch index
- UI: timeline view celé crew execution s proklikem do session
...
Sidecar uz bezi v kazdem team kontejneru (ADR-001 v2). Pridame:
...
- (-) Sidecar complexity roste (delegace + MCP gateway)
...
**Status:** Accepted
**Datum:** 2026-02-16
**Kontext:** Sidecar spousti MCP servery jako stdio procesy UVNITR team
...
## ADR-018: Claude Agent Teams jako optional crew execution mode
...
Crewship ma vlastni orchestracni system (Leader/Worker delegace pres sidecar,
...
**Rozhodnuti:** Agent Teams jako ALTERNATIVNI crew execution mode, ne nahrada.

Novy `CrewExecutionMode`:
- `CREWSHIP_ORCHESTRATED` (default): nase Leader/Worker delegace pres sidecar
...
- MCP: lead session vidi sidecar, muze delegovat MCP tool cally.

**Dsledky:**
- (+) Nativni Claude lateral communication (teammates si mohou psat)
- (+) Mene overhead nez nase delegace pro Claude-only crew
...
verze runtime, nepredvidatelne build chyby. Kazdy team kontejner by musel
...
v team kontejneru (Docker-in-Docker nebo bind-mounted socket).
...
- (-) Docker-in-Docker nebo bind-mounted socket v team kontejneru
...
| ADR-004 | Leader modes (active/passive) | Accepted | ORCHESTRATION.md 5.6 |
| ADR-005 | Worker output compression | Accepted | ORCHESTRATION.md 5.7 |
| ADR-006 | Circuit breaker | Accepted | ORCHESTRATION.md 5.8 |
| ADR-007 | Director bez kontejneru | Accepted | ORCHESTRATION.md 5.2, AGENT-RUNTIME.md 15.2 |
...
| ADR-012 | Trace ID | Accepted | ORCHESTRATION.md 5.10, DelegationLog |
