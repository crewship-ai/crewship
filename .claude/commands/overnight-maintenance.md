...
- **Remote dev server:** All Go builds and tests run via `ssh dev-server "cd /opt/crewship && <command>"`. Never compile Go locally.
...
| Change type | Command |
|---|---|
| Go code | `ssh dev-server "cd /opt/crewship && git pull && go test ./... -count=1 && go vet ./..."` |
| Frontend/Next.js | `ssh dev-server "cd /opt/crewship && git pull && pnpm lint && pnpm build"` |
