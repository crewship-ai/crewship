...
export default function NewCrewPage() {
  ...
  // renderers (CrewIcon, crew-group-node, cruise overviews) all resolve the
  ...
  const handleCreateWithAI = async () => {
    ...
    try {
      ...
      if (coordinator) {
        ...
        router.push(`/cruise/agents/${coordinator.id}/chat?prefill=${prefill}&workspace_id=${workspaceId}`)
        return
      }
  ...
  const handleDeployAI = async () => {
    ...
    try {
      ...
      router.push(`/cruise/crews/${crew.id}`)
    } catch {
      toast.error("Network error. Please try again.")
      setSubmitting(false)
    }
  ...
  const handleDeployTemplate = async () => {
    ...
    try {
      ...
      router.push(`/cruise/crews/${data.crew_id}`)
    } catch {
      toast.error("Network error. Please try again.")
      setSubmitting(false)
    }
      ...
      <PageShell
        ...
        actions={
          <Button variant="outline" size="sm" asChild>
            <Link href="/cruise/crews"><ArrowLeft className="mr-2 h-4 w-4" />Back</Link>
          </Button>
        }
        className="max-w-3xl"
      >
