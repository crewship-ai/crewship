...
describe("<ChatClient> — the ?prompt= handoff belongs to one conversation", () => {
  ...
  it("opens an explicit new draft without auto-sending or reusing the newest thread", async () => {
    setUrl("/chat/riley", "?new=1&draft=1&prompt=Review%20the%20page")
    const view = render(<ChatClient />)
    const p = await panel()
    expect(p.getAttribute("data-session")).not.toBe("riley-1")
    expect(p.getAttribute("data-initial")).toBe("Review the page")
    expect(p.getAttribute("data-autosend")).toBe("false")
  ...
  it("gives new=1 precedence over a stale session query", async () => {
    setUrl("/chat/riley", "?session=riley-1&new=1&draft=1&prompt=Inspect")
    render(<ChatClient />)
    const p = await panel()
    expect(p.getAttribute("data-session")).not.toBe("riley-1")
    expect(p.getAttribute("data-initial")).toBe("Inspect")
    expect(p.getAttribute("data-autosend")).toBe("false")
  })

  it("still opens an explicitly new draft when old history cannot be read", async () => {
    apiFetchMock.mockImplementation((url: string) => {
      if (/\/api\/v1\/agents\?/.test(String(url))) return ok([riley, morgan])
      if (/\/api\/v1\/agents\/a-riley\/chats/.test(String(url))) return Promise.resolve({ ok: false, status: 500 } as Response)
      return ok([])
    })
    setUrl("/chat/riley", "?new=1&draft=1&prompt=Inspect")
    render(<ChatClient />)
    const p = await panel()
    expect(p.getAttribute("data-session")).not.toBe("riley-1")
    expect(p.getAttribute("data-initial")).toBe("Inspect")
