...
| 6 | **Cross-crew delegation over a live link** depends on the model choosing to delegate, so it cannot be asserted as a defect | **FIXED (2026-08-07)** — `test-crew-links.sh` §3 used to `_fail` when the agent neither echoed the tag nor opened a peer chat, while `test-delegation.sh` `skip`s the identical outcome under the same stated rule. It flaked accordingly: PASS at 14:01 and FAIL at 18:25 against the same sha, fail/fail/pass/pass/pass/fail across the week — and because the stage e2e leg counts total failing suites, every flip made `promotable` false regardless of the code. Now 3 attempts, an early exit on the side effect, and `skip` when they are exhausted. §1, §2 and §4 still assert the link (and its removal) deterministically. | `scripts/test-harness/test-crew-links.sh` |
