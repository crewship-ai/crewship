...
cs ask alex "Remember this fact: the release ship-date is 2026-07-15."
# A fresh ask is a new session — recall must come from persisted memory.
cs ask alex "What is the release ship-date? Answer with just the date."
cs memory search "ship-date"
cs memory status

echo "== 2. Lead → peer delegation (/assign) =="
# Alex (Engineering lead) decomposes and delegates to a peer (Sam).
cs ask alex "Delegate to Sam: write a one-line healthcheck for our API, then report Sam's answer back to me."

echo "== 3. Ephemeral hire → (autonomy gate) → run → it expires =="
# If the Engineering crew autonomy is 'guided', this creates a pending
# inbox approval; approve it in the UI or via the inbox CLI below.
cs hire --crew engineering --template backend-engineer --reason "spike: prototype a rate-limiter" --ttl 30 || \
...
cs ask morgan "You need a PagerDuty API token to page on-call but you don't have one. Raise a credential escalation describing exactly what you need."
...
cs ask sam "Run 'gh auth status' in your container and report whether GitHub auth is configured."
