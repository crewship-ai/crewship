 ...
 *
 * These are read-only presets shown in the "Built-in" tab of the Create
 * Agent wizard. Mirrors the seed data in cmd/crewship/seeddata/agents.go +
 * cmd/crewship/seeddata/prompts/{slug}.md so a new agent created from one
 * of these matches what the seed scripts would produce.
 *
 * Future: a /api/v1/agent-templates endpoint will expose these (plus
 * user-created ones) — keep the shape stable so the swap is mechanical.
 */
