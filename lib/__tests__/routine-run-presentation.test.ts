...
describe("routine result and execution identity", () => {
  it("keeps a completed engine run with failed review visibly unsuccessful", () => {
    expect(routineRunPresentation({ status: "completed", outcome: "FAILED" }).tone).toBe(
      "destructive",
    )
    expect(routineRunPresentation({ status: "completed", outcome: "NEEDS_HUMAN" }).tone).toBe(
      "warn",
    )
  })
  ...
  it("uses only declared labels from the executed recipe, including object outputs", () => {
    const definition = {
      outputs: [
        { name: "spike", value_labels: { false: "Limit not exceeded", true: "Limit exceeded" } },
      ],
    }
  ...
  it("shows a human label without changing the programmatic key or type", () => {
    const fields = slashFieldsFromRoutineInputs([
      { name: "threshold_usd", label: "Cost limit (USD)", type: "number", default: 5 },
    ])
...
describe("client run explanations", () => {
  ...
  it("does not mistake every wait for a human decision", () => {
    ...
    expect(routineRunExplanation({ status: "running" }, "datetime").title).toBe(
      "Waiting until a scheduled time",
    )
    expect(routineRunExplanation({ status: "waiting" }, "approval").title).toBe(
      "A review is needed",
    )
    expect(routineRunExplanation({ status: "waiting" }).detail).toContain(
      "No human decision has been confirmed",
    )
  })
  it("keeps recorded work separate from missing completion and arbitrary errors", () => {
    ...
    ).toContain("result was recorded")
    expect(routineRunExplanation({ status: "failed", error_message: "fetch failed" }).title).toBe(
      "This run could not finish",
    )
    expect(
      routineRunExplanation({ status: "completed", outcome: "SUCCEEDED", output: "false" }).title,
    ).toBe("Completed")
  })
...
describe("one priority for runtime state and recorded outcome", () => {
  it.each([
    ["cancelled", "PARTIAL", "Stopped", "Run stopped"],
    ["canceled", "NO_CHANGE", "Stopped", "Run stopped"],
    ["cancelled", "FAILED", "Stopped", "Run stopped"],
    ["failed", "NEEDS_HUMAN", "Failed", "This run could not finish"],
    ["interrupted", "FAILED", "Interrupted", "Run interrupted"],
    ["running", "NEEDS_HUMAN", "Running", "Work is in progress"],
  ])("%s with %s does not produce contradictory headings", (status, outcome, label, title) => {
