...
describe("useFleetStatus", () => {
  ...
  it("returns null and does not fetch when workspaceId is null", async () => {
    const { result } = renderHook(() => useFleetStatus(null))
  ...
  it("fetches fleet status on mount and surfaces the record", async () => {
    mockFetch.mockResolvedValueOnce({
      ok: true,
      json: async () => ({ total: 10, running: 2, error: 1, idle: 7 }),
    })

    const { result } = renderHook(() => useFleetStatus("ws-1"))
    await act(async () => { await flushAsync() })

    expect(mockFetch).toHaveBeenCalledWith("/api/v1/agents/fleet-status?workspace_id=ws-1")
  ...
  it("debounces a burst of realtime events to a single refresh", async () => {
    ...
    renderHook(() => useFleetStatus("ws-1"))
    await act(async () => { await flushAsync() })
  ...
  it("unmount cancels a pending debounce timer", async () => {
    ...
    const { unmount } = renderHook(() => useFleetStatus("ws-1"))
    await act(async () => { await flushAsync() })
    expect(mockFetch).toHaveBeenCalledTimes(1)
