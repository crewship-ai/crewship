...
describe("useFleetSelection", () => {
  ...
  describe("selectAgent", () => {
    it("writes agent slug to URL with scroll:false", () => {
      ...
      expect(mocks.replace).toHaveBeenCalledWith("/fleet?agent=mia", { scroll: false })
    })
    ...
    it("clears agent param when called with null", () => {
      ...
      expect(mocks.replace).toHaveBeenCalledWith("/fleet?crew=research", { scroll: false })
    })
  ...
  describe("clearSelection", () => {
    it("removes both agent and crew", () => {
      ...
      expect(mocks.replace).toHaveBeenCalledWith("/fleet", { scroll: false })
    })
    ...
    it("preserves unrelated params", () => {
      ...
      expect(mocks.replace).toHaveBeenCalledWith("/fleet?tab=sessions", { scroll: false })
    })
  })

  describe("pathname agnostic", () => {
    it("uses current pathname, not hardcoded /fleet", () => {
      mocks.pathname = "/fleet/agents/mia"
      const { result } = renderHook(() => useFleetSelection())
      act(() => {
        result.current.selectCrew("research")
      })
      expect(mocks.replace).toHaveBeenCalledWith(
        "/fleet/agents/mia?crew=research",
        { scroll: false },
      )
    })
  })
