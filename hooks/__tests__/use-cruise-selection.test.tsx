...
describe("useCruiseSelection", () => {
  beforeEach(() => {
    mocks.replace.mockClear()
    mocks.pathname = "/cruise"
    setSearchParams("")
  })
  ...
  describe("reading URL state", () => {
    it("returns null slugs when query is empty", () => {
      setSearchParams("")
      const { result } = renderHook(() => useCruiseSelection())
  ...
  describe("selectAgent", () => {
    it("writes agent slug to URL with scroll:false", () => {
      ...
      expect(mocks.replace).toHaveBeenCalledWith("/cruise?agent=mia", { scroll: false })
    })
    ...
    it("clears agent param when called with null", () => {
      ...
      expect(mocks.replace).toHaveBeenCalledWith("/cruise?crew=research", { scroll: false })
    })
  ...
  describe("clearSelection", () => {
    it("removes both agent and crew", () => {
      ...
      expect(mocks.replace).toHaveBeenCalledWith("/cruise", { scroll: false })
    })
    ...
    it("preserves unrelated params", () => {
      ...
      expect(mocks.replace).toHaveBeenCalledWith("/cruise?tab=sessions", { scroll: false })
    })
  })

  describe("pathname agnostic", () => {
    it("uses current pathname, not hardcoded /cruise", () => {
      mocks.pathname = "/cruise/agents/mia"
      const { result } = renderHook(() => useCruiseSelection())
      act(() => {
        result.current.selectCrew("research")
      })
      expect(mocks.replace).toHaveBeenCalledWith(
        "/cruise/agents/mia?crew=research",
        { scroll: false },
      )
    })
  })
