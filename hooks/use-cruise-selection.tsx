...
export interface CruiseSelectionUpdate {
  agent?: string | null
  crew?: string | null
}

export interface CruiseSelection {
  selectedAgentSlug: string | null
  selectedCrewSlug: string | null
  update: (updates: CruiseSelectionUpdate) => void
...
/**
 * URL-driven selection for the Cruise page. Reads/writes `?agent=<slug>` and
 * `?crew=<slug>` via shallow routing so deep-links, refresh, and back-button
 * behave naturally.
 *
 * `selectCrew` clears the agent (focus is on the crew). Use `update({ agent, crew })`
 * for atomic multi-field changes (e.g. clicking an agent in a different crew).
 */
export function useCruiseSelection(): CruiseSelection {
