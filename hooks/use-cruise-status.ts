...
/** Aggregate agent counts by status for the cruise toolbar badge. */
...
/**
 * Lightweight hook for toolbar cruise status.
 * Fetches agent counts by status and auto-refreshes on real-time events.
 */
export function useCruiseStatus(workspaceId: string | null): CruiseStatus | null {
  const [status, setStatus] = useState<CruiseStatus | null>(null)

  const refresh = useCallback(async () => {
    if (!workspaceId) return
    try {
      const res = await fetch(`/api/v1/agents/cruise-status?workspace_id=${workspaceId}`)
      if (res.ok) {
