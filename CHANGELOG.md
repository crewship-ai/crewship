...
### Improved
- Dashboard: Results & review now shows live agent and routine runs, issues in progress or awaiting review, and recent outcomes in a filterable scroll area. Attention highlights issues needing review; the crew list scrolls through the whole fleet, Up next links to the routine calendar, and run-volume bars use crew colours. The run summary cards share one layout. The dashboard header keeps only a 24h/7d/30d selector in the product font, visible while scrolling; the redundant status panel and header actions are gone. Host CPU/RAM gauges remain available through `/metrics` for Prometheus and Grafana without SQLite history writes. (#2696)
