...
- Inbox now previews the first item without changing its unread state, keeps decisions and operational alerts in distinct list sections, and replaces the repeated overview cards with the selected item's detail. Its crew filter uses each crew's icon and colour, searchable names and slugs, and current-view counts; source-type filtering is tucked under Advanced. Dashboard decision counts now include human-input runs and exclude system advisories, matching the focused Inbox list. (#2696)
- Inbox now shows human decisions before older routine alerts, separates those groups in its overview, and gives the desktop list more room for request titles. Missed schedules use blue, paused schedules use red, and history stays neutral. Dashboard attention links now filter the main Inbox pane as well as the list. (#2696)
- Dashboard: Results & review now shows live agent and routine runs, issues in progress or awaiting review, and recent outcomes in a filterable scroll area. Attention highlights issues needing review and opens Inbox with the matching approvals, run alerts or schedule alerts filter already selected; the crew list scrolls through the whole fleet, Up next links to the routine calendar, and run-volume bars use crew colours. Routine type icons stay neutral, while Done is green, calendar alerts are blue, and the larger run-summary icons regain their coloured accents. The run summary cards share one layout. On desktop, the work and crew panels share one height and scroll independently with visible controls; smaller screens stack them. The sticky dashboard header keeps its title and a 24h/7d/30d selector in the product font; the redundant status panel, counts and header actions are gone. Host CPU/RAM gauges remain available through `/metrics` for Prometheus and Grafana without SQLite history writes. (#2696)
