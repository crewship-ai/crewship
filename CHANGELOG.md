...
### Fixed
- Testing a stored OpenAI Codex provider login no longer sends its `auth.json` payload as an API bearer key or reports the resulting 403 as an invalid login. The API and CLI state that this login was not checked by an API-key probe; a controlled CLI run is still needed. (#2643)
