...
- **A cancel before the agent process exists is confirmed only when nothing can still create one.** The webhook runtime learned where its agent was only at launch, so a cancel during a cold crew-container start could not be answered — every probe said "unknown", the grace period ran out, and work that had never reached an agent was parked for reconciliation; a container start that then completed launched the agent anyway. The runtime now tracks three launch phases. Before launch, a stop is recorded against the attempt and every step that leads to an agent (container start, run record, launch) refuses afterwards, so the stop is a fact: a user cancel ends `cancelled`, a shutdown ends `retry_wait`. While the launch is pending — location recorded, the process not yet confirmed — a stop cancels the creation's context and sends the kill probe, but an absent answer settles nothing, because creation may still complete; the ledger settles once the process is confirmed or the run returns, and an absent process after the run returned counts as "never existed" only when the orchestrator's journal shows no exec was requested. A run that completes before a late cancel is delivered is recorded `succeeded`, not `cancelled`. A stop after the agent is confirmed goes to the provider's probe as before, and a stop that does not take is still reconciliation.
