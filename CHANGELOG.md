...
- **A cancel during a cold crew-container start is confirmed, and no agent starts behind it.** The webhook runtime learned where its agent was only at launch, so a cancel that arrived while the container was still being prepared could not be answered — every probe said "unknown", the grace period ran out, and work that had never reached an agent was parked for reconciliation. A container start that then completed launched the agent anyway. The runtime now records a stop against the attempt's launch state before anything exists: the stop is answered as a fact, the preparation's context is cancelled, and every step that leads to an agent (container start, run record, launch) refuses once a stop was recorded. A user cancel in that window ends `cancelled`; a server shutdown in it ends `retry_wait` and runs again after the restart. A stop after the agent is launched goes to the provider's own probe as before, and a stop that does not take is still reconciliation, not a cancellation.
