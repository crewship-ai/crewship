...
- **Providers tab on Credentials** (#2428) — a provider login is a seat that pays for a model, not a secret an agent reads, and the vault now says so: **Credentials → Providers** lists one row per login with its brand, plan, owner, expiry, what it pays for and — where the provider exposes them — its 5-hour and weekly quota windows, under four tiles (*Seats*, *At limit*, *Expiring ≤ 30 d*, *Unassigned*) and a rail of Status / Provider / Mode / Owner facets. Provider logins are no longer counted in the Overview's "N secrets"; the header reads "12 secrets · 5 provider logins". A login's own page adds *Seat*, *Quota* ("not readable for this provider" when the server has no windows), *Validity & refresh* (the refresh token shown as **SEALED**, a **Refresh now** button where the provider has a refresh flow, and "paste a new token before …" where it does not) and *Delivered to the agent as*; its actions are **Assign to agent** (an AGENT-scope binding, the first way the console could write one), **Test** (metered keys only — a subscription cannot be probed), **Re-login** and **Revoke**. The *Add a credential* wizard's Provider login shape now creates a `PROVIDER_LOGIN` with the mode and owner as their own fields, names the owner (seats are per person), and for an OpenAI subscription offers **Sign in with a code** beside the `auth.json` paste: Crewship shows a one-time code and the page to enter it on, waits for the provider, and the login is saved with its refresh token sealed before the wizard reaches the delivery step. On an agent's Config tab, **Pays with** sits under the CLI adapter: the provider's seats with plan, owner and status, other providers' seats greyed as "wrong provider", and a warning when the chosen seat is at its limit with no pool partner.

- **Sign in to ChatGPT with a device code** (#2428) — `crewship credential login --provider OPENAI` prints a one-time code and a URL; enter the code in OpenAI's device page and the **server** finishes the sign-in — it drives the same device-authorization flow `codex login --device-auth` speaks (user code, poll, PKCE code exchange), with no `codex` binary involved — and stores the resulting `auth.json` as an `AI_CLI_TOKEN` credential owned by you, named "ChatGPT Plus · jana@unify.cz" from the token's plan and the account's email, through the same create path as pasting it. Pending sign-ins are persisted, so a closed terminal (`--no-wait` + `credential login-status`) or a server restart does not lose one; a code expires after 15 minutes, the provider's back-off is honoured, and starts are limited to five per person per ten minutes. Two endpoints back it (start, status), documented in the API reference.

- **Gemini CLI on a Google AI plan, and one declaration of where every CLI's login goes** (#2428) — each CLI adapter now declares its `AuthDelivery`: the environment variable its binary reads an account from (`CLAUDE_CODE_OAUTH_TOKEN`, `CURSOR_API_KEY`, `FACTORY_API_KEY`), the file under the agent's HOME it reads a subscription login from (`.codex/auth.json`, `.gemini/oauth_creds.json`), and the renderer that writes that file with a placeholder refresh token. The Claude, Codex, Cursor and Factory rows restate what already happened; the new row is Gemini: an `AI_CLI_TOKEN` credential with provider `GOOGLE` holding the `oauth_creds.json` that `gemini` writes after Login with Google is delivered to a `GEMINI_CLI` agent as `~/.gemini/oauth_creds.json`, with `GOOGLE_GENAI_USE_GCA=true` and no dummy API key (a key would win over the login), billed `flat_rate` as "Google AI Pro" / "Google AI Ultra" / "Google AI". The file is refused at submit time when it lacks `access_token`, `refresh_token` or `expiry_date`, removed on revoke and at the next run start after an unassignment, and withheld from the Files API.

- **Codex on a ChatGPT plan** (#2428) — an `AI_CLI_TOKEN` credential with provider `OPENAI` holding the `auth.json` that `codex login` writes is delivered to a `CODEX_CLI` agent as `$CODEX_HOME/auth.json`, rendered with the real id/access tokens and a **placeholder refresh token**, so any number of agents can share one login without their containers rotating each other's tokens. The run is billed `flat_rate` with the plan read from the token ("ChatGPT Plus"), the file is removed on revoke and on the next run start after an unassignment, and it is withheld from the Files API like the other generated config files. A value Codex would refuse (a bare token, a file without `id_token`) is refused at submit time. The *Add secret* wizard gains a seventh shape, **Provider login** — a subscription (Claude Max setup-token, ChatGPT `auth.json`) or a metered API key that pays for a model — which is also the first way the console could create an `AI_CLI_TOKEN` or `API_KEY` at all: its "Token" tile stores `CLI_TOKEN`, a file under `/secrets` that no model CLI reads.
