...
### Improved
- Dashboard: Results & review now shows live agent and routine runs, issues in progress or awaiting review, and recent outcomes in a filterable scroll area. Attention highlights issues needing review; the crew list scrolls through the whole fleet, Up next links to the routine calendar, and run-volume bars use crew colours. The run summary cards share one layout; System details shows server CPU/RAM gauges refreshed every 15 seconds and up to 30 days of minute-level history. Host readings are also exposed through `/metrics` for Prometheus and Grafana, with no extra history writes. The reporting controls stay visible while scrolling. (#2696)
