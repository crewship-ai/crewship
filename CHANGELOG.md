...
- **Experimental decisions CLI:** `crewship decisions evaluate|triage|rerank` calls Jev through TypeSafe or OpenRouter with typed probabilities, bounded requests, and a credential-free `--dry-run`. Triage suggestions and supplied-passage rankings are advisory; the pilot does not change server routing or permissions. Includes an English/Czech evaluation probe and benchmark research. (#2629)
