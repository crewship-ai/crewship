...
### Improved
- Dashboard: Results & review now shows live agent and routine runs, issues in progress or awaiting review, and recent outcomes in a filterable scroll area. Attention highlights issues needing review and opens Inbox with the matching approvals, run alerts or schedule alerts filter already selected; the crew list scrolls through the whole fleet, Up next links to the routine calendar, and run-volume bars use crew colours. Completed work, routine schedule icons and summary metrics use neutral colours; amber and red draw attention to decisions and failures. The run summary cards share one layout. On desktop, the work and crew panels share one height and scroll independently with visible controls; smaller screens stack them. The sticky dashboard header keeps its title and a 24h/7d/30d selector in the product font; the redundant status panel, counts and header actions are gone. Host CPU/RAM gauges remain available through `/metrics` for Prometheus and Grafana without SQLite history writes. (#2696)
