...
### Improved
- Inbox now previews the first item without changing its unread state, keeps decisions and operational alerts in distinct list sections, and replaces the repeated overview cards with the selected item's detail. Its crew filter uses each crew's icon and colour, searchable names and slugs, and current-view counts; source-type filtering is tucked under Advanced. (#2696)
