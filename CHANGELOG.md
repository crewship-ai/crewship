...
- Chat's right rail is two panels — Artifacts (⌘1) and Work (⌘2) — and the sidebar keeps shared conversations findable. Favorites pin agents, people and rooms into one section (browser-local, per workspace and user); search runs against names and session titles on the server, including sessions beyond the sidebar's loaded page; Filter carries the conversation category, agent session kind, crew and status. The Artifacts panel lists only client-facing outputs (PDF, images, sandboxed HTML, CSV/TSV preview; XLS/XLSX download), previews inline or expanded with a five-second Follow/Pause, and opens and saves through the file download and save routes instead of writing a directory listing over the file. Work names a conversation's exact issue or routine source: `GET …/agents/{id}/chats` gains `q`, `chat_id`, `routine_id` and `source=1` — the `source` object carries the routine/issue id, name, run and step — and new routine chats persist that association through the `chat_routine_provenance` migration rather than a title match. (#2699)
- Chat now uses the same subbar and collapsible navigation pattern as Issues and Routines. Its agent context opens live artifact previews beside the conversation and links to the agent's assigned issues and authored routines through Work. (#2698)
- The default demo seed is now four connected Harbor Goods business stories — Sales, Finance, Marketing and Shipping — plus Crewship Lab: 3 crews, 9 agents, 5 projects with one TODO Issue each bound to its check routine, 6 Pages and 15 routines, all running on local sample data with no third-party accounts. Page actions expose check, optional AI draft and resolution separately; human approval gates the local Sales, Finance and Shipping delivery, and a completed action leaves a local outbox receipt and moves its Issue to DONE. Crewship Lab keeps real workspace telemetry — live container CPU and memory per crew through a workspace-scoped sidecar read — plus a bounded live container monitor, and the chat-reply-inbox demo use case checks that a saved agent Chat reply appears in Inbox. `crewship seed verify` now verifies the business demo by default (`--complete-demo` also approves the local actions and checks completed Issues and outbox evidence); the legacy ci-watch/docs-drift packs are no longer installed and remain reachable only through `seed verify --pack`. (#2700)
...
- Credential details now link to saved routines that declare the credential's type and show whether those routines would currently select it. Scope edits and deletion show known dependencies; the same view is available through `crewship credential dependents`. The view marks actual run use as unattributed and warns when a lookup fails. (#2674)
- Run again checks the previewed recipe hash and pins the verified version before execution; a changed HEAD returns 409 with a reload action. CLI routine run now accepts `--version` and `--expected-hash`. Reviewed starts skip mutable step overrides. (#2664)
- Routine run details now show the trigger, executed recipe version and declared credential types from that run's archived definition. They distinguish declarations from per-run use, which is not recorded, and do not present legacy invoking IDs as verified identities. (#2662)
...
### Fixed
- **Forked restores no longer silently lose capability tokens, inbox items and message feedback.** A fork (`--as-workspace` / `--as-crew`) re-mints every capability token it carries — invitation links, port exposures, pipeline webhooks, page public links and page webhooks — together with its digest, so no secret that worked against the source works against the fork; invitations stay usable from the fork's member list and the other capabilities arrive revoked and are reported per table in the CLI, API, log and audit row. `inbox_items`' and `message_feedback`'s unique keys are now scoped per workspace, so forks keep their inbox and feedback rows instead of dropping them. (#2274)
- **A workspace export no longer fails when a crew container is stopped, and no longer resumes a container it did not pause.** Running containers are paused for collection and always resumed; already-paused ones are collected under their existing pause and left paused; stopped ones are collected without being started. A provisioned crew whose container is gone from the daemon is backed up DB-rows-only and named as such in the create response, manifest, CLI warning and audit row — an incomplete export no longer reads as a complete one. (#2612)
- **The dashboard's "Needs your attention" counts stopped at the inbox's 100-row window.** `GET /api/v1/inbox` now returns `active_by_kind`, an exact per-kind count of the caller's visible not-resolved items, and the dashboard counts from it rather than the windowed rows. An idle dashboard tab also no longer triggers a live provider (Docker) service listing per crew on every realtime burst. (#2187)
- **Durations over an hour rendered in minutes.** Dashboard and mission durations roll over to hours (`2h 5m`), `/issues?create=1` no longer reopens the composer on a plain reload, and colourless crews rotate the existing chart palette instead of all rendering the same grey. (#2187)
- **WCAG contrast regressions on the dashboard, crews, routines, credentials, settings, admin and chat surfaces.** The toolbar build-status badge, alpha-muted secondary text and brand-tinted count chips moved onto the contrast-safe tokens, and the axe wcag2a/wcag2aa scan is now part of the nightly gate instead of an unread drift report. (#1617)
- **A human-approved Keeper escalation now actually grants what was approved.** Resolving an escalation as ALLOW was a dead end: the agent had no route to learn the outcome and a retry was judged afresh, where the L4 tier floor re-escalated every read — an approved production credential could never execute. The agent can now poll `GET /keeper/request/{id}` on its sidecar (scoped to the acting agent) and present the approved request id on the retry (`approval_request_id` or `X-Keeper-Approval`). The approval binds to the same agent, credential, request type and — for `/execute` — the identical command, is single-use (races resolve to exactly one winner), expires after 15 minutes, and is spent before execution. Only a row resolved by a person is consumable, and the retry's own row is never spendable, so approvals cannot chain. (#2574)
- **The sampled behaviour watchdog no longer discards WARN and ESCALATE verdicts.** Every fired sample now writes the same keeper log row and inbox fan-out the synchronous review endpoint produces — WARN → non-blocking inbox item, ESCALATE → operator review (blocking in block mode at strict/guided autonomy), block-mode DENY → blocking inbox item plus the existing journal entry — following the workspace's governance configuration; ALLOW and full-autonomy verdicts stay journal telemetry. Previously only a block-mode DENY left a trace. (#2575)
- An L4 DENY now clears the default DENY-notify risk threshold: the tier's risk floor was 6 while unconfigured workspaces notify at 7, so a critical refusal never reached the inbox there — despite the comment claiming otherwise. (#2575)
- LLM proxy provider keys are now bound to a specific agent run. A finished run's token is refused before credential injection, including after a sidecar restart; old agent-only route tokens are refused. Drain active agent processes when deploying this protocol change. (#2643)
...
- **Editing or re-enabling an agent schedule could run an old due occurrence.** Schedule settings and the next due cursor now commit together; disabling clears the cursor, enabling chooses a future occurrence, and invalid enabled cron expressions are refused before any write. (#2643)
- **A run could be acknowledged before its start record reached durable storage.** Internal run creation now commits `run.started` before allowing the agent to start; an unavailable durable journal fails closed. (#2643)
...
### Security
- ⚠️ **Behaviour change:** HTTP routine steps with `credential_ref` now stop before sending a request if the credential cannot be resolved. Public calls should omit the reference. Required credential checks block on store errors (HTTP 503) instead of silently allowing dispatch; confirmed missing credentials still return 422. (#2639)
...
- **Credentials: the rotation grace window is consumed.** After an upstream 401 to a rotated credential's new value, the crew sidecar replays the request once with the previous value — only that credential's, only while the rotation is ACTIVE and unexpired; ending the grace early reaches running sidecars on the next successful metadata poll (60-second cadence) — and records the fallback as a journal entry naming the rotation. (#1882)
- **Operator model: newly extracted facts retain their source evidence.** The quote, the message it was said in, the source type and the time are stored beside the 1.5 KB model file, not inside it; `crewship privacy user-model list --provenance` and `show <field>` render them, `GET /api/v1/users/me/user-model` returns a `provenance` object per fact, and every delete path (opt-out, delete, forget, Art. 17 erasure) purges them. (#1693, #1669)
...
- **Routines read as an operator console.** The routine page says in one line which version is published, whether a draft is waiting and which version Run will use; "In one look" answers what you get, what you provide, who does the work, what is checked, when it needs you and what it can touch. Steps are grouped into phases from their declared dependencies, helper transformations fold into one row, `foreach` blocks nest, hooks show around the run, and recipes with more than twelve steps open on the map with one node per group. A new "Files this routine runs" card lists the scripts a recipe declares as a tree under the crew share, with the Files panel's read-only code editor beside it, and says when a declared file is missing or could not be checked (`files[].status`). Runs that could not finish explain the stopping point in plain words (`failure` on the run), what was kept and what was not done, and what to do next; the raw error stays under Technical details. The Routines overview is built from the dashboard's own pieces — attention strip, outcome tiles, run outcomes per day, running now, up next — and the catalog stays in the sidebar; runs are read in Activity's Routines lens. Month cells in the calendar show a day's first two starts and open the Day view. Plan rows say whether a schedule follows the latest published version or is pinned (`effective_version`, `version_pinned`). Publishing a draft, editing inputs, agent prompts and limits, and creating a routine (describe it to the lead, copy one as a draft, or build it with the CLI) are popups on the routine page; a slug with only a draft opens as a routine page that publishes into the first version. The in-browser recipe editor is gone — steps, checks, scripts and agent behaviour are authored with the CLI or by the lead and arrive as drafts to publish.
...
- **Docs gates:** `docs-inventory -strict` now fails an API path no `crewship` command builds (exemptions with a reason in `scripts/docs-inventory/cli-parity-exemptions.txt`) and a CLI flag documented only under another command's heading (baselined in `flag-section-baseline.txt`, ratchet-only); `docs-surface-check` fails a heading with an unescaped `{param}`, which MDX renders as nothing. The integrated CLI fixes also remove seven temporary parity exemptions, eighteen obsolete flag-baseline entries and two repaired-heading exemptions. (#2576)
...
- **Issues CLI and docs:** `crewship issue create|update --assignee-type user` sets the human owner (email or user id), `issue review --revision/--brief-revision` pin #2448's compare-and-set, `stop --hard` reports what Tier 2 actually does; `--force` is documented on `update` (not `create`), and the docs gain the work/hand-off/Lead-review contract, the Get Issue work and execution fields, the routine Start path, the four Review 409s, sessions/checkpoints/context pack, leases, the `take_over` hold, the waitpoint decider rule and the last `COMPLETED` leftovers. (#2587)
- **Docs:** the credentials API reference documents the five provider-pool routes (`GET`/`POST /provider-logins/pools`, `GET`/`PUT`/`DELETE …/{poolId}`) with the ETag revision contract, the OWNER/ADMIN-only visibility of provider accounts, the reveal fresh-sign-in and default-password gates, the `PROVIDER_LOGIN` re-login on `PATCH`, and the `REQUESTED`/`PENDING_APPROVAL` statuses; the workspace capability tables list all ten capabilities with `credentials:reveal` in the admin preset and OWNER/ADMIN default; the four-eyes "unknown initiator" case is described as failing closed; "account groups" is tied to `crewship credential pool`. Binary-driven acceptance tests cover `credential pool *`, `escalation supply`, `credential refresh` and `credential list --kind`. (#2588)
- **CLI:** `crewship me` / `now` "Pending approvals" was always empty (the fetch decoded a `data` envelope the approvals API never sends); `inbox get` now passes `source_missing` through and says whether a waitpoint/escalation can be resolved here; `seed verify` honours `--format yaml|ndjson` and the CLI config `format` like every other command; `approvals get` prints the row's real `decision_comment` / `timeout_at`; docs for backup verify completeness, `inbox_item_reads` in GDPR export/erasure, the `-f json` contract on mutating commands, `queued_reason`, and the mission-start `403` autonomy gate (#2584)
- Test collector accounting failures by their actual exit code and diagnostic; distinguish subprocess timeouts from expected input rejection instead of accepting any killed process. (#2610)
- **OpenAPI:** the hand-declared schemas are now held to the handler structs by a two-way field-set test, and the drift it found is corrected — `Run` gains `kind`, `mission_id`, `mission_identifier`, `crew_slug`, `pipeline_slug` and a required list; backup verify/restore gain their completeness, migration and journal re-sign fields; `Workspace` and its update body gain `approvals_retention_days`; the crew-assignments row is the one the handler emits (with `queued_reason`); the agent-webhook trigger declares its 202/200 receipts, plain-text errors and 409/413/429/503; approvals rows carry `routine_version`; `q`/`limit`/`offset` on crews, agents, credentials and agent chats and `run_id` on the journal are declared; and statuses written through `writeProblem`/`replyError` literals and the internal-error helpers are inferred (266 operations gain their 500). (#2583)
...
- The `@sentry/nextjs` test-load crash is fixed upstream in 10.74.0 (getsentry/sentry-javascript#23789); the ceiling had already left `pnpm.overrides`, and the `vitest.config.ts` alias's docs now say it stays on its own merits. (#2235)
...
- **Demo packs in `crewship seed`** — the external-source demo use cases: a **nightly CI watch** over the scheduled GitHub Actions workflows of `crewship-ai/crewship` (token-zero probe as the wake gate, Sonnet triage only when something is red or silently stale), a **docs-drift audit** of that repository's documentation against its code, and a **site replica** in which the engineering lead delegates the copy of `www.seznam.cz` across an analyst, a data engineer, a frontend engineer and a tester. Each pack is one crew, its deterministic scripts (with their own unit tests, run by `go test`) delivered to the crew's shared volume, its routines, its Page and its issues — the issues carry labels, and the cross-crew file hand-offs of the previous seed are gone (`/crew/shared` is per crew). Crew leads run on `claude-sonnet-5`, workers on `claude-haiku-4-5`. The default seed has since replaced the packs with the Harbor Goods business stories (#2700); the remaining pack catalogue (`ci-watch`, `docs-drift`) is legacy and reachable only through `seed verify --pack`.
- **`crewship seed verify`** — runs the selected demo end to end and checks the agents against the probes: delivered scripts byte-identical to the seed, the probe's verdict against an independent read of GitHub, the agent's `COUNTS:` line reconciled with the probe, no token in the report, the notification in the inbox and the Page panels written by that run. A pack whose requirement is missing is reported as skipped, never as green; `--strict` makes that a failure. The default target is now the business demo (#2700); this row-by-row contract applies to the legacy `--pack` selection.
