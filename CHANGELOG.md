...
### Improved
- Dashboard: Results & review now shows live agent and routine runs, issues in progress or awaiting review, and recent outcomes in a filterable scroll area. Attention highlights issues needing review; the crew list scrolls through the whole fleet, Up next links to the routine calendar, and run-volume bars use crew colours. The run summary cards now share one layout; System details shows readable run capacity, crew, agent and schedule status plus real server CPU/RAM gauges and up to 30 days of collected history. The reporting controls stay visible while scrolling. (#2696)
