...
- **Experimental typed decisions:** `crewship decisions evaluate|triage|rerank` calls Jev through TypeSafe or OpenRouter with typed probabilities, bounded requests, and a credential-free `--dry-run`. A signed webhook can now run an opt-in routine `decision` step and branch to declared agents or review; the server requires an explicit provider key and egress target. Existing routing and permissions are unchanged by default. Includes an English/Czech evaluation probe, benchmark research, and a separate local SemIf/MLX smoke test for Mac covering webhook, journal, and infrastructure dry-run decisions. (#2629)
