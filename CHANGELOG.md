...
- **Routines read as an operator console.** The routine page says in one line which version is published, whether a draft is waiting and which version Run will use; "In one look" answers what you get, what you provide, who does the work, what is checked, when it needs you and what it can touch. Steps are grouped into phases from their declared dependencies, helper transformations fold into one row, `foreach` blocks nest, hooks show around the run, and recipes with more than twelve steps open on the map with one node per group. A new "Files this routine runs" card lists the scripts a recipe declares as a tree under the crew share with a read-only preview, and says when a declared file is missing. Runs that could not finish explain the stopping point in plain words (`failure` on the run), what was kept and what was not done, and what to do next; the raw error stays under Technical details. The calendar groups a busy day by routine with counts and opens a day agenda instead of scrolling inside a cell. Plan rows say whether a schedule follows the latest published version or is pinned (`effective_version`, `version_pinned`). Publishing a draft, editing inputs, agent prompts and limits, and creating a routine (describe it to the lead, copy one, or build it with the CLI) are popups on the routine page; the in-browser recipe editor is gone — steps, checks, scripts and agent behaviour are authored with the CLI or by the lead and arrive as drafts to publish.
