...
func BuildEnvVarsSidecar(req AgentRunRequest, keeperEnabled bool) []string {
	...
	if hasOAuth {
		...
		env = append(env, "CREWSHIP_SUBSCRIPTION_PLAN="+claudeDelivery.PlanLabel(oauthToken))
	} else {
	...
	if hasFileLogin {
		...
		env = overrideEnv(env, "CREWSHIP_SUBSCRIPTION_PLAN", fileDelivery.PlanLabel(fileLoginCred.PlainValue))
	}
