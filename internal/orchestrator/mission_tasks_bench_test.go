...
func BenchmarkUnblockDependentTasks(b *testing.B) {
	...
	// :memory: with database/sql can map to separate DBs per pooled connection;
	// pin to a single connection so benchmark data stays deterministic.
	...
	mustExec := func(query string, args ...any) {
		...
		if _, err := db.Exec(query, args...); err != nil {
			b.Fatalf("seed exec failed: %v | query=%s", err, query)
		}
	}
	...
	mustExec(`INSERT INTO workspaces (id, name, slug) VALUES (?, 'Bench WS', 'bench-ws')`, wsID)
	mustExec(`INSERT INTO crews (id, workspace_id, name, slug) VALUES (?, ?, 'Bench Crew', 'bench-crew')`, crewID, wsID)
	mustExec(`INSERT INTO agents (id, workspace_id, crew_id, name, slug, agent_role) VALUES (?, ?, ?, 'Lead', 'lead', 'LEAD')`, leadID, wsID, crewID)
	mustExec(`INSERT INTO agents (id, workspace_id, crew_id, name, slug, agent_role) VALUES (?, ?, ?, 'Worker', 'worker', 'AGENT')`, agentID, wsID, crewID)
	mustExec(`INSERT INTO missions (id, workspace_id, crew_id, lead_agent_id, trace_id, title, status, created_at, updated_at)
	...
	mustExec(`INSERT INTO mission_tasks (id, mission_id, assigned_agent_id, title, status, task_order, depends_on, created_at, updated_at)
