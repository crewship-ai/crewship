...
type AuthDelivery struct {
	...
	// RenderSet renders a multi-provider API-key file from the run's grants,
	// preserving per-run sidecar isolation. Mutually exclusive with Render.
	RenderSet func(req AgentRunRequest) ([]byte, error)
...
func (opencodeAdapter) AuthDelivery() AuthDelivery {
	return AuthDelivery{File: OpenCodeAuthFileRel, RenderSet: renderOpenCodeAuth}
}
...
func loginPlanLabel(d AuthDelivery, login Credential) string {
	if login.isProviderLogin() && login.part(providerlogin.PartPlan) != "" {
		return providerlogin.PlanLabel(login.Provider, login.part(providerlogin.PartPlan))
	}
	return d.PlanLabel(login.PlainValue)
}
...
func syncLoginFile(
	...
	var body []byte
	var err error
	if d.Kind == oauthOpenAI {
		var f codexauth.File
		f, err = codexFileFor(login)
		if err == nil {
			body, err = codexauth.Render(f, time.Now())
		}
	} else if d.Kind == oauthGoogle && login.isProviderLogin() {
		var expires time.Time
		expires, err = time.Parse(time.RFC3339, login.part(providerlogin.PartExpiresAt))
		if err == nil {
			body, err = geminiauth.Render(geminiauth.File{
				AccessToken: login.PlainValue, IDToken: login.part(providerlogin.PartIDToken),
				Scope: login.part(providerlogin.PartScope), ExpiryDate: expires.UnixMilli(),
			})
		}
	} else {
	...
	if logger != nil {
		logger.Info("login delivered",
			...
			"plan", loginPlanLabel(d, login), "path", d.File)
	}
	return nil
}
