...
// domain allowlist primitive — the port/case-insensitive host check the
// sidecar proxy enforces for in-container egress, supporting both exact hosts
// and "*." subdomain wildcards (#1377). It carries NO crewship imports
// (std-lib only) so it can be shared by BOTH the sidecar package (which
// enforces the allowlist on agent-container egress) AND internal/egresspolicy
// (which resolves the same boundary for the app-layer paths — routine http
// steps, notify/webhook channels, hooks, and the MCP gateway). Keeping it a
// leaf is what lets the sidecar's MCP gateway build its gated client through
// egresspolicy without an import cycle: egresspolicy no longer reaches back
// into sidecar for this type.
//
...
// DefaultAllowedDomains / PackageRegistryDomains) are unchanged.
package egressallow
