...
func TestCovCopyAndObserveLLM_SSEUsageAndActor(t *testing.T) {
	...
	w := httptest.NewRecorder()
	proxy.copyAndObserveLLM(w, resp, "openai", "openai", "agent-7", "credential-7")
	if w.Body.String() != body {
		t.Fatalf("SSE pass-through changed: %q", w.Body.String())
	}
	if got.AgentID != "agent-7" || got.CredentialID != "credential-7" || got.Provider != "openai" || got.InputTokens != 12 || got.OutputTokens != 3 {
