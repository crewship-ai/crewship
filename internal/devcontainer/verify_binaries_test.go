...
func TestVerifyBinariesResolve_RecordedAsBuildLayer(t *testing.T) {
	...
	for _, want := range []string{"command -v", "claude", "codex", "/opt/mise/data/shims", "/opt/crewship/bin", ":$PATH", "adapter CLI not installed"} {
...
func TestVerifyBinariesResolve_ExecOutcomes(t *testing.T) {
	...
	err := VerifyBinariesResolve(context.Background(), "c1", []string{"claude"}, "/x/bin:${containerEnv:PATH}", failing)
...
// The aggregated PATH is searched, with the devcontainer reference turned
// into the shell's own $PATH.
func TestVerifyBinariesResolve_UsesAggregatedPath(t *testing.T) {
	rec := &dockerfileRecorder{}
	if err := VerifyBinariesResolve(context.Background(), "", []string{"claude"}, "/usr/local/py-utils/bin:/opt/mise/data/shims:${containerEnv:PATH}", rec.exec); err != nil {
		t.Fatal(err)
	}
	line := strings.Join(rec.groups[0].cmds, "\n")
	if !strings.Contains(line, `export PATH="/usr/local/py-utils/bin:/opt/mise/data/shims:$PATH"`) {
		t.Errorf("aggregated PATH not used:\n%s", line)
	}
}
