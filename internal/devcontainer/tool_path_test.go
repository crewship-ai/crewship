...
func TestEnsureAgentToolPath(t *testing.T) {
	tests := []struct {
		...
		{"nil env gets the default path behind the tool dirs", nil,
			"/home/agent/.local/bin:/opt/mise/data/shims:" + defaultImagePath},
		...
		{"operator PATH is kept, tool dirs go first", map[string]string{"PATH": "/opt/x/bin:/usr/bin"},
			"/home/agent/.local/bin:/opt/mise/data/shims:/opt/x/bin:/usr/bin"},
		{"already complete is untouched", map[string]string{"PATH": "/home/agent/.local/bin:/opt/mise/data/shims:/usr/bin"},
			"/home/agent/.local/bin:/opt/mise/data/shims:/usr/bin"},
		{"partially present adds only the missing dir", map[string]string{"PATH": "/opt/mise/data/shims:/usr/bin"},
			"/home/agent/.local/bin:/opt/mise/data/shims:/usr/bin"},
	}
...
func TestGenerateDockerfile_CarriesAgentToolEnv(t *testing.T) {
	...
	for _, want := range []string{
		"ENV PATH=/home/agent/.local/bin:/opt/mise/data/shims:",
...
func TestImageEnvChanges(t *testing.T) {
	got := imageEnvChanges(map[string]string{
		"PATH":       "/opt/mise/data/shims:/usr/bin",
		"TZ":         "Europe/Prague",
	...
	want := []string{
		`ENV NOTE="has \"q\""`,
		`ENV PATH="/opt/mise/data/shims:/usr/bin"`,
		`ENV TZ="Europe/Prague"`,
	}
	if strings.Join(got, "\n") != strings.Join(want, "\n") {
		t.Errorf("changes =\n%s\nwant\n%s", strings.Join(got, "\n"), strings.Join(want, "\n"))
	}
	if imageEnvChanges(nil) != nil {
