...
// --- PATCH: the comment rides the same transaction -------------------------

// A PATCH can carry a comment alongside the column changes. That comment used
// to be a bare Exec whose error was logged under the same 200 the label bug
// had — and this surface makes the comment the agent's primary way of telling a
// human WHY the status moved. "IN_PROGRESS, explanation silently dropped" is a
// worse outcome than the call failing, so the comment now commits with the rest.
func TestInternalIssueUpdate_CommentCommitsWithTheStatus(t *testing.T) {
	h, wsID, crewID, leadID, _ := newInternalIssueHandler(t)
	issueID := seedIssue(t, h.db, wsID, crewID, leadID, "ENG-1", "BACKLOG")

	rr := httptest.NewRecorder()
	h.UpdateStatus(rr, internalPatch("ENG-1",
		`{"workspace_id":"`+wsID+`","agent_id":"agent-worker","status":"TODO","comment":"picked this up"}`,
		crewBoundCtx1186(wsID, crewID)))
	if rr.Code != http.StatusOK {
		t.Fatalf("status = %d body=%s", rr.Code, rr.Body.String())
	}
	...
	if err := h.db.QueryRow(
		`SELECT author_type, author_id, body FROM mission_comments WHERE mission_id = ?`, issueID).
		Scan(&authorType, &authorID, &body); err != nil {
		t.Fatalf("read comment: %v", err)
	}
	if authorType != "agent" || authorID != "agent-worker" || body != "picked this up" {
		t.Errorf("comment = (%s,%s,%q)", authorType, authorID, body)
	}
}

// The other half: when the transaction rolls back, the comment goes with it.
// A comment describing a status change that did not happen is a lie in the
// audit trail, which is worse than no comment at all.
func TestInternalIssueUpdate_CommentRollsBackWithTheTransaction(t *testing.T) {
	h, wsID, crewID, leadID, _ := newInternalIssueHandler(t)
	issueID := seedIssue(t, h.db, wsID, crewID, leadID, "ENG-1", "BACKLOG")
	if _, err := h.db.Exec(`DROP TABLE mission_labels`); err != nil {
		t.Fatalf("drop mission_labels: %v", err)
	...
	rr := httptest.NewRecorder()
	h.UpdateStatus(rr, internalPatch("ENG-1",
		`{"workspace_id":"`+wsID+`","agent_id":"agent-worker","status":"TODO",`+
			`"comment":"moved it","labels":["whatever"]}`,
		crewBoundCtx1186(wsID, crewID)))
	if rr.Code != http.StatusInternalServerError {
	...
	var n int
	if err := h.db.QueryRow(`SELECT COUNT(*) FROM mission_comments WHERE mission_id = ?`, issueID).Scan(&n); err != nil {
		t.Fatalf("count comments: %v", err)
	}
	if n != 0 {
		t.Errorf("comment count = %d, want 0 — a comment about a change that rolled back must not survive", n)
	}
}
