...
func TestDeviceLogin_CompletesIntoCredential(t *testing.T) {
	...
	if name != "ChatGPT Plus · jana@unify.cz" || typ != "PROVIDER_LOGIN" || provider != "OPENAI" ||
	...
	if plain == "" || strings.Contains(plain, "rt.FAKE-REFRESH-SECRET") {
		t.Fatal("access value contains rotating material or is empty")
	}
	var sealed, refreshEnc string
	if err := f.h.db.QueryRow(`SELECT sensitivity FROM credentials WHERE id = ?`, *done.CredentialID).Scan(&sealed); err != nil || sealed != "SEALED" {
		t.Fatalf("device login not sealed: %q %v", sealed, err)
	}
	if err := f.h.db.QueryRow(`SELECT encrypted_value FROM credential_fields WHERE credential_id = ? AND key = 'refresh_token'`, *done.CredentialID).Scan(&refreshEnc); err != nil {
		t.Fatal(err)
	}
	refresh, err := encryption.Decrypt(refreshEnc)
	if err != nil || refresh != "rt.FAKE-REFRESH-SECRET" {
		t.Fatal("refresh token not stored as an encrypted part")
	}
