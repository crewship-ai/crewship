...
// U2 (#974): OLLAMA model listing must target the workspace's ENDPOINT_URL
// credential (its /v1 stripped to reach Ollama's native /api/tags), not just
// the server-global keeper daemon URL — so set-time model validation checks
// the endpoint the agent actually uses.
func TestModelsList_OllamaUsesWorkspaceEndpoint(t *testing.T) {
	var gotURL string
	h, wsID, userID := newModelsHandler(t, func(provider, apiKey, ollamaURL string) (llm.ModelLister, bool) {
		if provider == "OLLAMA" {
			gotURL = ollamaURL
			return &fakeLister{models: []llm.ModelInfo{{ID: "qwen2.5-coder:7b"}}}, true
		}
		return nil, false
	})

	// Seed a workspace ENDPOINT_URL credential (openai-compatible, .../v1).
	enc, err := encryption.Encrypt("http://ws-ollama:11434/v1")
	if err != nil {
		t.Fatalf("encrypt: %v", err)
	}
	if _, err := h.db.Exec(`INSERT INTO credentials
		(id, workspace_id, name, encrypted_value, type, provider, scope, status, created_by, created_at, updated_at)
		VALUES (?, ?, 'ollama-local', ?, 'ENDPOINT_URL', 'OLLAMA', 'WORKSPACE', 'ACTIVE', ?, datetime('now'), datetime('now'))`,
		"cred-endpoint", wsID, enc, userID); err != nil {
		t.Fatalf("seed endpoint credential: %v", err)
	}
	...
	if gotURL != "http://ws-ollama:11434" {
		t.Fatalf("lister ollamaURL = %q, want the workspace endpoint with /v1 stripped", gotURL)
	}
	if body.Source != "live" || len(body.Models) != 1 {
		t.Fatalf("expected 1 live model from the workspace endpoint, got source=%s n=%d", body.Source, len(body.Models))
	}
}

// Without a workspace ENDPOINT_URL credential, OLLAMA listing falls back to the
// server-global keeper daemon URL (unchanged behaviour).
func TestModelsList_OllamaFallsBackToServerGlobal(t *testing.T) {
	var gotURL string
	h, wsID, _ := newModelsHandler(t, func(provider, apiKey, ollamaURL string) (llm.ModelLister, bool) {
		if provider == "OLLAMA" {
			gotURL = ollamaURL
			return &fakeLister{models: []llm.ModelInfo{{ID: "llama3"}}}, true
		}
		return nil, false
	})
	if _, _ = doModelsList(t, h, wsID, "?provider=OLLAMA"); gotURL != "http://ollama.test:11434" {
		t.Fatalf("lister ollamaURL = %q, want the server-global keeper URL fallback", gotURL)
	}
}
