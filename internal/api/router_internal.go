...
func (r *Router) registerInternalRoutes(pipes *PipelineHandler, oh orchestrationHandlers) {
	...
	r.mux.Handle("POST /api/v1/internal/pipelines/drafts/get", internalAuth(http.HandlerFunc(pipes.InternalGetDraft)))
	r.mux.Handle("POST /api/v1/internal/pipelines/drafts/save", internalAuth(http.HandlerFunc(pipes.InternalSaveDraft)))
	...
	r.mux.Handle("GET /api/v1/internal/crews/telemetry", internalAuth(http.HandlerFunc(internal.WorkspaceCrewTelemetry)))
