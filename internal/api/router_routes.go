...
// HTTP route table extracted from router.go. registerRoutes is the
// single source of truth for every URL the server exposes; keeping
// it in its own file means the route map can be scanned without
// scrolling past struct definitions and option wiring.
