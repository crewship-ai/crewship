...
func (h *WorkspaceHandler) Delete(w http.ResponseWriter, r *http.Request) {
	...
	if role != "OWNER" {
		writeProblem(w, r, http.StatusForbidden, "Only the workspace owner can delete a workspace")
		return
	}
	...
	if err == sql.ErrNoRows {
		writeProblem(w, r, http.StatusNotFound, "Workspace not found")
		return
	}
	if err != nil {
	...
	if req.ConfirmSlug != slug {
		writeProblem(w, r, http.StatusBadRequest, "confirm_slug does not match the workspace slug")
		return
	}
	...
	if otherCount == 0 {
		writeProblem(w, r, http.StatusConflict, "Cannot delete your only workspace")
		return
	}
