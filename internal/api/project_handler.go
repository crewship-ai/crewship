	...
	query := `
		...
		LEFT JOIN (
		    SELECT project_id,
		           ...
		           SUM(CASE WHEN status IN ('DONE','REVIEW') THEN 1 ELSE 0 END) AS done_count
...
func getProjectByID(ctx context.Context, db *sql.DB, projectID, wsID string) (projectResponse, error) {
	...
	err := db.QueryRowContext(ctx, `
		SELECT p.id, p.workspace_id, p.name, p.slug,
		       ...
		       (SELECT COUNT(*) FROM missions WHERE project_id = p.id AND mission_type = 'issue' AND status IN ('DONE','REVIEW')) AS done_count
		FROM projects p
...
func (h *ProjectHandler) Stats(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.db.QueryRowContext(r.Context(), `
		SELECT COUNT(*),
		       COALESCE(SUM(CASE WHEN status IN ('DONE','REVIEW') THEN 1 ELSE 0 END), 0)
	...
	assigneeRows, err := h.db.QueryContext(r.Context(), `
		SELECT m.assignee_id, COALESCE(a.name, 'Unassigned'),
		       COUNT(*),
		       SUM(CASE WHEN m.status IN ('DONE','REVIEW') THEN 1 ELSE 0 END)
		FROM missions m
