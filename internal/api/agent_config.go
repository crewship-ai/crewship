...
func (h *InternalHandler) resolveAgentConfigWithOpener(w http.ResponseWriter, r *http.Request, agentID, openedByUserID, visibility string) {
	...
	// CREWSHIP_LOCAL_MODEL_BASE_URL. base_url is "" when none is set, in which
	// case the orchestrator applies the deprecated env fallback. api_key/headers
	// carry optional auth for an authenticated endpoint and travel only on this
	// internal resolve channel (never on a user-facing read path).
	localEndpoint := resolveLocalModelEndpoint(r.Context(), h.db, h.logger, data.wsID, creds)
	resp["local_model_base_url"] = localEndpoint.BaseURL
	if localEndpoint.APIKey != "" {
		resp["local_model_api_key"] = localEndpoint.APIKey
	}
...
func (h *InternalHandler) resolveAgentCredentials(r *http.Request, agentID string) ([]mcpCredEntry, error) {
	delivered, slotNotices, err := loadDeliveredCredentialsForRun(r.Context(), h.db, agentID, h.loginRefresher)
	if err != nil {
...
func (h *InternalHandler) buildKeeperBlock(agentSlug string, creds []mcpCredEntry) string {
	...
	keeperBlock.WriteString("(auth comes from the fd-3 config — never -H, see SIDECAR AUTH above):\n\n")
	...
	keeperBlock.WriteString("    -K /dev/fd/3 3<<AUTH\n")
	keeperBlock.WriteString("header = \"Authorization: Bearer $CREWSHIP_AGENT_TOKEN\"\n")
	keeperBlock.WriteString("AUTH\n\n")
