...
func TestRefuseIfBackupInProgress_NilDB(t *testing.T) {
	// Nil db short-circuits to nil so unit-test harnesses that fake
	// the handler do not need a real lock table.
	if err := refuseIfBackupInProgress(context.Background(), nil, "ws_test"); err != nil {
		t.Errorf("nil db should short-circuit, got %v", err)
	}
}

func TestRefuseIfBackupInProgress_EmptyWorkspace(t *testing.T) {
	db := newGuardTestDB(t)
	if err := refuseIfBackupInProgress(context.Background(), db, ""); err != nil {
...
func TestRefuseIfBackupInProgress_NoLockHeld(t *testing.T) {
	db := newGuardTestDB(t)
	if err := refuseIfBackupInProgress(context.Background(), db, "ws_test"); err != nil {
...
func TestRefuseIfBackupInProgress_LockHeld(t *testing.T) {
	...
	err = refuseIfBackupInProgress(context.Background(), db, "ws_test")
	if err == nil {
