...
// the invoking crew when one is named. Anything else is 403; nothing is
// silently rewritten, because a rewritten provenance is still a lie.
func assertInvokingIdentity(w http.ResponseWriter, r *http.Request, db *sql.DB, logger *slog.Logger, workspaceID, crewID, agentID string) bool {
	if db == nil {
		return true
	}
	ctx := r.Context()
