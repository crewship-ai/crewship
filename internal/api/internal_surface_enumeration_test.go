...
// internalSurfaceProbe sends one request through the full router stack.
func internalSurfaceProbe(t *testing.T, r *Router, method, target, remoteAddr string, headers map[string]string) *httptest.ResponseRecorder {
	t.Helper()
...
func TestInternalSurface_MethodMismatchIs404_NotEnumerable(t *testing.T) {
	...
	ref := internalSurfaceProbe(t, r, http.MethodGet, "/api/v1/internal/credentials?workspace_id="+wsID, fenceAttackerAddr, nil)
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			rr := internalSurfaceProbe(t, r, tc.method, tc.target, fenceAttackerAddr, tc.headers)
...
func TestInternalSurface_MethodMismatchIs404_FromPrivateOrigin(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			rr := internalSurfaceProbe(t, r, tc.method, tc.target, fenceLoopbackAddr,
...
func TestInternalSurface_LegitimateCallStillWorks(t *testing.T) {
	...
	rr := internalSurfaceProbe(t, r, http.MethodGet, "/api/v1/internal/crews?workspace_id="+wsID, fenceLoopbackAddr, auth)
	...
	rr = internalSurfaceProbe(t, r, http.MethodHead, "/api/v1/internal/crews?workspace_id="+wsID, fenceLoopbackAddr, auth)
	if rr.Code != http.StatusOK {
	...
	rr = internalSurfaceProbe(t, r, http.MethodGet, "/api/v1/internal/agents/no-such-agent/resolve?workspace_id="+wsID, fenceLoopbackAddr, auth)
...
func TestInternalSurface_NonCanonicalPathGetsTheFence404(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			rr := internalSurfaceProbe(t, r, tc.method, tc.target, fenceAttackerAddr, nil)
			assertIndistinguishable404(t, tc.name, rr, wantBody)
	...
	rr := internalSurfaceProbe(t, r, http.MethodGet, "//api/v1/internal/keeper/request", fenceLoopbackAddr,
		map[string]string{"X-Internal-Token": fenceInternalToken})
	if rr.Code != http.StatusNotFound {
