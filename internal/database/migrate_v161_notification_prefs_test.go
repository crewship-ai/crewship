...
// 'email'/'webhook' (#1412).
func TestMigrate_V161_NotificationChannelsWidened(t *testing.T) {
	t.Parallel()
	...
	for col, found := range want {
		if !found {
			t.Errorf("notification_channels missing column %q after v161", col)
		}
	}
	...
	if _, err := db.Exec(`INSERT INTO notification_channels
		...
		t.Fatalf("insert shoutrrr-type channel should succeed post-v161: %v", err)
	}
	if _, err := db.Exec(`INSERT INTO notification_channels
...
// TestMigrate_V161_UserNotificationPrefs asserts the matrix table's
...
func TestMigrate_V161_UserNotificationPrefs(t *testing.T) {
	t.Parallel()
	db := migrateChainSetup(t)
...
// TestMigrate_V161_NotificationDeliveries asserts the outbox/delivery-log
...
func TestMigrate_V161_NotificationDeliveries(t *testing.T) {
	t.Parallel()
	db := migrateChainSetup(t)
...
// TestMigrate_V161_IdempotentReapply guards the writable_schema CHECK
...
// pins the v161-specific "already widened" short-circuit) must not error
// and must not double-append 'shoutrrr' into the CHECK text.
func TestMigrate_V161_IdempotentReapply(t *testing.T) {
	t.Parallel()
	db := migrateChainSetup(t)
