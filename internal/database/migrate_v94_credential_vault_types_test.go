...
//
// Table-driven so a single failure (e.g. CHECK constraint not
...
func TestMigrateV94_CredentialVaultTypes(t *testing.T) {
	...
	// so failures isolate cleanly to the row that regressed.
	...
	cases := []struct {
		...
		{
			name: "schema/credentials.username_is_nullable_text",
			assert: func(t *testing.T, db *sql.DB) {
				if got := columnType(t, db, "credentials", "username"); strings.ToUpper(got) != "TEXT" {
					t.Errorf("credentials.username type = %q, want TEXT", got)
				}
			},
		},
		{
			name: "schema/agent_credentials.mount_type_defaults_to_env",
			assert: func(t *testing.T, db *sql.DB) {
				// The discriminator only works if existing rows
				// resolve as env-mounted, so verify the default
				// is literally 'env'.
				if got := columnDefault(t, db, "agent_credentials", "mount_type"); strings.Trim(got, "'\"") != "env" {
					t.Errorf("agent_credentials.mount_type default = %q, want 'env'", got)
				}
			},
		},
		{
			name: "legacy_credential_insert_without_username_works",
			assert: func(t *testing.T, db *sql.DB) {
				// Additive nullable column must not break the pre-v94
				// insert shape — every existing caller keeps working.
				mustExec(t, db, `
...
VALUES ('c_legacy', 'ws1', 'legacy api key', 'v1:abc', 'API_KEY', 'u1')`)
		...
		{
			name: "userpass_insert_carries_cleartext_username",
			assert: func(t *testing.T, db *sql.DB) {
				mustExec(t, db, `
...
VALUES ('c_userpass', 'ws1', 'gmail', 'v1:enc', 'USERPASS', 'user@gmail.com', 'u1')`)
		...
		{
			name: "ssh_key_reuses_encrypted_value_no_extra_column",
			assert: func(t *testing.T, db *sql.DB) {
				// PEM body lives in the existing encrypted_value
				// column — no second column was added in v94.
				mustExec(t, db, `
INSERT INTO credentials (id, workspace_id, name, encrypted_value, type, created_by)
VALUES ('c_ssh', 'ws1', 'github deploy', 'v1:pem', 'SSH_KEY', 'u1')`)
		...
		{
			name: "agent_credentials_mount_type_default_applies_when_omitted",
			assert: func(t *testing.T, db *sql.DB) {
				// Keeping the legacy agent_credentials insert path
				// working untouched across the migration boundary is
				// the entire reason for the DEFAULT 'env'.
				mustExec(t, db, `
...
VALUES ('ac1', 'a1', 'c_legacy', 'GITHUB_TOKEN')`)
		...
		{
			name: "agent_credentials_explicit_file_mount_binding_accepted",
			assert: func(t *testing.T, db *sql.DB) {
				// SSH key → ~/.ssh/keys/github inside the container.
				// env_var_name carries the basename in file-mount
				// mode, not an actual env var name.
				mustExec(t, db, `
...
VALUES ('ac2', 'a1', 'c_ssh', 'github', 'file')`)
		...
		{
			name: "agent_credentials_mount_type_CHECK_rejects_unknown_values",
			assert: func(t *testing.T, db *sql.DB) {
				// Defense in depth — the API validator already
				// rejects non-{env,file}, but the DB constraint
				// also fails closed for manual writes / future
				// backfills / migrations that bypass the API.
				_, err := db.Exec(`
INSERT INTO agent_credentials (id, agent_id, credential_id, env_var_name, mount_type)
VALUES ('ac_bogus', 'a1', 'c_ssh', 'bogus', 'filesystem')`)
	...
	// Sequential (not t.Parallel()) because cases share the same db
	// handle and several depend on rows seeded by earlier cases
	// (e.g. agent_credentials inserts FK to credentials.c_legacy /
	// c_ssh). The cost is negligible — every assert is sub-ms.
