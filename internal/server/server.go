...
func New(cfg *config.Config, logger *slog.Logger, deps *Deps) *Server {
	...
	orch, ipcBase := buildOrchestrator(cfg, logger, ctr, sta)
	...
	tokenPool, tokenSyncer, credMonitor := newLLMProxy(cfg, logger, wsHub)
	...
	if deps != nil && deps.DB != nil && cfg.Auth.JWTSecret != "" {
		s.mountAPIRouter(cfg, logger, deps, orch, ctr, wsHub, logW, missionEngine, convStore, mux, ipcBase)
	}
...
// mountAPIRouter wires the DB-backed API surface onto the server: the Go
// API router with all its options, the journal writer and its orchestrator
// and stats adapters, telemetry (OTel, pprof, Pyroscope), the keeper
// gatekeeper, consolidator, Phase-2 evaluators, and the embedded static UI
// handler. Pure code motion out of New(); called only when deps.DB and
// cfg.Auth.JWTSecret are present.
...
// buildOrchestrator constructs the orchestrator and applies its config-driven
// wiring (sidecar proxy, workspace memory tier, keeper flag, local model
...
// newWSHub enforces the auth prerequisites (JWT secret, DB-backed sessions
// store) and constructs the WebSocket hub. Panics on a missing prerequisite
// so an unauthenticated server can never start. Pure code motion out of
// New(); the JWT validator is returned too because the terminal handler
// needs it.
func newWSHub(cfg *config.Config, logger *slog.Logger, deps *Deps) (*auth.JWTValidator, *ws.Hub) {
	...
	return jwtValidator, wsHub
}

// newLLMProxy constructs the LLM-proxy token pool and, when the proxy is
// enabled and INTERNAL_TOKEN is set, the token syncer and credential
// monitor (with provider-status broadcasts wired to the hub). Pure code
// motion out of New(); syncer/monitor are nil when the proxy is disabled.
func newLLMProxy(cfg *config.Config, logger *slog.Logger, wsHub *ws.Hub) (*llmproxy.TokenPool, *llmproxy.TokenSyncer, *llmproxy.CredentialMonitor) {
