...
func (e *Executor) completeRun(ctx context.Context, in RunInput, emit *pipelineEmitContext, result *RunResult) {
	...
	emit.emitRunCompleted(ctx, result.DurationMs, result.CostUSD,
		announcesOwnCompletion(in.dsl, result.StepOutputs))
...
func (e *Executor) persistStepOutput(ctx context.Context, in RunInput, depth int, runID, stepID, output string, costUSD float64, startedAt time.Time) {
	...
	// #1416 item 5: scrub before persist. The in-memory result.StepOutputs
	// entry stays raw for template chaining; only the persisted projection
	// copy is scrubbed so secrets never leak into the run row.
	if err := e.runStore.UpsertStepOutput(ctx, runID, stepID, scrubStepOutput(output), costUSD, time.Since(startedAt).Milliseconds()); err != nil {
...
func (e *Executor) persistWaveOutputs(ctx context.Context, in RunInput, depth int, runID string, stepOutputs map[string]string, costUSD float64, startedAt time.Time) {
	...
	// #1416 item 5: scrub each step's output before the wave flush. Copy
	// rather than mutate the caller's snapshot so template chaining still
	// sees raw values.
	scrubbed := make(map[string]string, len(stepOutputs))
	for k, v := range stepOutputs {
		scrubbed[k] = scrubStepOutput(v)
	}
	if err := e.runStore.FlushStepOutputs(ctx, runID, scrubbed, costUSD, time.Since(startedAt).Milliseconds()); err != nil {
...
func (e *Executor) persistRunTerminal(runCtx context.Context, runID string, in RunInput, pipelineID string, result *RunResult, startedAt time.Time) {
	...
	if result == nil {
		...
		if len(result.StepOutputs) > 0 {
			...
			if err := e.runStore.FlushStepOutputs(ctx, runID, scrubbed, result.CostUSD, dur); err != nil {
