...
// user turn.
//
// Trade-offs vs. the full orchestrator path:
//
...
//     loops are out of scope. For pipelines that need real tool
//     invocation (gmail fetch, terraform apply), Phase 1.5 will add
//     a full-orchestrator runner that satisfies the same interface.
//   - Con: only Anthropic provider is wired today. The tier resolver
//     still picks model names freely; this runner clamps any
//     non-anthropic adapter to the workspace's anthropic key.
//     OpenAI + Ollama support is a small follow-up — the wrappers
//     exist, just need the credential lookup branch.
//
...
var errNoAnthropicCred = errors.New("no active Anthropic credential in workspace")
