...
func Validate(dsl *DSL, agentSlugs map[string]struct{}, pipelineSlugs map[string]struct{}) error {
	...
	for i, st := range dsl.Steps {
		...
		case StepCallPipeline:
			...
			// Cross-pipeline existence is best-effort; produce a
			// warning-shaped error only if the caller passed a
			// non-nil set AND the slug is outside it. This lets
			// authors stage two related pipelines in one session
			// without ordering issues.
		default:
			return fmt.Errorf("pipeline: step %q has unsupported type %q (MVP supports: agent_run, call_pipeline)", st.ID, st.Type)
		}
