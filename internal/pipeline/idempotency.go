...
VALUES (?, ?, ?, ?, ?)`,
		...
		//
		// pipeline_id is part of the PK (v160, #1415): two different
		// pipelines can legitimately hold rows with the same
		// (workspace_id, idempotency_key) pair, so this filter selects the
		// specific row THIS caller's pipeline reserved, rather than
		// returning an arbitrary match from an unrelated pipeline that
		// happens to share the key string.
		var existing string
