...
func TestValidate_TemplateFieldsBreadth(t *testing.T) {
	...
	cases := []struct {
		...
		{"code body", Step{ID: "c", Type: StepCode, Code: &CodeStep{Runtime: "bash", Code: "echo " + badRef}}},
		{"code env value", Step{ID: "c", Type: StepCode, Code: &CodeStep{Runtime: "bash", Code: "echo hi", Env: map[string]string{"X": badRef}}}},
