...
generate_env_local() {
  ...
  local managed_keys=(
    ...
    CREWSHIP_BOLT_PATH
    CREWSHIP_NEXTJS_URL
    # Emitted below since the origin allowlist landed, but never listed here,
    # so every start appended one more copy: clone 3 carried 100+ identical
    # CREWSHIP_ALLOWED_ORIGINS lines by 2026-09-12. Managed means stripped.
    CREWSHIP_ALLOWED_ORIGINS
  )
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    local skip=false
    if [[ "$line" == "# --- Auto-generated by dev.sh"* ]]; then
      skip=true
    fi
...
# deploy_staleness PROJECT_DIR BINARY — one sentence when what this slot
# serves is not what it has checked out, empty when it is.
#
# Two ways a slot lies about its code, both seen on crewship-dev on
# 2026-09-12: the frontend marker records a HEAD the repo has since moved
# past (a commit landed and nobody reloaded — dev2 sat 102 frontend files
# behind for a day), or the running binary is older than the commit it is
# supposed to be running (the build was skipped or failed and the previous
# binary kept serving). `status` is where a person looks before saying
# "it's deployed", so this is where the sentence belongs. Never fails the
# caller: a missing marker or binary is reported by the lines around it.
deploy_staleness() {
  local project="$1" binary="$2"
  local head marker_head="" binary_epoch commit_epoch
  head=$(git -C "$project" rev-parse HEAD 2>/dev/null) || return 0
  if [[ -f "$project/.web-build-marker" ]]; then
    marker_head=$(head -n 1 "$project/.web-build-marker" 2>/dev/null || true)
    if [[ -n "$marker_head" && "$marker_head" != "$head" ]]; then
      echo "web/out/ was built from ${marker_head:0:8}, repo is at ${head:0:8}"
      return 0
    fi
  fi
  if [[ -f "$binary" ]]; then
...
cmd_status() {
  ...
  if git -C "$PROJECT_DIR" rev-parse --git-dir >/dev/null 2>&1; then
    ...
    if [[ -f "$PROJECT_DIR/.web-build-marker" ]]; then
      echo -e "  web/out/:    built $(date -r "$PROJECT_DIR/.web-build-marker" '+%Y-%m-%d %H:%M' 2>/dev/null || stat -c '%y' "$PROJECT_DIR/.web-build-marker" 2>/dev/null | cut -d. -f1) from $(cut -c1-8 "$PROJECT_DIR/.web-build-marker")"
    else
    ...
    if [[ -n "$stale" ]]; then
      echo -e "  ${YELLOW}STALE:${NC}       $stale"
      echo -e "               run ${CYAN}sudo systemctl reload crewship-ws@${INSTANCE_NUM}${NC} (or ./dev.sh restart) to serve HEAD"
    fi
  fi
